...
describe("DM-2507 native arm64 source-priority workflow", () => {
  it("runs the exact logical discriminator in the pinned noble image", () => {
    ...
    expect(workflow).toContain("packages/text-engine/src/render/emoji-presentation-priority.ts");
    expect(workflow).toContain("packages/text-engine/src/render/emoji-presentation-priority.test.ts");
