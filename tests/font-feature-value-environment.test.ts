    ...
    faces: [
      {
        ...
        sources: [
          {
            kind: "data",
            cssText: "url(data:font/otf;base64,...)",
            sourceOrder: 0,
            status: "loaded",
...
describe("authenticated multi-face font-feature-values environments", () => {
  ...
  it("rejects wrong-face and stale-byte mutations", () => {
    expect(() => authenticateFontFeatureEnvironment({ ...environment(), selectedFaceId: "face-regular-x" })).toThrow(
      /absent or ambiguous/,
    );
  ...
  it("partitions cache identity by document, selected face, descriptors, aliases, and bytes", () => {
    ...
    for (const mutate of [
      (value: AuthenticatedFontFeatureEnvironment) => {
        value.documentId = "document-b";
      },
      (value: AuthenticatedFontFeatureEnvironment) => {
        value.selectedFaceId = "face-regular";
        value.selectedSourceOrder = 0;
      },
      (value: AuthenticatedFontFeatureEnvironment) => {
        value.faces[1].weightDescriptor = "600";
      },
      (value: AuthenticatedFontFeatureEnvironment) => {
        value.effectiveAliasTable["exact family"].stylistic!.fancy = [2];
      },
    ]) {
  ...
  it("pins Blink selection, Unicode range, cache ownership, and HarfBuzz feature handoff", () => {
    ...
    const cache = readFileSync("external/chromium/third_party/blink/renderer/core/css/font_face_cache.cc", "utf8");
