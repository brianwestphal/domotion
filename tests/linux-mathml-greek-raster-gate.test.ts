...
async function exactRow(
  ...
  machine: string,
): Promise<{ root: string; row: LinuxMathmlGreekRasterRow }> {
  const root = mkdtempSync(join(tmpdir(), "dm2512-gate-"));
  roots.push(root);
  ...
  writeFileSync(join(root, "hinting.png"), hinting);
  const preterminal = exactLinuxMathmlGreekPreterminal();
  const tokens = preterminal.tokens.map((token) => ({
    id: token.id,
    glyph: token.glyph,
  ...
  const residual = await measurePathsRasterResidual(native, paths),
    hintingOffResidual = await measurePathsRasterResidual(native, hinting);
  const fingerprint = {
    platform: "linux" as const,
    osImage: "ubuntu24",
    osImageVersion: "20260818.1",
    arch: "x64",
    osRelease: "6.11",
    chromium: "140.0",
    chromiumRevision: "r1",
    browserExecutableSha256: "c".repeat(64),
    fontconfigVersion: preterminal.inventory.fontconfigVersion,
    fontconfigConfigSha256: preterminal.inventory.configSha256,
    fontInventorySha256: preterminal.inventory.inventorySha256,
    rendererSourceSha256: "d".repeat(64),
    oracleSourceSha256: "e".repeat(64),
    consumerRasterizer: "playwright/chromium",
  ...
  return {
    ...
    row: {
      ...
      runProvenance: {
        ...
        githubJob: runLabel,
        runnerName: runLabel,
        runnerBootIdSha256: machine.repeat(64),
        workflowRef: "owner/repo/.github/workflows/linux.yml@refs/heads/test",
      },
      cellSha256: linuxMathmlGreekCellSha256(),
      fingerprint,
      preterminal,
      pathsLogical: {
        sourceSha256: preterminal.sourceFont.sha256,
        faceIndex: 0,
      ...
      hintingControl: {
        requestedFamily: "DMUnhinted",
        computedFamily: "DMUnhinted",
        fontFaceRuleFamily: "DMUnhinted",
        fontFaceRuleCount: 1,
        sourceSha256: LINUX_MATHML_GREEK_SUBSETS.unhinted.sha256,
        sourceByteLength: LINUX_MATHML_GREEK_SUBSETS.unhinted.byteLength,
        isCustomFont: true,
        glyphCount: 4,
        tokens,
      },
...
describe("DM-2512 Linux MathML Greek logical-first gate", () => {
  it("reauthenticates PNG bytes, dimensions, residuals, and the exact logical seam", async () => {
    ...
    const mutated: any = structuredClone(row);
    mutated.nativeArtifact.sha256 = "f".repeat(64);
  ...
  it("requires independent machines and an active hinting-off control", async () => {
    ...
    expect(adjudicateLinuxMathmlGreekRows([proposal, validation], { schemaVersion: 1, entries: [] }).verdict).toBe(
      "incomplete-independent-evidence",
    );
    const inert: any = structuredClone(proposal);
    inert.hintingOffArtifact = inert.nativeArtifact;
    inert.hintingOffResidual.changedPixels = 0;
    inert.hintingOffResidual.totalChannelDelta = 0;
