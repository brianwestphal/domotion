...
const cases = [
  {
    ...
    height: 70,
    zoom: 1,
    gradient: "37deg, #e11d48 0px 7px, #2563eb 7px 19px",
    physical: ["0px", "7px", "19px"],
  },
  {
    id: "tall-143",
    width: 70,
    height: 180,
    zoom: 1,
    gradient: "143deg, #e11d48 -5px 7px, #2563eb 7px 19px",
    physical: ["-5px", "7px", "19px"],
  },
  {
    id: "zoomed",
    width: 90,
  ...
  },
  {
    ...
    height: 70,
    zoom: 1,
    gradient: "90deg, #e11d48 0px, #2563eb 240px",
    physical: ["0px", "240px"],
  },
      ...
      try {
        for (const row of cases) {
          await source.setContent(
            `<!doctype html><style>html,body{margin:0;background:white}#${row.id}{width:${row.width}px;height:${row.height}px;zoom:${row.zoom};background:repeating-linear-gradient(${row.gradient})}</style><div id="${row.id}" data-domotion-anim="${row.id}"></div>`,
          );
          ...
          for (const token of row.physical)
            expect(captured!.styles.backgroundImage, `${row.id}/${token}`).toContain(token);
          ...
          if (row.id === "wide-37") {
            ...
            expect(logicalInteriorComparison(a, mutated, W * dpr).mismatches, "repeat mutation").toBeGreaterThan(
              1_000 * dpr * dpr,
            );
          }
        }
