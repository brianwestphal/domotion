...
// (packages/text-engine/tools/linux-glyph-extractor). Mirrors the macOS CoreText helper test
// (packages/text-engine/src/render/glyph-helper.test.ts). Skipped automatically when:
//   - we're not on Linux (the helper is platform-specific), or
//   - the helper binary isn't built yet (run packages/text-engine/tools/linux-glyph-extractor/build.sh).
