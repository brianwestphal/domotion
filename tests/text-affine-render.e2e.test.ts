...
const CASES = [
  ...
  {
    id: "skew-reflect",
    css: "transform:matrix(-1.05,.27,.43,.82,13,-7);transform-origin:73% 18%",
    text: "Skew reflection",
  },
  { id: "rtl", css: "direction:rtl;transform:skewY(13deg) scale(.83,1.2)", text: "RTL matrix العربية" },
  {
    id: "vertical",
    css: "writing-mode:vertical-rl;height:175px;transform:rotate(11deg) scaleX(-1)",
    text: "縦書Affine",
  },
  {
    id: "vertical-lr",
    css: "writing-mode:vertical-lr;height:175px;transform:rotate(11deg) scaleX(-1)",
    text: "縦書左組",
  },
  {
    id: "sideways-rl",
    css: "writing-mode:sideways-rl;height:175px;transform:rotate(11deg) scaleX(-1)",
    text: "Sideways RL",
  },
  {
    id: "sideways-lr",
    css: "writing-mode:sideways-lr;height:175px;transform:rotate(11deg) scaleX(-1)",
    text: "Sideways LR",
  },
  {
    id: "decorated",
    css: "transform:rotate(-17deg) scale(1.15,.76);text-decoration:underline wavy;text-shadow:4px 3px 2px #3973db;-webkit-text-stroke:1px #c026d3",
    text: "Shadow stroke",
  },
  {
    id: "wrapped",
    css: "width:185px;white-space:normal;transform:skewX(16deg) scale(.9,1.12)",
    text: "Wrapped affine fragments across several physical lines",
  },
  { id: "zoom-local", outerCss: "zoom:1.35", css: "transform:rotate(23deg) scale(.8,1.1)", text: "Zoom remains local" },
  {
    id: "content-reference",
    css: "border:7px solid transparent;padding:9px 21px;transform-box:content-box;transform:rotate(19deg) scale(1.2,.8);transform-origin:100% 0",
    text: "Content reference",
  },
  { id: "bitmap-fallback", css: "transform:matrix(.82,.31,-.24,1.14,9,-4)", text: "Affine 🧭 bitmap" },
  {
    id: "line-clamp",
    css: "display:-webkit-box;width:210px;white-space:normal;overflow:hidden;-webkit-box-orient:vertical;-webkit-line-clamp:2;transform:rotate(9deg) scale(1.04,.88)",
    text: "Line clamp affine text continues well beyond the second physical line to retain its generated marker",
  },
] as const;
      ...
      try {
        for (const row of CASES) {
          ...
          if (row.id === "line-clamp") {
            ...
            expect(["vertical", "vertical-lr"], "only a leading-CJK vertical fallback may fail closed").toContain(
              row.id,
            );
            expect(
              walk(tree).some((element) => element.transformSubtreeRaster != null),
            ...
            for (const fragment of owner?.textPaintGeometry?.fragments ?? []) {
              ...
              expect(fragment.lineOrigin.source, `${row.id}: structured line origin`).toBe(
                "blink-text-fragment-line-origin-v1",
              );
              expect(fragment.lineOrigin.primaryFontIntegerAscent, `${row.id}: integer primary ascent`).toBe(
                Math.round(fragment.lineOrigin.primaryFontIntegerAscent),
              );
          ...
          expect(body, `${row.id}: scalar wrapper mutation`).not.toMatch(
            /anisotropicCorrection|cumScale|scale\([^)]*\/[^)]*\)/,
          );
          await output.setContent(
            `<style>html,body{margin:0;background:white}svg{display:block}</style><svg xmlns="http://www.w3.org/2000/svg" width="${WIDTH}" height="${HEIGHT}" viewBox="0 0 ${WIDTH} ${HEIGHT}">${body}</svg>`,
          );
          ...
          for (const edge of ["left", "top", "right", "bottom"] as const) {
            expect(Math.abs(actual[edge] - expected[edge]), `${row.id}: ${edge} ink bound`).toBeLessThanOrEqual(
              4 * dpr,
            );
          }
        }
