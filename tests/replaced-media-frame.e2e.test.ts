...
import { captureElementTreeWithWarnings, installCaptureRafClock } from "../src/capture/index.js";
...
describe("atomic replaced-media capture transaction", () => {
  ...
  it("screenshots every canvas/video owner in one stable epoch across repeated production captures", async () => {
    ...
    const first = await captureElementTreeWithWarnings(page, "body", viewport, {
      animationTimeMs: 250,
      rafClock: handle,
    });
    const second = await captureElementTreeWithWarnings(page, "body", viewport, {
      animationTimeMs: 250,
      rafClock: handle,
    });
    ...
    const capturedOwners = first.tree
  ...
  it("waits for a concrete video frame and rejects a hostile post-bind seek", async () => {
    await page.setContent(
      `<video muted preload="auto" src="data:video/webm;base64,${VIDEO_WEBM}" style="width:120px;height:80px"></video>`,
    );
    const transaction = await directTransaction(page, handle);
    try {
      ...
      await page.locator("video").evaluate((video) => {
        (video as HTMLVideoElement).currentTime = 0.75;
      });
