...
describeBrowser("gradient font-relative length Chromium image oracle (DM-2194)", () => {
  it("matches Chromium after em/rem become px and calc percentage resolves against the box", async () => {
    ...
    try {
      ...
      expect(await source.locator("#x").evaluate((el) => getComputedStyle(el).backgroundImage)).toContain("24px");
      expect(await source.locator("#x").evaluate((el) => getComputedStyle(el).backgroundImage)).toContain(
        "calc(50% + 20px)",
      );
