...
import type { CapturedElement, CapturedTextPaintAffine, CapturedTextPaintQuad } from "../src/capture/types.js";
...
function ownerFor(tree: readonly CapturedElement[], label: string): CapturedElement {
  const owner = walk(tree).find((node) => node.textSegments?.some((segment) => segment.text.includes(label)) === true);
...
function maxMappedResidual(
  ...
  for (let corner = 0; corner < 4; corner++) {
    ...
    result = Math.max(result, Math.abs(mapped.x - paint[corner * 2]), Math.abs(mapped.y - paint[corner * 2 + 1]));
  }
  return result;
}
...
describeBrowser("DM-2469 authoritative affine text-fragment capture", () => {
  it("splits mixed fallback, bidi, ligature, wrap, first-letter, vertical, zoom, and nested transforms by exact UTF-16 spans", async () => {
    ...
    try {
      ...
      await page.locator("#mixed").evaluate((element, text) => {
        element.textContent = text;
      }, mixedText);
      await page.locator("#vertical").evaluate((element, text) => {
        element.textContent = text;
      }, verticalText);
      ...
      for (const [label, expectedText, firstLetter] of [
        ...
        const owner = walk(capture.tree).find((element) =>
          element.textPaintGeometry?.neutral?.textSegments?.some(
            (segment) => segment.sourceMapping?.domText === expectedText,
          ),
        );
        ...
        expect(
          geometry!.sourceFragments.map((fragment) => fragment.physicalFragmentIndex),
          `${label}: ordered FragmentItems`,
        ).toEqual(Array.from({ length: geometry!.sourceFragments.length }, (_, index) => index));
        expect(
          geometry!.sourceFragments.filter((fragment) => fragment.role === "ordinary").length,
          `${label}: one source span per CDP quad`,
        ).toBe(geometry!.fragments.length);
        expect(
          geometry!.sourceFragments.filter((fragment) => fragment.role === "first-letter").length,
          `${label}: first-letter source ownership`,
        ).toBe(firstLetter ? 1 : 0);
        if (firstLetter) {
          expect(geometry!.sourceFragments.find((fragment) => fragment.role === "first-letter")?.domUtf16Span).toEqual([
            0, 2,
          ]);
        }
        ...
        for (const [sourceFragmentIndex, source] of geometry!.sourceFragments.entries()) {
          const matching = neutralSegments.filter(
            (segment) =>
      ...
      try {
        ...
        await page.evaluate(() =>
          Promise.all([
            ...
            ...Array.from(document.querySelectorAll("iframe"), (frame) => frame.contentDocument!.fonts.ready),
          ]),
        );
        const capture = await captureElementTreeWithWarnings(page, "#scene", { x: 0, y: 0, ...VIEWPORT });
        ...
        for (const fragment of wrapped) {
          ...
          expect(fragment.shapedOrigins).toHaveLength(
            ownerFor(capture.tree, "Zoomed").textSegments![fragment.textSegmentIndex].text.length,
          );
        ...
        expect(rtl.textPaintGeometry!.fragments[0].inlineOffset).toBeCloseTo(
          rtl.textSegments![0].x + rtl.textSegments![0].width,
          5,
        );

        const verticalOwner = ownerFor(capture.tree, "縦書Affine");
        const vertical = verticalOwner.textPaintGeometry?.fragments[0];
        const fragmentWarnings = capture.warnings.filter(
          (warning) => warning.feature === "<transform>" && warning.detail.includes("text-fragment"),
        );
        if (vertical == null) {
          ...
          expect(walk(capture.tree).some((node) => node.transformSubtreeRaster != null)).toBe(true);
        ...
        expect(
          ownerFor(capture.tree, "FrameAffine").textPaintGeometry?.fragments[0].affineResidual,
        ).toBeLessThanOrEqual(0.05);
        expect(
          capture.warnings.filter(
            (warning) => warning.detail.includes("text-fragment") && warning.feature !== "<transform>",
          ),
        ).toEqual([]);
      } finally {
        await context.close();
  ...
  it("retains transform-box/origin translation and routes only a projective plane to the outer surface", async () => {
    ...
    try {
      await page.setContent(
        ...
        <div id=projective><span id=plane>ProjectiveText</span></div><span id=flat3d>FlatMatrix3D</span></div>`,
        { waitUntil: "load" },
      );
      const capture = await captureElementTreeWithWarnings(page, "#scene", { x: 0, y: 0, ...VIEWPORT });
      ...
      for (let index = 0; index < 4; index++)
        expect(border.paintMatrix[index]).toBeCloseTo(content.paintMatrix[index], 5);
      expect(
      ...
      expect(
        projectiveOwner == null ? [] : walk(projectiveOwner.children).filter((node) => node.textPaintGeometry != null),
      ).toEqual([]);
      expect(ownerFor(capture.tree, "FlatMatrix3D").textPaintGeometry?.fragments[0].affineResidual).toBeLessThanOrEqual(
        0.05,
      );
