...
export const FEATURES: FeatureEntry[] = [
  ...
  },
  {
    ...
    tests: ["src/animation/animator.test.ts", "src/cli/animate.test.ts", "tests/overlay-window.e2e.test.ts", "tests/animate-examples.tsx"],
    transition: "three plain continue+cut frames whose MIDDLE member anchors an overlay to a MOVING element collapse into one run; the composed SVG is rasterized at each state's midpoint and the overlay's painted pixels are asserted absent in state 0, present in state 1 at STATE 1's anchor position (not the run's final position), and absent again in state 2 (the window closed with its state).",
  },
  {
