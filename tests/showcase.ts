 ...
 *
 * Usage: npx tsx tests/showcase.ts [--only frame-name]
 */
...
function page(body: string): string {
  return `<div style="padding: 32px; width: ${WIDTH}px; height: ${HEIGHT}px;">
${body}
</div>`;
}
...
function wrap(body: string): string {
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><style>${CSS}</style></head><body>${page(body)}</body></html>`;
}
