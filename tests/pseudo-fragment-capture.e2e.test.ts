    ...
    try {
      ...
      await page.locator("#frame").evaluate((frame) => {
        (frame as HTMLIFrameElement).srcdoc =
          `<!doctype html><style>body{margin:0}.inner::before{content:"frame"}</style><div class="inner">body</div>`;
      });
      ...
      expect(
        host?.pseudoFragments?.[0].fragments.some(
          (fragment) => fragment.kind === "text" && fragment.text.includes("😀"),
        ),
      ).toBe(true);
      ...
      expect(
        elements.some((element) =>
          element.pseudoFragments?.some(
            (record) => record.status === "exact" && record.contentItems.some((item) => item.text === "frame"),
          ),
        ),
      ).toBe(true);
