...
const evidence = (
  ...
  mode: "representative" | "exhaustive" = "representative",
): SourceDriftEvidence => ({
  fingerprint: {
    chromiumRevision: revision,
    chromiumHarfBuzzRevision: revision,
    harfbuzzRevision: revision,
    icuRevision: revision,
    icuDataSha256: `data-${revision}`,
    helperBinaries: { icu: `icu-${revision}`, glyph: `glyph-${revision}` },
    generatedClassifiers: { script: `script-${revision}`, ignorable: `ignorable-${revision}` },
  },
  mode,
...
describe("ICU/HarfBuzz source drift gate", () => {
  it("withholds changed property branches until source refs and exact oracle rows are updated", () => {
    ...
    expect(
      compareSourceDrift(evidence("old"), evidence("new", "Lowercase_Letter"), {
        sourceRefs: ["icu/uchar.cpp:1"],
        updatedPropertyRows: ["gc.0041"],
        updatedShapingRows: [],
      }).verdict,
    ).toBe("comparable");
  });
  it("withholds missing helper/classifier fingerprints and mixed representative/exhaustive profiles", () => {
    const missing = evidence("new");
    missing.fingerprint.helperBinaries = {};
    expect(compareSourceDrift(evidence("old"), missing).blockers).toContain("incomplete-source-fingerprint");
    expect(compareSourceDrift(evidence("old"), evidence("new", "Letter", "exhaustive")).blockers).toContain(
      "incomparable-conformance-mode",
    );
  });
