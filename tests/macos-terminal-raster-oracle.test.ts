...
import {
  POSITIVE_RECORDS,
  ZERO_DIFF_CONTROLS,
  classifyTerminalRaster,
  type TerminalRasterRecord,
} from "../tools/macos-terminal-raster-oracle.js";
...
describe("macOS terminal-raster evidence oracle", () => {
  ...
  it.each(["face", "gid", "cluster", "advance", "upem", "fontSize", "paint", "outline", "position"])(
    "withholds when %s identity moves",
    (field) => {
  ...
  it("reports envelope violations without changing a visual threshold", () => {
    const edge = copy(POSITIVE_RECORDS[0]);
    edge.residual.maxEdgeDistance = 2;
    const area = copy(POSITIVE_RECORDS[0]);
    area.residual.area = 48;
  ...
  it("requires controls to remain exact-zero", () => {
    const r = copy(ZERO_DIFF_CONTROLS[0]);
    r.residual.area = 1;
