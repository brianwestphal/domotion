...
async function pngWithInk(x: number, y: number): Promise<Buffer> {
  return sharp(rawWithInk(x, y), { raw: { width: 3, height: 3, channels: 4 } })
    .png()
    .toBuffer();
}
...
function partialRow(cell = pathsNativeRasterMatrix()[0]): PathsRasterRow {
  ...
  return {
    ...
    fingerprint: {
      ...
      consumerRasterizer: "chromium-svg-headless",
    ...
    expectedLogical: structuredClone(logical),
    actualLogical: structuredClone(logical),
    nativeFace: {
    ...
    pathsArtifact: { path: "paths.png", sha256: sha("d"), width: 3, height: 3 },
    warnings: [],
  };
}
...
describe("paths/native source-owned collection contract", () => {
  ...
  it("declares the deconfounded 348-cell technology/phase/transform/DPR union", () => {
    ...
    for (const technology of PATHS_NATIVE_RASTER_FIXTURES.map((fixture) => fixture.technology)) {
      ...
      expect(new Set(rows.map((cell) => cell.dimensions.transform))).toEqual(
        new Set(["none", "translate", "scale", "rotate", "affine"]),
      );
      ...
      expect(
        new Set(
          rows
            .filter(
              (cell) =>
                cell.dimensions.transform === "none" &&
                cell.dimensions.fontSizePx === 20 &&
                cell.dimensions.weight === 400,
            )
            .map((cell) => `${cell.dimensions.phaseX},${cell.dimensions.phaseY}`),
        ).size,
      ).toBe(16);
      expect(new Set(rows.map((cell) => cell.dimensions.weight))).toEqual(new Set([400, 600, 800]));
    }
    const declared = matrix.map((cell) => ({
      id: cell.id,
      dimensions: cell.dimensions,
      cellSha256: pathsRasterCellSha256(cell),
    }));
  ...
  it("uses the unique custom CSS family and source facts, never a backend display alias", () => {
    ...
    const identity = {
      ...
      actual: {
        sourceSha256: sha("a"),
        faceIndex: 0,
        variationAxes: { wght: 600, wdth: 90 },
        glyphs: structuredClone(glyphs),
      },
      native: {
    ...
    const mutations = [
      {
        expected: "requested-family",
        value: {
          ...identity,
          native: { ...identity.native, requestedFamily: "HostFallback", computedFamily: "HostFallback" },
        },
      },
      {
        expected: "native-source-bytes",
        value: { ...identity, native: { ...identity.native, sourceSha256: sha("c") } },
      },
      {
        expected: "native-variation-axes",
        value: { ...identity, native: { ...identity.native, computedVariationAxes: { wdth: 90, wght: 500 } } },
      },
      ...
      {
        expected: "variation-axes",
        value: { ...identity, actual: { ...identity.actual, variationAxes: { wdth: 90, wght: 500 } } },
      },
      {
        expected: "helper-source-bytes",
        value: {
          ...identity,
          native: { ...identity.native, helper: { ...identity.native.helper!, sourceSha256: sha("c") } },
        },
      },
      {
        expected: "helper-variation-axes",
        value: {
          ...identity,
          native: { ...identity.native, helper: { ...identity.native.helper!, resolvedAxes: { wdth: 90, wght: 500 } } },
        },
      },
      {
        expected: "glyph-outline-stream",
        value: { ...identity, actual: { ...identity.actual, glyphs: [{ ...glyphs[0], outlineSha256: sha("c") }] } },
      },
    ];
  ...
  it("rehashes bytes and recomputes residuals instead of trusting supplied observations", async () => {
    const root = mkdtempSync(join(tmpdir(), "dm2499-producer-"));
    roots.push(root);
    const native = await pngWithInk(1, 1),
      paths = await pngWithInk(2, 1);
    writeFileSync(join(root, "native.png"), native);
    writeFileSync(join(root, "paths.png"), paths);
    ...
    expect(rows[0].residual).toEqual(
      expect.objectContaining({ changedPixels: 2, area: 0, width: 0, height: 0, maxEdgeDistance: 1 }),
    );
    expect(rows[0].residual.severity).toBeCloseTo(1.5, 12);
  });

  it.each(["source", "axes"] as const)(
    "rejects a joint all-witness %s forgery against the declared cell",
    async (kind) => {
      ...
      await expect(producePathsRasterRows([row], root, { requireComplete: false })).rejects.toThrow(
        kind === "source" ? /sourceSha256.*declared fixture/ : /variationAxes.*declared matrix/,
      );
    },
  );
  ...
  it("rejects artifact paths outside the authenticated observation root", async () => {
    ...
    row.nativeArtifact.path = "../native.png";
  ...
  it("rejects role/path reuse before symmetric residuals can hide a swapped arm", async () => {
    ...
    writeFileSync(join(root, "native.png"), await pngWithInk(1, 1));
    const row = partialRow();
    row.pathsArtifact.path = "native.png";
