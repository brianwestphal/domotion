...
describe("strict animated-image static frame capture (DM-2579)", () => {
  ...
  it.each(ANIMATED_IMAGE_FIXTURES)("freezes authenticated nonzero $format frame as PNG", async (fixture) => {
    ...
    expect(record.requestedFrameIndex).toBe(1);
    expect(record.track.animated).toBe(true);
    expect(record.observation.complete).toBe(true);
    expect(record.pngDataUrl).toMatch(/^data:image\/png;base64,/);
  ...
  it.each(["svg-href", "background-image", "border-image-source", "mask-image", "list-style-image"] as const)(
    "substitutes only the authenticated %s owner slot",
    async (slot) => {
      const fixture = ANIMATED_IMAGE_FIXTURES[0];
      const page = await browser.newPage();
      const collector = await AuthenticatedAnimatedImageByteCollector.install(page);
      ...
      const data = `data:${fixture.mimeType};base64,${fixture.base64}`;
      const cssValue =
        slot === "background-image" || slot === "mask-image"
          ? `url('${data}'),linear-gradient(red,blue)`
          : `url('${data}')`;
      ...
      const value = await page
        ...
        .evaluate(
          (owner, selectedSlot) =>
            selectedSlot === "svg-href"
              ? (owner as SVGImageElement).href.baseVal
              : (owner as HTMLElement).style.getPropertyValue(selectedSlot),
          slot,
        );
