...
describe("Glassbox Studio acceptance benchmark", () => {
  it("keeps the AI-authored story, real workflow, brand treatments, hooks, and outputs in durable project JSON", () => {
    ...
    expect(live.tracks?.flatMap((track) => track.events.map((event) => event.id))).toEqual(
      expect.arrayContaining(["event-risk", "event-type-note", "event-complete", "event-apply-fix"]),
    );
    ...
    expect(value.scenes.flatMap((scene) => scene.treatments ?? []).map((item) => item.kind)).toEqual(
      expect.arrayContaining(["title-card", "logo-reveal", "terminal-chrome", "browser-chrome", "scene-transition"]),
    );
  ...
  it("revises persistent brand framing once, then accepts only a capture containing every causal beat", () => {
    const value = project();
    const evidence = value.scenes
      .find((scene) => scene.id === "scene-live-review")!
      .tracks![0].events.map((event) => ({
        version: 1 as const,
        eventId: event.id,
    ...
    revised.review.revisions.push({
      id: "revision-review-framing",
      parentId: revised.review.headRevisionId,
      createdAt: "2026-09-06T00:00:01.000Z",
      author: { kind: "ai" },
      kind: "content",
      summary: "Clarified the live browser frame.",
      metadata: { automation: { phase: "review" } },
    });
