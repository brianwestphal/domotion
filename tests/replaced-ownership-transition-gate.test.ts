...
const rows: ReplacedOwnershipTransitionRow[] = [
  {
    id: "control.checkbox.auto",
    family: "native-control",
    pairId: "control.checkbox",
    pairRole: "source",
    pairMode: "ownership-transition",
    expectedOwner: "whole-host-raster",
    actualOwner: "whole-host-raster",
    source: "Blink EffectiveAppearance",
    facts: { appearance: "checkbox" },
    exactCapture: true,
  },
  {
    id: "control.checkbox.none",
    family: "native-control",
    pairId: "control.checkbox",
    pairRole: "author",
    pairMode: "ownership-transition",
    expectedOwner: "structural-vector",
    actualOwner: "structural-vector",
    source: "Blink author-style adjustment",
  ...
  },
  {
    ...
    source: "LayoutImage natural dimensions",
    facts: { zoom: 1 },
    exactCapture: true,
    maxDevicePixelDelta: 0.5,
  },
  {
    id: "object.zoom.1_25",
    family: "object-geometry",
...
describe("DM-2364 replaced ownership transition adjudicator", () => {
  it("accepts complete source-owned paired evidence", () => {
    expect(adjudicateReplacedOwnershipTransitions(rows, fingerprint, requirements)).toMatchObject({
      pass: true,
      errors: [],
    });
  });
  ...
  it.each([
    [
      "ownership swap",
      (copy: ReplacedOwnershipTransitionRow[]) => {
        copy[0].actualOwner = "structural-vector";
      },
    ],
    [
      "partial record",
      (copy: ReplacedOwnershipTransitionRow[]) => {
        copy[0].exactCapture = false;
      },
    ],
    [
      "one-device-pixel breach",
      (copy: ReplacedOwnershipTransitionRow[]) => {
        copy[2].maxDevicePixelDelta = 1.01;
      },
    ],
    [
      "capture warning",
      (copy: ReplacedOwnershipTransitionRow[]) => {
        copy[0].unexpectedWarnings = ["native crop unavailable"];
      },
    ],
