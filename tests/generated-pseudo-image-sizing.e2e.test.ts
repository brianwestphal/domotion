...
describeBrowser("generated URL image intrinsic paint", () => {
  it("keeps a 128px SVG paint owner separate from its 24px pseudo layout slot", async () => {
    ...
    await Promise.all([
      ...
      writeFile(
        htmlPath,
        '<style>body{margin:0}#host{font:16px/24px Arial}#host::before{content:url("asset.svg");display:inline-block;width:24px;height:24px;vertical-align:middle;margin-right:6px}</style><p id="host">text</p>',
      ),
    ]);
    ...
    try {
      ...
      const record = all
        .flatMap((entry) => entry.pseudoFragments ?? [])
        .find(
          (entry) =>
