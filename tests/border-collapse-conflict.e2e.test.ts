...
describeBrowser("DM-1260: border-collapse conflict resolution", () => {
  it("keeps block-displayed td boxes outside the collapsed table grid (DM-2654)", async () => {
    ...
    try {
      await page.setContent(
        `<style>body{margin:0}table{border-collapse:collapse}td{padding:6px 12px;border:1px solid rgb(226,232,240)}td.lab{display:block;margin-top:4px}</style><table><tr><td class="lab">Label</td><td>A</td><td>B</td></tr></table>`,
        { waitUntil: "load" },
      );
      ...
      expect(
        rects.every(
          (rect) =>
            rect.x + rect.width >= geometry.firstCellLeft &&
            (rect.axis !== "row" || rect.x >= geometry.firstCellLeft - 1),
        ),
      ).toBe(true);
    } finally {
      await page.close();
  ...
  it("resolves shared edges to the winner and suppresses structural box borders", async () => {
    ...
    try {
      ...
      expect(rects).toContainEqual(
        expect.objectContaining({ axis: "column", width: 8, style: "solid", color: "rgb(255, 0, 0)" }),
      );
      ...
      expect(rects).toContainEqual(
        expect.objectContaining({ axis: "column", width: 6, style: "solid", color: "rgb(0, 128, 0)" }),
      );
  ...
  it("segments a rowspan edge at each neighboring cell (DM-2246)", async () => {
    ...
    try {
      await page.setContent(
        `<style>table{border-collapse:collapse}td{width:60px;height:30px;border:2px solid blue}.r{border-right:6px solid red}.b{border-left:8px dashed green}</style><table><tr><td class="r" rowspan="2">R</td><td>A</td></tr><tr><td class="b">B</td></tr></table>`,
        { waitUntil: "load" },
      );
      const tree = await captureElementTree(page, "body", { x: 0, y: 0, width: W, height: H });
      const table = find(tree, (n) => n.tag === "table")!;
      const right = ((table.styles as any).collapsedBorderRects as Array<any>).filter(
        (rect) => rect.axis === "column" && (rect.color === "rgb(255, 0, 0)" || rect.color === "rgb(0, 128, 0)"),
      );
  ...
  it("uses physical unequal row-track boundaries for rowspan segments (DM-2252)", async () => {
    ...
    try {
      await page.setContent(
        `<style>table{border-collapse:collapse}td{width:60px;border:2px solid blue}.r{border-right:6px solid red}.a{height:20px}.b{height:70px;border-left:8px dashed green}</style><table><tr><td class="r" rowspan="2">R</td><td class="a">A</td></tr><tr><td class="b">B</td></tr></table>`,
        { waitUntil: "load" },
      );
      const tree = await captureElementTree(page, "body", { x: 0, y: 0, width: W, height: H });
      const table = find(tree, (n) => n.tag === "table")!;
  ...
  it("uses physical unequal column-track boundaries for colspan segments (DM-2252)", async () => {
    ...
    try {
      await page.setContent(
        `<style>table{border-collapse:collapse}td{height:30px;border:2px solid blue}.c{border-bottom:6px solid red}.a{width:25px}.b{width:95px;border-top:8px dashed green}</style><table><tr><td class="c" colspan="2">C</td></tr><tr><td class="a">A</td><td class="b">B</td></tr></table>`,
        { waitUntil: "load" },
      );
      const tree = await captureElementTree(page, "body", { x: 0, y: 0, width: W, height: H });
      const table = find(tree, (n) => n.tag === "table")!;
      const bottom = ((table.styles as any).collapsedBorderRects as Array<any>).filter(
        (rect) => rect.axis === "row" && (rect.color === "rgb(255, 0, 0)" || rect.color === "rgb(0, 128, 0)"),
      );
  ...
  it("maps vertical-rl RTL logical edges to physical table rectangles (DM-2320)", async () => {
    ...
    try {
      await page.setContent(
        `<style>table{border-collapse:collapse;writing-mode:vertical-rl;direction:rtl}td{width:34px;height:44px;border:2px solid blue}.a{border-right:7px solid red}.b{border-right:5px dashed green}</style><table><tr><td class="a">A</td><td class="b">B</td></tr></table>`,
        { waitUntil: "load" },
      );
      const tree = await captureElementTree(page, "body", { x: 0, y: 0, width: W, height: H });
      const table = find(tree, (n) => n.tag === "table")!;
      ...
      expect(rects.some((rect) => rect.width === 5 && rect.style === "dashed" && rect.color === "rgb(0, 128, 0)")).toBe(
        true,
      );
  ...
  it("paints collapsed edges independently in each multicol table fragment (DM-2322)", async () => {
    ...
    try {
      await page.setContent(
        `<style>body{margin:0}.cols{columns:3;column-fill:auto;width:720px;height:120px}.t{border-collapse:collapse;width:100%}.t td{box-sizing:border-box;height:38px;border:4px solid rgb(0,0,255);padding:0}</style><div class="cols"><table class="t"><tbody>${Array.from({ length: 8 }, (_, i) => `<tr><td>${i}</td></tr>`).join("")}</tbody></table></div>`,
        { waitUntil: "load" },
      );
      const sourceFragments = await page.locator("table").evaluate((table) =>
        Array.from(table.getClientRects(), (rect) => ({
          left: rect.left,
          right: rect.right,
          top: rect.top,
          bottom: rect.bottom,
          ...
          .every((fragment) =>
            rects.some(
              (rect) =>
                rect.axis === "row" &&
                rect.height === 2 &&
                rect.x < fragment.right &&
                rect.x + rect.width > fragment.left &&
                rect.y > fragment.top,
            ),
          ),
      ).toBe(true);
      expect(
        sourceFragments
          .slice(1)
          .every((fragment) =>
            rects.some((rect) => rect.axis === "row" && rect.height === 2 && rect.y === Math.round(fragment.top)),
          ),
      ).toBe(true);
    } finally {
      await page.close();
  ...
  it("omits row-axis edges where one table row itself fragments (DM-2322)", async () => {
    ...
    try {
      await page.setContent(
        `<style>body{margin:0}.cols{columns:3;column-fill:auto;width:720px;height:110px}.t{border-collapse:collapse;width:100%}.t td{border:5px solid rgb(220,0,0);padding:4px}.tall{height:245px}</style><div class="cols"><table class="t"><tbody><tr class="tall"><td>A<br>${"line<br>".repeat(14)}</td></tr><tr><td>B</td></tr></tbody></table></div>`,
        { waitUntil: "load" },
      );
      const fragments = await page.locator("table").evaluate((table) =>
        Array.from(table.getClientRects(), (rect) => ({
      ...
      for (const fragment of fragments.slice(0, -1)) {
        expect(
          rects.some(
            (rect) => rect.axis === "row" && (rect.y === fragment.bottom || rect.y + rect.height === fragment.bottom),
          ),
        ).toBe(false);
      }
    } finally {
  ...
  it("does not invent edges for an empty trailing table fragment or clip outer ink (DM-2322)", async () => {
    ...
    try {
      await page.setContent(
        `<style>body{margin:0}.cols{columns:4;column-fill:auto;width:920px;height:110px}.t{border-collapse:collapse;width:100%}.t td{border:5px solid red;padding:4px}.tall{height:245px}</style><div class="cols"><table class="t"><tbody><tr class="tall"><td>A<br>${"line<br>".repeat(14)}</td></tr><tr><td>B</td></tr></tbody></table></div>`,
        { waitUntil: "load" },
      );
      const fragments = await page.evaluate(() => {
        const serial = (element: Element) =>
      ...
      expect(
        rects.some(
          (rect) =>
            rect.x < lastPainted.right &&
            rect.x + rect.width > lastPainted.left &&
            rect.y + rect.height > lastPainted.bottom,
        ),
      ).toBe(true);
    } finally {
      await page.close();
  ...
  it("owns repeated header/footer occurrences independently from their CSSOM prototype aliases (DM-2558)", async () => {
    ...
    try {
      ...
      const cssom = await page.evaluate(() => {
        ...
        const serial = (el: Element) =>
          Array.from(el.getClientRects(), (rect) => [rect.left, rect.top, rect.right, rect.bottom]);
        return {
          table: serial(table),
          head: serial(document.querySelector("thead")!),
          foot: serial(document.querySelector("tfoot")!),
        };
      });
      ...
      expect(
        sections
          .filter((section: any) => section.repeatRole !== "non-repeated")
          .every(
            (section: any) =>
        ...
        expect(
          rects.some(
            (rect) =>
              rect.x < fragment.right &&
              rect.x + rect.width > fragment.left &&
              rect.y < fragment.bottom &&
              rect.y + rect.height > fragment.top,
          ),
        ).toBe(true);
      }
      expect(
        rects.some(
          (rect) =>
            rect.axis === "row" && rect.width === 8 && rect.style === "dashed" && rect.color === "rgb(220, 0, 0)",
        ),
      ).toBe(true);
    } finally {
      await page.close();
