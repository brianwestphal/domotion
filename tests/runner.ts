 ...
 *
 * Usage: npx tsx tests/runner.ts [--only feature-name]
 */
...
export async function runFeatureTests(tests: FeatureTest[], suiteName?: string): Promise<SuiteResult[]> {
  ...
  for (const test of tests) {
    ...
    const fullHtml = `<!DOCTYPE html><html><head><meta charset="utf-8"><style>* { margin: 0; padding: 0; box-sizing: border-box; } body { background: #0d1117; overflow: hidden; }</style></head><body>${test.html}</body></html>`;
    ...
    const svgHtml = `<!DOCTYPE html><html><head><meta charset="utf-8"><style>* { margin: 0; } body { overflow: hidden; }</style></head><body><img src="file://${svgPath}" width="${w}" height="${h}" /></body></html>`;
    const svgHtmlPath = resolve(OUTPUT_DIR, `${test.name}-svg-render.html`);
    writeFileSync(svgHtmlPath, svgHtml);
  ...
  // Write a per-suite manifest the review tool (tests/review-server.ts) reads
  ...
  if (failed > 0) {
    ...
    console.log("\nReview tool: npx tsx tests/review-server.ts");
    process.exit(1);
  }
