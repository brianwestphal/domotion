...
describeBrowser("recursed iframe inner mask/clip/filter defs (DM-1446)", () => {
  it("hoists same-document mask / clip-path / filter defs defined inside the iframe", async () => {
    ...
    try {
      ...
      const unresolved = warnings.filter((w) => /did not resolve to an inline/.test(JSON.stringify(w)));
  ...
  it.each([1, 2])(
    "keeps iframe fragment identity, coordinate systems, and mask semantics exact at DPR %i",
    async (dpr) => {
      ...
      try {
        ...
        await source.waitForFunction(
          () => document.querySelector("iframe")?.contentDocument?.querySelectorAll(".case,.zoomcase").length === 8,
        );
        ...
        expect((root.maskDefs ?? []).find((def) => def.id === "objectContentMask")?.maskContentUnits).toBe(
          "objectBoundingBox",
        );
        expect((root.maskDefs ?? []).find((def) => def.id === "userRegionMask")?.userSpaceRegion).toEqual({
          x: 0,
          y: 0,
          width: 32,
          height: 60,
        ...
        const innerSharedMask = sharedMasks.find((def) =>
          def.dependencyGraph?.nodes.some((node) => node.id === "tone"),
        );
        ...
        expect(
          innerSharedMask?.dependencyGraph?.edges.every(
            (edge) => edge.from === 0 && edge.to === 1 && edge.target === "tone" && edge.status === "resolved",
          ),
        ).toBe(true);

        const scopedConsumers = flatten(tree).filter(
          (node) => /^url\(/.test(node.styles.clipPath) || /^url\(/.test(node.styles.maskImage),
        );
        ...
        await rendered.setContent(
          `<body style="margin:0"><svg xmlns="http://www.w3.org/2000/svg" width="${EXACT_WIDTH}" height="${EXACT_HEIGHT}" viewBox="0 0 ${EXACT_WIDTH} ${EXACT_HEIGHT}"><rect width="100%" height="100%" fill="white"/>${svg}</svg></body>`,
          { waitUntil: "load" },
        );
