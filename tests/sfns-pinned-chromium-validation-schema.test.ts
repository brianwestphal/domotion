...
const retained = JSON.parse(
  readFileSync(new URL("../.pr-notes/artifacts/dm2575-sfns-pinned-chromium-validation.json", import.meta.url), "utf8"),
...
function reseal(artifact: SfnsPinnedChromiumValidationArtifact): void {
  ...
  for (const control of artifact.controls) {
    control.changedEvidenceGroups = sfnsValidationChangedEvidenceGroups(baseline, control.observation);
  }
...
function selectedEvent<T>(observation: SfnsValidationObservation, event: SfnsHookEventName): T {
  const sequence =
    event === "shape"
      ? observation.selection.shapeSequence
      : event === "raw"
        ? observation.selection.rawSequence
        : event === "filtered"
          ? observation.selection.filteredSequence
          : event === "gamma"
            ? observation.selection.gammaSequence
            : event === "run"
              ? observation.selection.runSequence
...
describe("pinned-Chromium SFNS validation evidence", () => {
  it("retains 26 isolated explicitly-headless observations and authenticates scale cancellation", () => {
    ...
    expect(
      observations.every(
        (observation) =>
          observation.browser.explicitlyHeadless && observation.browser.launchArgs.includes("--headless=new"),
      ),
    ).toBe(true);
    const baseline = retained.scenarios.find((scenario) => scenario.id === "zoom-2")!.observations[0];
    const baselineRaw = baseline.events.find((event) => event.sequence === baseline.selection.rawSequence)!
      .payload as SfnsRawPayload;
    const baselineFiltered = baseline.events.find((event) => event.sequence === baseline.selection.filteredSequence)!
      .payload as SfnsFilteredPayload;
    expect([baselineRaw.rawRec.maskFormat, baselineFiltered.after.maskFormat]).toEqual(["LCD16", "A8"]);
    const cancellation = retained.scenarios.find((scenario) => scenario.id === "zoom-2-transform-half")!
      .observations[0];
    const filtered = cancellation.events.find((event) => event.sequence === cancellation.selection.filteredSequence)!
      .payload as SfnsFilteredPayload;
    expect(filtered.matrices.scale).toEqual([13, 13]);
    expect(firstMask(cancellation).glyph.metrics.maskFormat).toBe("A8");
    const surface = retained.controls.find((control) => control.id === "surface-mask-format")!.observation;
    const surfaceRaw = surface.events.find((event) => event.sequence === surface.selection.rawSequence)!
      .payload as SfnsRawPayload;
  ...
  it("rejects missing observations and dropped, duplicate, reordered, or stale events", () => {
    ...
    reseal(duplicate);
    expect(validateSfnsPinnedChromiumValidation(duplicate)).toContain("validation-zoom-2-cold-1:duplicate-sequence");
    ...
    [reorderedObservation.events[0], reorderedObservation.events[1]] = [
      reorderedObservation.events[1],
      reorderedObservation.events[0],
    ];
    reseal(reordered);
    expect(validateSfnsPinnedChromiumValidation(reordered)).toContain("validation-zoom-2-cold-1:event-reordered");

    const dropped = evidence();
    dropped.scenarios[0].observations[0].events.shift();
    reseal(dropped);
    expect(validateSfnsPinnedChromiumValidation(dropped)).toContain("validation-zoom-2-cold-1:event-dropped");

    const stale = evidence();
    stale.scenarios[0].observations[0].events[0].observationId = "prior-process-evidence";
    reseal(stale);
    expect(validateSfnsPinnedChromiumValidation(stale)).toContain("validation-zoom-2-cold-1:stale-envelope");
  });
  ...
  it("rejects source drift, non-headless launches, and corrupt exact mask bytes", () => {
    ...
    reseal(source);
    expect(validateSfnsPinnedChromiumValidation(source)).toContain("validation-zoom-2-cold-1:source-revision");
    ...
    visibleBrowser.launchArgs = visibleBrowser.launchArgs.filter((argument) => !argument.startsWith("--headless"));
    reseal(visible);
    expect(validateSfnsPinnedChromiumValidation(visible)).toContain("validation-zoom-2-cold-1:not-explicitly-headless");

    const corrupt = evidence();
    firstMask(corrupt.scenarios[0].observations[0]).glyph.mask.bytes = "AAAA";
    reseal(corrupt);
    expect(validateSfnsPinnedChromiumValidation(corrupt)).toEqual(
      expect.arrayContaining(["validation-zoom-2-cold-1:mask:0:byte-length", "validation-zoom-2-cold-1:mask:0:sha256"]),
    );
  });
  ...
  it("retains exact rec bytes while rejecting a mismatched process-local typeface id", () => {
    ...
    const raw = observation.events.find((event) => event.sequence === observation.selection.rawSequence)!
      .payload as SfnsRawPayload;
    ...
    reseal(artifact);
    expect(validateSfnsPinnedChromiumValidation(artifact)).toContain("validation-zoom-2-cold-1:rec-typeface-identity");
  });
  ...
  it.each([
    ...
    },
    {
      ...
      mutate: (artifact: SfnsPinnedChromiumValidationArtifact) => {
        artifact.scenarios[0].observations[0].events[0].typeface.fontBytes.sha256 = "0".repeat(64);
      },
    },
    {
    ...
    },
    {
      ...
      mutate: (artifact: SfnsPinnedChromiumValidationArtifact) => {
        const shape = selectedEvent<SfnsShapePayload>(artifact.scenarios[0].observations[0], "shape");
    ...
    },
    {
      ...
      mutate: (artifact: SfnsPinnedChromiumValidationArtifact) => {
        const run = selectedEvent<SfnsRunPayload>(artifact.scenarios[0].observations[0], "run");
    ...
    },
    {
      ...
      mutate: (artifact: SfnsPinnedChromiumValidationArtifact) => {
        const observation = artifact.controls.find((control) => control.id === "anti-aliasing")!.observation;
    ...
    },
    {
      ...
      mutate: (artifact: SfnsPinnedChromiumValidationArtifact) => {
        const observation = artifact.controls.find((control) => control.id === "hinting")!.observation;
    ...
    },
    {
      ...
      mutate: (artifact: SfnsPinnedChromiumValidationArtifact) => {
        selectedEvent<SfnsRawPayload>(artifact.scenarios[0].observations[0], "raw").deviceMatrix[2] += 0.25;
      },
    },
    {
      ...
      mutate: (artifact: SfnsPinnedChromiumValidationArtifact) => {
        const observation = artifact.controls.find((control) => control.id === "optical-size")!.observation;
        const sequence = observation.selection.runSequence;
        observation.events
          .find((event) => event.sequence === sequence)!
    ...
    },
    {
      ...
      mutate: (artifact: SfnsPinnedChromiumValidationArtifact) => {
        const observation = artifact.controls.find((control) => control.id === "surface-mask-format")!.observation;
        selectedEvent<SfnsRawPayload>(observation, "raw").surfaceProps.pixelGeometry = "rgb-h";
      },
    },
    {
      ...
      mutate: (artifact: SfnsPinnedChromiumValidationArtifact) => {
        const gamma = selectedEvent<SfnsGammaPayload>(artifact.scenarios[0].observations[0], "gamma");
  ...
  it("requires every control to move its owned exact evidence groups", () => {
    const artifact = evidence();
    const baseline = structuredClone(artifact.scenarios.find((scenario) => scenario.id === "zoom-2")!.observations[0]);
    ...
    reseal(artifact);
    expect(validateSfnsPinnedChromiumValidation(artifact)).toEqual(
      expect.arrayContaining(["subpixel-phase:inactive:phase", "subpixel-phase:inactive:mask"]),
    );
  });

  it("rejects a 26px cancellation claim and an unsealed artifact mutation", () => {
    const artifact = evidence();
    const cancellation = artifact.scenarios.find((scenario) => scenario.id === "zoom-2-transform-half")!;
    for (const observation of cancellation.observations) {
      const filtered = observation.events.find((event) => event.sequence === observation.selection.filteredSequence)!
        .payload as SfnsFilteredPayload;
    ...
    const baseline = geometry.scenarios.find((scenario) => scenario.id === "zoom-2")!.observations[0];
    const run = baseline.events.find((event) => event.sequence === baseline.selection.runSequence)!;
    (run.payload as { glyphs: Array<{ deviceOrigin: number[] }> }).glyphs[1].deviceOrigin[0] += 0.000_000_000_001;
    reseal(geometry);
    ...
    expect(validateSfnsPinnedChromiumValidation(unsealed)).toEqual(
      expect.arrayContaining(["corpus", "artifact-digest"]),
    );
  });
});
