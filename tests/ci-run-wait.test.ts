...
describe("waitForRunCompletion", () => {
  it("does not mistake a failed matrix job for a completed aggregate", async () => {
    ...
    await expect(waitForRunCompletion(inspect, { sleep, pollMs: 1, timeoutMs: 10_000 })).resolves.toEqual({
      status: "completed",
      conclusion: "failure",
    });
  ...
  it("fails with the last observed status after the deadline", async () => {
    ...
    try {
      await expect(
        waitForRunCompletion(async () => ({ status: "queued", conclusion: "" }), { sleep, pollMs: 50, timeoutMs: 100 }),
