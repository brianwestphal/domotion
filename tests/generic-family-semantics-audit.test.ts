...
function validOrders(target: GenericFamilyGatePlatform = "win32"): GenericFamilySemanticsOrder[] {
  return ["forward", "reverse"].map((order) => ({
    ...
    rows: (order === "forward" ? GENERIC_FAMILY_SEMANTICS_CASES : [...GENERIC_FAMILY_SEMANTICS_CASES].reverse()).map(
      (spec) => validRow(spec, order as GenericFamilySemanticsOrder["order"], target),
    ),
  }));
}
...
describe("exact generic-family semantic ownership gate", () => {
  it("keeps quoted commas and quoted generic-looking names literal", () => {
    expect(splitComputedFontFamily("\"A, B\", Courier, 'monospace'")).toEqual([
    ...
    expect(sourceDeclaredFamilyHeadIdentity('"monospace", Courier')).toBe("name:monospace");
  });
  ...
  it("models Blink's rightmost enum-bearing generic and non-occupying controls", () => {
    ...
    expect(blinkGenericFamilyFromComputedStack("monospace, system-ui, math")).toBe("monospace");
  ...
  it("transcribes each platform's candidate and terminal ownership order", () => {
    ...
    expect(sourcePlatformCandidateOrder("win32", "arabic", "monospace")[0]).toBe("courier new");
    expect(sourcePlatformCandidateOrder("win32", "arabic", "monospace")).toHaveLength(17);

    expect(sourceTerminalQuestionOrder("darwin", "monospace")).toEqual(["Times", "Lucida Grande"]);
    expect(sourceTerminalQuestionOrder("linux", "monospace")).toEqual(["monospace", "Sans", "Arial", "<unnamed>"]);
    expect(sourceTerminalQuestionOrder("win32", "none")).toEqual(
      expect.arrayContaining(["MS UI Gothic", "Courier New", "<unnamed>"]),
    );
    expect(sourceTerminalOwnerOrder("darwin", "monospace")).toEqual(["times", "lucida-grande"]);
    expect(sourceTerminalOwnerOrder("linux", "none")).toEqual(["helvetica"]);
    expect(sourceTerminalOwnerOrder("linux", "monospace")).toEqual(["courier", "helvetica"]);
    expect(sourceTerminalOwnerOrder("win32", "serif")).toEqual(["times", "arial"]);
  });
  ...
  it("keeps the production carrier and terminal adapter source-exact on all platforms", () => {
    for (const row of GENERIC_FAMILY_SEMANTICS_CASES) {
      ...
      expect(productionGeneric, row.id).toBe(blinkGenericFamilyFromComputedStack(row.fontFamily));
      const context = createFontFallbackSemanticContext(row.fontFamily);
      for (const target of ["darwin", "linux", "win32"] as const) {
        expect(skiaLastResortFamilyQuestionOrder(productionGeneric, target), `${target}:${row.id}`).toEqual(
          sourceTerminalQuestionOrder(target, productionGeneric),
        );
        expect(__skiaLastResortKeysForTest(context, target), `${target}:${row.id}`).toEqual(
          sourceTerminalOwnerOrder(target, productionGeneric),
        );
      }
    }
  });
  ...
  it("matches independent and production cache identity components", () => {
    ...
    expect(sourceSemanticCacheIdentity("linux", "Courier", "none")).toBe("name:courier|initial:<unnamed>");
  ...
  it.each([
    ...
    [
      ...
      (row: GenericFamilySemanticsRow) => ({ ...row, productionCandidateOrder: ["Wrong"] }),
    ],
    [
      "source-terminal-question-order",
      (row: GenericFamilySemanticsRow) => ({ ...row, sourceTerminalQuestionOrder: ["Wrong"] }),
    ],
    [
      "production-terminal-question-order",
      (row: GenericFamilySemanticsRow) => ({ ...row, productionTerminalQuestionOrder: ["Wrong"] }),
    ],
    [
      "production-terminal-owner-order",
      (row: GenericFamilySemanticsRow) => ({ ...row, productionTerminalOwnerOrder: ["wrong"] }),
    ],
    [
      "production-terminal-activation",
      (row: GenericFamilySemanticsRow) => ({ ...row, productionTerminalActivated: !row.productionTerminalActivated }),
    ],
    ...
    [
      "key-derived-loss-classification",
      (row: GenericFamilySemanticsRow) => ({ ...row, keyDerivedSemanticMismatch: !row.keyDerivedSemanticMismatch }),
    ],
  ...
  it("requires a complete strict matrix with every semantic discriminator active", () => {
    ...
    expect(classifyGenericFamilySemanticsEvidence(validOrders(), false).verdict).toBe("source-drift");
    expect(classifyGenericFamilySemanticsEvidence([validOrders()[0]], true).verdict).toBe("invalid-evidence");
  });
  ...
  it("clears once before both orders and accepts only source-exact at the CLI", () => {
    ...
    expect(source.indexOf("clearFontResolutionCaches()")).toBeLessThan(
      source.indexOf('for (const order of ["forward", "reverse"]'),
    );
