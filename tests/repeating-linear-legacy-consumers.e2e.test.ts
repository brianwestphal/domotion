...
describeBrowser("legacy border gradient consumer repeat domains", () => {
  for (const dpr of [1, 2])
    it(`matches Chromium logical interiors at DPR ${dpr}`, async () => {
      ...
      try {
        await source.setContent(
          '<style>html,body{margin:0;background:white}#x{margin:20px;width:180px;height:100px;border:20px solid transparent;border-image:repeating-linear-gradient(90deg,#e11d48 0,#2563eb 240px) 20 fill}</style><div id="x"></div>',
        );
