  ...
  return adjudicateNativePaletteRow(
    {
      id: "named-base2",
      dpr: 1,
      computedFontPalette: "--base2",
      cssomRule: { name: "--base2", fontFamily: "DomotionPaletteAudit", basePalette: "2", overrideColors: "" },
      sourceGlyphId: 35,
      paintedFamilyDisplayName: "Ahem",
      paintedPostscriptDisplayName: "Ahem",
      isCustomFont: true,
      paintedGlyphCount: 1,
...
function order(first: "base2" | "base3", captured: string): ProductionOrderEvidence {
  ...
  return {
    order: [first, second],
    sourcePngSha256: source,
    sourceOpaqueColors: [
      ["red", "magenta"],
      ["cyan", "green"],
    ],
    capturedPngSha256: [captured, captured],
    capturedPngCount: 2,
    capturedUniquePngCount: 1,
    svgImageCount: 2,
    captureHasFontPaletteFact: false,
...
describe("DM-2350 font-palette ownership audit", () => {
  it("pins and decodes the source CPAL/COLR identity", () => {
    ...
    expect(FONT_PALETTE_SOURCE_PINS).toEqual(
    ...
    expect(
      expectedPaletteColors(
        facts,
        FONT_PALETTE_CASES.find((row) => row.id === "named-base2")!,
      ),
    ).toEqual(["255,0,0,255", "255,0,255,255"]);
    expect(
      expectedPaletteColors(
        facts,
        FONT_PALETTE_CASES.find((row) => row.id === "named-base3")!,
      ),
    ).toEqual(["0,255,0,255", "0,255,255,255"]);
  });

  it.each(["computed", "source-gid", "custom", "glyph", "colors", "inert", "rule", "rule-family"])(
    "rejects a hostile %s mutation",
    (kind) => {
      const row = nativeRow();
      const input = {
        ...row,
        cssomRule: row.cssomRule == null ? null : { ...row.cssomRule },
        observedColorCounts: { ...row.observedColorCounts },
      };
  ...
  it("accepts only one-level native antialias fringes around exact palette colors", () => {
    ...
    expect(opaquePaletteColorsWithinTolerance([...expected, "255,255,1,255", "255,255,254,255"], expected)).toBe(true);
  ...
  it("classifies order-dependent first-raster reuse as a confirmed gap", () => {
    ...
    expect(classifyPaletteAudit(rows, [order("base2", sha("a")), order("base3", sha("b"))], [1])).toBe(
      "confirmed-palette-identity-gap",
    );
    const inert = order("base3", sha("a"));
    expect(classifyPaletteAudit(rows, [order("base2", sha("a")), inert], [1])).toBe("inconclusive");
    const bad = nativeRow();
    bad.pass = false;
    expect(classifyPaletteAudit([bad], [order("base2", sha("a")), order("base3", sha("b"))], [1])).toBe(
      "invalid-evidence",
    );
  });
});
