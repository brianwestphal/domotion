...
import type { AuthenticatedFontFeatureEnvironment } from "../tools/font-feature-value-environment.js";
import { resolvedFeaturesForRun, shapingProbePageHtml, type RunSpec } from "../tools/shaping-conformance.js";
    ...
    faces: [
      ...
      },
      {
        ...
        sources: [
          ...
          {
            kind: "remote",
            cssText: "url(https://fixture.invalid/font.otf)",
            sourceOrder: 1,
            status: "loaded",
            bytesBase64: base64,
            sha256,
...
function spec(documentId = "document-a"): RunSpec {
  return {
    text: "Xnophijklmqrstuvwxyz",
    fontFamily: `"${family}"`,
    fontSize: 32,
    fontWeight: 700,
    fontStyle: "normal",
    fontVariantAlternates: "stylistic(fancy)",
    resolvedFontFeatures: ["salt=1"],
    fontFeatureEnvironment: environment(documentId),
    fixtures: 1,
    example: "dm2545",
  };
}
...
describe("multi-face font-feature-values exact shaping", () => {
  ...
  it("emits only authenticated selected bytes and partitions distinct documents", () => {
    ...
    expect(() => shapingProbePageHtml([spec("document-a"), spec("document-b")])).toThrow(/mixes distinct/);
  });
