...
describeBrowser("pixel content at Chromium-owned raster boundaries", () => {
  ...
  it("captures replaced canvas pixels at the used size with live frame colors", async () => {
    ...
    try {
      ...
      await page.locator("#c").evaluate((canvas) => {
        ...
        x.fillStyle = "#e22";
        x.fillRect(0, 0, 60, 80);
        x.fillStyle = "#25c";
        x.fillRect(60, 0, 60, 80);
      });
  ...
  it("captures native-control chrome with authoritative geometry and nonempty alpha", async () => {
    ...
    try {
      await page.setContent(
        `<input id="native" type="checkbox" checked style="margin:20px;width:22px;height:22px;accent-color:#d21f5b">`,
      );
  ...
  it("captures image-replacement sprite pixels without absorbing a vector sibling", async () => {
    ...
    try {
      await page.setContent(
        `<div id="sprite" aria-label="icon" style="position:absolute;left:20px;top:20px;width:48px;height:36px;text-indent:-9999px;overflow:hidden;white-space:nowrap;background:linear-gradient(90deg,#e22,#25c)">icon</div><div id="vector" style="position:absolute;left:90px;top:20px;width:40px;height:36px;background:#2c5"></div>`,
      );
      ...
      // and crop are exact.
      const sprite =
        spriteNode?.replacedSnapshot?.dataUri != null
      ...
      expect(
        nodes.some(
          (node) =>
            ...
            node.styles.backgroundColor === "rgb(34, 204, 85)",
        ),
      ).toBe(true);
    } finally {
      await page.close();
  ...
  it("captures unreadable iframe pixels while preserving its used box", async () => {
    ...
    try {
      await page.setContent(
        `<iframe id="frame" src="data:text/html,<style>html,body{margin:0;width:100%;height:100%;background:linear-gradient(90deg,%23e22,%2325c)}</style>" style="border:0;width:100px;height:60px"></iframe>`,
      );
  ...
  it("captures projective subtree pixels instead of flattening them to an affine SVG", async () => {
    ...
    try {
      await page.setContent(
        `<div style="margin:30px;width:150px;height:110px;perspective:260px"><div style="width:120px;height:90px;background:linear-gradient(90deg,#e22 0 50%,#25c 50%);transform:rotateY(34deg)"></div></div>`,
      );
  ...
  it("captures color-glyph pixels through the selected outline or bitmap ownership route", async () => {
    ...
    try {
      ...
      const segments = walk(tree).flatMap(
        (node) => node.textPaintGeometry?.neutral?.textSegments ?? node.textSegments ?? [],
      );
      ...
      if (rasterSegments.length === 1) {
        expect(rasterSegments[0].colorGlyphIdentities?.some((identity) => identity.representation === "bitmap")).toBe(
          true,
        );
  ...
  it("replaces stale advanced-gradient cache content with Chromium pixels at the physical tile size", async () => {
    ...
    try {
      await page.setContent(
        `<div id="g" style="width:120px;height:90px;background:linear-gradient(90deg in oklab,#e22,#25c);background-size:40px 30px"></div>`,
      );
