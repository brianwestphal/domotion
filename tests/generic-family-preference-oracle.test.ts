...
const rows = (): BlinkPreferenceRow[] =>
  logicalProbeTargets().map((target, index) => ({
    ...target,
...
describe("DM-2351 generic preference oracle contract", () => {
  it("defaults local automation to headless modes and requires explicit headed opt-in", () => {
    ...
    expect(() => selectedLaunchModeIds(["--modes=pinned-headed"])).toThrow(
  ...
  it("crosses Common and ten settings scripts with system-ui and quoted-literal controls", () => {
    ...
    expect(
      targets
        .filter((target) => target.generic === "system-ui")
        .every((target) => target.text === "Regna" || target.text === "A"),
    ).toBe(true);
    expect(targets.filter((target) => target.generic === "quoted-serif")).toHaveLength(11);
    expect(new Set(targets.map((target) => `${target.script ?? "COMMON"}/${target.generic}`)).size).toBe(
      targets.length,
    );
  });
  ...
  it("derives every mutation from observed faces instead of an OS snapshot table", () => {
    ...
    for (const row of source.filter((item) => item.generic !== "system-ui" && item.generic !== "quoted-serif")) {
      expect(plan.expectedFaceByTarget[`${row.script ?? "COMMON"}/${row.generic}`]).not.toBe(row.postScriptName);
    }
  });

  it("derives each script mutation only from faces proven to paint that script", () => {
    const source = rows();
    const hebrew = source.filter(
      (row) => row.script === "HEBREW" && row.generic !== "system-ui" && row.generic !== "quoted-serif",
    );
    ...
    expect(
      Object.entries(plan.expectedFaceByTarget)
        .filter(([key]) => key.startsWith("HEBREW/"))
        .every(([, value]) => value.startsWith("Hebrew")),
    ).toBe(true);
  });
  ...
  it("grades a single-face script's default route without inventing an inert mutation", () => {
    ...
    for (const row of source.filter(
      (item) => item.script === "DEVANAGARI" && item.generic !== "system-ui" && item.generic !== "quoted-serif",
    )) {
    ...
    expect(Object.keys(plan.expectedFaceByTarget).some((key) => key.startsWith("DEVANAGARI/"))).toBe(false);
  });
});
