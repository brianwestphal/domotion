...
const runs: CorpusRun[] = Array.from({ length: 100 }, (_, i) => ({
  text: `run-${i}`,
  fontFamily: i % 2 ? "A" : "B",
  fontSize: 16,
}));
describe("Unicode shaping corpus orchestration", () => {
  ...
  it("uses a stable bounded representative population before sharding", () => {
    ...
    expect(new Set(selected.map(runIdentity))).toEqual(
      new Set(
        [...runs]
          .sort((a, b) => runIdentity(a).localeCompare(runIdentity(b)))
          .slice(0, 17)
          .map(runIdentity),
      ),
    );
  ...
  it("reduces repeated logical signatures to one ticket-sized example", () => {
    expect(
      reduceMismatches([
        { verdict: "mismatch-count", fontFamily: "A", chromeGlyphs: 2, ourGlyphs: 1, maxDelta: null, text: "x" },
        { verdict: "mismatch-count", fontFamily: "A", chromeGlyphs: 2, ourGlyphs: 1, maxDelta: null, text: "y" },
      ])[0].count,
    ).toBe(2);
  });
});
