...
import { blinkUsesTextCombine, resolveVerticalOrientations } from "../src/vertical-orientation.js";
  ...
  it("retains exact Range placement and source orientation across the representative corpus", async () => {
    ...
    try {
      ...
      await page.setContent(
        `<style>html,body{margin:0}.case{}div{font:32px/1.25 Arial,sans-serif;color:#111}</style>${markup}`,
      );

      const live = await page.evaluate(
        (ids): Promise<Record<string, LiveFixture>> => {
      ...
      for (const row of fixtures) {
        ...
        if (combine) {
          expect(segment.verticalCombineXOffsets, `${row.id} combine placement`).toEqual(
            source.points.map((point) => point.x - minX),
          );
          ...
          expect(segment.verticalOrientations, `${row.id} orientation array`).toEqual(
            resolveVerticalOrientations(source.text, effective),
          );
          expect(segment.yOffsets, `${row.id} y placement`).toEqual(repeated(source.points, (point) => point.y));
          expect(segment.verticalAdvances, `${row.id} captured advances`).toEqual(
            repeated(source.points, (point) => point.height),
          );
        }
      }
    } finally {
