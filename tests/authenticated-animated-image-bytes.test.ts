...
const logical: Omit<AuthenticatedAnimatedImageByteRecord, "epochDigest"> = {
  ...
  ownerSlotIndex: null,
  ownerSerializedValue: "https://example.test/image.gif",
  backendNodeId: 10,
  frameId: "frame",
  documentLoaderId: "loader",
  documentNonce: "nonce",
  selectedUrl: "https://example.test/image.gif",
  currentSrc: "https://example.test/image.gif",
  devicePixelRatio: 1,
  viewport: { width: 800, height: 600 },
  requestId: "request",
  redirectHops: [],
  responseUrl: "https://example.test/image.gif",
  responseStatus: 200,
  mimeType: "image/gif",
...
describe("authenticated animated-image exact comparator", () => {
  ...
  it.each([
    [
      "logical drift",
      (value: AuthenticatedAnimatedImageByteRecord) => {
        value.responseStatus = 304;
      },
      "response-drift",
    ],
    [
      "length drift",
      (_value: AuthenticatedAnimatedImageByteRecord) => undefined,
    ...
    const error = (() => {
      try {
        verifyAuthenticatedAnimatedImageBytes(candidate, supplied);
      } catch (value) {
        return value;
      }
    })();
