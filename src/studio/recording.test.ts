...
describe("Studio real-interaction recording import (DM-2696)", () => {
    ...
    const result = await importStudioInteractionRecording(project, recording(), {
      ai: {
        heal: async ({ aiPolicy, recording: raw }) => {
          ...
          return {
            kind: "clarify",
            question: "Should the redacted field be replayed?",
            reason: "The value was intentionally removed.",
            evidence: { summary: "One sensitive input is ambiguous." },
          };
        },
        review,
      },
  ...
  it("requires AI healing and review, emits a normal validated scene, and persists only redacted evidence", async () => {
    ...
    try {
      ...
      const heal = vi.fn(async () => ({
        ...
        scene: importedScene(),
        summary: "Collapsed raw keystrokes and pointer noise into two semantic actions.",
        evidence: { summary: "Used accessible labels and a stable test id." },
      }));
      const review = vi.fn(async ({ candidate }) => ({
        kind: "accept" as const,
        summary: `Reviewed ${candidate.tracks?.[0].events.length} semantic actions.`,
        evidence: { summary: "Targets and pacing are unambiguous." },
      }));
      const result = await importStudioInteractionRecording(project, recording(), {
        ai: { heal, review },
      ...
      expect(result.project.review.revisions.at(-1)).toMatchObject({
        author: { kind: "ai" },
        kind: "content",
        metadata: { operation: "studio.recording.import" },
      });
      expect(result.project.artifacts.at(-1)).toMatchObject({
        kind: "capture-evidence",
        sceneIds: [result.scene.id],
        sourceRevisionId: result.project.review.headRevisionId,
  ...
  it("rejects AI output that is replay-only or reuses an existing scene identity", async () => {
    ...
    await expect(
      importStudioInteractionRecording(project, recording(), {
        ai: {
          heal: async () => ({
            kind: "candidate",
            scene: duplicate,
