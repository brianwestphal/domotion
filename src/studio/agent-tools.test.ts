...
describe("Studio agent tools (DM-2693)", () => {
  ...
  it("pins the protocol version and reuses canonical Studio scene/track validation", () => {
    ...
    expect(
      studioAgentToolRequestSchema.safeParse({
        version: 2,
        tool: "project.create",
  ...
  it("supports an agent create → capture → preview → annotate → revise workflow", async () => {
    ...
    const options: RunStudioAgentToolOptions = {
      ...
      capture: async ({ project, absolutePath, workspacePath, selection, aiPolicy }) => {
        ...
        writeFileSync(
          evidencePath,
          JSON.stringify({ selector: "button", computedStyle: { color: "rgb(255, 255, 255)" } }),
        );
    ...
    const created = await runStudioAgentTool(
      null,
      {
        version: STUDIO_AGENT_TOOL_VERSION,
        tool: "project.create",
    ...
    const authored = await runStudioAgentTool(
      project,
      {
        version: STUDIO_AGENT_TOOL_VERSION,
        tool: "project.edit",
          ...
          scenes: [
            {
              ...
              render: {
                kind: "storyboard",
                recipe: { capture: { url: "https://example.test/checkout", selector: "main" }, duration: 1800 },
              },
              tracks: [
                {
                  id: "track-checkout",
    ...
    const captured = await runStudioAgentTool(
      project,
      {
        version: STUDIO_AGENT_TOOL_VERSION,
    ...
    const previewed = await runStudioAgentTool(
      project,
      {
        version: STUDIO_AGENT_TOOL_VERSION,
        tool: "render.preview",
    ...
    expect(previewed.artifacts?.[0]).toMatchObject({
      path: join(workspace, "artifacts/preview.svg"),
      workspacePath: "artifacts/preview.svg",
    });

    const annotated = await runStudioAgentTool(
      project,
      {
        version: STUDIO_AGENT_TOOL_VERSION,
            ...
            regions: [
              {
                ...
                width: 180,
                height: 48,
                coordinateSpace: "artifact-pixels",
    ...
    expect(project.review.annotations[0]).toMatchObject({
      author: { kind: "ai", name: "Demo Director" },
      status: "open",
    });

    const revised = await runStudioAgentTool(
      project,
      {
        version: STUDIO_AGENT_TOOL_VERSION,
        tool: "project.edit",
    ...
    const inspected = await runStudioAgentTool(
      project,
      { version: STUDIO_AGENT_TOOL_VERSION, tool: "project.inspect", include: "annotations" },
      options,
    );
  ...
  it("returns explicit clarification, conflict, and bounded permission states", async () => {
    ...
    const baseOptions: RunStudioAgentToolOptions = {
      ...
      timestamp: () => T0,
      preview: async () => {
        throw new Error("ambiguous request must not call the adapter");
      },
    };
    let project = projectFrom(
      await runStudioAgentTool(
        null,
        {
          version: STUDIO_AGENT_TOOL_VERSION,
          tool: "project.create",
            ...
            addScenes: [
              {
                ...
                render: {
                  kind: "storyboard",
                  recipe: { template: "title-card", params: { title: "Second" }, duration: 1000 },
                },
              },
            ],
          },
    ...
    const clarification = await runStudioAgentTool(
      project,
      {
        version: STUDIO_AGENT_TOOL_VERSION,
        tool: "render.preview",
        outputPath: "preview.svg",
      },
      baseOptions,
    );
    expect(clarification).toMatchObject({
      status: "clarification",
      clarification: { choices: ["complete project", "scene-opening", "scene-second"] },
    });

    const stale = await runStudioAgentTool(
      project,
      {
        version: STUDIO_AGENT_TOOL_VERSION,
        tool: "project.edit",
    ...
    const unconfirmed = await runStudioAgentTool(
      project,
      {
        version: STUDIO_AGENT_TOOL_VERSION,
        tool: "project.edit",
    ...
    expect(unconfirmed).toMatchObject({
      ...
      permission: { name: "destructiveProjectEdits", destructiveIds: ["scene-second"] },
    });

    const denied = await runStudioAgentTool(
      project,
      {
        version: STUDIO_AGENT_TOOL_VERSION,
        tool: "project.edit",
  ...
  it("keeps artifact writes inside the workspace and makes required AI video review observable", async () => {
    ...
    const review = await runStudioAgentTool(
      project,
      {
        version: STUDIO_AGENT_TOOL_VERSION,
        tool: "render.video",
    ...
    expect(review).toMatchObject({
      status: "clarification",
      data: { inspected: ["dom", "computed-css", "rendered-frame"] },
    });
    ...
    await expect(
      runStudioAgentTool(
        ...
        { ...options, preview: async () => ({ artifacts: [] }) },
      ),
    ).rejects.toThrow(StudioAgentToolError);
  });
});
