...
export interface StudioObservedRect {
  x: number;
  y: number;
  width: number;
  height: number;
...
function classifyRawObservation(raw: RawObservation, eventId: string, path: string): StudioInteractionEvidence {
  ...
  const actionMutationRefs = new Set(
    actionMutations.flatMap((item) => [item.targetRef, ...(item.addedRefs ?? []), ...(item.removedRefs ?? [])]),
  );
  const ambientActionRefs = new Set(
    actionMutations
  ...
  for (const ref of refs) {
    ...
    const pseudoDeltas = [...new Set([...Object.keys(a?.pseudos ?? {}), ...Object.keys(b?.pseudos ?? {})])]
      .sort()
      .flatMap((pseudo) =>
        deltaRecord(a?.pseudos[pseudo] ?? {}, b?.pseudos[pseudo] ?? {}).map((delta) => ({ pseudo, ...delta })),
      );
    ...
    const hasRenderedOrDocumentEffect =
      addedOrRemoved ||
      styleDeltas.length > 0 ||
      pseudoDeltas.length > 0 ||
      geometryChanged ||
    ...
    const classification: StudioEvidenceClassification =
      !hasRenderedOrDocumentEffect || ambient.has(ref) || ambientActionRefs.has(ref)
        ? "incidental-churn"
        : direct
          ? "direct-feedback"
          : "action-effect";
  ...
  const targetSnapshots = (items: StudioObservedElement[]): ElementStyleSnapshot[] =>
    items
    ...
    const next = mutations
      ...
      .find(
        (candidate) =>
          ...
          candidate.targetRef === current.targetRef &&
  ...
  const meaningfulSignal = signals.some(
    (item) => item.phase === "action" && /^(focusin|focusout|transition|animation|scroll)/.test(item.type),
  );
  return {
    version: 1,
    eventId,
    ...
    async ({ target, related, options, key, properties }) => {
      ...
      const snapshotElement = (element: Element): StudioObservedElement => {
        ...
        for (const property of properties)
          styles[property] = (style as unknown as Record<string, string>)[property] ?? "";
        ...
        const animations = element
          ...
          .sort((a, b) =>
            `${a.type}:${a.name ?? ""}:${a.property ?? ""}`.localeCompare(
              `${b.type}:${b.name ?? ""}:${b.property ?? ""}`,
            ),
          );
        return {
          ref: refFor(element),
          ...
          text: (element.childNodes.length === 1 && element.firstChild?.nodeType === Node.TEXT_NODE
            ? (element.textContent ?? "")
            : ""
          ).slice(0, 240),
          ...
          animations,
          scroll: {
            x: round(element.scrollLeft),
            y: round(element.scrollTop),
      ...
      const recordSignal = (type: string, node: EventTarget | null): void => {
        ...
        signals.push({
          ...
          type,
          targetRef: ref,
          ...(element instanceof Element ? { sample: snapshotElement(element) } : {}),
        });
        lastSignalAt = performance.now();
      };
      const consume = (records: MutationRecord[]): void => {
        for (const record of records) {
          if (record.type === "attributes" && record.attributeName != null && ignored.has(record.attributeName))
            continue;
          ...
          const addedRefs =
            record.type === "childList" ? [...record.addedNodes].map((node) => refFor(node)) : undefined;
          const removedRefs =
            record.type === "childList" ? [...record.removedNodes].map((node) => refFor(node)) : undefined;
          mutations.push({
            sequence: sequence++,
            phase,
            atMs: stamp(),
            ...
            ...(record.type === "attributes"
              ? {
                  ...
                  newValue: (record.target as Element).getAttribute(record.attributeName ?? ""),
                }
              : {}),
            ...(record.type === "characterData"
              ? { oldValue: record.oldValue, newValue: record.target.textContent }
              : {}),
      ...
      root[key] = {
        ...
        eventTypes,
        listener,
        getLastSignalAt: () => lastSignalAt,
        getTruncated: () => truncated,
  ...
  return page.evaluate(
    async ({ key, settleMs, debounceMs }) => {
      ...
      while (performance.now() - state.getLastSignalAt() < debounceMs) {
        if (performance.now() - waitStarted >= settleMs) {
...
export async function observeStudioInteraction(
  ...
  if (
    ![settleMs, debounceMs, baselineMs, maxNodes].every(Number.isFinite) ||
    settleMs <= 0 ||
    debounceMs < 0 ||
    baselineMs < 0 ||
  ...
  try {
    ...
    const evidence = classifyRawObservation(
      await finishObservation(page, settleMs, debounceMs),
      options.eventId,
      options.path,
    );
    if (actionError != null) {
      throw new StudioInteractionObservationError(
        `Studio interaction ${options.path} (${options.eventId}) failed while evidence was captured`,
        evidence,
        { cause: actionError },
      );
    }
    ...
    await page
      .evaluate((key) => {
        const root = globalThis as unknown as Record<string, unknown>;
        const state = root[key] as
          { observers?: MutationObserver[]; eventTypes?: string[]; listener?: EventListener } | undefined;
        for (const observer of state?.observers ?? []) observer.disconnect();
...
export async function observeStudioSemanticStep(
  ...
  const observesAttachmentLifecycle =
    event.kind === "waitForState" && (event.state === "attached" || event.state === "detached");
  ...
  return observeStudioInteraction(
    page,
    { ...options, eventId: event.id, path: step.path, target, relatedTargets },
    action,
  );
}
