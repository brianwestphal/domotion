...
describe("Studio unified review annotations (DM-2690)", () => {
  ...
  it("uses the same command API for a fully grounded AI finding", () => {
    ...
    expect(parseStudioProjectJson(serializeStudioProject(result.project)).review.annotations[0]).toEqual(
      result.annotation,
    );
  });

  it("records recoverable edit/status provenance and enforces lifecycle + stale-head rules", () => {
    const created = applyStudioAnnotationCommand(
      sparseProject(),
      {
        kind: "create",
        body: "First wording",
    ...
    const edited = applyStudioAnnotationCommand(
      created.project,
      {
        kind: "edit",
    ...
    const resolved = applyStudioAnnotationCommand(
      edited.project,
      {
        kind: "set-status",
        annotationId: "annotation-one",
    ...
    expect(resolved.annotation).toMatchObject({
      status: "resolved",
      statusRevisionId: "revision-resolve",
      resolvedRevisionId: "revision-resolve",
    });
    const reopened = applyStudioAnnotationCommand(
      resolved.project,
      {
        kind: "set-status",
        annotationId: "annotation-one",
    ...
    const superseded = applyStudioAnnotationCommand(
      reopened.project,
      {
        kind: "set-status",
        annotationId: "annotation-one",
    ...
    expect(() =>
      applyStudioAnnotationCommand(superseded.project, {
        kind: "set-status",
        annotationId: "annotation-one",
        author: { kind: "human" },
        status: "open",
    ...
    expect(() =>
      applyStudioAnnotationCommand(
        ...
        { expectedHeadRevisionId: "revision-initial" },
      ),
    ).toThrow(/stale annotation change/);
    expect(() =>
      applyStudioAnnotationCommand(created.project, {
        kind: "edit",
        annotationId: "annotation-one",
        author: { kind: "human" },
        body: "First wording",
  ...
  it("losslessly migrates modern and legacy SVG Scrubber review tickets", () => {
    const modern = importSvgScrubberReviewAnnotation(
      ...
      },
      {
        ...
        sourceReference: "imports/logo.svg",
        frameReference: "imports/logo.png",
      },
    );
    ...
    expect(() =>
      importSvgScrubberReviewAnnotation(
        ...
        { author: { kind: "human" }, sourceReference: "/private/source.svg" },
      ),
