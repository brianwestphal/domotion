...
describe("Studio interaction observation boundary", () => {
  it.each([
    ...
    ).rejects.toThrow("Studio interaction observation timing must be finite and maxNodes must be a positive integer");
  ...
  it("fails clearly when the resolved target is detached", async () => {
    ...
    ).rejects.toThrow("Studio interaction observation $.events.missing: target is not attached");
  });
