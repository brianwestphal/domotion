    ...
    try {
      ...
      const heal = vi.fn(
        async ({
          project,
          failure,
        }: Parameters<Parameters<typeof runStudioHealingLoop>[2]["heal"]>[0]): Promise<StudioAiHealingDecision> => {
          ...
          return {
            ...
            evidence: {
              summary: "Chromium exposes Start tour at the control's new location.",
              data: { ariaSnapshot: failure.inspection?.ariaSnapshot ?? "", rect: currentControl?.rect ?? null },
            },
          };
        },
      );
      const review = vi.fn(async () => ({
        ...
        evidence: { summary: "The repaired flow reaches Ready and the SVG is self-contained." },
      }));
      ...
      expect(result.project.review.revisions[2]).toMatchObject({
        ...
        metadata: {
          automation: {
            phase: "heal",
            changes: [
              { path: "$.scenes[0].tracks[0].events[1].target.name", before: "Launch tour", after: "Start tour" },
            ],
          },
        },
      });
      ...
      expect(
        result.project.artifacts.every(
          (artifact) => artifact.sourceRevisionId === result.project.review.headRevisionId,
        ),
      ).toBe(true);
    ...
    try {
      ...
      const first = await runStudioHealingLoop(browser, ambiguous, {
        ...
        heal: async ({ failure }) => {
          ...
          return {
            kind: "clarify",
            question: "Which Start action expresses the story intent?",
            reason: "Two exact button candidates remain.",
            evidence: { summary: "Chromium exposes Start tour and Start settings." },
          };
        },
        review: async () => {
          throw new Error("review should wait for a successful replay");
        },
      });
      ...
      const resumed = await resumeStudioHealingLoop(browser, first.checkpoint, "Start tour", {
        ...
        heal: async ({ project, clarification }) => {
          ...
          return {
            kind: "edit",
            project: edited,
            summary: "Applied the clarified control.",
            evidence: { summary: "Human selected Start tour." },
          };
        },
        review: async () => ({
          kind: "accept",
          summary: "Ready.",
          evidence: { summary: "Replay passed after clarification." },
        }),
      });
      ...
      const unrecoverable = await runStudioHealingLoop(browser, missing, {
        ...
        heal: async () => ({
          kind: "unrecoverable",
          reason: "The authored intent has no safe equivalent.",
          evidence: { summary: "No destructive action exists in the current application." },
        }),
        review: async () => {
