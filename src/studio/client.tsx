  ...
  bootstrap.project == null
    ...
    : {
        ...
        scenes: bootstrap.project.scenes.map((scene) => ({
          id: scene.id,
          generated: bootstrap.project!.artifacts.some(
            (artifact) =>
              artifact.sourceRevisionId === studioContentRevisionId(bootstrap.project!) &&
              artifact.sceneIds?.includes(scene.id) === true,
          ),
        })),
      },
);
...
function sourceLabel(scene: StudioScene): string {
  if (scene.render.kind === "composition")
    return `Composition · ${scene.render.composition.layers.length} layer${scene.render.composition.layers.length === 1 ? "" : "s"}`;
...
function scenePresentation(
  scene: StudioScene,
): Pick<StoryboardRecipe, "duration" | "trimStart" | "trimEnd" | "fit" | "transition"> {
...
function replaceSource(scene: StudioScene, kind: SceneSourceKind, value: string): StoryboardRecipe {
  ...
  if (kind === "url")
    return {
      capture: { url: value, ...(previous.capture?.selector == null ? {} : { selector: previous.capture.selector }) },
      ...presentation,
    };
  if (kind === "file")
    return {
      capture: { file: value, ...(previous.capture?.selector == null ? {} : { selector: previous.capture.selector }) },
      ...presentation,
    };
...
function cinematicPreset(scene: StudioScene): string {
  ...
  if (
    treatments.length === 1 &&
    ["browser-chrome", "device-frame", "zoom-pan", "spotlight", "title-card"].includes(treatments[0].kind)
  )
    return treatments[0].kind;
  return "custom";
}
    ...
    return [
      {
        ...
        transform: {
          ...
          to: { x: 0, y: 0, scale: 1.18 },
          origin: { x: current.canvas.width / 2, y: current.canvas.height / 2 },
        },
      },
    ];
  if (preset === "spotlight")
    return [
      {
        ...
        mask: {
          region: {
            x: current.canvas.width * 0.25,
            y: current.canvas.height * 0.25,
            width: current.canvas.width * 0.5,
            height: current.canvas.height * 0.5,
            radius: 18,
          },
        },
        color: "#000000",
...
function updateSceneRecipe(scene: StudioScene, patch: Partial<StoryboardRecipe>): StudioAuthoringCommand {
  return {
    kind: "scene.update",
    sceneId: scene.id,
    patch: { render: { kind: "storyboard", recipe: { ...storyboardRecipe(scene), ...patch } } },
  };
}

function updateScenePresentation(
  scene: StudioScene,
  patch: Partial<Pick<StoryboardRecipe, "duration" | "trimStart" | "trimEnd" | "fit" | "transition">>,
...
function applyAuthoring(command: StudioAuthoringCommand): void {
  ...
  try {
    ...
    generation.value = {
      ...
      scenes: result.project.scenes.map((scene) => ({ id: scene.id, generated: false })),
    };
    issues.value = [];
    message.value = "";
  } catch (error) {
...
function hasScenePreview(sceneId: string): boolean {
  ...
  return project.value.artifacts.some(
    (artifact) =>
      artifact.kind === "svg" &&
      artifact.sourceRevisionId === revision &&
      artifact.sceneIds?.length === 1 &&
      artifact.sceneIds[0] === sceneId,
  );
}
...
function annotationTargetLabel(target: StudioProject["review"]["annotations"][number]["target"]): string {
  ...
  else if (target.atMs != null)
    parts.push(target.endMs == null ? `${target.atMs}ms` : `${target.atMs}–${target.endMs}ms`);
        ...
        <main class="main">
          ...
          {current == null ? (
            ...
            <>
              <header class="topbar">
                <div>
                  <div class="eyebrow">{current.id}</div>
                  <h2>{current.narrative.title}</h2>
                  <div class="muted">{projectPath.value}</div>
                </div>
                <div class="statusline">
                  <span class={`badge ${dirty.value ? "warn" : "good"}`}>
              ...
              {(() => {
                ...
                return (
                  <section class="panel timeline-panel" aria-label="Multitrack timeline">
                    <div class="section-head">
                      <h3>Detailed timeline</h3>
                      <span class="badge">{timeline.rows.length} tracks</span>
                    </div>
                    <div class="timeline-toolbar">
                      <label class="field">
                        <span>Zoom</span>
                        <input
                          data-field="timeline-zoom"
                          type="range"
                          min="0.5"
                          max="4"
                          step="0.25"
                          value={String(timelineZoom.value)}
                          aria-label="Timeline zoom"
                        />
                      </label>
                      <label class="field">
                        <span>Snap</span>
                        <select data-field="timeline-snap" aria-label="Timeline snap">
                          {[1, 10, 25, 50, 100, 250].map((value) => (
                            <option value={String(value)} selected={timelineSnapMs.value === value}>
                              {value} ms
                            </option>
                          ))}
                        </select>
                      ...
                      <button
                        data-action="timeline-earlier"
                        disabled={busy.value || timelineSelection.value.length === 0}
                      >
                        Earlier
                      </button>
                      <button
                        data-action="timeline-later"
                        disabled={busy.value || timelineSelection.value.length === 0}
                      >
                        Later
                      </button>
                      <button
                        data-action="timeline-shorter"
                        disabled={busy.value || timelineSelection.value.length === 0}
                      >
                        Shorter
                      </button>
                      <button
                        data-action="timeline-longer"
                        disabled={busy.value || timelineSelection.value.length === 0}
                      >
                        Longer
                      </button>
                      <button data-action="timeline-undo" disabled={busy.value || timelineUndo.value.length === 0}>
                        Undo timeline
                      </button>
                      <button data-action="timeline-redo" disabled={busy.value || timelineRedo.value.length === 0}>
                        Redo timeline
                      </button>
                    </div>
                            ...
                            <div class="timeline-lane" style={`width:${width}px`}>
                              {track.items.map((item) => (
                                ...
                                >
                                  {item.label}
                                  {item.resizable && (
                                    <span class="timeline-resize" data-timeline-resize="end" aria-hidden="true"></span>
                                  )}
                                </button>
                              ))}
                            </div>
                    ...
                    <p class="timeline-summary">
                      {Math.round(timeline.durationMs)}ms · click to select, Cmd/Ctrl-click for multiselect · arrows
                      move by {timelineSnapMs.value}ms · Alt+arrows resize
                    </p>
                  </section>
                );
              })()}
...
function sendPreviewToScrubber(preview: PreviewResponse, seekMs?: number): void {
  ...
  const restoreState =
    seekMs == null
      ? previous
      : {
          ...normalizeScrubberEmbedViewState(previous, preview.durationMs),
          playheadMs: Math.max(0, Math.min(preview.durationMs, seekMs)),
        };
...
window.addEventListener("message", (event) => {
  const frame = app.querySelector<HTMLIFrameElement>("[data-scrubber-frame]");
  if (event.origin !== location.origin || event.source !== frame?.contentWindow || !isScrubberEmbedEvent(event.data))
    return;
...
async function loadPreview(
  selection: { kind: "story" } | { kind: "scene"; sceneId: string },
  seekMs?: number,
): Promise<void> {
...
async function applyTimeline(command: StudioTimelineCommand, mode: "edit" | "undo" | "redo" = "edit"): Promise<void> {
  ...
  acceptLoaded(
    result,
    mode === "edit" ? "Updated timeline in" : mode === "undo" ? "Undid timeline change in" : "Redid timeline change in",
    true,
  );
  if (mode === "edit") {
...
async function importRecording(): Promise<void> {
  ...
  try {
    recording = JSON.parse(recordingJson.value);
  } catch {
    throw new Error("Recorded interaction JSON is invalid.");
  }
  const result = await post("/api/recording/import", {
    path: projectPath.value,
    expectedHeadRevisionId: current.review.headRevisionId,
    recording,
  });
  acceptLoaded(
    result,
    result.recordingImportResult?.status === "imported" ? "Imported recording into" : "Recording import paused for",
  );
...
function handleTimelineClick({ action }: ClickActionContext): boolean {
  if (
    action === "timeline-earlier" ||
    action === "timeline-later" ||
    action === "timeline-shorter" ||
    action === "timeline-longer"
  ) {
  ...
  switch (action) {
    case "undo": {
      ...
      if (previous != null) {
        ...
        dirty.value = true;
        generation.value = {
          artifactCount: previous.artifacts.length,
          scenes: previous.scenes.map((scene) => ({ id: scene.id, generated: false })),
        };
      }
      return true;
    }
    ...
    case "scene-duplicate":
      if (sceneId != null) applyAuthoring({ kind: "scene.duplicate", sceneId });
      return true;
    case "scene-up":
      if (sceneId != null) applyAuthoring({ kind: "scene.move", sceneId, toIndex: sceneIndex - 1 });
      return true;
    case "scene-down":
      if (sceneId != null) applyAuthoring({ kind: "scene.move", sceneId, toIndex: sceneIndex + 1 });
      return true;
    case "scene-remove":
      if (
        sceneId != null &&
        window.confirm("Remove this scene? Generated scene artifacts will no longer belong to the story.")
      ) {
    ...
    case "beat-add":
      ...
      return true;
    case "beat-remove":
      if (beatId != null) applyAuthoring({ kind: "beat.remove", beatId });
      return true;
    default:
      return false;
  }
...
function handlePreviewClick({ action, target, sceneId }: ClickActionContext): boolean {
  switch (action) {
    case "generate-story":
      ...
      return true;
    case "generate-scene":
      if (sceneId != null) void run(() => generate({ kind: "scene", sceneId }));
      return true;
    ...
    case "preview-refresh":
      if (previewSelection.value != null) void loadPreview(previewSelection.value);
      return true;
    default:
      return false;
  }
...
function handleProjectClick({ action }: ClickActionContext): boolean {
  if (action === "create") {
    void run(async () =>
      acceptLoaded(await post("/api/create", { path: projectPath.value, title: newTitle.value }), "Created"),
    );
    return true;
  }
  if (action === "open" || action === "reopen") {
    void run(async () =>
      acceptLoaded(await post("/api/open", { path: projectPath.value }), action === "reopen" ? "Reopened" : "Opened"),
    );
    return true;
  }
  if (action === "save") {
    if (project.value != null) {
      void run(async () =>
        acceptLoaded(
          await post("/api/save", {
            path: projectPath.value,
            expectedHeadRevisionId: project.value!.review.headRevisionId,
...
async function createAnnotation(current: StudioProject): Promise<void> {
  ...
  const regionValues = [
    annotationRegionX.value,
    annotationRegionY.value,
    annotationRegionWidth.value,
    annotationRegionHeight.value,
  ];
  const hasAnyRegion = regionValues.some((value) => value.trim() !== "");
  if (hasAnyRegion && regionValues.some((value) => value.trim() === ""))
    throw new Error("A region requires x, y, width, and height.");
  ...
  const target = {
    scope:
      annotationScene.value === ""
        ...
        : { kind: "scene" as const, sceneId: annotationScene.value },
    ...(atMs == null ? {} : { time: { pointMs: atMs, ...(endMs == null ? {} : { range: { startMs: atMs, endMs } }) } }),
    ...(hasAnyRegion
      ? {
          regions: [
            { x: region[0], y: region[1], width: region[2], height: region[3], coordinateSpace: "scene" as const },
          ],
        }
      : {}),
  };
  const result = await post("/api/annotation", {
    ...
    command: {
      ...
      author: {
        kind: "human",
        ...(annotationAuthor.value.trim() === "" ? {} : { name: annotationAuthor.value.trim() }),
      },
      target,
      origin: { kind: "studio" },
    },
...
async function updateAnnotation(
  ...
  const draftBody = (
    card.querySelector<HTMLTextAreaElement>('[data-field="annotation-existing-body"]')?.value ?? ""
  ).trim();
...
function handleAnnotationClick({ action, target }: ClickActionContext): boolean {
  ...
  if (current != null && card != null && annotationId != null)
    void run(() => updateAnnotation(action, current, card, annotationId));
  return true;
}
...
void delegate(app, "click", "[data-action]", (_event, target) => {
  ...
  const context = {
    ...
    sceneId,
    sceneIndex,
    beatId: element.closest<HTMLElement>("[data-beat-id]")?.dataset.beatId,
  };
...
void delegate(app, "input", "[data-field]", (_event, target) => {
  ...
  if (field === "annotation-new-body") {
    annotationBody.value = control.value;
    return;
  }
  if (field === "annotation-author") {
    annotationAuthor.value = control.value;
    return;
  }
  if (field === "annotation-scene") {
    annotationScene.value = control.value;
    return;
  }
  if (field === "annotation-start") {
    annotationStartMs.value = control.value;
    return;
  }
  if (field === "annotation-end") {
    annotationEndMs.value = control.value;
    return;
  }
  if (field === "annotation-region-x") {
    annotationRegionX.value = control.value;
    return;
  }
  if (field === "annotation-region-y") {
    annotationRegionY.value = control.value;
    return;
  }
  if (field === "annotation-region-width") {
    annotationRegionWidth.value = control.value;
    return;
  }
  if (field === "annotation-region-height") {
    annotationRegionHeight.value = control.value;
    return;
  }
  if (field === "annotation-existing-body") return;
  if (field === "recording-json") {
    recordingJson.value = control.value;
    return;
  }
  const current = project.value;
  if (current == null) return;
  ...
  dirty.value = true;
  generation.value = {
    artifactCount: next.artifacts.length,
    scenes: next.scenes.map((scene) => ({ id: scene.id, generated: false })),
  };
  issues.value = [];
  message.value = "";
});
...
void delegate(app, "change", "[data-field]", (_event, target) => {
  ...
  if (field === "scene-action-at") {
    ...
    if (!Number.isFinite(atMs) || atMs < 0) {
      setFailure(new Error("Action timing must be non-negative."));
      return;
    }
    ...
    applyAuthoring({
      kind: "scene.update",
      sceneId: scene.id,
      patch: { narrativeBeatIds: control.value === "" ? [] : [control.value] },
    });
    ...
    if (value === "") {
      setFailure(new Error("A scene source is required."));
      return;
    }
    ...
    applyAuthoring(
      updateSceneRecipe(scene, { capture: { ...recipe.capture, selector: control.value.trim() || "body" } }),
    );
    ...
    if (recipe.trimEnd != null && recipe.trimStart != null) {
      if (recipe.trimEnd <= recipe.trimStart) {
        setFailure(new Error("Trim end must be after trim start."));
        return;
      }
    ...
    if (!Number.isFinite(duration) || duration < 0) {
      setFailure(new Error("Transition duration must be non-negative."));
      return;
    }
    const previous = scenePresentation(scene).transition;
    ...
    applyAuthoring({
      kind: "scene.update",
      sceneId: scene.id,
      patch: { treatments: presetTreatments(scene, control.value) },
    });
  }
});
...
void delegate(app, "click", "[data-timeline-id]", (event, target) => {
  ...
  const item =
    project.value == null
      ? undefined
      : buildStudioTimeline(project.value).items.find((candidate) => candidate.id === itemId);
...
void delegate(app, "keydown", "[data-timeline-id]", (event) => {
  ...
  void run(() =>
    applyTimeline(
      selectedTimelineCommand(keyboard.altKey ? "resize" : "move", direction * multiplier * timelineSnapMs.value),
    ),
  );
});
