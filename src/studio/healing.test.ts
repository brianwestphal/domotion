...
describe("Studio AI healing state machine", () => {
  it("requires AI review, records review edits with exact provenance, then hands the candidate to a human", () =>
    withArtifacts(async (artifactDir) => {
      const review = vi.fn(async ({ project: current }: { project: StudioProject }) => {
        if (current.narrative.title === "Original") {
          ...
          return {
            ...
            evidence: { summary: "The title did not state the outcome.", data: { rubric: "clarity" } },
          };
        }
        return {
          kind: "accept" as const,
          summary: "Ready for human review.",
          evidence: { summary: "Clarity check passed." },
        };
      });
      const result = await runStudioHealingLoop(noBrowser, project(), {
        ...
        artifactDir,
        heal: async () => {
          throw new Error("healing should not run");
        },
        review,
  ...
  it("pauses review for clarification and resumes from the exact project snapshot", () =>
    withArtifacts(async (artifactDir) => {
      ...
      await expect(
        resumeStudioHealingLoop(noBrowser, tampered, "Emphasize trust.", {
          projectDir: fixtureDir,
          artifactDir,
          heal: async () => {
            throw new Error("healing should not run");
      ...
      const resumed = await resumeStudioHealingLoop(noBrowser, first.checkpoint, "Emphasize trust.", {
        ...
        review: async ({ clarification }) => {
          expect(clarification).toEqual({
            phase: "review",
            question: first.checkpoint.question,
            answer: "Emphasize trust.",
          });
          return {
            kind: "accept",
            summary: "Trust is clear.",
            evidence: { summary: "The human resolved the ambiguity." },
          };
        },
      });
  ...
  it("isolates callback inputs so mutation cannot bypass revision provenance", () =>
    withArtifacts(async (artifactDir) => {
      const result = await runStudioHealingLoop(noBrowser, project(), {
        projectDir: fixtureDir,
        artifactDir,
  ...
  it("returns unrecoverable intent without invoking review", () =>
    withArtifacts(async (artifactDir) => {
      ...
      const result = await runStudioHealingLoop(noBrowser, active, {
        ...
        heal: async ({ failure }) => ({
          kind: "unrecoverable",
          reason: "No current product action matches the intent.",
  ...
  it("rejects no-op AI edits instead of silently cycling", () =>
    withArtifacts(async (artifactDir) => {
      await expect(
        runStudioHealingLoop(noBrowser, project(), {
          projectDir: fixtureDir,
          artifactDir,
          heal: async () => {
            throw new Error("healing should not run");
  ...
  it("requires clarification before AI changes a script trust boundary", () =>
    withArtifacts(async (artifactDir) => {
      const initial = project();
      initial.scriptHooks = [{ id: "hook", module: "./trusted.ts" }];
      await expect(
        runStudioHealingLoop(noBrowser, initial, {
          projectDir: fixtureDir,
          artifactDir,
          heal: async () => {
            throw new Error("healing should not run");
          ...
          review: async ({ project: current }) => {
            ...
            return {
              kind: "edit",
              project: edited,
              summary: "Change executable code.",
              evidence: { summary: "Requested by review." },
            };
          },
        }),
