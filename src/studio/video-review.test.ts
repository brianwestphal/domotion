...
describe("Studio structured AI video review", () => {
  it("renders through the media boundary and adds every grounded finding to the shared revision queue", () =>
    withDir(async (dir) => {
  ...
  it("keeps the general rubric authoritative when comparing revisions", () =>
    withDir(async (dir) => {
      ...
      const result = await renderAndReviewStudioVideo(project(), {
        ...
        comparison: {
          ...
          media: {
            ...
            sha256: "b261985457e281a3015ac85c953553a8914f2d2f25f5dc160c25da2ad6b63590",
      ...
      if (result.status === "reviewed") {
        expect(result.project.review.revisions.at(-1)?.metadata).toMatchObject({
          automation: { comparison: { summary: "Pacing improved." } },
        });
      }
    }));

  it("rejects a report that skips any rubric dimension", () =>
    withDir(async (dir) => {
      await expect(
        renderAndReviewStudioVideo(project(), {
          projectDir: dir,
          svgPath: "source.svg",
          outputPath: "review.mp4",
  ...
  it("pauses for clarification, verifies its media and project, and resumes without rerendering", () =>
    withDir(async (dir) => {
      ...
      await expect(
        resumeStudioVideoReview(initial.checkpoint, "Trust", {
