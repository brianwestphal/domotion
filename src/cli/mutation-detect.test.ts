...
import {
  resolveJsRevealSpec,
  JS_REVEAL_DEFAULTS,
  MUTATION_DETECT_EVENTS,
  synthMutationTween,
} from "./mutation-detect.js";
...
describe("synthMutationTween (DM-1580)", () => {
  ...
  it("builds a transform tween (with fused opacity) for a target motion delta", () => {
    const anims = synthMutationTween(
      diff([
  ...
  it("builds an opacity-only tween when there's no transform delta", () => {
    ...
    expect(anims).toEqual([
      { animId: "jr0", property: "opacity", from: "1", to: "0.5", duration: 200, easing: "ease-out" },
    ]);
  });

  it("returns nothing when the motion deltas aren't on the target (key !== '')", () => {
    expect(
      synthMutationTween(diff([{ key: "3", property: "transform", from: "none", to: "scale(2)" }]), "jr0", 300),
    ).toEqual([]);
