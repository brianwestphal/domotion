      ...
      async ({ selector, key, token, top }) => {
        ...
        function cssUrls(content: string): string[] {
          ...
          while ((match = expression.exec(content)) != null) {
            ...
            try {
              output.push(new URL(value, document.baseURI).href);
            } catch {
              output.push(value);
            }
          }
        ...
        for (let elementIndex = 0; elementIndex < elements.length; elementIndex++) {
          ...
          for (const pseudo of ["checkmark", "before", "after"] as const) {
            ...
            if ("letterSpacing" in drawing && style.letterSpacing !== "normal")
              drawing.letterSpacing = `${number(style.letterSpacing) * zoom}px`;
            if ("wordSpacing" in drawing && style.wordSpacing !== "normal")
              drawing.wordSpacing = `${number(style.wordSpacing) * zoom}px`;
            ...
            rows.push({
              ...
              style: {
                ...
                border: {
                  top: edge(style, "border", "Top") * zoom,
                  right: edge(style, "border", "Right") * zoom,
                  bottom: edge(style, "border", "Bottom") * zoom,
                  left: edge(style, "border", "Left") * zoom,
                },
                padding: {
                  top: edge(style, "padding", "Top") * zoom,
                  right: edge(style, "padding", "Right") * zoom,
                  bottom: edge(style, "padding", "Bottom") * zoom,
                  left: edge(style, "padding", "Left") * zoom,
                },
                margin: {
                  top: edge(style, "margin", "Top") * zoom,
                  right: edge(style, "margin", "Right") * zoom,
                  bottom: edge(style, "margin", "Bottom") * zoom,
                  left: edge(style, "margin", "Left") * zoom,
                },
                typography: {
        ...
        await Promise.all(
          rows.map(async (row) => {
            const zoom = row.style.typography.effectiveZoom > 0 ? row.style.typography.effectiveZoom : 1;
            row.style.contentImageNaturalSizes = await Promise.all(
              row.style.contentUrls.map(async (url) => {
...
async function runtimeContexts(session: CDPSession, key: string): Promise<Map<string, number>> {
  ...
  for (const contextId of ids) {
    const result = await session
      ...
      .catch(() => null);
    if (typeof result?.result.value === "string" && result.result.value !== "")
      contexts.set(result.result.value, contextId);
  }
  return contexts;
}
      ...
      prepared.candidates.map(async (candidate) => {
        ...
        try {
          ...
          if (pseudo != null && pseudo.nodeId > 0) {
            const platform = await session
              .send("CSS.getPlatformFontsForNode", { nodeId: pseudo.nodeId })
              .catch(() => null);
            candidate.style.typography.resolvedFonts =
              platform?.fonts.map((font) => ({
...
function snapshotPaintNodes(snapshot: SnapshotResult, targetBackendNodeId: number): PseudoBackdropSnapshotNode[] {
...
function pseudoBackdropRect(record: CapturedPseudoFragmentSet): Rect | null {
  const rects = record.boxFragments
    .map((box) => box.physicalRect)
    .filter(
      (value) =>
        value.width > 0 && value.height > 0 && [value.x, value.y, value.width, value.height].every(Number.isFinite),
    );
 ...
 */
function neutralPseudoContent(candidate: Candidate, record: CapturedPseudoFragmentSet): string | null | undefined {
  ...
  const sizes = images.map((item) => {
    ...
    const zoom = candidate.style.typography.effectiveZoom > 0 ? candidate.style.typography.effectiveZoom : 1;
  ...
  try {
    for (const backendNodeId of backendNodeIds) {
      ...
      const changed = await session.send("Runtime.callFunctionOn", {
        objectId,
        functionDeclaration:
          "function(){if(!(this instanceof Element))return null;const v=this.style.getPropertyValue('visibility');const p=this.style.getPropertyPriority('visibility');this.style.setProperty('visibility','hidden','important');return {v,p};}",
        returnByValue: true,
      });
...
async function restoreBackdropLaterNodes(session: CDPSession, restores: readonly VisibilityRestore[]): Promise<void> {
...
async function capturePseudoBackdropBoundary(
  ...
  const candidateByBackendId = new Map(
    candidates.flatMap((row) => (row.backendNodeId == null ? [] : [[row.backendNodeId, row] as const])),
  );
  const hiddenPseudos = plan.hideBackendNodeIds.flatMap((backendNodeId) => {
    const row = candidateByBackendId.get(backendNodeId);
    return row == null ? [] : [row];
  });
  const ordinaryBackendNodeIds = plan.hideBackendNodeIds.filter(
    (backendNodeId) => !candidateByBackendId.has(backendNodeId),
  );
...
function exactRecord(
  ...
  const fragments = decoded.fragments
    .flatMap((fragment) => {
...
async function isolatePseudoSurface(
  ...
  for (let cursor: Frame | null = candidate.frame; cursor != null; cursor = cursor.parentFrame())
    ancestorFrames.add(cursor);
  try {
    await Promise.all(
      prepared.map(({ frame }) =>
        frame.evaluate(
    ...
    for (let cursor: Frame | null = candidate.frame; cursor.parentFrame() != null; cursor = cursor.parentFrame()!) {
      ...
      await handle.evaluate((element, value) => {
        for (let owner: Element | null = element as Element; owner != null; owner = owner.parentElement)
          owner.setAttribute("data-domotion-pseudo-ancestor", value);
      ...
      frame.evaluate(
        ({ key, facts }) => {
          ...
          if (registry != null) {
            ...
            const complete: Record<number, CapturedPseudoFragmentSet[]> = Object.create(null) as Record<
              number,
              CapturedPseudoFragmentSet[]
            >;
