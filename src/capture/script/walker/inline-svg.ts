...
export const captureInlineSvg = (el, cs, warn, sel) => {
  ...
  const _hasTransformSignal = (node, style) => {
    ...
    for (const child of node.children || []) {
      if (isSvgTransformAnimation(child.localName, child.getAttribute && child.getAttribute("attributeName")))
        return true;
    }
    return false;
  };
  ...
  const _freezeUsedAffine = (origNode, cloneNode, ocs) => {
    ...
    const freeze =
      usedCtm != null && parentCtm != null && neutralCtm != null
    ...
    try {
      ...
      /* Unit-basis validation remains authoritative. */
    }
  ...
  if (svgStroke && svgStroke !== "" && svgStroke !== "none" && !_hasConcreteAttr(el, "stroke"))
    clone.setAttribute("stroke", svgStroke);
  if (svgStrokeWidth && svgStrokeWidth !== "" && !_hasConcreteAttr(el, "stroke-width"))
    clone.setAttribute("stroke-width", svgStrokeWidth);
  ...
  const _bakeSvgNativePaintProps = ["marker-start", "marker-mid", "marker-end", "vector-effect", "color-interpolation"];
    ...
    for (var ui = 0; ui < uses.length; ui++) {
        ...
        try {
          ...
          if (anims != null && anims.length > 0) {
            warn(
              ...
              '<use href="#' +
                targetId +
                '"> resolved to a CSS-animated subtree; the inlined SVG carries the t=0 computed paint state (no animation in the output)',
            );
          }
        } catch (e) {
          /* getAnimations not supported — fall through, no warning */
        }
      }
      ...
      if (targetTag === "symbol") {
        ...
        if (clonedTarget.tagName && clonedTarget.tagName.toLowerCase() === "svg" && clonedTarget.removeAttribute) {
          ...
          if (
            shouldStripPromotedViewportDimension(target.getAttribute("height"), clonedTarget.getAttribute("height"))
          ) {
  ...
  for (const animation of Array.from(
    clone.querySelectorAll
      ? clone.querySelectorAll("animateMotion, animateTransform, animate[attributeName], set[attributeName]")
      : [],
  )) {
  ...
  const _sanitizeClonedTransformCss = () => {
    for (const styleNode of Array.from(clone.querySelectorAll ? clone.querySelectorAll("style") : [])) {
      ...
      try {
        ...
        styleNode.textContent = Array.from(sheet.cssRules)
