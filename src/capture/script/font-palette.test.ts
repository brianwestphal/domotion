...
describe("font-palette rule ownership", () => {
  it("joins the computed token to the matching requested family and preserves override order", () => {
    ...
    expect(
      resolveFontPalette(element([new CSSFontPaletteValuesRule("--brand", "Audit", "3", "7 red, 3 magenta")]), {
        fontPalette: "--brand",
        fontFamily: "Audit, serif",
      }),
    ).toEqual({
      token: "--brand",
      kind: "custom",
  ...
  it("collapses a missing or family-mismatched rule to normal", () => {
    const { resolveFontPalette } = createFontPaletteResolver();
    expect(
      resolveFontPalette(element([new CSSFontPaletteValuesRule("--brand", "Other", "2", "")]), {
        fontPalette: "--brand",
        fontFamily: "Audit",
      }),
    ).toEqual({
  ...
  it("uses Blink's last document rule and includes document adopted sheets", () => {
    ...
    expect(resolveFontPalette(el, { fontPalette: "--brand", fontFamily: "Audit" })).toEqual(
      expect.objectContaining({
        ruleScope: "document",
        basePalette: "3",
        overrides: [{ index: 7, color: "cyan" }],
      }),
    );
  });
  ...
  it("records recursive palette-mix normalization and every resolved endpoint", () => {
    ...
    expect(
      resolveFontPalette(el, {
        fontPalette: "palette-mix(in hsl longer hue, --a 20%, palette-mix(in srgb, --b, normal) 30%)",
        fontFamily: "Audit",
      }),
    ).toEqual({
  ...
  it("treats animation's computed Oklab percentage as paint-time identity", () => {
    ...
    expect(resolved.mix).toEqual(
      expect.objectContaining({
        colorSpace: "oklab",
        startPercentage: 70,
        endPercentage: 30,
        normalizedPercentage: 0.3,
        alphaMultiplier: 1,
  ...
  it("keeps raw calc percentages in identity while clamping only normalized paint progress", () => {
    ...
    const resolved = resolveFontPalette(element([]), {
      fontPalette: "palette-mix(in srgb, --from -30%, --to)",
      fontFamily: "Audit",
    });
    expect(resolved.mix).toEqual(
      expect.objectContaining({
