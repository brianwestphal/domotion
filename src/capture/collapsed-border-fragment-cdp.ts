    ...
    const raw = await frame.evaluate(
      ({ selector, key, token, top }) => {
        ...
        for (const table of candidates) {
          ...
          const sectionElements = Array.from(table.children).filter(
            (child): child is HTMLTableSectionElement =>
              child.tagName === "THEAD" || child.tagName === "TBODY" || child.tagName === "TFOOT",
          );
          const sectionDisplays = new Map(
            sectionElements.map((section) => [section, getComputedStyle(section).display]),
          );
          const selectedHeader = sectionElements.find(
            (section) => sectionDisplays.get(section) === "table-header-group",
          );
          const selectedFooter = sectionElements.find(
            (section) => sectionDisplays.get(section) === "table-footer-group",
          );
          ...
          const parsedFragmentainerBlockSize =
            columnOwnerStyle == null ? Number.NaN : Number.parseFloat(columnOwnerStyle.blockSize);
          const fragmentainerBlockSize =
            Number.isFinite(parsedFragmentainerBlockSize) && parsedFragmentainerBlockSize > 0
              ? parsedFragmentainerBlockSize
              : null;
          ...
          const outsideNestedRepeatableContent =
            nestedStyle == null ||
            !(
          ...
          for (const descendant of [table, ...Array.from(table.querySelectorAll("*"))]) {
            ...
            if (
              ...
              descendantStyle.transformStyle === "preserve-3d"
            )
              transformOwners.add(descendant);
          }
...
function sourceRestoredExactly(frame: PreparedFrame, live: QuadMap, restored: QuadMap): boolean {
