...
export const ANIMATED_IMAGE_STATIC_FRAME_CHROMIUM_SOURCE_REVISION = "7d859f271cbda744098ac69f44978d4edfa62be3" as const;

export type AnimatedImageStaticFrameFailureCode =
  | "decoder-unavailable"
  | "insecure-decoder-context"
  | "unsupported-type"
  | "unselected-animation-track"
  | "static-image-track"
  | "unbounded-frame-count"
  | "frame-index-out-of-range"
  | "partial-frame"
  | "decoder-facts-mismatch"
  | "source-record-drift"
  | "replacement-missing";
...
export class AnimatedImageStaticFrameError extends Error {
  constructor(readonly code: AnimatedImageStaticFrameFailureCode) {
    super(code);
    this.name = "AnimatedImageStaticFrameError";
  }
}
...
export interface AnimatedImageStaticFrameRecord {
  ...
  mimeType: string;
  browser: {
    sourceRevision: typeof ANIMATED_IMAGE_STATIC_FRAME_CHROMIUM_SOURCE_REVISION;
...
const fail = (code: AnimatedImageStaticFrameFailureCode): never => {
  throw new AnimatedImageStaticFrameError(code);
};
...
export function verifyAnimatedImageStaticFrameRecord(record: AnimatedImageStaticFrameRecord): void {
  ...
  if (
    !Number.isInteger(record.track.frameCount) ||
    record.track.frameCount < 2 ||
    record.track.frameCount > ANIMATED_IMAGE_STATIC_FRAME_MAX_COUNT
  )
    fail("unbounded-frame-count");
  if (record.requestedFrameIndex < 0 || record.requestedFrameIndex >= record.track.frameCount)
    fail("frame-index-out-of-range");
}
...
export async function freezeAuthenticatedAnimatedImageFrames(
  ...
  for (const input of inputs) {
    ...
    try {
      verifyAuthenticatedAnimatedImageBytes(input.record, sourceBytes);
    } catch (error) {
    ...
    const output = await page.evaluate(
      async ({ selector, frameIndex, mimeType, base64, expected }) => {
        type Track = { frameCount: number; animated: boolean; repetitionCount: number };
        type DecodeResult = { image: VideoFrame; complete: boolean };
        type Decoder = {
          tracks: { ready: Promise<unknown>; selectedIndex: number; selectedTrack: Track | null };
          decode(value: { frameIndex: number; completeFramesOnly: boolean }): Promise<DecodeResult>;
          close(): void;
        };
        type Constructor = {
          new (value: {
          ...
          try {
            ...
            return {
              ...
              observation: {
                ...
                rgbaSha256: await hash(rgba),
                pngSha256: await hash(png),
        ...
        const create = () =>
          new ImageDecoderCtor({ data: bytes, type: mimeType, colorSpaceConversion: "default", preferAnimation: true });
        const proposal = create();
        await proposal.tracks.ready;
        const selected = proposal.tracks.selectedTrack;
        if (selected == null) {
          proposal.close();
          return { failure: "unselected-animation-track" as const };
        }
        const track = {
          selectedIndex: proposal.tracks.selectedIndex,
          frameCount: selected.frameCount,
          animated: selected.animated,
          repetitionCount: Number.isFinite(selected.repetitionCount) ? String(selected.repetitionCount) : "Infinity",
        };
        if (!track.animated) {
          proposal.close();
          return { failure: "static-image-track" as const };
        }
        if (!Number.isInteger(track.frameCount) || track.frameCount < 2 || track.frameCount > 4096) {
          proposal.close();
          return { failure: "unbounded-frame-count" as const };
        }
        if (frameIndex < 0 || frameIndex >= track.frameCount) {
          proposal.close();
          return { failure: "frame-index-out-of-range" as const };
        }
        const first = await observe(proposal, frameIndex);
        proposal.close();
        const validation = create();
        await validation.tracks.ready;
        for (let index = track.frameCount - 1; index > frameIndex; index--) {
          const control = await validation.decode({ frameIndex: index, completeFramesOnly: true });
          control.image.close();
        }
        const second = await observe(validation, frameIndex);
        validation.close();
        if (!first.observation.complete || !second.observation.complete) return { failure: "partial-frame" as const };
        if (
          JSON.stringify(first.observation) !== JSON.stringify(second.observation) ||
          first.pngBase64 !== second.pngBase64
        ) {
        ...
        const current =
          expected.ownerKind === "html-image" && owner instanceof HTMLImageElement
            ? owner.currentSrc || owner.src
            : expected.ownerKind === "input-image" && owner instanceof HTMLInputElement && owner.type === "image"
              ? owner.src
              : expected.ownerKind === "svg-image" && owner instanceof SVGImageElement
                ? new URL(owner.href.baseVal, document.baseURI).href
                : expected.ownerKind === "css-image"
                  ? getComputedStyle(owner).getPropertyValue(expected.ownerSlot)
                  : "";
        const nonce =
          (globalThis as typeof globalThis & { __domotionAnimatedImageDocumentNonce?: string })
            .__domotionAnimatedImageDocumentNonce ?? "";
        const expectedCurrent =
          expected.ownerKind === "css-image" ? expected.ownerSerializedValue : expected.selectedUrl;
        if (current !== expectedCurrent || nonce !== expected.documentNonce)
          return { failure: "source-record-drift" as const };
        ...
        if (expected.ownerKind === "html-image" && owner instanceof HTMLImageElement) {
          ...
          owner.src = pngDataUrl;
        } else if (expected.ownerKind === "input-image" && owner instanceof HTMLInputElement && owner.type === "image")
          owner.src = pngDataUrl;
        else if (expected.ownerKind === "svg-image" && owner instanceof SVGImageElement)
          owner.href.baseVal = pngDataUrl;
        ...
        if (expected.ownerKind === "html-image" || expected.ownerKind === "input-image")
          await new Promise<void>((resolve, reject) => {
            const image = owner as HTMLImageElement;
      ...
      {
        selector: input.record.selector,
        frameIndex: input.record.requestedFrameIndex,
        mimeType: input.record.mimeType,
        base64: Buffer.from(sourceBytes).toString("base64"),
        expected: {
          selectedUrl: input.record.selectedUrl,
          documentNonce: input.record.documentNonce,
          ownerKind: input.record.ownerKind,
          ownerSlot: input.record.ownerSlot,
          ownerSlotIndex: input.record.ownerSlotIndex,
          ownerSerializedValue: input.record.ownerSerializedValue,
        },
      },
    );
    sourceBytes.fill(0);
    ...
    const logical = {
      selector: input.record.selector,
      requestedFrameIndex: input.record.requestedFrameIndex,
      sourceEpochDigest: input.record.epochDigest,
      sourceSha256: input.record.sha256,
      mimeType: input.record.mimeType,
      browser: {
        sourceRevision: ANIMATED_IMAGE_STATIC_FRAME_CHROMIUM_SOURCE_REVISION,
    ...
    verifyAnimatedImageStaticFrameRecord(record);
    records.push(record);
  }
  return records;
}
