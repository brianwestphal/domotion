  ...
  await context.addInitScript(
    ({ key, protocol }) => {
      ...
      if ("Worker" in globalThis)
        Object.defineProperty(globalThis, "Worker", { configurable: false, writable: false, value: blockWorker });
      if ("SharedWorker" in globalThis)
        Object.defineProperty(globalThis, "SharedWorker", { configurable: false, writable: false, value: blockWorker });
        ...
        Object.defineProperty(canvasPrototype, "transferControlToOffscreen", {
          ...
          value(): never {
            ...
            throw new DOMException(
              "OffscreenCanvas is unavailable during an authenticated capture frame",
              "NotSupportedError",
            );
          },
        });
      ...
      ({ key, timeMs, bound }) => {
        ...
        return (
          control as (
            time: number,
            max: number,
          ) => Omit<CaptureRafTargetState, "targetId" | "frameUrl" | "frameIdentity">
        )(timeMs, bound);
      },
...
export async function sampleCaptureRafClock(
  ...
  const targets = await Promise.all(
    before.map((frame) => sampleFrame(page, frame, handle, requestedTimeMs, maxCallbacks)),
  );
