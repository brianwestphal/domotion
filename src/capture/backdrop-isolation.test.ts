...
const n = (
  ...
  attributes: string[] = [],
): SnapshotNode => ({ backendNodeId, parentIndex, paintOrder, layoutOrder: backendNodeId, bounds, attributes });
...
describe("backdrop raster diagnostics", () => {
  ...
  it("reports a planner miss as a partial unisolated Chromium crop", () => {
    expect(
      ...
      status: "partial",
      detail:
        "partial: DOMSnapshot token did not map to one painted layout owner; fallback: unisolated Chromium page crop",
    });
  });
  ...
  it("reports screenshot failure as unavailable and names the vector fallback", () => {
    expect(
      ...
      status: "unavailable",
      detail:
        "unavailable: Chromium backdrop screenshot failed; fallback: captured frosted-background color without a sampled backdrop",
    });
  });

  it("keeps generated-pseudo ownership distinct from the host selector", () => {
    expect(
      backdropRasterWarning("#host::before", {
        status: "unavailable",
