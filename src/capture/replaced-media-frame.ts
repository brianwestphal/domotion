...
export type ReplacedMediaKind =
  "canvas" | "video" | "iframe" | "object" | "embed" | "custom-element" | "image-replacement";
...
export async function prepareReplacedMediaFrameTransaction(
    ...
    documentIdentity = await page.evaluate(
      async ({ key, rootSelector, rect, requestedSeconds, protocol }) => {
        ...
        const candidates = [root, ...Array.from(root.querySelectorAll("canvas,video"))]
          ...
          .filter(
            (element) =>
              (element instanceof HTMLCanvasElement || element instanceof HTMLVideoElement) && visible(element),
          );
      ...
      ({ key, protocol }) => {
        ...
        const dimensions = (element: Element): ReplacedMediaDimensions => {
          ...
          return {
            cssWidth: box.width,
            cssHeight: box.height,
            intrinsicWidth: element.clientWidth,
            intrinsicHeight: element.clientHeight,
          };
        };
        ...
        return state.bound.map((entry): BrowserOwnerFact => {
          ...
          const frameEpoch =
            canvas != null
              ...
              : video != null
                ? `video:${videoEpoch}`
                : `document:${state.documentIdentity}`;
          return {
            rid: entry.rid,
            kind: entry.kind,
  ...
  return {
    ...
    async finalize(tree: CapturedElement[]): Promise<StableReplacedMediaFrameState> {
      ...
      if (
        currentTargets.length !== boundTargets.length ||
