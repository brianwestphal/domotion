...
describe("Blink URL background candidate selection", () => {
  ...
  it("filters unsupported MIME types, stable-dedupes densities, and chooses first >= DPR", () => {
    const layer =
      'image-set(url("unsupported.png") 1x type("image/lolcat"), url("one.png") 1x type("image/png"), url("duplicate.png") 1x, url("two.png") 2x, url("three.png") 3x)';
  ...
  it("preserves direct URLs and makes unsupported image-set state explicit", () => {
    ...
    expect(selectBackgroundCandidate('image-set(url("bad.png") 1x type("image/lolcat"))', 1)).toMatchObject({
      source: "image-set",
      selectedUrl: null,
