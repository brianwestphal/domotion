...
describe("color glyph raster identity", () => {
  ...
  it("separates palette rules and ordered overrides for the same face/gid", () => {
    ...
    const b = colorGlyphRasterIdentity(
      { fontPalette: "--b", fontPaletteIdentity: palette("--b", "3", [{ index: 7, color: "red" }]) },
      span,
    );
    expect(a).not.toBe(b);
  });
  ...
  it("drops override indices outside the selected face CPAL entry count", () => {
    const key = colorGlyphRasterIdentity(
      {
        fontPalette: "--a",
        fontPaletteIdentity: palette("--a", "0", [
          { index: 3, color: "red" },
          { index: 99, color: "blue" },
        ]),
      },
      [{ ...span[0], paletteEntryCount: 8 }],
    );
  ...
  it("resolves theme and invalid numeric bases against the selected CPAL table", () => {
    expect(
      resolvedPaletteIdentity({ fontPalette: "light" }, { ...span[0], paletteCount: 4, paletteTypes: [0, 0, 1, 2] })
        .resolvedBasePalette,
    ).toBe(2);
    expect(
      resolvedPaletteIdentity(
        { fontPalette: "--bad", fontPaletteIdentity: palette("--bad", "99") },
        { ...span[0], paletteCount: 4 },
      ).resolvedBasePalette,
    ).toBe(0);
  });
  ...
  it("separates time, interpolation space, hue method, and recursively resolved endpoint rules", () => {
    ...
    const identities = [mix(0.3), mix(0.5), mix(0.3, "srgb"), mix(0.3, "hsl", "longer")].map((fontPaletteIdentity) =>
      colorGlyphRasterIdentity({ fontPaletteIdentity }, [{ ...span[0], paletteEntryCount: 8 }]),
    );
    expect(new Set(identities).size).toBe(4);
    expect(identities[0]).not.toContain('"index":99');
    const resolved = resolvedPaletteIdentity(
      { fontPaletteIdentity: mix(0.3) },
      { ...span[0], paletteCount: 4, paletteEntryCount: 8 },
    );
