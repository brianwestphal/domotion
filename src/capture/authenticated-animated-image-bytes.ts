...
export const AUTHENTICATED_ANIMATED_IMAGE_BYTE_PROTOCOL = "domotion-animated-image-bytes-v1" as const;
...
export type AnimatedImageOwnerSlot =
  | "html-current"
  | "input-src"
  | "svg-href"
  | "background-image"
...
function dataUrlBytes(url: string): Uint8Array {
  ...
  try {
    ...
    return fail("data-url-parse-failed");
  }
}
...
async function snapshotOwner(
  ...
  const facts = await handle.evaluate((node, requested) => {
    ...
    if (requested.slot == null && node instanceof HTMLImageElement)
      return {
        ownerKind: "html-image" as const,
        ownerSlot: "html-current" as const,
        ownerSlotIndex: null,
        ownerSerializedValue: node.currentSrc || node.src,
        selectedUrl: node.currentSrc || node.src,
        currentSrc: node.currentSrc,
        nonce,
        dpr: devicePixelRatio,
        viewport: { width: innerWidth, height: innerHeight },
      };
    if (requested.slot == null && node instanceof HTMLInputElement && node.type === "image")
      return {
        ownerKind: "input-image" as const,
        ownerSlot: "input-src" as const,
        ownerSlotIndex: null,
        ownerSerializedValue: node.src,
        selectedUrl: node.src,
        currentSrc: "",
        nonce,
        dpr: devicePixelRatio,
        viewport: { width: innerWidth, height: innerHeight },
    ...
    if (requested.slot === "svg-href" && node instanceof SVGImageElement && requested.index == null) {
      ...
      if (raw === "") return null;
      return {
        ownerKind: "svg-image" as const,
        ownerSlot: "svg-href" as const,
        ownerSlotIndex: null,
        ownerSerializedValue: raw,
        selectedUrl: new URL(raw, document.baseURI).href,
        currentSrc: "",
        nonce,
        dpr: devicePixelRatio,
        viewport: { width: innerWidth, height: innerHeight },
    ...
    if (
      requested.slot != null &&
      cssSlots.has(requested.slot) &&
      Number.isInteger(requested.index) &&
      (requested.index ?? -1) >= 0
    ) {
      const serialized = getComputedStyle(node).getPropertyValue(requested.slot);
      const item = splitTopLevel(serialized)[requested.index!];
      const raw = item == null ? null : plainUrl(item);
      if (raw == null) return null;
      return {
        ownerKind: "css-image" as const,
        ownerSlot: requested.slot,
        ownerSlotIndex: requested.index!,
        ownerSerializedValue: serialized,
        selectedUrl: new URL(raw, document.baseURI).href,
        currentSrc: "",
        nonce,
        dpr: devicePixelRatio,
  ...
  return {
    ownerKind: facts.ownerKind,
    ownerSlot: facts.ownerSlot,
    ownerSlotIndex: facts.ownerSlotIndex,
    ownerSerializedValue: facts.ownerSerializedValue,
    backendNodeId,
    frameId: tree.frameTree.frame.id,
    documentLoaderId: tree.frameTree.frame.loaderId,
    documentNonce: facts.nonce,
    selectedUrl: facts.selectedUrl,
    currentSrc: facts.currentSrc,
    devicePixelRatio: facts.dpr,
...
export class AuthenticatedAnimatedImageByteCollector {
  ...
  static async install(page: Page): Promise<AuthenticatedAnimatedImageByteCollector> {
    const collector = new AuthenticatedAnimatedImageByteCollector(page, await page.context().newCDPSession(page));
    ...
    await Promise.all([
      collector.cdp.send("DOM.enable"),
      collector.cdp.send("Runtime.enable"),
      collector.cdp.send("Page.enable"),
  ...
  private async collectOne(request: StrictAnimatedImageFrameRequest): Promise<AuthenticatedAnimatedImageBytes> {
    ...
    if (url.protocol === "http:" || url.protocol === "https:") {
      ...
      const candidates = [...this.ledger.values()].filter(
        (item) =>
          item.requestedUrls.includes(before.selectedUrl) &&
          item.frameId === before.frameId &&
          item.loaderId === before.documentLoaderId &&
          item.completed &&
          !item.failed,
      );
      ...
      if (entry.redirects.length > AUTHENTICATED_ANIMATED_IMAGE_LIMITS.maximumRedirects)
      ...
      try {
        response = await this.cdp.send("Network.getResponseBody", { requestId: entry.requestId });
      } catch {
        return fail("body-evicted");
      }
      ...
      try {
        ...
        bytes = first.bytes;
        if (sha256(bytes) !== sha256(repeated.bytes) || first.mimeType !== repeated.mimeType)
          fail("body-digest-mismatch");
        transportMime = first.mimeType;
      } catch (error) {
        if (error instanceof AnimatedImageByteCollectorError) throw error;
        return fail("blob-unavailable");
      }
      transport = "blob-read";
    } else fail("unsupported-scheme");
    if (bytes.byteLength === 0 || bytes.byteLength > AUTHENTICATED_ANIMATED_IMAGE_LIMITS.maximumResourceBytes)
    ...
    const logical = {
      ...
      requestedFrameIndex: request.frameIndex,
      ...before,
      requestId: entry?.requestId ?? null,
      redirectHops: entry?.redirects ?? [],
      ...
      requestMode: entry?.requestMode ?? "same-origin",
      credentialsMode: entry?.credentialsMode ?? "include",
      transport,
    ...
    const record: AuthenticatedAnimatedImageByteRecord = {
      ...logical,
      epochDigest: authenticatedAnimatedImageRecordDigest(logical),
    };
