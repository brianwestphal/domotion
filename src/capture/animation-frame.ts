...
async function seekDocumentFrame(
  ...
  return frame.evaluate(
    async ({ requestedTimeMs, epsilonMs, settleWithAnimationFrame }) => {
      ...
      const sourceState = (source: Element | null, axis: string, subject: Element | null): string => {
        ...
        const subjectState =
          subject == null
            ? null
            : subject instanceof SVGGraphicsElement
              ? (() => {
                  const box = subject.getBBox();
                  const rect = subject.getBoundingClientRect();
                  return {
                    branch: "svg-mapped-bounds",
                    id: elementIdentity(subject),
                    box: [box.x, box.y, box.width, box.height],
                    rect: [rect.x, rect.y, rect.width, rect.height],
                  };
                })()
              : {
                  branch: "html-stitched-size",
                  id: elementIdentity(subject),
                  offsetLeft: (subject as HTMLElement).offsetLeft,
                  offsetTop: (subject as HTMLElement).offsetTop,
                  offsetWidth: (subject as HTMLElement).offsetWidth,
                  offsetHeight: (subject as HTMLElement).offsetHeight,
                };
      ...
      if (failures.length === 0)
        for (const fact of progressAnimations) {
          try {
      ...
      for (let index = 0; index < progressAnimations.length; index++) {
        ...
        if (
          ...
          currentTime.unit !== "percent" ||
          currentTime.value !== fact.time.value
        ) {
      ...
      for (let index = 0; index < smilRoots.length; index++) {
        ...
        if (!Number.isFinite(currentTimeSeconds) || Math.abs(currentTimeSeconds * 1000 - requestedTimeMs) > epsilonMs) {
