...
describe("replaced snapshot geometry", () => {
  ...
  it("maps an isolated off-page sample back with exactly one translation", () => {
    ...
    expect(
      mapTranslatedQuadToScreenshot(output, sampled, {
        x: 0,
        y: 0,
        width: 160,
        height: 100,
  ...
  it("clips in sampled CSS space and transfers only that clip to output space", () => {
    const mapped = mapTranslatedQuadToScreenshot(quad(10.2, 30.2, 100, 50), quad(80.2, 70.2, 100, 50), {
      x: 0,
      y: 0,
      width: 150,
      height: 100,
  ...
  it("rejects scale/stretch between the sampled and output quads", () => {
    expect(
      mapTranslatedQuadToScreenshot(quad(0, 0, 100, 50), quad(10, 10, 150, 50), {
        x: 0,
        y: 0,
        width: 300,
        height: 200,
