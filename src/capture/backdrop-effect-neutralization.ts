  ...
  for (const property of neutralize) {
    switch (property) {
      case "opacity":
        ...
        break;
      case "filter":
        declarations.push(["filter", "none"]);
        break;
      case "clip-path":
        declarations.push(["clip-path", "none"]);
        break;
      case "mask":
      ...
      case "rotate-skew":
        ...
        declarations.push(["transform", "translate(0)"], ["translate", "none"], ["rotate", "none"], ["scale", "none"]);
        break;
    }
  }
  return declarations;
...
export async function prepareBackdropEffectSpace(
  ...
  const plans = raster.effectSpace.ancestors.map((plan) => ({
    depth: plan.depth,
    declarations: backdropNeutralizationDeclarations(
      options.preserveRelativeTransform
        ? plan.neutralize.filter((effect) => effect !== "rotate-skew")
        : plan.neutralize,
    ),
    preserveRoot: rootPreservingWillChange(
      options.preserveRelativeTransform
        ? plan.neutralize.filter((effect) => effect !== "rotate-skew")
        : plan.neutralize,
    ),
  ...
  try {
    prepared = await page.evaluate(
      ({ targetToken, plans, restoreToken }) => {
        type RestoreItem = {
          element: HTMLElement;
          property: string;
          value: string;
  ...
  return prepared === "exact" ? { status: "exact", restore } : { status: "unavailable", reason: prepared, restore };
}
