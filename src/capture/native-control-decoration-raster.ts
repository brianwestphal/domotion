...
export async function rasterizeNativeControlDecorations(
  ...
  try {
      ...
      ({ sourceNodeKey, decorationNodeKey, restoreKey, rows }) => {
        ...
        const result = rows.map((row, rowIndex) => {
          ...
          if (row.selectArrow) {
            ...
            if (
              Math.abs(frozenRect.left - row.hostRect.left) > 0.25 ||
              ...
              return {
                ...
                reason: "select host geometry changed during arrow isolation",
              };
            }
            ...
            if (
              ...
              return {
                connected: true,
                fingerprintMatches: false,
                reason: "select interaction state changed during isolation",
              };
            }
          ...
          if (row.exactPartBox) {
            ...
            if (!buttonFound || !statusFound) {
              return {
                connected: true,
                fingerprintMatches: false,
                reason: "file-selector button/status topology changed",
              };
            }
            const initial = interactionStates[rowIndex];
            if (
              ...
              return {
                connected: true,
                fingerprintMatches: false,
                reason: "file input interaction state changed during isolation",
              };
            }
            return { connected: true, fingerprintMatches: true };
          }
          ...
          if (
            ...
            return {
              connected: true,
              fingerprintMatches: false,
              reason: "control interaction state changed during isolation",
            };
          }
          return { connected: true, fingerprintMatches: true };
        });
    ...
    const restored = await page
      .evaluate(
        ({ key, sourceNodeKey, decorationNodeKey, rows }) => {
          type RestoreEntry =
            | { type: "style"; element: HTMLElement; property: string; value: string; priority: string }
            | { type: "node"; node: Node; parent: Node; nextSibling: Node | null };
          type RetainedPart = { kind?: string; node?: Element };
          ...
          for (let index = (state?.entries.length ?? 0) - 1; index >= 0; index--) {
            ...
            if (entry.type === "node") {
              ...
              try {
                ...
                failures.push(`style restore failed: ${entry.property}`);
              }
            } else {
              try {
                entry.element.style.setProperty(entry.property, entry.value, entry.priority);
              } catch {
                failures.push(`style restore failed: ${entry.property}`);
              }
            }
          ...
          for (const row of rows) {
            ...
            if (
              Math.abs(hostRect.left - row.hostRect.left) > 0.25 ||
  ...
  try {
    for (let index = 0; index < active.length; index++) {
      ...
      if (
        !target.raster.exactPartBox &&
        nativeDecorationTouchesBoundary(rgba, pixelCrop.width, pixelCrop.height, {
          left: !clippedLeft,
          top: !clippedTop,
          right: !clippedRight,
