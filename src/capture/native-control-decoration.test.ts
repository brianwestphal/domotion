...
import { decorationFingerprintMatches, nativeControlDecorationKinds } from "./native-control-decoration.js";
...
describe("partial native-control decoration ownership", () => {
  it("splits only the styled menulist and keeps complete/base select routes intact", () => {
    ...
    expect(nativeControlDecorationKinds(select, "menulist-button")).toEqual(["menulist-button-arrow"]);
  ...
  it("routes only partial number/search/temporal UA-shadow decorations", () => {
    expect(nativeControlDecorationKinds({ tag: "input", type: "number" }, "none")).toEqual(["inner-spin-button"]);
    expect(nativeControlDecorationKinds({ tag: "input", type: "search" }, "none")).toEqual(["search-cancel-button"]);
    for (const type of ["date", "datetime-local", "month", "time", "week"]) {
      expect(nativeControlDecorationKinds({ tag: "input", type }, "none")).toEqual(["calendar-picker-indicator"]);
    }
    expect(nativeControlDecorationKinds({ tag: "input", type: "number" }, "textfield")).toEqual([]);
    expect(nativeControlDecorationKinds({ tag: "input", type: "text" }, "none")).toEqual([]);
    expect(nativeControlDecorationKinds({ tag: "input", type: "search" }, "base")).toEqual([]);
  });

  it("uses the real file-selector child's EffectiveAppearance, not the file host", () => {
    const file = { tag: "input", type: "file" };
    expect(nativeControlDecorationKinds(file, "none", "push-button")).toEqual(["file-selector-button"]);
    expect(nativeControlDecorationKinds(file, "none", null)).toEqual(["file-selector-button"]);
    expect(nativeControlDecorationKinds(file, "none", undefined)).toEqual(["file-selector-button"]);
