...
describe("ordered Blink Range FragmentItem join", () => {
  it("joins an exact child-frame translation and retains an unmatched first-letter item", () => {
    const joined = joinBlinkRangeFragmentsToContentQuads(
      4,
      [range(0, [0, 2], 5, 6, 24, 31), range(1, [2, 7], 29, 15, 50), range(2, [7, 13], 79, 15, 60)],
      [quad(129, 215, 50, 20), quad(179, 215, 60, 20)],
    );
  ...
  it("rejects collapsed, reordered, and indistinguishable joins", () => {
    ...
    expect(
      joinBlinkRangeFragmentsToContentQuads(0, fragments.slice(0, 1), [quad(10, 10, 30, 20), quad(40, 10, 30, 20)])
        .failureReason,
    ).toContain("cardinality");
    expect(
      joinBlinkRangeFragmentsToContentQuads(0, fragments, [quad(40, 10, 30, 20), quad(10, 10, 30, 20)]).failureReason,
    ).toContain("do not exactly join");
    expect(
      joinBlinkRangeFragmentsToContentQuads(
        0,
...
describe("UTF-16 FragmentItem segment splitting", () => {
  it("splits one mixed-script segment before correlation and rebases shaped/raster indices", () => {
    ...
    segment.rasterGlyphs = [
      { charIndex: 7, charLength: 2, rect: { x: 80, y: 10, width: 20, height: 20 }, suppressGlyph: true },
    ];
    const joined = joinBlinkRangeFragmentsToContentQuads(
      0,
      [range(0, [0, 4], 10, 10, 40), range(1, [4, 7], 50, 10, 30), range(2, [7, 9], 80, 10, 20)],
      [quad(10, 10, 40, 20), quad(50, 10, 30, 20), quad(80, 10, 20, 20)],
    ).fragments!;
    const split = splitTextSegmentsOnFragmentSpans([segment], joined);
    expect(split.failureReason).toBeUndefined();
    expect(
      split.segments?.map((item) => ({
        text: item.text,
    ...
    ).toEqual([
      {
        text: "Lat·",
        span: [0, 4],
        xOffsets: [10, 20, 30, 40],
        xAdvances: [10, 10, 10, 10],
        dotted: [3],
        raster: undefined,
      },
    ...
    expect(split.sourceFragments.map((item) => item.domUtf16Span)).toEqual([
      [0, 4],
      [4, 7],
      [7, 9],
    ]);
  ...
  it("keeps a dedicated first-letter and removes its suppressed body duplicate", () => {
    ...
    const joined = joinBlinkRangeFragmentsToContentQuads(
      0,
      [range(0, [0, 2], 10, 2, 25, 32), range(1, [2, 6], 35, 10, 40)],
      [quad(35, 10, 40, 20)],
    ).fragments!;
  ...
  it("preserves a length-changing transform chunk and rejects a wrong span split", () => {
    ...
    const correct = joinBlinkRangeFragmentsToContentQuads(
      0,
      [range(0, [0, 1], 10, 10, 20), range(1, [1, 2], 30, 10, 10)],
      [quad(10, 10, 20, 20), quad(30, 10, 10, 20)],
    ).fragments!;
    expect(splitTextSegmentsOnFragmentSpans([transformed], correct).segments?.map((segment) => segment.text)).toEqual([
      "SS",
      "a",
    ]);
    ...
    expect(splitTextSegmentsOnFragmentSpans([transformed], wrong).failureReason).toContain(
  ...
  it("splits one-to-one source chunks across adjacent fallback FragmentItems", () => {
    ...
    const joined = joinBlinkRangeFragmentsToContentQuads(
      0,
      [range(0, [0, 2], 10, 10, 20, 40), range(1, [2, 8], 10, 50, 20, 120)],
      [quad(10, 10, 20, 40), quad(10, 50, 20, 120)],
    ).fragments!;
