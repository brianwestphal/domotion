...
describe("buildMagicMove (DM-898)", () => {
  ...
  it("emits a translate·scale affine for an element that moves AND resizes (DM-899)", () => {
    ...
    const prev = el({
      ...
      x: 0,
      y: 0,
      children: [el({ tag: "div", x: 10, y: 10, width: 100, height: 40, text: "card" })],
    });
    const next = el({
      tag: "body",
      x: 0,
      y: 0,
      children: [el({ tag: "div", x: 60, y: 90, width: 200, height: 80, text: "card" })],
    });
    const mm = buildMagicMove(prev, next, stubRender, "mm0-")!;
    expect(mm.slides).toHaveLength(1);
  ...
  it("animates only the highest moved ancestor, not its moved children", () => {
    ...
    const prev = el({
      ...
      x: 0,
      y: 0,
      children: [el({ tag: "div", x: 10, y: 10, children: [el({ tag: "span", x: 12, y: 12, text: "x" })] })],
    });
    const next = el({
      tag: "body",
      x: 0,
      y: 0,
      children: [el({ tag: "div", x: 50, y: 50, children: [el({ tag: "span", x: 52, y: 52, text: "x" })] })],
    });
    const mm = buildMagicMove(prev, next, stubRender, "mm0-")!;
    expect(mm.slides).toHaveLength(1);
  ...
  it("force-pairs by data-magic-key even when content differs (DM-900)", () => {
    ...
    const prev = el({
      ...
      x: 0,
      y: 0,
      children: [el({ tag: "div", x: 10, y: 10, text: "A", magicKey: "hero" })],
    });
    const next = el({
      tag: "body",
      x: 0,
      y: 0,
      children: [el({ tag: "div", x: 200, y: 200, text: "B", magicKey: "hero" })],
    });
  ...
  it("dual-renders a cross-fade when a mover's PAINT changes, not just its box (DM-903)", () => {
    ...
    const prev = el({
      ...
      x: 0,
      y: 0,
      children: [el({ tag: "div", x: 10, y: 10, text: "Card", styles: blue })],
    });
    const next = el({
      tag: "body",
      x: 0,
      y: 0,
      children: [el({ tag: "div", x: 120, y: 60, text: "Card", styles: red })],
    });
    const mm = buildMagicMove(prev, next, stubRender, "mm0-")!;
  ...
  it("fades in added elements and fades out removed ones", () => {
    const prev = el({
      ...
      x: 0,
      y: 0,
      children: [el({ tag: "p", x: 10, y: 10, text: "stays" }), el({ tag: "p", x: 10, y: 40, text: "goes away" })],
    });
    const next = el({
      tag: "body",
      x: 0,
      y: 0,
      children: [el({ tag: "p", x: 10, y: 10, text: "stays" }), el({ tag: "p", x: 10, y: 40, text: "brand new" })],
    });
    const mm = buildMagicMove(prev, next, stubRender, "mm0-")!;
