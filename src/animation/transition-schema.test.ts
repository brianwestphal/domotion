...
describe("canonical transition schema", () => {
  ...
  it("accepts bounded custom recipes and rejects unsafe or unsupported channels actionably", () => {
    ...
    expect(
      transitionSchema.safeParse({ ...recipe, custom: { ...recipe.custom, css: "filter: blur(2px)" } }).success,
    ).toBe(false);
    const outgoingClip = transitionSchema.safeParse({
      ...recipe,
      custom: { ...recipe.custom, outgoing: { clip: { shape: "linear" } } },
    });
    expect(outgoingClip.success).toBe(false);
    if (!outgoingClip.success)
      expect(outgoingClip.error.issues.map((issue) => issue.message).join(" ")).toMatch(/unrecognized|outgoing/i);
    const mismatchedOrigins = transitionSchema.safeParse({
      type: "custom",
      duration: 300,
      custom: {
        incoming: {
          scale: { from: 0.8, origin: { x: 0.2, y: 0.2 } },
          clip: { shape: "radial", origin: { x: 0.8, y: 0.8 } },
        },
        outgoing: { opacity: 0 },
      },
    });
    expect(mismatchedOrigins.success).toBe(false);
    if (!mismatchedOrigins.success)
      expect(mismatchedOrigins.error.issues[0].message).toContain("same viewport-relative origin");
  });
  ...
  it("discriminates family parameters and rejects irrelevant channels", () => {
    expect(transitionSchema.parse({ type: "push", duration: 300, push: { angle: 35, distance: 0.8 } })).toMatchObject({
      push: { angle: 35, distance: 0.8 },
    });
    expect(transitionSchema.parse({ type: "reveal", duration: 300, reveal: { shape: "radial" } })).toMatchObject({
      reveal: { shape: "radial", origin: { x: 0.5, y: 0.5 }, radius: 1 },
    });
    expect(transitionSchema.parse({ type: "zoom", duration: 300, zoom: {} })).toMatchObject({
      zoom: { fromScale: 0.9, origin: { x: 0.5, y: 0.5 } },
    });
    expect(
    ...
    expect(
      transitionSchema.safeParse({
        type: "reveal",
        duration: 300,
        reveal: { shape: "linear", origin: { x: 0.5, y: 0.5 } },
      }).success,
    ).toBe(false);
...
describe("legacy transition normalization", () => {
  ...
  it("maps every legacy type onto only opacity, translate, scale, clip, and overlay channels", () => {
    for (const type of TYPES) {
      ...
      expect(Object.keys(plan.incoming).every((key) => ["opacity", "translate", "scale", "clip"].includes(key))).toBe(
        true,
      );
  ...
  it("normalizes parameter families to bounded viewer-safe channels", () => {
    const translate = normalizeTransition({ type: "push", duration: 200, push: { angle: 45, distance: 1 } }).incoming
      .translate;
    ...
    expect(
      normalizeTransition({ type: "zoom", duration: 200, zoom: { fromScale: 1.4, origin: { x: 0.25, y: 0.75 } } })
        .incoming.scale,
    ).toEqual({ from: 1.4, origin: { x: 0.25, y: 0.75 } });
  });

  it("retains legacy wipe and clock parameters in their normalized clip plans", () => {
    expect(normalizeTransition({ type: "wipe", duration: 200, wipeAngle: 37 }).incoming.clip).toEqual({
      shape: "wipe",
      angle: 37,
    });
    expect(
      normalizeTransition({ type: "wipe-clock", duration: 200, wipeStartAngle: 90, wipeCounterclockwise: true })
        .incoming.clip,
    ).toEqual({
      shape: "clock",
      startAngle: 90,
