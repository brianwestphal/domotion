...
function render(
  transition: NonNullable<AnimationFrame["transition"]>,
  nextTransition: NonNullable<AnimationFrame["transition"]> = { type: "cut", duration: 0 },
): string {
...
describe("parameterized transition rendering", () => {
  ...
  it("applies viewport-relative origins to radial reveals and zooms", () => {
    const radial = render({
      type: "reveal",
      duration: 300,
      reveal: { shape: "radial", origin: { x: 0.25, y: 0.75 }, radius: 1 },
    });
  ...
  it("passes bounded shine parameters into the shared safe sweep", () => {
    const svg = render({
      type: "shine",
      duration: 300,
      shine: { angle: 30, bandWidth: 0.2, color: "#ffeeaa", opacity: 0.4 },
    });
    ...
    const svg = render(
      {
        type: "reveal",
        duration: 300,
        reveal: { shape: "clock", origin: { x: 0.4, y: 0.6 }, startAngle: 90, direction: "counterclockwise" },
      },
    ...
    const svg = render({
      ...
      custom: {
        incoming: {
          ...
          scale: { from: 0.8, origin: { x: 0.5, y: 0.5 } },
