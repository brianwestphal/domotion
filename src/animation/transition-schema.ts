...
const plainLegacy = (type: "crossfade" | "cut" | "magic-move") =>
  z.object({ type: z.literal(type), duration }).strict();
const pushLegacy = (type: "push-left" | "push-right" | "push-up" | "push-down" | "scroll") =>
  z.object({ type: z.literal(type), duration }).strict();
...
const pushParamsSchema = z.union([
  z
    .object({
      ...
      distance: z.number().positive().max(2).default(1),
    })
    .strict(),
...
const revealParamsSchema = z.discriminatedUnion("shape", [
  ...
  z
    .object({
      shape: z.literal("radial"),
      origin: originSchema.default({ x: 0.5, y: 0.5 }),
      radius: z.number().min(1).max(2).default(1),
    })
    .strict(),
  z
    .object({
      shape: z.literal("clock"),
      origin: originSchema.default({ x: 0.5, y: 0.5 }),
      startAngle: z.number().min(-360).max(360).default(0),
...
const shineParamsSchema = z
  ...
  .strict();
const translatePrimitiveSchema = z
  .object({
  ...
  .strict()
  .refine((value) => value.x !== 0 || value.y !== 0, "translate must move on at least one axis");
const scaleFromPrimitiveSchema = z
  .object({ from: z.number().min(0.01).max(4), origin: originSchema.default({ x: 0.5, y: 0.5 }) })
  .strict();
const scaleToPrimitiveSchema = z
  .object({ to: z.number().min(0.01).max(4), origin: originSchema.default({ x: 0.5, y: 0.5 }) })
  .strict();
const customIncomingSchema = z
  .object({
  ...
  .strict()
  .refine((value) => Object.keys(value).length > 0, "incoming must declare at least one primitive");
const customOutgoingSchema = z
  .object({
  ...
  .refine(
    ...
    "outgoing must declare at least one primitive; outgoing clip is unsupported—put clip on incoming",
  );
const customRecipeSchema = z
  .object({
  ...
  .superRefine((recipe, ctx) => {
    ...
    const clipOrigin =
      recipe.incoming.clip != null && recipe.incoming.clip.shape !== "linear" ? recipe.incoming.clip.origin : undefined;
    if (
      scaleOrigin != null &&
      clipOrigin != null &&
      (scaleOrigin.x !== clipOrigin.x || scaleOrigin.y !== clipOrigin.y)
    ) {
      ctx.addIssue({
        code: "custom",
        path: ["incoming"],
        message: "combined scale + clip must use the same viewport-relative origin",
      });
    }
  });

export const transitionSchema = z.discriminatedUnion("type", [
  plainLegacy("crossfade"),
  plainLegacy("cut"),
  plainLegacy("magic-move"),
  pushLegacy("push-left"),
  pushLegacy("push-right"),
  pushLegacy("push-up"),
  pushLegacy("push-down"),
  pushLegacy("scroll"),
  z.object({ type: z.literal("wipe"), duration, easing, wipeAngle: z.number().optional() }).strict(),
  radialLegacy("iris"),
  radialLegacy("wipe-radial"),
  z
    .object({
      type: z.literal("wipe-clock"),
      duration,
      easing,
      wipeStartAngle: z.number().optional(),
      wipeCounterclockwise: z.boolean().optional(),
    })
    .strict(),
  zoomLegacy("zoom-in"),
  zoomLegacy("zoom-out"),
...
export const storyboardTransitionSchema = z.union(
  transitionSchema.options.filter((option) => option.shape.type.value !== "magic-move"),
);
...
export function normalizeTransition(transition: Transition): NormalizedTransitionPlan {
  ...
  switch (type) {
    case "push-left":
      return push(type, durationMs, "X", -1);
    case "push-right":
      return push(type, durationMs, "X", 1);
    case "push-up":
    case "scroll":
      return push(type, durationMs, "Y", -1);
    case "push-down":
      return push(type, durationMs, "Y", 1);
    case "push": {
      ...
      return {
        ...
        parameterized: true,
        incoming: { opacity: "hold", translate: v },
        outgoing: { opacity: "hold", translate: v },
      };
    }
    case "wipe":
      return {
        ...
        easing: transition.easing,
        parameterized: false,
        incoming: { opacity: "hold", clip: { shape: "wipe", angle: transition.wipeAngle } },
        outgoing: { opacity: "hold" },
      };
    case "iris":
    case "wipe-radial":
      return {
        ...
        easing: transition.easing,
        parameterized: false,
        incoming: { opacity: "hold", clip: { shape: "iris" } },
        outgoing: { opacity: "hold" },
      };
    case "wipe-clock":
      return {
        ...
        incoming: {
          ...
          clip: {
            shape: "clock",
            startAngle: transition.wipeStartAngle,
            counterclockwise: transition.wipeCounterclockwise,
          },
        },
        outgoing: { opacity: "hold" },
      };
    case "reveal": {
      ...
      const clip =
        r.shape === "linear"
          ? { shape: "wipe" as const, angle: r.angle }
          : r.shape === "radial"
            ? { shape: "iris" as const, origin: r.origin, radius: r.radius }
            : {
                shape: "clock" as const,
                origin: r.origin,
                startAngle: r.startAngle,
                counterclockwise: r.direction === "counterclockwise",
              };
      return {
        legacyType: type,
        duration: durationMs,
    ...
    case "zoom-out":
      return {
        ...
        easing: transition.easing,
        parameterized: false,
        incoming: { opacity: "fade", scale: { from: type === "zoom-in" ? 0.9 : 1.1 } },
        outgoing: { opacity: "fade" },
      };
    case "zoom":
      return {
        legacyType: type,
        ...
        easing: transition.easing,
        parameterized: true,
        incoming: { opacity: "fade", scale: { from: transition.zoom.fromScale, origin: transition.zoom.origin } },
        outgoing: { opacity: "fade" },
      };
    case "shine":
      return {
        legacyType: type,
        duration: durationMs,
        parameterized: transition.shine != null,
        incoming: { opacity: "fade" },
        outgoing: { opacity: "fade" },
        overlay: "shine",
        shine: transition.shine,
      };
    case "custom": {
      ...
      const normalizedClip =
        clip == null
          ? undefined
          : clip.shape === "linear"
            ? { shape: "wipe" as const, angle: clip.angle }
            : clip.shape === "radial"
              ? { shape: "iris" as const, origin: clip.origin, radius: clip.radius }
              : {
                  shape: "clock" as const,
                  origin: clip.origin,
                  startAngle: clip.startAngle,
                  counterclockwise: clip.direction === "counterclockwise",
                };
      return {
        legacyType: type,
        duration: durationMs,
        ...
        parameterized: true,
        incoming: {
          opacity: recipe.incoming.opacity == null ? "hold" : "fade",
          opacityFrom: recipe.incoming.opacity,
          translate:
            recipe.incoming.translate == null
              ? undefined
              : { x: -recipe.incoming.translate.x, y: -recipe.incoming.translate.y },
          scale:
            recipe.incoming.scale == null
              ? undefined
              : { from: recipe.incoming.scale.from, origin: recipe.incoming.scale.origin },
          clip: normalizedClip,
        },
        outgoing: {
          opacity: recipe.outgoing.opacity == null ? "hold" : "fade",
          opacityTo: recipe.outgoing.opacity,
          translate: recipe.outgoing.translate,
          scale: recipe.outgoing.scale,
        },
        overlay: recipe.overlay == null ? undefined : "shine",
        shine: recipe.overlay,
    ...
    case "magic-move":
      return {
        ...
        duration: durationMs,
        parameterized: false,
        incoming: { opacity: "hold" },
        outgoing: { opacity: "cut" },
        overlay: "magic-move",
      };
    case "cut":
      return {
