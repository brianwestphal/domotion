...
function img(intrinsic: { w: number; h: number }, effectiveZoom?: number): CapturedElement {
  return {
    ...
    children: [],
    imageSrc: PNG,
    imageIntrinsic: intrinsic,
    imageEffectiveZoom: effectiveZoom,
    styles: STYLES,
...
describe("Blink concrete object-fit rectangle", () => {
  it("resolves arbitrary percentage alignment instead of SVG min/mid/max buckets", () => {
    expect(computeObjectFitRect(10, 20, 200, 100, 80, 80, "contain", "25% 75%")).toEqual({
      x: 35,
      y: 20,
      width: 100,
      height: 100,
  ...
  it("resolves computed calc positions against negative free space for cover", () => {
    expect(computeObjectFitRect(0, 0, 200, 100, 80, 80, "cover", "calc(100% - 10px) calc(100% - 20px)")).toEqual({
      x: -10,
      y: -120,
      width: 200,
      height: 200,
  ...
  it("uses the smaller concrete object for scale-down", () => {
    expect(computeObjectFitRect(0, 0, 200, 100, 60, 40, "scale-down", "50% 50%")).toEqual({
      x: 70,
      y: 30,
      width: 60,
      height: 40,
