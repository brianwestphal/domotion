...
export function emitVerticalRasterText(ctx: PaintCtx, el: CapturedElement, indent: string): void {
  ...
  ctx.defsParts.push(
    `<clipPath id="${erCid}"><rect x="${r(er.x)}" y="${r(er.y)}" width="${r(er.width)}" height="${r(er.height)}" /></clipPath>`,
  );
  ctx.svgParts.push(
    `${indent}<image href="${dataUri}" x="${r(er.x)}" y="${r(er.y)}" width="${r(er.width)}" height="${r(er.height)}" preserveAspectRatio="none" clip-path="url(#${erCid})"/>`,
  );
}
...
export function buildPseudoBoxBgLayers(
  ...
  for (let li = layers.length - 1; li >= 0; li--) {
    ...
    const rxAttr =
      pb.borderRadius != null && pb.borderRadius > 0 ? ` rx="${r(pb.borderRadius)}" ry="${r(pb.borderRadius)}"` : "";
    out.push(
      `<rect x="${r(pb.x)}" y="${r(pb.y)}" width="${r(pb.width)}" height="${r(pb.height)}"${rxAttr} fill="url(#${defId})" />`,
    );
  }
  return out.join("");
}
...
export function renderOneText(
  ...
  opts: {
    ...
    affineMatrix?: CapturedTextPaintAffine;
  },
  emit: (pb: {
    x: number;
    y: number;
  ...
  try {
    ...
    if (opts.el.tag === "input" || opts.el.tag === "textarea")
      return wrapAffineTextPaint(opts.affineMatrix, renderInputText(optsWithEmit));
    ...
    return wrapAffineTextPaint(
      opts.affineMatrix,
      `<g clip-path="url(#${opts.clipId})">${renderSourceOwnedTextBoundary(el.text, "element-render-failed")}</g>`,
    );
  } finally {
...
export function resolveTextFill(
  ...
  if (
    topmostTextBgClipFill == null &&
    textIsTransparent &&
    el.styles.inheritedTextFillGradient != null &&
    el.styles.inheritedTextFillGradient !== "" &&
    el.styles.inheritedTextFillGradient !== "none"
  ) {
  ...
  const fillColor =
    topmostTextBgClipFill != null && textIsTransparent
...
export function paintText(
  ...
  if (paintsClampFragments ? hasCapturedClampFragment : el.text !== "") {
    const { fillColor, textIsTransparent } = resolveTextFill(
      ctx,
      el,
      textColor,
      textBgClipFills,
    ...
    if (
      el.elementRaster != null &&
      el.elementRaster.dataUri != null &&
      el.styles.writingMode != null &&
      el.styles.writingMode !== "horizontal-tb"
    ) {
