...
const bar = (
  ...
  orientation,
  route: "native-raster",
  frameRect: { x: 0, y: 0, width: 4, height: 5 },
  usedWidth: "auto",
  logicalSide: orientation === "horizontal" ? "bottom" : "right",
  visibleSize: 10,
  totalSize: 20,
  currentPosition: 0,
  enabled: true,
...
const set = (overrides: Partial<CapturedScrollbarSet> = {}): CapturedScrollbarSet => ({
  ...
  rootScroller: false,
  horizontal: bar("horizontal", raster(1)),
  vertical: bar("vertical", raster(2)),
  nativeCornerRaster: raster(3),
  overlay: false,
