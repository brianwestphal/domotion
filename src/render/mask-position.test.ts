...
describe("Blink mask-position geometry (DM-2379)", () => {
  it("keeps calc components intact and resolves percentage + physical px", () => {
    expect(splitMaskPositionComponents("calc(25% + 7px) calc(75% - 3px)")).toEqual([
      "calc(25% + 7px)",
      "calc(75% - 3px)",
    ]);
    expect(resolveMaskPosition("calc(25% + 7px) calc(75% - 3px)", 120, 40)).toEqual({ x: 37, y: 27 });
  });

  it("resolves positions against negative free space for cover", () => {
    expect(resolveMaskPosition("25% 75%", -100, -40)).toEqual({ x: -25, y: -30 });
  ...
  it("fits contain, then positions at a non-quantized percentage", () => {
    ...
    expect(
      resolveMaskContainCoverRect({ x: 10, y: 20, width: 180, height: 120 }, { w: 200, h: 100 }, "contain", "23% 73%"),
