...
describe("drift guard: the transcription equals the installed playwright-core's table", () => {
  ...
  interface PwPlatform {
    fontFamilies: PwFontFamilies;
    forScripts?: Array<{ script: string; fontFamilies: PwFontFamilies }>;
  }
  ...
  const pw = (
    require(join(pwRoot, "lib", "server", "chromium", "defaultFontFamilies.js")) as {
  ...
  it("mac/win forScripts match key-for-key after the ScriptNameToCode conversion the browser applies", () => {
    for (const [ourPlatform, pwPlatform] of [
      ["darwin", "mac"],
      ["win32", "win"],
    ] as const) {
...
describe("firstAvailableOrFirst (font_cache.cc:220-227 → ui/gfx/font_list.cc:246)", () => {
  it("a single name returns as-is without an availability check", () => {
    expect(
      firstAvailableOrFirst("Songti SC", () => {
...
describe("resolveFontKey(family, lang) moves the settings-mapped generics per script", () => {
  ...
  it.runIf(process.platform === "darwin")(
    "the per-script entry outranks the session-probed Common-script override, mirroring the settings lookup order",
    () => {
      withHostPlatform("darwin", () => {
  ...
  it.runIf(process.platform === "darwin")(
    "a session-probed per-script answer outranks the static Playwright transcription",
    () => {
      withHostPlatform("darwin", () => {
        setSessionGenericFamilyOverrides({
  ...
  it.runIf(process.platform === "darwin")(
    "a hidden Hebrew painted face remains a fallback from the captured Common cursive primary",
    () => {
      withHostPlatform("darwin", () => {
        setSessionGenericFamilyOverrides({
  ...
  it.runIf(process.platform === "darwin" && isGlyphHelperAvailable())(
    "CoreText reaches the hidden Hebrew identity only from the Apple Chancery handle",
    () => {
  ...
  it.runIf(process.platform === "darwin")(
    "darwin host: lang=zh-Hans serif resolves the Songti SC face; ja sans-serif the Hiragino Sans key",
    () => {
      withHostPlatform("darwin", () => {
  ...
  it.runIf(process.platform === "darwin")(
    "the chain ends with the script-keyed STANDARD family — Blink's FontDataAt list's final entry",
    () => {
      withHostPlatform("darwin", () => {
