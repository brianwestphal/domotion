  ...
  return [
    {
      ...
      height: 50,
      children: [],
      styles: { ...baseStyles, webkitBoxReflect: reflect },
    } as CapturedElement,
  ];
}
...
describe("-webkit-box-reflect vector duplication", () => {
  it("parses directions, signed/percentage offsets, and the computed nine-piece mask suffix", () => {
    expect(
      parseBoxReflection("below -8px linear-gradient(transparent 30%, black) 0 fill / auto / 0 stretch", 100, 50),
    ).toEqual({
      direction: "below",
      offset: -8,
      maskImage: "linear-gradient(transparent 30%, black)",
    });
  ...
  it("applies a computed gradient as an alpha mask to only the reflected copy", () => {
    const svg = elementTreeToSvg(
      tree("below 0px linear-gradient(rgba(0,0,0,0), rgba(0,0,0,0.8)) 0 fill / auto / 0 stretch"),
      260,
      180,
    );
