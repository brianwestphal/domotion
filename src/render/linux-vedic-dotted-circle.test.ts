...
const FAMILY =
  '"Mukta", "Arial Unicode MS", "Apple Symbols", "Apple Color Emoji", "Noto Sans", "Noto Serif", sans-serif';
...
describe("Linux Vedic selected-face dotted circles", () => {
  ...
  beforeAll(() => {
    ...
    noCircleSubset = hbSubsetRetainGids(
      fs.readFileSync(FREE_SERIF),
      [0, face.glyphForCodePoint(0x1cd1).id],
      0,
      true,
      null,
    );
  ...
  itLinux("keeps a well-formed base+mark cluster free of an inserted U+25CC", () => {
    ...
    expect(run).toMatchObject({
      ...
      request: { script: "Deva" },
      selected: { postscriptName: "FreeSerif", sourcePath: FREE_SERIF, faceIndex: 0 },
    });
  ...
  itLinux("does not insert U+25CC when the selected face has no nominal circle glyph", () => {
    ...
    expect(run).toMatchObject({
      mechanism: "declared-family",
      request: { script: "Deva" },
      selected: { fontKey: "webfont:dm2420 no circle", postscriptName: "FreeSerif" },
    });
    expect(compactGlyphs(run)).toEqual([{ id: 3402, cluster: 0, xAdvance: 0, yAdvance: 0, xOffset: 0, yOffset: 0 }]);
  });
