...
/**
 * DM-1900: Apple's system UI faces are optically SIZE-dependent, and opening
 * them at `unitsPerEm` for design-unit geometry silently pinned them to their
 * largest optical cut at every run size.
 *
 * The gap this closes, measured against Chrome on macOS 26.5.2 by
 * `Range.getBoundingClientRect` over a run of U+0915 in a `system-ui` stack
 * (advance normalised to upem 1000):
 *
 *     size    Chrome    was      now
 *       13     867.4    792      867
 *       17     853.0    792      853
 ...
 *       28     802.0    792      802
 *
 * The "was" column is flat because every size resolved to the same handle. The
 * full Blink pipeline that produces Chrome's column — UI-font base ->
 * `CTFontCreateForString` -> clone at `opsz` = specified size, clamped — is
 * reproduced in `tools/scratch/dm1900-pipeline.swift` and agrees to <1 unit.
 */
describeHelper("optically size-dependent faces (DM-1900)", () => {
  const SF_INDIA = "/System/Library/Fonts/SFIndia.ttc";
  const itIfPresent = existsSync(SF_INDIA) ? it : it.skip;

  /** Advance of U+0915 in design units, opened as a run of `size` would. */
  function advanceAt(size: number, variations?: Record<string, number>): number | null {
    const f = createGlyphHelperFont({
      postscriptName: ".SFDevanagari-Regular", fontPath: SF_INDIA,
      variations, opticalSize: size,
    });
    const g = f?.glyphForCodePoint?.(0x0915);
    return g?.advanceWidth ?? null;
  }

  itIfPresent("tracks the run size instead of collapsing to one optical cut", () => {
    // Before this, all four were 792 — the largest cut — regardless of size.
    const a13 = advanceAt(13, { opsz: 17 });
    const a20 = advanceAt(20, { opsz: 20 });
    const a28 = advanceAt(28);
    expect(a13).not.toBeNull();
    // Monotonically tighter as the run grows, which is what optical sizing does.
    expect(a13!).toBeGreaterThan(a20!);
    expect(a20!).toBeGreaterThan(a28!);
    // …and each lands on Chrome's measured value.
    expect(a13!).toBeCloseTo(867, 0);
    expect(a20!).toBeCloseTo(838.6, 0);
    expect(a28!).toBeCloseTo(802, 0);
  });

  itIfPresent("leaves a SIZE-INVARIANT face byte-identical across sizes", () => {
    // The guard that keeps this change inert for ordinary fonts: Helvetica's
    // advance is the same at every size, so it must stay on the untouched path
    // and produce one number. A regression here would mean every ordinary face
    // started round-tripping through the run-size path for no reason.
    const helvetica = "/System/Library/Fonts/Helvetica.ttc";
    if (!existsSync(helvetica)) return;
    const at = (size: number) => {
      const f = createGlyphHelperFont({ postscriptName: "Helvetica", fontPath: helvetica, opticalSize: size });
      return f?.glyphForCodePoint?.(0x0061)?.advanceWidth ?? null;
    };
    const a = at(13), b = at(28), c = at(1000);
    expect(a).not.toBeNull();
    expect(a).toBe(b);
    expect(b).toBe(c);
  });

  itIfPresent("is inert when no run size is supplied", () => {
    // Callers that do not know the size keep the previous behavior exactly,
    // rather than silently getting a different instance.
    const f = createGlyphHelperFont({ postscriptName: ".SFDevanagari-Regular", fontPath: SF_INDIA });
    expect(f?.glyphForCodePoint?.(0x0915)?.advanceWidth).toBeCloseTo(792, 0);
  });
});
