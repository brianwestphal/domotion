...
export type HtmlClipGeometryBox =
  | "border-box"
  | "padding-box"
  | "content-box"
  | "margin-box"
  ...
  | "view-box"
  | "half-border-box";
export type SvgEffectGeometryBox =
  "content-box" | "padding-box" | "fill-box" | "border-box" | "margin-box" | "stroke-box" | "view-box";
...
export function svgEffectReferenceBox(
  boxes: SvgEffectReferenceBoxes,
  geometryBox: SvgEffectGeometryBox,
...
export function clipReferenceBox(
  el: ClipPathBoxInput,
  geometryBox: HtmlClipGeometryBox,
): { x: number; y: number; width: number; height: number } {
  const border = {
    top: px(el.styles.borderTopWidth),
    right: px(el.styles.borderRightWidth),
    bottom: px(el.styles.borderBottomWidth),
    left: px(el.styles.borderLeftWidth),
  };
  const padding = {
    top: px(el.styles.paddingTop),
    right: px(el.styles.paddingRight),
    bottom: px(el.styles.paddingBottom),
    left: px(el.styles.paddingLeft),
  };
  const margin = {
    top: px(el.styles.marginTop),
    right: px(el.styles.marginRight),
...
function roundedReferenceBox(el: ClipPathBoxInput, geometryBox: HtmlClipGeometryBox): string {
  ...
  if (geometryBox === "padding-box")
    radii = insetCornerRadii(radii, border.top, border.right, border.bottom, border.left);
  else if (geometryBox === "content-box" || geometryBox === "fill-box")
    ...
    radii = insetCornerRadii(radii, border.top / 2, border.right / 2, border.bottom / 2, border.left / 2);
  else if (geometryBox === "margin-box")
    radii = outsetCornerRadiiWithCorrection(
      radii,
      px(el.styles.marginTop),
      px(el.styles.marginRight),
      px(el.styles.marginBottom),
      px(el.styles.marginLeft),
    );
...
export function clipPathShapeForElement(el: ClipPathBoxInput, clipPathCss: string): string {
  const match =
    /\b(content-box|padding-box|border-box|margin-box|fill-box|stroke-box|view-box|half-border-box)\b/i.exec(
      clipPathCss,
    );
  ...
  const shape =
    match == null
      ? clipPathCss.trim()
      : (clipPathCss.slice(0, match.index) + clipPathCss.slice(match.index + match[0].length)).trim();
