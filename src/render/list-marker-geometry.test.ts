...
describe("Blink symbol marker geometry oracle (DM-2192)", () => {
  it.each([
    ...
    "transcribes integer disc/circle/square geometry at ascent $ascent",
    ({ ascent, font, width, box, block, end }) => {
      for (const type of ["disc", "circle", "square"] as const) {
        expect(blinkSymbolMarkerGeometry(ascent, font, type)).toEqual({
          markerInlineSize: box,
          inlineOffset: 1,
          blockOffset: block,
          inlineSize: width,
          blockSize: width,
          outsideEndMargin: end,
        });
      }
    },
  );
  ...
  it("maps the captured first-line marker fragment through Blink's line writing mode", () => {
    ...
    expect(blinkPhysicalSymbolMarkerRect(fragment, 18, 24, 1, "disclosure-closed", "horizontal-tb")).toEqual({
      x: 100,
      y: 42.16,
      width: 15.84,
      height: 15.84,
    ...
    expect(blinkPhysicalSymbolMarkerRect(fragment, 18, 24, 1, "disclosure-closed", "vertical-rl")).toEqual({
      x: 112,
      y: 40,
      width: 15.84,
      height: 15.84,
    });
    expect(blinkPhysicalSymbolMarkerRect(fragment, 18, 24, 1, "disclosure-closed", "vertical-lr")).toEqual({
      x: 112,
      y: 40,
      width: 15.84,
      height: 15.84,
    });
    expect(blinkPhysicalSymbolMarkerRect(fragment, 18, 24, 1, "disclosure-closed", "sideways-lr")).toEqual({
      x: 102.16,
      y: 40,
      width: 15.84,
      height: 15.84,
  ...
  it("maps disclosure orientation through writing direction", () => {
    ...
    expect(disclosureTriangle(rect, "disclosure-closed", "horizontal-tb", "rtl")).toBe(
      "10,0 1.4000000000000001,5 10,10",
    );
    expect(disclosureTriangle(rect, "disclosure-open", "horizontal-tb", "ltr")).toBe(
      "0,0.7000000000000001 5,9.3 10,0.7000000000000001",
    );
    ...
    expect(disclosureTriangle(rect, "disclosure-closed", "sideways-lr", "ltr")).toBe(
      "0,9.3 5,0.7000000000000001 10,9.3",
    );
  });
});
