...
describe("capture-owned text terminal provenance", () => {
      ...
      textSegments: [
        {
          ...
          width: 18,
          height: 22,
          rasterRect: { x: 8, y: 12, width: 18, height: 22 },
          rasterDataUri: dataUri,
        },
      ],
    } as CapturedElement;

    expect(
      renderSingleLineText({ el: element, idPrefix: "dm2511", clipId: "dm2511-clip", fillColor: "black" }),
    ).toContain(dataUri);
    expect(getTextRunProvenance()).toEqual({
      runs: [],
  ...
  it("records a segment raster before the multi-segment text emitter is bypassed", () => {
    ...
    expect(
      renderMultiSegmentText({ el: element, idPrefix: "dm2511", clipId: "dm2511-clip", fillColor: "black" }, [segment]),
    ).toContain(dataUri);
  ...
  it("records a legacy element raster before input text emission is bypassed", () => {
    ...
    expect(renderInputText({ el: element, idPrefix: "dm2511", clipId: "dm2511-clip", fillColor: "black" })).toContain(
      dataUri,
    );
    expect(getTextRunProvenance().transitions).toEqual([
