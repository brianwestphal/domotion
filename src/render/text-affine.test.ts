...
describe("source-owned affine text paint", () => {
  it("composes and inverts the complete signed affine matrix", () => {
    ...
    expect(multiplyTextAffine(inverse, composed)).toEqual(
      expect.arrayContaining([
        expect.closeTo(local[0], 10),
        expect.closeTo(local[1], 10),
        expect.closeTo(local[2], 10),
        expect.closeTo(local[3], 10),
        expect.closeTo(local[4], 10),
        expect.closeTo(local[5], 10),
      ]),
    );
  });
  ...
  it("removes an already-emitted ancestor matrix without losing shear/reflection", () => {
      ...
      fragments: [
        {
          ...
          paintMatrix: paint,
          affineResidual: 0,
          lineOrigin: horizontalLineOrigin(10, 20, 60, 18, 14),
          inlineOffset: 10,
          shapedOrigins: [],
          shapedAdvances: [],
          writingMode: "horizontal-tb",
    ...
    expect(multiplyTextAffine(ctm, prepared.residualMatrix!)).toEqual(
      expect.arrayContaining([
        expect.closeTo(paint[0], 8),
        expect.closeTo(paint[1], 8),
        expect.closeTo(paint[2], 8),
        expect.closeTo(paint[3], 8),
        expect.closeTo(paint[4], 8),
        expect.closeTo(paint[5], 8),
      ]),
    );
      ...
      fragments: [
        {
          ...
          domUtf16Span: [0, 1],
          physicalFragmentIndex: 0,
          neutralQuad: [0, 0, 1, 0, 1, 1, 0, 1],
          paintQuad: [0, 0, 1, 0, 1, 1, 0, 1],
          paintMatrix: [1, 0, 0, 1, 0, 0],
          affineResidual: 0,
          lineOrigin: horizontalLineOrigin(0, 0, 1, 1, 1),
          inlineOffset: 0,
          shapedOrigins: [0],
          shapedAdvances: [1],
          writingMode: "horizontal-tb",
  ...
  it("consumes the structured writing-plane origin once before the affine matrix", () => {
    ...
    expect(prepareAffineTextPaint(el, [1, 0, 0, 1, 0, 0]).failureReason).toContain(
      "disagrees with its UTF-16 source owner",
    );
    el.textPaintGeometry.fragments[0].domUtf16Span = [0, 2];
    lineOrigin.writingModeRotation = [1, 0, 0, 1, 0, 0];
    expect(prepareAffineTextPaint(el, [1, 0, 0, 1, 0, 0]).failureReason).toContain("writing-mode rotation changed");
  });
});
