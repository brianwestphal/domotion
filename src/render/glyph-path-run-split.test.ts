...
function splitFull(
  fam: string,
  text: string,
): Array<{ text: string; key: string; decomposed: boolean; mechanism: string | undefined }> {
  const key = resolveFontKey(fam);
  const font = resolveFont(fam, 400, 32, 0)!;
  const chain = resolveFontKeyChain(fam);
  const runs = splitTextIntoGlyphPathRuns(
  ...
  return runs.map((r) => ({
    text: r.text,
    key: r.fontKey,
    decomposed: r.decomposed === true,
    mechanism: r.routeMechanism,
  }));
}
...
(MACOS_FONTS ? describe : describe.skip)(
  "partial webfont primary at cluster granularity (docs/113 §2 case 6, paths mode)",
  () => {
...
(MACOS_FONTS ? describe : describe.skip)("raster emoji uses Chromium's ordinary fallback terminal", () => {
  ...
  it("keeps emoji cases on the shaped Chromium face without a chain-tail terminal", () => {
    for (const [fam, text] of emojiTexts) {
      ...
      expect(
        shaped.some((run) => run.key === "last-resort"),
        `${fam} ${text}`,
      ).toBe(false);
    }
  });
