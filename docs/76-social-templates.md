...
for `them`) so it grows out of its tail, staggered down the thread. All messages
stay in the flex layout (only scaled/faded), so nothing reflows as each appears.
...
click — three distinct elements, one animation each.
