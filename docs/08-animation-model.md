...
Each frame boundary composes two independent halves — how the previous frame LEAVES (its own transition's exit) and how the current frame ENTERS (the previous transition's entrance). DM-1414 composed the slide/fade families; **DM-1548** unifies this ACROSS families (a unified `emitComposedFrame` — reveal/dolly/slide entrances composed with any exit), so mixed chains like `zoom-out → wipe` or `wipe → push-left` compose instead of dropping half the effect. Same-type chains and slide/fade mixes stay byte-identical. See `docs/88`.
...
- **An `opacity` animation owns the element's opacity channel.** Normally the renderer bakes an element's captured opacity onto its wrapper (`<g opacity="0.2">`) and drops `opacity: 0` elements from the markup entirely (a real size win — an invisible subtree is dead weight). Both behaviors would break a fade-in: SVG group opacity composes multiplicatively, so a keyframe animating 0.2→1 under a baked 0.2 wrapper peaks at 0.2·1 = 0.2 (the element could never brighten past its captured value), and a dropped element leaves nothing in the SVG to animate. So when an intra-frame animation (primary `property` or a `fuse` track) animates `opacity` on an element, the renderer suppresses the baked wrapper opacity AND exempts the element from the `opacity: 0` drop — the animation's keyframes are the single source of the element's opacity. (Internally: the `animate` pipeline runs the `annotateAnimatedProperties` tree-ops pass between capture and render, recording each animation's animated properties on its target elements; the renderer keys off that annotation, not off `animId` alone, so magic-move / cull wrappers are unaffected.) Two consequences for authors:
  - **Rest state**: outside the animation's window the element shows the keyframes' held value (`from` before the delay, `to` after the window — one-shot animations hold both ends on the scene clock; `repeat` loops with a positive delay hold `from` via fill-mode `both`). To make the frame at rest paint identically to the capture, set `from` to the captured opacity (e.g. an element captured at `opacity: 0.2` fading in should use `from: "0.2"`). Setting `from: "0"` on a partially-transparent capture is also valid — it just means the element rests fully hidden before the fade rather than at its captured dimness.
  - **Fade-in from invisible**: an element captured at `opacity: 0` can now be faded in directly (`from: "0"`, `to: "1"`) — the markup exists, kept invisible at rest by the keyframes instead of a baked zero-opacity wrapper. Plain `opacity: 0` elements without an opacity animation are still dropped. (Before this, the workaround was capturing the element fully opaque and animating an opaque cover rect out on top of it.)
