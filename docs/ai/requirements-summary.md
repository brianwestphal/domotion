...
Replaced video/canvas output owns one frozen Chromium-presented frame only. Pairing exact animation time with the pre-navigation capture-rAF handle activates the shipped atomic preflight/freeze/capture/reverify transaction: finite video must decode, seek and present; origin-clean canvas bytes, media facts, every owner PNG, page/frame navigation and compositor/surface epochs must remain exact or capture fails closed. Captures without both authorities stay observational. For animated GIF/APNG/WebP, docs 231–236 and DM-2583–2585/DM-2579/DM-2580/DM-2581 ship the macOS/Linux-ratified strict path: only an explicit `animatedImageFrames` list installs the pre-navigation ledger; authenticated unique same-origin base owners, SVG `href`, or explicit indexed ordinary CSS `url(...)` items pass through two fresh `ImageDecoder` instances with `preferAnimation`, complete-frame and reverse-order exact controls. Track/VideoFrame/RGBA/PNG facts and source epoch must agree before only the named owner slot is replaced. The frozen PNG alone enters embedding, Sharp/libvips, resize/dedup caches and SVG emission. DM-2582's optional native macOS/Linux workflow retains exact decoder, production-test, and environment artifacts and requires the complete two-platform aggregate. Missing options preserve legacy capture exactly; errors expose stable codes but no denied byte facts. Cross-origin CORS, cache/revalidation/SW, repeated-resource ambiguity, image-set, pseudo/shadow owners, Windows evidence, global native release, and live playback remain unsupported. DM-2590 owns Windows/global evidence.
