...
Replaced video/canvas output owns one frozen Chromium-presented frame only. Pairing exact animation time with the pre-navigation capture-rAF handle activates the shipped atomic preflight/freeze/capture/reverify transaction: finite video must decode, seek and present; origin-clean canvas bytes, media facts, every owner PNG, page/frame navigation and compositor/surface epochs must remain exact or capture fails closed. Captures without both authorities stay observational. Playback, decoder continuation, live canvas mutation and media-time effects after capture remain unsupported. For animated GIF/APNG/WebP, doc 229 source-proves a separate exact-byte WebCodecs route for arbitrary complete **static** frame selection, including fresh forward/reverse logical controls. Doc 231 proves that CDP can transport the exact selected Blink `ResourceBuffer` without a refetch once its request is authenticated, but public CDP does not itself prove the owner/slot-to-request relation or authorize cross-origin bytes for every element/CSS route. DM-2583 supplies the pinned private, evidence-only owner/resource/security truth oracle and exact proposal/validation schema; it never exposes denied body facts or becomes a production runtime dependency. The source-gated public join, production byte collector, strict frozen-PNG transaction, downstream/CSS enablement, and native release gates remain unimplemented. The independent macOS proposal/validation pair agrees exactly; the global six-artifact verdict remains withheld on DM-2589 until fresh Linux and Windows proposal/validation evidence is retained. Legacy capture stays unchanged and live playback remains out of scope.
