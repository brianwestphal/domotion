...
Replaced video/canvas output owns one frozen Chromium-presented frame only. Pairing exact animation time with the pre-navigation capture-rAF handle activates the shipped atomic preflight/freeze/capture/reverify transaction: finite video must decode, seek and present; origin-clean canvas bytes, media facts, every owner PNG, page/frame navigation and compositor/surface epochs must remain exact or capture fails closed. Captures without both authorities stay observational. Playback, decoder continuation, live canvas mutation and media-time effects after capture remain unsupported. For animated GIF/APNG/WebP, doc 229 now source-proves a separate exact-byte WebCodecs route for arbitrary complete **static** frame selection, including fresh forward/reverse logical controls, but this is investigation evidence only. Production arbitrary-frame capture remains unsupported until authenticated encoded-byte acquisition and the strict opt-in frozen-PNG transaction are implemented and gated; live playback remains out of scope.
