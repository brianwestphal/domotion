...
Replaced video/canvas output owns one frozen Chromium-presented frame only. Pairing exact animation time with the pre-navigation capture-rAF handle activates the shipped atomic preflight/freeze/capture/reverify transaction: finite video must decode, seek and present; origin-clean canvas bytes, media facts, every owner PNG, page/frame navigation and compositor/surface epochs must remain exact or capture fails closed. Captures without both authorities stay observational. For animated GIF/APNG/WebP, docs 231–234 and DM-2583–2585/DM-2579/DM-2580/DM-2591 ship the macOS/Linux-ratified strict owner path: only an explicit `animatedImageFrames` list installs the pre-navigation ledger; authenticated unique same-origin `<img>/<picture>`, image-input, SVG image href, or explicit ordinary CSS `url(...)` property/index HTTP(S), ordinary data, and same-partition blob bytes pass through two fresh `ImageDecoder` instances with `preferAnimation`, complete-frame and reverse-order exact controls. Track/VideoFrame/RGBA/PNG facts and source epoch must agree before replacement. Only that frozen PNG enters embedding, Sharp/libvips, resize/dedup caches and SVG emission; its digest is checked and exact post-resize dimensions/length/SHA retain source epoch and requested index. Missing options preserve legacy capture exactly; errors expose stable codes but no denied byte facts. Cross-origin CORS, cache/revalidation/SW, repeated-resource ambiguity, CSS image-set/pseudo/shadow owners, Windows evidence, native release gates, and live playback remain unsupported or downstream. DM-2590 owns Windows/global evidence; DM-2581 owns downstream CSS/SVG substitution.
