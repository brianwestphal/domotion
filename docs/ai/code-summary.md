...
DM-2583 implements doc 231's evidence-only pinned Chromium owner/resource truth oracle without putting patched Chromium on the production path. `tools/animated-image-owner-resource-truth-schema.ts` owns the exact record, 38-case corpus, redaction rules, normalization, and six-artifact adjudicator; the `-probes.ts` and `-fixtures.ts` files own source citations and live mutations. `-chromium.patch` is the exact 26-file private Blink change, `-manifest.mjs` authenticates its applied source, `-collector.ts` runs fresh explicitly headless arms and authenticates live browser/renderer mappings, and `-adjudicator.ts` reopens and seals retained reports fail closed. The helper proves owner/slot -> content -> Resource -> inspector-request and CORS/body authority directly; it does not decode, substitute, paint, or enable a production byte collector. DM-2589 and DM-2590 retain independent macOS, Linux, and Windows proposal/validation reports: all six agree on the exact 38-row logical digest, preserve body-free denied envelopes, and pass the canonical adjudicator with zero failures and `proposal-validation-agreement`.

DM-2584/DM-2585 implement the macOS/Linux-ratified base-owner acquisition boundary. `tools/animated-image-stock-cdp-support.ts` publishes the exact 38-case support matrix; `src/capture/authenticated-animated-image-bytes.ts` owns the opt-in pre-navigation Network ledger, document/owner/request epoch, bounded same-origin HTTP(S), data, and same-partition blob transports, immutable-copy handoff, and body-free stable failures. `DemoRecorder` installs it only for a non-empty `animatedImageFrames` request. Doc 232 is normative. Decoding, frame selection, PNG replacement, CSS/SVG owners, broad CORS/cache/SW joins, Windows evidence, and live playback remain downstream or unsupported.

DM-2579 continues that exact opt-in in `src/capture/animated-image-static-frame.ts` (doc 233): two fresh `ImageDecoder` instances with `preferAnimation`, complete-frame decoding and reverse-order validation must agree in track, VideoFrame, raw-RGBA and PNG identities before the authenticated owner is replaced. Production capture tests prove the final SVG retains only the frozen PNG. DM-2580/doc 234 extends that ownership through Sharp/libvips resize and SVG emission: only the frozen PNG enters downstream caches, its digest is checked before decode, and exact post-resize dimensions/length/SHA retain the encoded-source epoch and requested index. DM-2581/doc 236 adds the DM-2591-ratified SVG `href` and explicit indexed ordinary CSS `url(...)` subset. DM-2582's native workflow retains fresh decoder, 18-test production, and runner-fingerprint artifacts on macOS/Linux/Windows and requires a complete three-platform aggregate; the retained verdict is `macos-linux-windows-production-exact`. Image-set, pseudo/shadow, repeated-resource, cache/SW, and cross-origin routes remain fail-closed.
