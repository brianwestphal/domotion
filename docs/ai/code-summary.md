...
DM-2584/DM-2585/DM-2591 implement the macOS/Linux-ratified owner acquisition boundary. `tools/animated-image-stock-cdp-support.ts` publishes the exact 38-case support matrix; `src/capture/authenticated-animated-image-bytes.ts` owns the opt-in pre-navigation Network ledger, document/owner/request epoch, bounded same-origin HTTP(S), data, and same-partition blob transports, immutable-copy handoff, and body-free stable failures. Base HTML/image-input owners, SVG image href, and explicit ordinary CSS `url(...)` property/index slots use a public-only unique URL/frame/loader join. CSS `image-set()`, pseudo/shadow owners, broad CORS/cache/SW joins, Windows evidence, and live playback remain unsupported. `DemoRecorder` installs the collector only for a non-empty `animatedImageFrames` request. Doc 232 is normative.

DM-2579 continues that exact opt-in in `src/capture/animated-image-static-frame.ts` (doc 233): two fresh `ImageDecoder` instances with `preferAnimation`, complete-frame decoding and reverse-order validation must agree in track, VideoFrame, raw-RGBA and PNG identities before the authenticated base owner is replaced. Production capture tests prove the final SVG retains only the frozen PNG. DM-2580/doc 234 extends that ownership through Sharp/libvips resize and SVG emission: only the frozen PNG enters downstream caches, its digest is checked before decode, and exact post-resize dimensions/length/SHA retain the encoded-source epoch and requested index. DM-2581 can consume DM-2591's public SVG and ordinary CSS URL-slot proof; image-set and pseudo/shadow ownership remain fail-closed.
