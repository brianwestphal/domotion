...
`down:-100px` ≡ `up:100px`. A signed delta magnitude **composes with the direction prefix by sign-multiplication** — it does _not_ take the magnitude as `abs` with the prefix unconditionally winning. The resolved delta is `prefixSign × signedMagnitude`, where the prefix sign is `+1` for `down` / `right`, `-1` for `up` / `left`, and `+1` (default `down`) when no prefix is present (`resolveScrollAction` in `src/scroll/executor.ts`). So a negative magnitude **reverses** the prefix's direction: `down:-100px` resolves to a delta of `+1 × (−100) = −100` and `up:100px` to `−1 × 100 = −100` — both scroll up 100, which is why they're equivalent.
...
- **Auto-chunking** (executor implementation detail, not grammar). A long single scroll (multiple visible slices covered by one `down:bottom/30s` action) is subdivided internally so the composer has enough anchor points to stack contiguous captures. A window-owned scroll steps by the configured output viewport; an element-owned scroll steps by that element's live `clientWidth` or `clientHeight` on the active axis, even when the captured subtree or clip is larger. The final step clamps to the requested destination and may overlap the preceding slice. The pattern grammar surface is unaffected — this is purely how the executor schedules captures.
