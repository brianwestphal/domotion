---
id: "requirements/declarative-animate-config"
title: "43 — Declarative animate config"
kind: "contract"
status: "current"
owners: ["animation"]
platforms: ["windows"]
...
- A `cast` / `template` / `scroll` frame's content is a **nested `<svg>`** inside its frame group. To avoid document-global name clashes, that nested document's ids / classes / `@keyframes` / `--scene-dur` are prefixed per frame — `cfN_` for casts, `tfN_` for templates, and `sfN_` for scroll composites (`namespaceEmbeddedAnimatedSvg`, doc 73 / DM-1287 / DM-1292). Every timed nested document, including a scroll composite (DM-2701), records its local period as `embeddedAnimationPeriodMs` so its timeline is re-anchored to the containing frame's offset on the master loop (DM-1319).
