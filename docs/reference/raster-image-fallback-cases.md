...
Trigger: `src/capture/script/emoji-detect.ts` ports Blink's pinned whole-sequence emoji grammar and records every non-whitespace grapheme as a temporary candidate. Node then runs the candidate through the same declared-family, presentation-priority, system-fallback, and shaping path used for vector text. `selectedGlyphRasterSpans()` keeps the candidate only when the selected glyph's concrete representation is sbix, COLR, CBDT/CBLC, or OpenType SVG without an available outline. There are no Unicode block lists, named glyph exceptions, canvas color probes, or font-name tests. Consequently arbitrary color icon webfonts are covered, while a monochrome glyph in an emoji-associated block—and an outline glyph in a mixed color-capable face—remains vector.
