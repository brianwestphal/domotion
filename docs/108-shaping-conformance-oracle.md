...
- **Only axis- and feature-bearing text nodes are split on whitespace.** Every other node must be whitespace-free as a whole or it is dropped, so multi-word runs are unswept. Splitting universally would grow the corpus 10.95× (2,385 → 26,121); whether that breadth is worth its runtime is unanswered.
- **The corpus's feature-bearing runs cannot currently exercise the feature machinery, and that is a property of the fixtures, not of the instrument.** See the section below.
...
So the zero is real: the fixtures ask for features from fonts that do not have them. What the corpus cannot yet exercise is a **known divergence** it would otherwise be the natural instrument for — `parseFontFeatureSettings` drops `0`/`off` entries entirely, because fontkit takes an enable-only list and cannot disable a default-on feature. The corpus contains `"dlig" 0, "liga" 0` on Georgia, but Georgia's ligatures do not fire for those samples in Chrome either (`office` and `waffle` are 6 glyphs both ways), so the divergence stays invisible. Exercising it needs a fixture that disables a ligature on a face that actually forms one.
