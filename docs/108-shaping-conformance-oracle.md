...
- **`RunSpec` carries no `font-stretch`.** Now the sharper of the two axis gaps: on macOS `system-ui`, `font-stretch` moves painted width from 133.52px to 272.70px across 50–200% (measured above) — a wider spread than `font-variation-settings` produces on the same face. The capture layer already records `fontStretch`, but the render side never reads it, so the property is captured and dropped. Neither the corpus nor the renderer models it.
