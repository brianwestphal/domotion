...
| `studioNarrativeSchema` / `studioReviewHistorySchema` / `studioReviewAuthorSchema` / `studioReviewRevisionSchema` / `studioReviewAnnotationSchema` / `studioAnnotationTargetSchema` / `studioAnnotationRegionSchema` / `studioArtifactSchema` / `studioScriptHookSchema` | schema | Runtime contracts for narrative beats, revisions/annotations and their grounding, generated-output provenance, and referenced (never inline) script hooks. |
...
| `StudioProjectValidationError` / `StudioProjectCompileError` / `StudioInteractionError` / `StudioInteractionObservationError` / `StudioInteractiveSceneError` / `StudioHealingError` / `StudioTreatmentError` / `StudioVideoReviewError` | class | Structured validation, replay, evidence, healing, treatment, and video-review failures; actionable scene errors retain current-page browser evidence and the completed causal trail. |
...
| `StudioNarrative`, `StudioSemanticTarget`, `StudioSemanticEvent`, `StudioSemanticTrack`, `StudioScriptHook`, `StudioReviewHistory`, `StudioReviewRevision`, `StudioArtifact` | type | Narrative, intent, extension, evidence-capable review, and generated-artifact value types. |
