...
| `StudioProjectValidationError` / `StudioProjectCompileError` / `StudioInteractionError` / `StudioInteractionObservationError` / `StudioInteractiveSceneError` / `StudioHealingError` / `StudioTreatmentError` / `StudioVideoReviewError` / `StudioAnnotationError` | class | Structured validation, replay, evidence, healing, treatment, video-review, and annotation transaction failures; actionable scene errors retain current-page browser evidence and the completed causal trail. |
