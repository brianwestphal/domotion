...
| `StudioProjectValidationError` / `StudioProjectCompileError` / `StudioInteractionError` / `StudioInteractionObservationError` / `StudioInteractiveSceneError` / `StudioHealingError` / `StudioVideoReviewError` | class | Structured validation, replay, evidence, healing, and video-review failures; actionable scene errors retain current-page browser evidence and the completed causal trail. |
