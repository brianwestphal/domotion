...
| `captureElementTreeWithDebug` | function | Capture the normal inline result plus in-memory `{ expectedPng, capturedTreeJson }` evidence from the same stable Chromium frame. It performs no filesystem writes and does not own or close the caller's browser context. |
...
| `assembleCaptureDebugBundle` | function | Combine `captureElementTreeWithDebug().debug`, caller-rendered SVG, and optional caller-recorded HAR text/bytes into one filesystem-neutral in-memory bundle. |
...
| `pagedCaptureBundleManifestSchema` / `parsePagedCaptureBundleManifest` | schema / function | Strict version-1 authenticated/unavailable paged-capture manifest contract. Runtime validation rejects source/helper drift, PDF-derived logical claims, unknown fields, nonconsecutive selection indices, unordered document pages, nondeterministic relative SVG paths, and bundle digest drift. See `docs/255-paged-media-capture-bundle.md`. |
| `buildAuthenticatedPagedCaptureBundleManifest` / `pagedCaptureBundleDigest` | function | Construct and canonically hash an authenticated manifest after the private print transport and page renderer have supplied their evidence. These functions do not launch a browser or make an unavailable record look authenticated. |
| `verifyPagedCaptureBundleAssets` | function | Re-read every page SVG through a caller-supplied asset reader and reject missing assets, byte-length/SHA-256 drift, non-SVG payloads, and external runtime references. Unavailable manifests perform no asset reads. |
| `buildPagedCaptureBundleJsonSchema` / `pagedCaptureBundleJsonSchemaText` / `PAGED_CAPTURE_BUNDLE_SCHEMA_ID` | function / const | Generate the shipped draft-2020-12 editor schema at `schemas/paged-capture-bundle.schema.json` from the runtime Zod source. Cross-page and digest invariants remain runtime checks. |
| `PagedCaptureBundleManifest` / `AuthenticatedPagedCaptureBundleManifest` / `UnavailablePagedCaptureBundleManifest` / `AuthenticatedPagedCaptureBundleInput` / `PagedCaptureBundleAssetReader` | type | Public manifest, builder-input, and asset-reader types for the paged bundle surface. |
| `capturePagedSvgBundle` / `writeAuthenticatedPagedCaptureBundle` | function | Explicit authenticated paged-capture orchestration and atomic bundle publication. The high-level API launches only the caller-pinned helper; the writer stages and verifies every SVG before publishing the manifest last. See `docs/258-paged-capture-api-cli.md`. |
| `normalizePagedPageRanges` / `PagedCaptureError` | function / class | Normalize bounded closed one-based print ranges and expose stable fail-closed diagnostic categories for paged capture. |
| `CapturePagedSvgBundleOptions` / `PagedCapturePrintOptions` / `WritePagedCaptureBundleOptions` | type | Public helper trust anchor, page preparation, print control, and approved-output inputs for paged capture. |
| `pagedCaptureHelperBundleManifestSchema` / `parsePagedCaptureHelperBundleManifest` | schema / function | Strict separately distributed helper manifest. It pins Chromium/Skia/depot_tools/patch/protocol identities, platform loader metadata, the executable, the complete runtime closure, licenses, and manual-only update policy. |
| `verifyPagedCaptureHelperBundle` | function | Locate a caller-supplied helper using a required external manifest SHA-256 trust anchor, then stream-verify every declared member, mode, link, path boundary, platform, and canonical runtime-closure digest. It never downloads or substitutes a browser. |
| `launchPagedCaptureHelper` / `authenticatePagedCaptureHelperTransport` / `authenticatePagedCaptureHelperProcesses` | function | Explicitly launch only the verified local executable, prove the default-off and active private-print handshake, authenticate the live DevTools product/protocol/schema plus response-bound browser/renderer image mappings, then reverify the bundle. The launched handle exposes a repeatable process-authentication hook for the capture epoch. |
| `buildPagedCaptureHelperBundleJsonSchema` / `pagedCaptureHelperBundleJsonSchemaText` / `PAGED_CAPTURE_HELPER_BUNDLE_SCHEMA_ID` | function / const | Generate the shipped draft-2020-12 editor schema at `schemas/paged-capture-helper-bundle.schema.json`; exact inventory, runtime-closure, and cross-field invariants remain runtime checks. |
| `PagedCaptureHelperBundleManifest` / `VerifiedPagedCaptureHelperBundle` / `PagedCaptureProcessAuthentication` / `LaunchedPagedCaptureHelper` | type | Helper distribution, verified locator, live-process evidence, and launched-runtime records for the opt-in paged path. |
| `captureAuthenticatedPagedPageRecord` / `isLiveAuthenticatedPagedPageRecord` | function | Run one all-pages private print transaction against a real page owned by a verified helper launch, authenticate both raw sidecars and live process/frame/source identity before promotion, and test the non-serializable in-memory authority brand. PDF stream bytes are never read. |
| `parsePagedPageRecord` / `validateAuthenticatedPagedPageRecord` / `unavailablePagedPageRecord` | function | Parse persisted unavailable records, structurally validate an authenticated shape, or produce a typed unavailable result. Persisted JSON and caller-constructed shapes cannot acquire live authentication; raw promotion is private to the capture transaction. |
| `PAGED_PAGE_RECORD_VERSION` / `PAGED_PAGE_RECORD_ABI` / `PAGED_PAGE_RECORD_MAX_BYTES` / `PAGED_PAGE_RECORD_CHROMIUM_REVISION` / `PAGED_PAGE_RECORD_SOURCE_PINS` | const | Version, helper ABI, 64 MiB transport bound, Chromium revision, and reviewed Blink/Skia source owners for the page-record contract. |
| `renderAuthenticatedPagedSvgPages` | function | Return the native self-contained SVG bytes for every live-authenticated page after internally exact-checking the helper's raw 1/64 CSS-pixel collapsed-border logical rectangles against the shared replay. The internal audit is not overpainted onto the SVG. |
| `PagedPageRecord` / `AuthenticatedPagedPageRecord` / `UnavailablePagedPageRecord` / `AuthenticatedPagedPage` / `PagedFragmentRecord` / `PagedVectorPaintRecord` / `PagedPoint` / `PagedSize` / `PagedPagePrintOptions` / `RenderedPagedSvgPage` | type | Public authenticated/unavailable record, explicit native-local geometry, all-page print-option, vector-scene, and rendered in-memory page shapes. |
...
| `CaptureElementTreeResult`, `CaptureElementTreeDebugResult` | type | Inline warning/state result and its debug-evidence extension returned by the two capture APIs. |
| `CaptureDebugArtifacts`, `CaptureDebugBundle`, `AssembleCaptureDebugBundleOptions` | type | In-memory PNG/tree evidence, the assembled SVG/HAR bundle, and the optional caller-owned HAR input. |
...
| `Transition` / `TransitionType` | type | Canonical discriminated frame-transition object and vocabulary, including strict parameterized `push`, `reveal`, `zoom`, and `shine` forms. Units and bounds: doc 117. |
| `custom` transition recipe | `Transition` variant | Bounded incoming/outgoing opacity, viewport translate, scale, incoming clip, and supported shine overlay composition; no raw CSS/script/SMIL. See doc 118. |
...
| `resolveCaretPoint` | function | Resolve `{ target, charOffset }` against a captured tree → `{ x, baselineY, ascentPx, descentPx, fontSize, cellWidthPx, rtl?, vertical?, columnWidthPx? }` on Chromium's own painted glyph positions (captured `xOffsets` / `yOffsets`). `charOffset` counts LOGICAL code points: inside an RTL bidi run the caret sits on the addressed character's right edge and `rtl: true` says the insertion cell extends left from `x`; for vertical writing modes `vertical` carries the captured `writing-mode`, `x` is the column's left edge and `baselineY` the along-column position. |
| `resolveRangeRects` | function | Resolve a `[charStart, charEnd)` range → one selection rect per covered text run — and per bidi level run within it, so a logical range over mixed-direction text yields the same visually discontiguous rects Chromium fragments its own selection into — each with per-character sweep edges in logical order (`rtl` rects sweep right-to-left; `vertical` rects span the column and sweep downward). |
...
| `studioNarrativeSchema` / `studioReviewHistorySchema` / `studioReviewAuthorSchema` / `studioReviewRevisionSchema` / `studioReviewAnnotationSchema` / `studioAnnotationTargetSchema` / `studioAnnotationScopeSchema` / `studioAnnotationTimeSchema` / `studioAnnotationRegionSchema` / `studioArtifactSchema` / `studioScriptHookSchema` | schema | Runtime contracts for narrative beats, content/review revisions, normalized annotation scope/time/region grounding, generated-output provenance, and referenced (never inline) script hooks. |
...
| `inspectStudioHealingPage` | function | Capture an AI-mode browser ARIA snapshot plus bounded raw DOM author hints, live rectangles, and computed CSS without inventing accessible roles or names. |
| `runStudioHealingLoop` / `resumeStudioHealingLoop` | function | Replay, require AI repair for actionable failures, append evidence-backed AI revisions, require AI review, pause/resume through digest-checked clarification checkpoints, and hand an accepted candidate to human review. |
| `renderAndReviewStudioVideo` / `resumeStudioVideoReview` | function | Render production MP4 review media, require all-dimension AI findings, append grounded findings to the shared review queue, and pause/resume ambiguous decisions through digest-checked clarification. |
| `studioVideoReviewFindingSchema` / `studioVideoReviewReportSchema` / `STUDIO_VIDEO_REVIEW_DIMENSIONS` | schema / const | Structured evidence, inference, severity, recommendation, grounding, comparison, and the authoritative seven-dimension visual-quality rubric. |
| `runStudioAgentTool` / `STUDIO_AGENT_TOOL_VERSION` | function / const | Execute one versioned, MCP-shaped Studio tool request in process. The host supplies the trusted actor, bounded capabilities, workspace root, and capture/preview/video adapters; results are compact and include exact absolute plus workspace-relative artifact paths. |
| `studioAgentToolRequestSchema` / `studioAgentToolResponseSchema` / `studioAgentToolArtifactSchema` | schema | Runtime and JSON-Schema-projectable transport source for project inspect/create/edit, capture compile, preview/video render, and annotation tools. Status is explicitly `ok`, `clarification`, `permission-required`, or `conflict`. |
| `buildStudioAgentToolRequestJsonSchema` | function | Project the draft-2020-12 MCP/CLI input schema directly from the strict runtime request union. |
| `studioAgentProjectDigest` | function | Produce the validated SHA-256 compare-and-swap token required by project and annotation mutations. |
| `applyStudioAnnotationCommand` / `studioAnnotationCommandSchema` | function / schema | Apply one bounded human, AI, or system annotation create/edit/lifecycle command, append reversible revision provenance, reject no-ops/stale heads, and return a revalidated project. |
| `normalizeStudioAnnotationTarget` | function | Convert readable legacy v1 scene/time aliases into the normalized project/scene scope plus independent point/range model used by every new annotation write. |
| `importSvgScrubberReviewAnnotation` / `svgScrubberReviewTicketSchema` | function / schema | Validate and migrate SVG Scrubber review JSON into a portable Studio annotation command while retaining frame/range/region source evidence without absolute paths. |
...
| `StudioHealingPageInspection`, `StudioReplayFailure`, `StudioHealRequest`, `StudioReviewRequest`, `StudioAiHealingDecision`, `StudioAiReviewDecision` | type | Browser-owned repair evidence and the required application AI callback protocol. |
| `StudioHealingLoopResult`, `StudioHealingClarificationCheckpoint`, `StudioAutomationEvidence`, `StudioAutomationChange`, `StudioClarificationAnswer` | type | Human-handoff, clarification, and evidence-backed automation revision values. |
| `StudioVideoReviewReport`, `StudioVideoReviewFinding`, `StudioVideoReviewRequest`, `StudioVideoReviewResult`, `StudioVideoReviewClarificationCheckpoint`, `StudioReviewMedia` | type | Structured rendered-media AI review, grounding, comparison, clarification, and shared-queue result values. |
...
ordered scene, generation, and review state. Generated scene or whole-story SVG
