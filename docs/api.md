...
| `resolveCaretPoint` | function | Resolve `{ target, charOffset }` against a captured tree → `{ x, baselineY, ascentPx, descentPx, fontSize, cellWidthPx, rtl? }` on Chromium's own painted glyph positions (captured `xOffsets`). `charOffset` counts LOGICAL code points: inside an RTL bidi run the caret sits on the addressed character's right edge and `rtl: true` says the insertion cell extends left from `x`. |
| `resolveRangeRects` | function | Resolve a `[charStart, charEnd)` range → one selection rect per covered text run — and per bidi level run within it, so a logical range over mixed-direction text yields the same visually discontiguous rects Chromium fragments its own selection into — each with per-character sweep edges in logical order (`rtl` rects sweep right-to-left). |
