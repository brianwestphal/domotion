...
`translateClipPath()` in `src/render/clip-path.ts` (DM-1305; re-exported from `element-tree-to-svg.ts`) recognises `inset` / `circle` / `ellipse` / `polygon` / `path` shape functions and returns "" for everything else — including `url(#id)`. When `translateClipPath` returns "", the renderer skips emitting a `<clipPath>` def and the element paints unclipped (the overflow-clip path takes over when an `overflow` is set, but that's a different clip).
