...
- **Radial gradients (SK-1225 / DM-2649)**: shipped via `<radialGradient>`. Supports `circle`/`ellipse` shapes, the four extent keywords (`closest-side` / `closest-corner` / `farthest-side` / `farthest-corner`), explicit length sizing, explicit two-axis `<length-percentage>` ellipse sizing, and the `at <position>` clause (single keyword, two-token keyword/length pairs, percent positions). Blink resolves the first explicit ellipse radius against the gradient-box width and the second against its height; Domotion preserves those separate bases when it emits `r` plus `gradientTransform`. A lone percentage is invalid because a one-value explicit size denotes a circle, whose radius must be a length. Ellipses use `gradientTransform="translate(cx, cy) scale(1, ry/rx) translate(-cx, -cy)"` to bend the natively-circular SVG radial into the desired aspect. For the corner keywords the ellipse keeps the matching side's aspect ratio and is scaled to pass through the corner — exactly `√2 ×` the side radii (DM-1243; verified against Chrome's painted ring).
...
- **Color hints (DM-1242 / DM-2305)**: the bare `<percentage>` between two color stops (e.g. `linear-gradient(red, 20%, blue)`) shifts the 50% transition via a power curve. Blink itself approximates that curve by replacing the hint with exactly nine ordinary stops: two on its short side and seven on its long side (`ReplaceColorHintsWithColorStops`, Chromium rev `7d859f27`). `parseGradientStops` now transcribes those positions and weights exactly instead of applying Domotion's former seven-sample uniform-weight approximation.
