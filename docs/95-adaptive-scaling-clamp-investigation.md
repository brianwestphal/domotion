...
_exponents_ if differential scaling is wanted — do NOT switch the cards to raw
`vw`/`clamp()`.** Viewport units _do_ capture faithfully (the one open question
in the ticket), but:

1. **Naive `vw` does the opposite of the DM-1541 goal.** `vw` is proportional to
   canvas _width_; a 9:16 reel is narrow, so a `vw`-sized headline comes out
   _smaller_ on the reel than on landscape — exactly the "type tiny and lost in
...
2. **`clamp()`/viewport units are always-on, breaking the opt-in contract.** The
   ...
   viewport units respond to _every_ viewport, so they'd change the default and
...
3. **They only help the wrapping text cards.** The number cards (`stat`,
   ...
   value from clipping; a `clamp()` can't express "as big as _fits_ this column
   count." The uniform path already covers both card families.

A properly _tuned_ `clamp()` (weighted toward `vh`) **can** match or beat the
...
- **naive `vw`** — headline is visibly _smaller_, floats in empty space: the
...
- **Opt-in contract breaks.** `formatScaleFactor(…, safeInset)` returns exactly
  ...
  authored into the card CSS is _always_ live — it rescales the default 1280×720
...
- **Number cards still need JS.** `stat`/`counter` clamp the odometer cell to the
  ...
  _fits_ N columns" isn't expressible in `clamp()`; those cards would keep their
...
The only thing the uniform factor _can't_ do is scale elements differentially.
