...
- **Action targets**: automatic click/hover/fill events snapshot both the target point and its effective cursor **before** the action runs. The generated move carries that keyword as an override, so dismissing a dialog, moving a button, or navigating cannot recompute the click glyph from unrelated content in the post-action capture at the same coordinates (DM-1995).
- **Hit-test**: `cursorAtPoint(roots, x, y)` (`src/animation/cursor-overlay.ts`) returns the cursor keyword of the **visually-topmost element** whose box contains the point, hit-testing in **true paint order** (DM-1742, via `hitTestTopmost` in `src/render/paint-order.ts`): the tree is flattened through the renderer's own stacking-context traversal (`gatherStackingContextChildren` / `sortChildrenByPaintOrder` — z-index buckets, float/inline hoisting, viewport-fixed pulling, preserve-3d re-sort) and the last painted hit wins, so the glyph matches whatever the rendered SVG paints on top. This fixes stacked layouts (overlapping windows switched via z-index, modals, dropdowns, sticky headers) where the earlier DFS-pre-order approximation answered with whatever happened to be later in the DOM. `pointer-events: none` elements are transparent to the hit-test (captured as `styles.pointerEvents`, only when `none`; the property inherits so descendants carry it themselves), `overflow != visible` ancestors clip their descendants' hit boxes (`position: fixed` escapes, matching the render path), and `opacity: 0` elements still hit — browsers hit-test transparent boxes. `cursor` inherits and was resolved per element, so the topmost element's stored value (or `default` when omitted) is what Chrome painted. The flattened sequence is memoized per frame tree.
