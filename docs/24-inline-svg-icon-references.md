...
Pattern (2) is broken: the cloned consumer's `outerHTML` contains a `<use href="#icon-check">` whose target is in a _different_ SVG that Domotion never emits. The reference dangles in the output — Chromium's SVG renderer falls back to nothing painted, and the icon disappears (or, worse, the host element's text label is captured and renders as PUA-glyph tofus where Chrome painted the icon).
