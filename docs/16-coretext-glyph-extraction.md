...
- A `glyphs` query takes an array of either Unicode codepoint (`cp`) or pre-resolved glyph id (`id`) entries. `cp` is resolved with `CTFontGetGlyphsForCharacters`; missing-glyph (id 0) entries return empty path data.
...
- The cached helper binary exists at the user-cache path (or downloads successfully on first need).
