...
- A `fallback` query (`{ "type": "fallback", "fontRef": "f1", "cps": [ … ], "cssWeight": 700, "bold": true, "italic": false }`) answers, per codepoint, which installed face the system would substitute when the referenced font lacks the character — `CTFontCreateForString` from that base, with `found: false` for LastResort. `cssWeight` / `bold` / `italic` carry the run's CSS description and are optional; when present the helper additionally performs Blink's **in-family cut re-selection** on the nominated face (`CTFontCreateWithFontDescriptor` over the substitute's family with the requested symbolic traits and `ToCTFontWeight(cssWeight)`, adopted only when the face moves and still covers the codepoint). Without them the nominated face is returned as-is, which is one cut too light or too heavy at most non-400 weights. See `docs/font-resolution-diagram.md` §8a.
