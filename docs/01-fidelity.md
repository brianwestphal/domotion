...
- [~] URL `background-size` / `background-position` / `background-repeat` —
  ...
  controls. Single-axis auto ratio, general calc sizing/phase, contain
  snapping, round/space, fixed/local attachment, effective zoom, transformed
  phase, cyclic longhand lists, and sliced fragments remain source-proven gaps.
  Blink's exact natural-sizing, snapped/unsnapped geometry, phase/spacing,
  attachment, and fragment contract is in
  [doc 163](163-url-background-image-geometry-audit.md); implementation is
  tracked by DM-2477/2478/2479, existing DM-2365, and gate DM-2480.
...
- [~] `::before` / `::after` generated content — aggregate positioned boxes and common single-line cases are captured, but in-flow generated text still uses a clone-derived width followed by host-edge and host-baseline heuristics. It does not preserve Blink's anonymous content-item fragments, used-font baseline, `vertical-align`, bidi visual order, vertical columns, mixed text/image sequence, or fragmentainer translation. The source-owned fragment record, fail-closed protocol boundary, mutation matrix, and implementation order are specified in [doc 157](157-pseudo-generated-fragment-geometry-audit.md) and tracked by DM-2466/DM-2467/DM-2468.
