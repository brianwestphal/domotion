...
- **Line coverage is necessary but NOT sufficient (DM-1459).** 100% line/branch coverage proves every line *executed* — it says nothing about whether every documented *behavior*, or every *transition between states*, is actually *asserted*. A bug in an untested interaction slips through a green 100% report because the lines still get hit by isolated, from-a-clean-state tests. The orthogonal axis is the **feature-coverage index** (`tests/feature-coverage.ts`): one entry per behavior → implementing export/verb → the test that would fail if it regressed. `npm run check:features` (and the `tests/conventions.test.ts` mirror in `npm test`) flags any **gap** (a behavior with no asserting test), **broken ref**, or **drift** (a new public export / CLI verb with no feature entry — so the index can't silently fall behind the code). Add a feature = add its index entry with a real test ref; if you can't name the test, you found a gap — write it. See `docs/83-feature-coverage.md`.
- **Stateful modules: test the TRANSITIONS, not just single operations.** For anything with modes / paths / a state machine (the process-global render mode, the scroll executor's `until` loops, the frame-transition-to-transition composition, the glyph-defs/embedded-font registries' per-generation lifecycle, nested-composite timeline re-anchoring, the scrubber's review mode), the feature index and the tests must cover *sequences* — operating on the module after it's already moved through prior states — because that's exactly the gap line coverage is structurally blind to. `render-text-mode-guard.test.ts` (set → scope → restore-even-on-throw) is the template.
