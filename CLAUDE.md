...
## Platform support — macOS and Linux first-class, Windows best-effort
...
**Font-capture PRECISION is a two-tier commitment:**

- **macOS and Linux are first-class.** Output is pixel-faithful to Chromium _on the platform the capture is running on_ — Chromium-on-macOS uses CoreText fallback (Hiragino, Apple Symbols, Zapf Dingbats, STIX Two Math), Chromium-on-Linux uses fontconfig (Noto / DejaVu / Liberation). Their fallback chains are calibrated against the actual painted output of Chromium on that platform, they carry per-PR fidelity gates, and a parity defect on either is a real defect.
- **Windows is best-effort.** Chromium-on-Windows uses DirectWrite (Segoe UI Symbol, Cambria Math, Consolas, Yu Gothic), and the win32 routing exists and is reasonably calibrated — but **exact font-capture precision is not a goal there**, and a Windows-only divergence is not a release blocker. Don't spend a session chasing one.

**What that means in practice:** don't invest in Windows-specific font precision work, per-PR Windows gates, or routine Windows conformance sweeps. Do keep Windows *working* — a crash, an unhandled path, a missing binary, or a hard failure on win32 is still a bug, and platform-aware routing is still required of new code (below). If a fix happens to improve Windows for free, take it; if it needs its own calibration cycle, it is not worth it.

Rationale, recorded so it is not re-litigated: Windows carried a disproportionate share of effort relative to its parity return — a resolver that was silently inert for months while reporting plausible numbers, a conformance sweep 23× slower than macOS, and a VM round trip for every measurement. macOS and Linux give better signal per hour, and the parity goal is better served by making those two exact.

All three platforms are now calibrated, not just macOS (the earlier "macOS-only" note is out of date). `src/render/font-resolution.ts` carries populated, empirically-calibrated `FONT_PATHS` (macOS), `LINUX_FONT_PATHS`, and `WIN32_FONT_PATHS` tables plus three real fallback chains (`darwinFallbackChain` / `linuxFallbackChain` / `win32FallbackChain`), each dispatched by `process.platform`, with per-platform glyph extractors (`tools/{macos,linux,win32}-glyph-extractor`) and CI gates (`.github/workflows/test-linux.yml`, `windows-fidelity.yml`). macOS is held to pixel-exact parity (`regionCount === 0`); Linux and Windows match Chromium's glyph selection and metrics within a documented native-hinting floor (Linux ≤1%, Windows ≤4% — see `docs/42-cross-platform-fallback-calibration.md`). The residual gap is unhinted-outline-vs-native-raster hinting, not missing calibration — and in the **embedded-font** render mode (the broad html/unicode sweeps) that gap is now largely RECOVERED: the embedded subsets preserve the source font's TrueType hinting program via hb-subset (variable fonts fully instanced at the resolved axis location), default-on — see `docs/99-hinted-embedded-subset.md`. The `paths`-mode floor (feature suite) still stands. The live per-codepoint system-fallback resolver is now calibrated and default-on on all three platforms (macOS CoreText, Linux fontconfig in DM-1416, Windows DirectWrite in DM-1424 — see `docs/80-cross-platform-system-fallback-resolver.md`). **Caveat worth internalising: "default-on" described the flag, not the behavior.** On Windows the resolver was flipped on and then answered "no fallback font" for every codepoint until DM-1889, because the Node side declared a base font DirectWrite does not take and cannot open, which one-shot mode treats as fatal — so Windows was silently scoring its static chain alone. It reported a stable, plausible number the whole time. This is exactly why the standing advice below is to disable the resolver and *require the answer to move*: a flag being on is not evidence a mechanism is in the loop. The **Linux** feature-suite fidelity gate ("Linux fidelity regression") is a **required status check on `main`** (baseline-relative: absolute pass/fail is advisory since the hinting floor is expected, the gate fails only on a regression vs the committed baseline). **The Windows equivalent is NOT a gate** — `windows-fidelity.yml` is manual-dispatch only under the best-effort policy, and its required-check status should be removed in the repo's branch-protection settings if still set (that is a GitHub setting, not something the workflow file controls). Remaining roadmap item (tracked in local Hot Sheet tickets): a Noto desktop-Linux profile. The broad `html-test` / `unicode` sweeps run on Linux only on-demand via `visual-tests.yml --os linux`, not per-PR; the Windows equivalents are available but should not be run routinely.
