...
- **`packages/text-engine/src/render/text-to-path.ts`** — fontkit glyph-to-path conversion (`renderTextAsPath`, run-based shaping for Arabic contextual joining, Devanagari cluster reordering, CJK GPOS). The font-routing tables + chains — `FONT_PATHS`/`LINUX_FONT_PATHS`/`WIN32_FONT_PATHS`, `fallbackFontChain()`, `resolveFontKey`, `matchFamilyNameToKey` — live in **`packages/text-engine/src/render/font-resolution.ts`** (DM-1305/DM-1307; still re-exported from `text-to-path.ts`), and the Unicode predicates in **`packages/text-engine/src/render/unicode-classification.ts`**.
...
- **`src/animation/animator.ts`** — stable public animation facade. `svg-generator.ts` owns animated SVG/body emission; `frame-timeline.ts` owns the deterministic frame clock shared with capture. The declarative pipeline keeps CLI parsing in `src/cli/animate-command.ts`, artifact optimization/writing in `animate-artifact.ts`, browser/font lifecycle in `animate-capture-session.ts`, ordinary DOM-frame serialization in `animate-frame-capture.ts`, and config/cross-frame orchestration in `animate-orchestrator.ts`.
...
The general create → track → complete ticket workflow lives in the "Ticket-Driven Work" section further down (inserted by Hot Sheet). These project-specific rules sit on top of it:
...
- **A user-mentioned problem always gets a Hot Sheet ticket immediately.** Treat credible reports framed as observations, questions, double-checks, or suspected regressions the same as explicit bug-filing requests; do not wait for the user to say “fix it” or “file a ticket.” Consolidate symptoms that demonstrably share one root cause into one bug, and create separately blocked follow-up tickets when remediation includes distinct work such as regenerating and approving public artifacts.
...
- **E2E tests** (`src/**/*e2e.test.ts`, `tests/**/*e2e.test.ts`): `vitest` under the separate `vitest.e2e.config.ts`, driving real Chromium via Playwright. Its required global preflight launches Chromium before test collection, so the authoritative `npm run test:e2e` and merged-coverage lanes fail when the Playwright Chromium browser or its platform dependencies cannot launch; individual suites must not downgrade that environment failure to skips. Run with `npm run test:e2e`.
...
- **Merged coverage** (`npm run test:coverage:all`, DM-1343/DM-2646/DM-FDMFB7): `npm run test:coverage` reflects ONLY the vitest unit suite, so browser-driven capture/CLI/server code and the big render modules read as under-covered even though the E2E and bespoke visual suites exercise them hard. `tools/coverage-all.mjs` runs the unit suite and ordinary browser E2Es (vitest `--pool=forks`, headless/no-open) plus the fast visual suites (features + showcase + snapshot-isolation + animate-examples). Unit and E2E Vitest lanes emit source-mapped Istanbul JSON; direct `tsx` visual runners emit raw Node profiles under `NODE_V8_COVERAGE` that c8 converts; and Playwright collects Chromium V8 ranges from source-mapped review, scrubber, and Studio bundles for `tools/browser-coverage-to-istanbul.mjs`. The four Istanbul maps are merged into `coverage/all/`. The intentionally headed authenticated-profile experiment remains in its dedicated `fonts:generic-profile-target` lane and is excluded from coverage. The merged command prints per-directory metrics and the instrumented files still below 50% statements, and writes the same data to `coverage/all/directory-summary.json`; its required-source gate fails if any of the three browser client entry points is absent from the converted browser map. The default intentionally skips the slow/CI-bound sweeps. `npm run test:coverage:full` (`node tools/coverage-all.mjs --full`) ALSO runs the ~277-fixture html-test sweep, the 331 per-Unicode-block fixtures, and the real-world HAR replays — which push the font/text/render modules higher (they exercise the fallback chains, shaping, and per-block routing the fast suites barely touch) but take the better part of an hour locally (the same reason CLAUDE.md shards those on CI). `--full` runs only the sweeps whose fixture checkouts exist (`external/html-test`, `../html-test/unicode`, `tests/cache/real-world`), skipping any that aren't cloned with a warning. Even `--full` won't reach 100%: platform-specific branches (Linux/Windows font extractors, the `<text>`-fallback path) don't execute on a single macOS run. Note: the merged aggregate combines Istanbul's statement accounting with c8's physical-line accounting over all `src` files, so its top-line percentage is not numerically comparable to `test:coverage`'s Vitest-only count — read the per-module numbers, not the aggregate, when comparing.
