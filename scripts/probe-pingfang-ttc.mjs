...
if (ttc.fonts) {
  ...
  for (let i = 0; i < ttc.fonts.length; i++) {
    ...
    // List tables present
    try {
      console.log(`  tables: ${Object.keys(f._directory?.tables ?? {}).join(",")}`);
    } catch (e) {}
    ...
    try {
      ...
      if (cm?.subtables)
        for (const st of cm.subtables)
          console.log(
            `    subtable platformID=${st.platformID} encodingID=${st.encodingID} format=${st.format ?? st.version ?? "?"}`,
          );
    } catch (e) {
      console.log(`  cmap THROW: ${e.message}`);
    }
    // glyphForCodePoint
    try {
      const g = f.glyphForCodePoint(SAMPLE_CP);
      console.log(
        `  glyphForCodePoint(0x${SAMPLE_CP.toString(16)}): ${g ? `id=${g.id} adv=${g.advanceWidth} bboxW=${g.bbox?.maxX - g.bbox?.minX}` : "null"}`,
      );
    } catch (e) {
      console.log(`  glyphForCodePoint THROW: ${e.message}`);
    }
    // Try direct getGlyph by id
    try {
      const g100 = f.getGlyph(100);
      console.log(
        `  getGlyph(100): id=${g100?.id} adv=${g100?.advanceWidth} bbox=${JSON.stringify(g100?.bbox ?? null)} pathOps=${g100?.path?.commands?.length ?? "?"}`,
      );
    } catch (e) {
      console.log(`  getGlyph(100) THROW: ${e.message}`);
    }
    try {
      const g500 = f.getGlyph(500);
      console.log(`  getGlyph(500): adv=${g500?.advanceWidth} pathOps=${g500?.path?.commands?.length ?? "?"}`);
    } catch (e) {
      console.log(`  getGlyph(500) THROW: ${e.message}`);
    }
    // Try layout-with-feature path
    try {
      const r = f.layout("漢");
      console.log(
        `  layout("漢"): ${r.glyphs.length} glyphs, [0]=id=${r.glyphs[0]?.id} adv=${r.glyphs[0]?.advanceWidth}`,
      );
    } catch (e) {
      console.log(`  layout THROW: ${e.message}`);
    }
    continue;
  }
  ...
  console.log(
    `  postscriptName=${ttc.postscriptName} family="${ttc.familyName}" subfamily="${ttc.subfamilyName}" numGlyphs=${ttc.numGlyphs} unitsPerEm=${ttc.unitsPerEm}`,
  );
