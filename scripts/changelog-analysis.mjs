...
function main() {
  ...
  L.push(
    `package.json ver:  base ${baseVer || "(?)"} → HEAD ${headVer || "(?)"}   (HEAD still holds the LAST released version)`,
  );
  L.push(`Next version:      ${args.next ?? "(NOT PROVIDED — the skill must ask the user)"}`);
  if (args.next)
    L.push(`Suggested output:  docs/technical-changelog/${base}-v${String(args.next).replace(/^v/, "")}.md`);
  ...
  L.push(
    `  PRODUCT CODE ONLY:  +${prodAdd} / -${prodDel}   (src + native helpers + tools + tests + scripts + examples)`,
  );
