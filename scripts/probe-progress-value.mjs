...
for (const [name, data] of Object.entries(result)) {
  ...
  if (rows.length === 0) {
    console.log(`${name}: no blue paint`);
    continue;
  }
  const yTop = rows[0].y,
    yBot = rows[rows.length - 1].y;
  const xLeft = Math.min(...rows.map((r) => r.left));
  const xRight = Math.max(...rows.map((r) => r.right));
  console.log(`\n=== ${name} (bbox h=${bbox.h}) ===`);
  console.log(
    `  blue bbox: y=[${yTop}..${yBot}] (h=${yBot - yTop + 1}) x=[${xLeft}..${xRight}] (w=${xRight - xLeft + 1})`,
  );
  console.log(
  ...
  for (const r of rows) {
    console.log(
      `    y=${r.y}: left=${r.left} right=${r.right} (inset L=${r.left - bbox.x}) mid_px=[${r.midPix.join(",")}]`,
    );
  }
}
