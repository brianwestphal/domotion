...
const result = await page.evaluate(async (b64) => {
  ...
  for (const cls of ["card", "notch", "offset", "grid", "dark", "badged"]) {
    ...
    const x0 = Math.round(r.x + 5),
      x1 = Math.round(r.x + r.width - 5);
    // scan y
    const scan = [];
    ...
    data.push({
      ...
      lr: lr ? { x: lr.x, y: lr.y, w: lr.width, h: lr.height } : null,
      btw,
      target,
      scan,
    });
...
for (const d of result) {
  console.log(`\n=== fieldset.${d.cls} ===`);
  console.log(
    `  fs y=${d.r.y.toFixed(1)}, h=${d.r.h.toFixed(1)} legend y=${d.lr ? d.lr.y.toFixed(1) : "n/a"} h=${d.lr ? d.lr.h.toFixed(1) : "n/a"} legend-center=${d.lr ? (d.lr.y + d.lr.h / 2).toFixed(1) : "n/a"}`,
  );
