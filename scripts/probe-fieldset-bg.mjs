...
for (const d of result) {
  console.log(`\n=== fieldset.${d.cls} ===`);
  console.log(
    `  fs.y=${d.fsY?.toFixed(1)} fs.h=${d.fsH?.toFixed(1)} legend.y=${d.lgY?.toFixed(1)} legend.h=${d.lgH?.toFixed(1)} legend.yCenter=${d.lgYC?.toFixed(1)}`,
  );
