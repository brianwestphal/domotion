...
const rows = await page.evaluate(
  async ({ ids, b64, W, H }) => {
    ...
    return ids.map((id) => {
      ...
      const yStart = Math.floor(rr.top),
        yEnd = Math.ceil(rr.bottom);
      let topRow = -1,
        bottomRow = -1;
      ...
      return {
        id,
        sz: cs.fontSize.replace("px", ""),
        font: cs.fontFamily.split(",")[0].slice(0, 6),
        lineH: (rr.bottom - rr.top).toFixed(1),
        cAsc: tm.fontBoundingBoxAscent,
        emAsc: emAsc.toFixed(3),
        emDesc: emDesc.toFixed(3),
