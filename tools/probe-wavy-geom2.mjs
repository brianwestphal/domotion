...
let html =
  '<!doctype html><meta charset="utf-8"><style>body{margin:24px;font-family:sans-serif;background:white;color:black;}.row{margin:18px 0;}.label{font-size:11px;color:#888;}.sample{text-decoration:underline wavy red;line-height:2em;}</style>';
...
const samples = await page.evaluate(() => {
  return Array.from(document.querySelectorAll(".sample")).map((el) => {
    ...
    return {
      ...
      thickness: parseFloat(cs.textDecorationThickness) || 1,
      rect: { x: r.x, y: r.y, w: r.width, h: r.height + parseFloat(cs.fontSize) * 0.6 },
    };
  });
});
for (const s of samples) {
  const buf = await page.screenshot({
    clip: { x: Math.floor(s.rect.x), y: Math.floor(s.rect.y), width: Math.ceil(s.rect.w), height: Math.ceil(s.rect.h) },
  ...
  if (vals.length < 50) {
    console.log(JSON.stringify({ fontSize: s.fontSize, thickness: s.thickness, error: "too-few-red-pixels" }));
    continue;
  }
