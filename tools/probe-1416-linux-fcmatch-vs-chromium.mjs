...
try {
  ...
  for (const file of files) {
    ...
    const lang = /cjk|hiragana|katakana|kana|\bhan\b|ideograph/i.test(block)
      ? "ja"
      : /hangul/i.test(block)
        ? "ko"
        : null;
  ...
  for (const r of rows) {
    ...
    if (fcBare && fcBare !== chrome0) {
      ...
      diverge.push({
        ...
        fcSans,
        fcLang,
        sansFixes: fcSans === chrome0,
        langFixes: fcLang === chrome0,
      });
    }
  }
  writeFileSync(
    OUT_PATH,
    JSON.stringify({ processed: rows.length, divergeCount: diverge.length, diverge, rows }, null, 2),
  );
