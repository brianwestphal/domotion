...
const html =
  `<style>div{font:32px sans-serif;background:white;color:black;height:48px}</style>` +
...
for (let i = 0; i < cps.length; i++) {
  ...
  console.log(
    `U+${cp.toString(16).toUpperCase().padStart(4, "0")}  ${String.fromCodePoint(cp)}  Chrome: ${fonts.fonts.map((f) => `${f.familyName}×${f.glyphCount}`).join(", ")}  paint-width: ${w.toFixed(2)}px`,
  );
}
await browser.close();
