...
function blendChannel(mode: BlendMode, s: number, d: number, sa: number, da: number): number {
  switch (mode) {
    case "normal":
      return s + d * inv(sa);
    case "multiply":
      return s * d + s * inv(da) + d * inv(sa);
    case "screen":
      return s + d - s * d;
    case "darken":
      return s + d - Math.max(s * da, d * sa);
    case "lighten":
      return s + d - Math.min(s * da, d * sa);
    case "difference":
      return s + d - 2 * Math.min(s * da, d * sa);
    case "exclusion":
      return s + d - 2 * s * d;
    case "color-burn":
    ...
    case "hard-light":
      return s * inv(da) + d * inv(sa) + (2 * s <= sa ? 2 * s * d : sa * da - 2 * (da - d) * (sa - s));
    case "overlay":
      return s * inv(da) + d * inv(sa) + (2 * d <= da ? 2 * s * d : sa * da - 2 * (da - d) * (sa - s));
    case "soft-light": {
      const m = da > 0 ? d / da : 0;
      const s2 = 2 * s;
      const m4 = 4 * m;
      ...
      const lightSource = d * sa + da * (s2 - sa) * (4 * d <= da ? darkDestination : lightDestination);
...
const luminance = (color: [number, number, number]): number => color[0] * 0.3 + color[1] * 0.59 + color[2] * 0.11;
const saturation = (color: [number, number, number]): number => Math.max(...color) - Math.min(...color);
...
function nonseparable(
  ...
  if (mode === "hue") {
    ...
    mixed = setSaturation(
      d.map((value) => value * source.a) as [number, number, number],
      saturation(s) * destination.a,
    );
    mixed = setLuminance(mixed, luminance(d) * source.a);
  } else if (mode === "color") {
...
export function blend(mode: BlendMode, source: Rgba, destination: Rgba): Rgba {
  ...
  const result =
    mode === "hue" || mode === "saturation" || mode === "color" || mode === "luminosity"
...
function colorMatrix(
  operation: Exclude<ColorFilterOperation, { type: "invert" | "opacity" | "brightness" | "contrast" }>,
): number[] {
...
const SOURCE: Rgba = { r: 38 / 255, g: 171 / 255, b: 224 / 255, a: 0.65 };
const OPAQUE_SOURCE: Rgba = { ...SOURCE, a: 1 };
const COLOR_CHAIN: readonly ColorFilterOperation[] = [
  { type: "grayscale", amount: 0.37 },
  { type: "sepia", amount: 0.44 },
  { type: "saturate", amount: 1.35 },
  { type: "hue-rotate", amount: 37 },
  { type: "invert", amount: 0.18 },
  { type: "brightness", amount: 1.12 },
  { type: "contrast", amount: 0.83 },
];
const REVERSE_COLOR_CHAIN = [...COLOR_CHAIN].reverse();

export function blendFilterFixtureHtml(): string {
  return readFileSync(
    resolve(dirname(fileURLToPath(import.meta.url)), "../tests/fixtures/html-test/37-blend-filter-pixel-stage.html"),
    "utf8",
  );
}
...
function expectedPixels(source: Rgba = SOURCE): Map<string, Pixel> {
  ...
  expected.set(
    "filter.color-chain",
    toPixel(blend("normal", applyColorFilters(OPAQUE_SOURCE, COLOR_CHAIN), DESTINATION)),
  );
  expected.set(
    "filter.reverse-chain",
    toPixel(blend("normal", applyColorFilters(OPAQUE_SOURCE, REVERSE_COLOR_CHAIN), DESTINATION)),
  );
  expected.set(
    "filter.own-source",
    toPixel(blend("normal", applyColorFilters(source, [{ type: "invert", amount: 1 }]), DESTINATION)),
  );
  expected.set(
    "filter.opacity-chain",
    toPixel(
      blend(
        "normal",
        applyColorFilters(source, [
          { type: "opacity", amount: 0.55 },
          { type: "brightness", amount: 1.18 },
        ]),
        DESTINATION,
      ),
    ),
  ...
  expected.set(
    "surface.filter-own-source",
    toPixel(blend("normal", applyColorFilters(source, [{ type: "invert", amount: 1 }]), DESTINATION)),
  );
  const backgroundBottom = blend(
    "multiply",
    { r: 40 / 255, g: 190 / 255, b: 96 / 255, a: 1 },
    { r: 220 / 255, g: 170 / 255, b: 32 / 255, a: 1 },
  );
  expected.set(
    "surface.background-stack",
    toPixel(blend("screen", { r: 48 / 255, g: 82 / 255, b: 220 / 255, a: 1 }, backgroundBottom)),
  );
  expected.set("surface.opacity-blend", toPixel(blend("normal", { ...OPAQUE_SOURCE, a: 0.58 }, DESTINATION)));
  return expected;
}
...
const maxDistance = (left: Pixel, right: Pixel): number =>
...
async function screenshotSvg(page: Page, svg: string): Promise<Buffer> {
  ...
  return Buffer.from(
    await page.screenshot({ clip: { x: 0, y: 0, width: WIDTH, height: HEIGHT }, omitBackground: true }),
  );
}
...
function probeSpecs(ids: readonly string[]): Map<string, ProbeSpec> {
  ...
  for (const id of ids) {
    if (id.startsWith("blend."))
      result.set(id, { mutation: "no-blend", movement: id === "blend.normal" ? "stable" : "moved" });
...
export async function runBlendFilterPixelStageOracle(
  ...
  return {
    ...
    sourceRules: {
      ...
      primitiveClip: "disabled",
      blurTileMode: "decal",
      blurIdentitySigma: BLUR_IDENTITY_SIGMA,
      blurKernelRadius: "ceil(3*sigma)",
      blendRepresentation: "premultiplied-rgba",
    },
    rows,
