...
function walk(n: CapturedElement, depth: number, pred: (n: CapturedElement, d: number) => boolean) {
  if (pred(n, depth)) {
    ...
    console.log(
      `${"  ".repeat(Math.min(depth, 10))}${n.tag} rect=(${Math.round(n.x)},${Math.round(n.y)},${Math.round(n.width)},${Math.round(n.height)}) op=${styles.opacity} disp=${styles.display} pos=${styles.position} transform=${styles.transform === "none" ? "-" : "YES"} src=${src || "-"}`,
    );
  }
