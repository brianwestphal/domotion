...
await page.setContent(
  readFileSync("/Users/westphal/Documents/html-test/unicode/12480-1254F-early-dynastic-cuneiform.html", "utf-8"),
);
...
function flat(node, arr = []) {
  arr.push(node);
  for (const c of node.children || []) flat(c, arr);
  return arr;
}
const all = flat(root);
...
for (const g of gNodes) {
  ...
  console.log(
    `char: ${JSON.stringify(text)} (${cp})  →  ${fonts.fonts.map((f) => `${f.familyName}×${f.glyphCount}`).join(", ")}`,
  );
}
await browser.close();
