...
try {
  ...
  console.log("No Geeza, trying SF Arabic");
}
if (!font) {
  try {
    font = fontkit.openSync("/System/Library/Fonts/SFArabic.ttf");
    console.log("Font:", font.fullName);
  } catch (e) {
    console.log("No SF Arabic either");
    process.exit(1);
  }
}
const text = "العربية";
...
for (let i = 0; i < layout.glyphs.length; i++) {
  ...
  console.log(
    `  ${i}: id=${g.id} name="${g.name || ""}" cps=[${(g.codePoints || []).map((c) => "U+" + c.toString(16).toUpperCase()).join(", ")}] xAdv=${pos.xAdvance}`,
  );
}
