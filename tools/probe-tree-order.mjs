...
await page.evaluate(async () => {
  for (let h = 0; h < document.body.scrollHeight; h += 600) {
    window.scrollTo(0, h);
...
function find(el, stack) {
  if (iconEl) return;
  if (el.svgContent && el.svgContent.includes("text-red-11")) {
    iconEl = el;
    parents = [...stack];
    return;
  }
...
function summ(el, depth, out) {
  out.push({
    ...
    pos: el.styles?.position !== "static" ? el.styles?.position : "",
    z: el.styles?.zIndex !== "auto" ? el.styles?.zIndex : "",
    xform: el.styles?.transform && el.styles.transform !== "none" ? "T" : "",
    bg:
      el.styles?.backgroundColor && el.styles.backgroundColor !== "rgba(0, 0, 0, 0)"
        ? el.styles.backgroundColor.slice(0, 22)
...
for (const e of out) {
  console.log(
    `${"  ".repeat(e.d)}${e.tag} y=${e.y} h=${e.h} x=${e.x} w=${e.w} ${e.pos}${e.z !== "" ? " z=" + e.z : ""}${e.xform ? " " + e.xform : ""} ${e.bg ? "bg=" + e.bg : ""} ${e.svg} ${e.txt}`,
  );
}
await browser.close();
