...
const targets = [
  {
    ...
    cropW: 275,
    cropH: 237,
    colorTest: (r, g, b) => r > 100 && g < 80 && b < 80,
  },
...
for (const tgt of targets) {
  for (const which of ["expected", "actual"]) {
    ...
    const absX = tgt.cropX + minX,
      absY = tgt.cropY + minY;
    const wd = maxX - minX + 1,
      ht = maxY - minY + 1;
    console.log(
      `${tgt.name.padEnd(20)} ${which.padEnd(8)} cropbox=${minX},${minY}+${wd}x${ht}  page=(${absX},${absY},${wd},${ht})  pixels=${count}`,
    );
  }
}
