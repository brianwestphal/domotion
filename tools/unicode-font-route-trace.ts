...
if (fixtureArg == null)
  throw new Error("usage: npx tsx tools/unicode-font-route-trace.ts <fixture.html> [--out file.json]");
...
const cells = await page.evaluate<CellStyle[]>(() =>
  [...document.querySelectorAll("x > g")].map((el, i) => {
    const cs = getComputedStyle(el);
    const text = el.textContent ?? "";
    return {
      selector: `x:nth-of-type(${i + 1}) > g`,
      text,
      cp: text.codePointAt(0) ?? 0,
...
async function probeChromeFamily(cell: CellStyle, family: string) {
  ...
  await page.evaluate(
    ({ id, text, family, weight, size, italic, stretch, lang }) => {
...
for (const cell of cells) {
  ...
  const chromeInstance =
    chromeInstalled == null
      ? null
      : createGlyphHelperFont({ fontPath: chromeInstalled.path, postscriptName: chromeInstalled.postscriptName });
  ...
  const declared = splitFamilies(cell.fontFamily).map((family, index) => {
    ...
    return {
      ...
      family,
      generic,
      usable: generic || installed != null,
      installed: installed != null,
      postscriptName: installed?.postscriptName ?? null,
  ...
  const hardcodedFamilies = blinkWinHardcodedFamilies(
    cell.cp,
    { lang: cell.lang, priority },
    (family) => resolveInstalledFont(family) != null,
  );
  const hardcoded = [];
  for (const family of hardcodedFamilies) {
    ...
    const inst =
      installed == null
        ? null
        : createGlyphHelperFont({ fontPath: installed.path, postscriptName: installed.postscriptName });
    hardcoded.push({
      family,
      installed: installed != null,
      postscriptName: installed?.postscriptName ?? null,
      glyphId: inst == null ? 0 : glyphIdForCp(inst, cell.cp),
      chrome: await probeChromeFamily(cell, family),
    });
  }
  const baseFamilyName = declared[0]?.family;
  const locale = blinkWinFallbackLocale(cell.cp, cell.lang, priority);
  const directWrite =
    resolveSystemFallbackFonts([cell.cp], "Helvetica", {
      weight: cell.weight,
      italic: cell.italic,
      fontSize: cell.size,
      ...(baseFamilyName != null ? { baseFamilyName } : {}),
      ...(locale !== "" ? { locale } : {}),
    }).get(cell.cp) ?? null;
  const ours = resolveFontForCodepoint(
    cell.cp,
    primary,
    primaryKey,
    cell.weight,
  ...
  const finalInst =
    ours.fontOverride ??
    (ours.key === primaryKey ? primary : getFontInstance(ours.key, cell.weight, cell.size, cell.italic ? 1 : 0));
  const source = getFontSourceInfo(finalInst);
  const shapedRuns = splitTextIntoFontRunsShaped(
    cell.text,
    primary,
    primaryKey,
    cell.weight,
  ...
  const rasterSpans = selectedGlyphRasterSpans(cell.text, [{ start: 0, end: cell.text.length }], {
    fontSize: cell.size,
  ...
  results.push({
    ...
    chrome:
      chrome == null
        ...
        : {
            ...
            postscriptName: chrome.postScriptName,
            glyphCount: chrome.glyphCount,
            reopenedPath: chromeInstalled?.path ?? null,
            glyphId: chromeInstance == null ? null : glyphIdForCp(chromeInstance, cell.cp),
          },
    hardcoded: { priority, candidates: hardcoded, accepted: hardcoded.find((h) => h.glyphId !== 0) ?? null },
    directWrite: {
      baseFamilyName: baseFamilyName ?? null,
      locale,
      weight: cell.weight,
      italic: cell.italic,
      stretch: cell.stretch,
    ...
    domotion: {
      ...
      chain,
      routeKey: ours.key,
      covered: ours.covered,
      glyphId: finalInst == null ? 0 : glyphIdForCp(finalInst, cell.cp),
      postscriptName: finalInst?.postscriptName ?? source?.postscriptName ?? null,
