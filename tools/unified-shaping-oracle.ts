...
export interface ExactRecord {
  ...
  face: {
    ...
    localPostscriptName: string | null;
    namedInstance: string | null;
...
export function joinShapingEvidence(
  record: ExactRecord,
  paintedFaces: PaintedFace[],
  paintedOrigins: PaintedOrigin[],
  faceObservation: FaceObservation = "matched",
): Record<string, unknown> {
  const localPostscriptName = record.face.localPostscriptName;
  const faceAgreement =
    localPostscriptName != null &&
  ...
  return {
    ...
    comparison: {
      faceAgreement,
      faceObservation,
      glyphIds: faceAgreement ? "source-equivalent-same-face" : "not-compared-across-different-faces",
      paintedOrigins: "recorded-not-inferred-from-pixels",
    },
  };
}
...
export function browserControlMovement(
  controls: Record<string, Array<{ left: number; top: number; right: number; bottom: number }>>,
): Record<string, boolean> {
  const base = rectSignature(controls.base ?? []);
  return Object.fromEntries(
    Object.entries(controls)
      .filter(([name]) => name !== "base")
      .map(([name, rects]) => [name, rectSignature(rects) !== base]),
  );
}
...
async function main(): Promise<number> {
  ...
  try {
    ...
    try {
      ...
      const faceRules = inputs
        .map((run) =>
          run.postscriptName == null
            ? ""
            : `@font-face{font-family:"${run.family}";src:local("${run.postscriptName.replaceAll('"', "")}")}`,
        )
        .join("");
      await page.setContent(
        `<!doctype html><style>${faceRules}body{margin:0}.run{display:inline-block;margin:2px;font-size:16px}</style><main></main>`,
      );
      ...
      for (let index = 0; index < exact.records.length; index++) {
        ...
        if (
          targetPostscript != null &&
          !fonts.some((font) => normalizeFace(font.postScriptName) === normalizeFace(targetPostscript))
        ) {
          const inferredFamily = targetPostscript
            .replace(/^\./, "")
            .replace(/-(?:Regular|Italic|Light|Medium|Bold).*$/i, "");
          const keyFamily = exactRecord.face.key
            .replace(/^(?:sysfb|winfam):/, "")
            .replace(/-(?:regular|italic|light|medium|bold).*$/i, "")
            .replaceAll("-", " ");
          const candidates = [
            ...new Set([
            ...
            [300, 400, 500, 700].map((weight) => ({
              family,
              weight,
              style: /italic/i.test(targetPostscript) ? "italic" : "normal",
            })),
          );
          ...
          for (let candidateIndex = 0; candidateIndex < candidates.length; candidateIndex++) {
            ...
            const generic = new Set([
              ...
              "serif",
              "monospace",
            ]).has(candidate.family);
            const cssFamily = generic ? candidate.family : `"${candidate.family}"`;
            await page.evaluate(
              `(() => { const source=document.querySelector(${JSON.stringify(selector)}); const span=source.cloneNode(true); span.id=${JSON.stringify(id)}; span.style.fontFamily=${JSON.stringify(cssFamily)}; span.style.fontWeight=${JSON.stringify(String(candidate.weight))}; span.style.fontStyle=${JSON.stringify(candidate.style)}; document.body.append(span); })()`,
            );
            await page.evaluate(
              (candidateSelector) => document.querySelector(candidateSelector)?.getBoundingClientRect(),
              `#${id}`,
            );
            ...
            if (
              candidateFonts.fonts.some(
                (font) => normalizeFace(font.postScriptName) === normalizeFace(targetPostscript),
              )
            ) {
      ...
      const browserControls = {
        ...
        paintedOrigins: moved.paintedOrigins,
      };
      const movementProven = exact.movementProven && Object.values(browserControls).every(Boolean);
      const complete =
        exact.completeEnvironment &&
        records.length === exact.pairs &&
        records.every((record) => {
          ...
          return (
            helper.localPostscriptName != null &&
            chrome.paintedOrigins.length > 0 &&
      ...
      const report = {
        ...
        boundary:
          "CDP exposes painted faces and DOM origins, not Blink glyph ids; glyph records come from the same concrete face shaped by Chromium-configured vendored HarfBuzz.",
        movementProven,
