...
export const SFNS_VALIDATION_CHROMIUM_REVISION = "7d859f271cbda744098ac69f44978d4edfa62be3";
export const SFNS_VALIDATION_SKIA_REVISION = "62efacd37737505732dbe3d8daa62abd679626a1";
export const SFNS_VALIDATION_DEPOT_TOOLS_REVISION = "612d70c7ccb01d4a405e822ad0505206de636d7e";
export const SFNS_VALIDATION_FONT_SHA256 = "2bfd40dc72e6759e248f82a52a40d551338979fffc9b5c070e685b4b7ad19e66";
export const SFNS_VALIDATION_FONT_BYTE_LENGTH = 7_909_644;
export const SFNS_VALIDATION_DECODED_FONT_SHA256 = "48eedcecfc1b0338a2b0deaac43b017df55b3023cff2c5e8ecc87570b4eacff4";
...
export type SfnsHookPayload =
  SfnsShapePayload | SfnsRawPayload | SfnsFilteredPayload | SfnsGammaPayload | SfnsRunPayload | SfnsMaskPayload;
...
function mapped(matrix: readonly number[], point: readonly number[]): [number, number] {
  return [
    Math.fround(Math.fround(matrix[0] * point[0]) + Math.fround(matrix[1] * point[1]) + matrix[2]),
    Math.fround(Math.fround(matrix[3] * point[0]) + Math.fround(matrix[4] * point[1]) + matrix[5]),
  ];
}
...
function expectedObservationId(
  ...
  return caseId.startsWith("control-") ? `validation-${caseId}-1` : `validation-${caseId}-${lifecycle}-${ordinal}`;
}
...
function selected(observation: SfnsValidationObservation): {
  ...
  return {
    ...
    masks: observation.selection.maskSequences
      .map((sequence) => eventAt(observation, sequence))
      .filter((event): event is SfnsHookEvent => event != null),
  };
}
...
export function sfnsValidationObservationDigest(observation: SfnsValidationObservation): string {
  ...
  const strip = (event: SfnsHookEvent | undefined): unknown =>
    event == null
      ? null
      : {
          event: event.event,
...
export function sfnsValidationEvidenceGroups(observation: SfnsValidationObservation): Record<string, string> {
  const evidence = selected(observation);
...
export function sfnsValidationArtifactDigest(
  artifact: Omit<SfnsPinnedChromiumValidationArtifact, "artifactDigest"> | SfnsPinnedChromiumValidationArtifact,
): string {
...
function validateRec(rec: SfnsValidationRec, label: string, errors: string[]): void {
  ...
  if (
    !Array.isArray(rec.singleMatrix) ||
    rec.singleMatrix.length !== 9 ||
    rec.singleMatrix.some((value) => !Number.isFinite(value))
  )
    errors.push(`${label}:matrix`);
}

function validateEventEnvelope(event: SfnsHookEvent, observation: SfnsValidationObservation, errors: string[]): void {
  ...
  if (
    event.observationId !== observation.observationId ||
    event.scenarioId !== observation.scenarioId ||
    event.lifecycle !== observation.lifecycle ||
    event.controlId !== observation.controlId ||
    event.ordinal !== observation.ordinal
  )
    errors.push(`${label}:stale-envelope`);
  if (
    event.source.chromiumRevision !== SFNS_VALIDATION_CHROMIUM_REVISION ||
  ...
  if (
    event.typeface.fontBytes.authority !== "ots-sanitized-sfnt" ||
...
function validateObservation(
  ...
  if (observation.observationId !== expectedObservationId(caseId, observation.lifecycle, observation.ordinal))
  ...
  if (
    ...
    observation.controlId !== manifestCase.controlId
  )
    errors.push(`${label}:case-identity`);
  if (
    !exact(request.run.deviceStart, mapped(request.run.liveDeviceMatrix, request.run.sourceStart)) ||
    request.run.deviceBaseline !== request.run.deviceStart[1]
  ) {
    errors.push(`${label}:manifest-coordinate-envelope`);
  }
  if (!observation.browser.explicitlyHeadless || !observation.browser.launchArgs.includes("--headless=new")) {
  ...
  if (
    !exact(observation.coordinateSpace, {
      source: "skia-source-space-y-down",
      device: "device-space-y-down",
      coreTextRaw: "coretext-y-up",
      normalized: "device-y-down",
  ...
  const evidence = selected(observation);
  if (
    evidence.shape?.event !== "shape" ||
    evidence.raw?.event !== "raw" ||
    evidence.filtered?.event !== "filtered" ||
    evidence.gamma?.event !== "gamma" ||
    evidence.run?.event !== "run" ||
    evidence.masks.length !== observation.selection.maskSequences.length ||
    evidence.masks.some((event) => event.event !== "mask")
  )
    errors.push(`${label}:selection`);
  if (
    evidence.shape == null ||
    evidence.raw == null ||
  ...
  if (
    selectedEvents.some(
      (event) =>
        event.processId !== observation.selection.processId ||
  ...
  if (!(
    evidence.shape.sequence < evidence.raw.sequence &&
  ...
  if (
    recTypefaceId(raw.rawRec) !== evidence.raw.typeface.uniqueId ||
  ...
  const expectedPostscript =
    request.axes.opsz === 26
      ? ".SFNS-Regular_wdth_opsz1A0000_GRAD_wght2BC0000"
      : ".SFNS-Regular_wdth_opsz110000_GRAD_wght2BC0000";
  if (
    typeface.family !== ".SF NS" ||
    typeface.postscriptName !== expectedPostscript ||
  ...
  for (const [tag, expected] of Object.entries(expectedAxes)) {
    ...
    if (axis == null || !exact([axis.min, axis.default, axis.max, axis.hidden, axis.actual], expected))
      errors.push(`${label}:axis:${tag}`);
  }
  if (
    observation.browser.platformFonts.length !== 1 ||
  ...
  if (!exact(raw.surfaceProps, request.surface) || !exact(run.surfaceProps, request.surface))
    errors.push(`${label}:surface`);
  if (raw.scalerContextFlags !== request.scalerContextFlags || run.scalerContextFlags !== request.scalerContextFlags) {
  ...
  const expectedRawMaskFormat =
    validationFont.edging === "subpixel" && request.surface.pixelGeometry !== "unknown" ? "LCD16" : "A8";
  const expectedFilteredMaskFormat =
    expectedRawMaskFormat === "LCD16" && filtered.smoothBehavior === "subpixel" ? "LCD16" : "A8";
  if (
    raw.rawRec.maskFormat !== expectedRawMaskFormat ||
  ...
  if (
    raw.rawRec.textSize !== request.fontSize ||
    filtered.before.textSize !== request.fontSize ||
    filtered.after.textSize !== request.fontSize
  )
  ...
  validateEncodedBytes(gamma.tableBytesBase64, tableLength, gamma.tableSha256, `${label}:gamma-table`, errors);
  ...
  run.glyphs.forEach((glyph, index) => {
    ...
    if (
      shaped.index !== index ||
      shaped.gid !== SFNS_VALIDATION_GLYPH_IDS[index] ||
      shaped.characterIndex !== index ||
      !shaped.horizontal ||
      shaped.shapedAdvance.length !== 2 ||
      shaped.shapedAdvance[1] !== 0 ||
      shaped.shapedOffset.length !== 2 ||
      !shaped.shapedOffset.every(Number.isFinite) ||
      !exactNumbers(shaped.accumulatedAdvance, accumulated) ||
      glyph.index !== index ||
      glyph.gid !== shaped.gid ||
      ...
      !exactNumbers(glyph.roundedDeviceOrigin, [
        Math.floor(expectedRoundedInput[0]),
        Math.floor(expectedRoundedInput[1]),
      ]) ||
    ...
    const expectedPhaseX = packedPhase(expectedRoundedInput[0], run.rounding.ignorePositionFieldMask[0]);
    const expectedPhaseY = packedPhase(expectedRoundedInput[1], run.rounding.ignorePositionFieldMask[1]);
    const expectedPackedId = ((glyph.gid << 2) | expectedPhaseX | (expectedPhaseY << 18)) >>> 0;
    if (
      glyph.packedId !== expectedPackedId ||
  ...
  if (
    !exactNumbers(
      masks.map((mask) => mask.glyph.packedId),
      expectedPackedIds,
    )
  ) {
  ...
  for (const [index, mask] of masks.entries()) {
    ...
    if (
      mask.glyph.metrics.maskFormat !== expectedFilteredMaskFormat ||
      mask.glyph.metrics.imageSize !== mask.glyph.metrics.rowBytes * mask.glyph.metrics.height
    ) {
    ...
    if (
      runGlyph == null ||
      runGlyph.gid !== mask.glyph.gid ||
      runGlyph.phase.x !== mask.glyph.phase.x ||
      runGlyph.phase.y !== mask.glyph.phase.y ||
      !exactNumbers(mask.glyph.subpixelOffsetFixed, [mask.glyph.phase.x << 14, mask.glyph.phase.y << 14])
    ) {
      errors.push(`${label}:mask:${index}:run-phase-link`);
    }
    const maskGamma = mask.gamma;
    if (
      maskGamma.recordDump !== mask.filteredRec.dump ||
  ...
  const rawMetricKeys = ["pointSize", "unitsPerEm", "ascent", "descent", "leading", "capHeight", "xHeight"] as const;
  if (
    rawMetricKeys.some((key) => !Number.isFinite(rawMetrics[key])) ||
    rawMetrics.pointSize !== effectiveScale ||
    !Array.isArray(rawMetrics.boundingBox) ||
...
const REQUIRED_HOST_COMPONENTS = ["xcode", "macOS", "metalToolchainIdentifier", "metal", "clang"] as const;

function validateSfnsPinnedChromiumValidationImpl(artifact: SfnsPinnedChromiumValidationArtifact): string[] {
  const errors: string[] = [];
  if (
    artifact.schemaVersion !== 2 ||
    artifact.authority !== "validation-test-only-pinned-chromium" ||
    artifact.arm !== "validation"
  )
    errors.push("artifact-envelope");
  if (
  ...
  if (
    !exact(artifact.collectionContract, {
      browserLaunches: 26,
      processIsolation: "one-explicitly-headless-browser-per-observation",
      inputDerivation: "source-owned-manifest-independent-arm-derivation",
      equality: "exact-bytes-no-tolerance",
  ...
  if (
    artifact.build.chromiumRevision !== SFNS_VALIDATION_CHROMIUM_REVISION ||
  ...
  if (
    artifact.build.platform !== "darwin" ||
    artifact.build.architecture !== "arm64" ||
    artifact.build.gnArgs !== SFNS_VALIDATION_GN_ARGS
  ) {
    errors.push("build-contract");
  }
  if (
    !artifact.build.binary.path.endsWith("/headless_shell") ||
  ...
  if (
    ...
    errors.push("corpus");
  if (
    artifact.scenarios.length !== SFNS_VALIDATION_SCENARIOS.length ||
    new Set(artifact.scenarios.map((scenario) => scenario.id)).size !== SFNS_VALIDATION_SCENARIOS.length
  )
    errors.push("scenario-set");
  ...
  for (const id of SFNS_VALIDATION_SCENARIOS) {
    ...
    if (scenario.observations.length !== 4) {
      errors.push(`${id}:observation-count`);
      continue;
    }
    ...
    scenario.observations.forEach((observation, index) => {
      if (
        observation.lifecycle !== expectedLifecycle[index] ||
        observation.ordinal !== expectedOrdinals[index] ||
  ...
  if (
    artifact.controls.length !== SFNS_VALIDATION_CONTROLS.length ||
    new Set(artifact.controls.map((control) => control.id)).size !== SFNS_VALIDATION_CONTROLS.length
  )
    errors.push("control-set");
  ...
  for (const id of SFNS_VALIDATION_CONTROLS) {
    ...
    if (
      ...
      observation.controlId !== id ||
      observation.ordinal !== 1 ||
      control.baselineScenarioId !== "zoom-2" ||
      control.caseId !== caseId ||
      !exact(control.request, request)
    ) {
...
export function validateSfnsPinnedChromiumValidation(artifact: SfnsPinnedChromiumValidationArtifact): string[] {
  try {
