...
const fingerprintSchema = z
  .object({
    ...
    skia: z.string().min(1),
    harfbuzz: z.string().min(1),
    oracleSkiaRevision: z.string().min(1),
    oracleHarfbuzzRevision: z.string().min(1),
    ...
    metricAlgorithm: z.string().min(1),
  ...
  .superRefine((fingerprint, ctx) => {
    if (fingerprint.platform === "win32" && fingerprint.nativeIdentityHelperSha256 == null) {
      ctx.addIssue({
        code: "custom",
        message: "Windows observations require the DirectWrite identity-helper fingerprint",
      });
    }
    if (fingerprint.platform !== "win32" && fingerprint.nativeIdentityHelperSha256 != null) {
      ctx.addIssue({
        code: "custom",
        message: "only Windows observations may carry a DirectWrite identity-helper fingerprint",
      });
    }
  });
const glyphSchema = z
  .object({
    ...
    offsetX: finite,
    offsetY: finite,
    outlineSha256: z.string().regex(/^[a-f0-9]{64}$/),
...
const logicalSchema = z
  .object({
    ...
    variationAxes: z.record(z.string(), finite),
    glyphs: z.array(glyphSchema).min(1),
    baseline: finite,
...
const inkGeometrySchema = z
  ...
  .strict();
const envelopeResidualSchema = z
  .object({
  ...
  .strict();
const residualSchema = envelopeResidualSchema
  .extend({
...
const artifactSchema = z
  ...
  .strict();
export const pathsRasterRowSchema = z
  .object({
    id: z.string().min(1),
    ...
    runProvenance: z
      ...
      .strict(),
    cellSha256: z.string().regex(/^[a-f0-9]{64}$/),
    fingerprint: fingerprintSchema,
    dimensions: dimensionsSchema,
    expectedLogical: logicalSchema,
    actualLogical: logicalSchema,
    nativeFace: nativeFaceSchema,
...
const envelopeSchema = z
  .object({
    ...
    reviewedArtifacts: z
      .object({
        proposal: z
          .object({ native: z.string().regex(/^[a-f0-9]{64}$/), paths: z.string().regex(/^[a-f0-9]{64}$/) })
          .strict(),
        validation: z
          .object({ native: z.string().regex(/^[a-f0-9]{64}$/), paths: z.string().regex(/^[a-f0-9]{64}$/) })
          .strict(),
  ...
  .strict();
export const envelopeFileSchema = z
  .object({
  ...
  .superRefine((file, ctx) => {
    if (file.ratified && (file.reviewer == null || file.reviewedAt == null))
      ctx.addIssue({ code: "custom", message: "ratified envelopes require reviewer and reviewedAt" });
    if (file.ratified && file.envelopes.length === 0)
      ctx.addIssue({ code: "custom", message: "ratified envelope file must contain reviewed rows" });
    const identities = new Set<string>();
    for (let i = 0; i < file.envelopes.length; i++) {
      const envelope = file.envelopes[i];
      if (envelope.key !== dimensionsKey(envelope.dimensions))
        ctx.addIssue({
          code: "custom",
          message: "envelope key does not match dimensions",
          path: ["envelopes", i, "key"],
        });
      const identity = `${envelope.fingerprintSha256}|${envelope.cellSha256}`;
      if (identities.has(identity))
        ctx.addIssue({ code: "custom", message: "duplicate fingerprint/dimensions envelope", path: ["envelopes", i] });
      ...
      for (const [label, reviewed] of Object.entries(envelope.reviewedArtifacts)) {
        ...
        if (reviewed.native === reviewed.paths)
          ctx.addIssue({
            code: "custom",
...
export const fingerprintSha256 = (fingerprint: PathsRasterRow["fingerprint"]): string =>
  createHash("sha256").update(canonical(fingerprint)).digest("hex");
export const dimensionsKey = (dimensions: PathsRasterRow["dimensions"]): string =>
  [
    dimensions.fontTechnology,
...
function assertRatifiedRunPairs(rows: PathsRasterRow[], envelopeFile: z.infer<typeof envelopeFileSchema>): void {
  ...
  const envelopes = new Map(
    envelopeFile.envelopes.map((entry) => [`${entry.fingerprintSha256}|${entry.cellSha256}`, entry]),
  );
...
function firstLogicalMismatch(
  expected: PathsRasterRow["expectedLogical"],
  actual: PathsRasterRow["actualLogical"],
): string | null {
  if (
    expected.postscriptName !== actual.postscriptName ||
    expected.sourceSha256 !== actual.sourceSha256 ||
    expected.faceIndex !== actual.faceIndex ||
    canonical(expected.variationAxes) !== canonical(actual.variationAxes)
  )
...
export interface PathsRasterVerdict {
  id: string;
  verdict:
    | "logical-mismatch"
    | "invalid-evidence"
...
export function adjudicatePathsRasterRows(
...
): { pass: boolean; rows: PathsRasterVerdict[] } {
  ...
  const verdicts = rows.map((row): PathsRasterVerdict => {
    if (row.warnings.length > 0 || row.nativeArtifact.sha256 === row.pathsArtifact.sha256)
      return {
        id: row.id,
        verdict: "invalid-evidence",
        reason: row.warnings.length > 0 ? "warnings" : "inert-raster-arm",
      };
    ...
    if (reviewed.native !== row.nativeArtifact.sha256 || reviewed.paths !== row.pathsArtifact.sha256)
      return { id: row.id, verdict: "missing-envelope", reason: "unreviewed-artifact-role" };
    const r = row.residual,
      m = envelope.max;
    const inside =
      r.changedPixels <= m.changedPixels &&
      r.area <= m.area &&
      r.width <= m.width &&
      r.height <= m.height &&
      r.maxEdgeDistance <= m.maxEdgeDistance &&
  ...
  return {
    pass: verdicts.length > 0 && verdicts.every((row) => row.verdict === "accepted-rasterization-only"),
    rows: verdicts,
  };
}
