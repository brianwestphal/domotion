...
console.log(
  `Chrome 1D400-1D7FF: ${result.length} cps, ${byHash.size} distinct, largest cluster ${clusters[0]} (${((100 * clusters[0]) / result.length).toFixed(1)}% — tofu if dominant)`,
);
...
for (const p of [
  ...
  try {
    ...
    for (const ff of f.fonts ?? [f]) faces.push({ p, ff });
  } catch {}
}
let cov = 0;
for (const cp of cps) {
  for (const { ff } of faces) {
    try {
      if (ff.glyphForCodePoint(cp).id !== 0) {
        cov++;
        break;
      }
    } catch {}
