...
console.log(
  "U+2F9B2 NFD:",
  [...ch.normalize("NFD")].map((c) => c.codePointAt(0).toString(16)),
  "NFC:",
  [...ch.normalize("NFC")].map((c) => c.codePointAt(0).toString(16)),
);
const faces = [
...
for (const [name, p, ps] of faces) {
  let f;
  try {
    f = ps ? fk.openSync(p, ps) : fk.openSync(p);
  } catch {
    console.log(name, "open fail");
    continue;
  }
