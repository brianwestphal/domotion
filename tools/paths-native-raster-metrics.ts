...
function edges(mask: Uint8Array, width: number, height: number): Point[] {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      if (at(x, y) === 1 && (at(x - 1, y) === 0 || at(x + 1, y) === 0 || at(x, y - 1) === 0 || at(x, y + 1) === 0))
        result.push({ x, y });
    }
  }
  return result;
...
function inkGeometry(
  ...
  width: number,
  height: number,
): { area: number; x: number; y: number; width: number; height: number } {
  let area = 0,
    minX = width,
    minY = height,
    maxX = -1,
...
export function measureDecodedPathsRasterResidual(
  ...
  if (native.width !== paths.width || native.height !== paths.height || native.channels !== paths.channels) {
    throw new Error(
      `raster dimensions differ: native ${native.width}x${native.height}x${native.channels}, paths ${paths.width}x${paths.height}x${paths.channels}`,
    );
  }
  let changedPixels = 0,
    totalChannelDelta = 0;
  for (let pixel = 0; pixel < native.width * native.height; pixel++) {
    ...
    let delta = 0;
    for (let channel = 0; channel < native.channels; channel++)
      delta += Math.abs(native.data[offset + channel] - paths.data[offset + channel]);
    if (delta === 0) continue;
    changedPixels++;
    totalChannelDelta += delta;
  }
  const nativeMask = inkMask(native),
    pathsMask = inkMask(paths);
  ...
  if (nativeEdges.length === 0 || pathsEdges.length === 0)
    throw new Error("native or paths raster contains no substantive ink edge");
  const maxEdgeDistance = Math.max(
    directedMaxDistance(nativeEdges, pathsEdges),
    directedMaxDistance(pathsEdges, nativeEdges),
  );
  return {
    changedPixels,
