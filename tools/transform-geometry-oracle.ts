...
function hasTransformRaster(elements: CapturedElement[]): boolean {
  return elements.some(
    (element) => element.transformSubtreeRaster?.dataUri != null || hasTransformRaster(element.children),
  );
}
...
export async function runTransformGeometryOracle(): Promise<{ rows: Row[]; mutationMoved: boolean }> {
  ...
  try {
    ...
    const cases = [
      ...
      {
        id: "matrix-translate",
        transform: "matrix(.8,.3,-.2,1.1,23,-17)",
        origin: "33px 44px",
        x: 90,
        y: 220,
        w: 170,
    ...
    for (const test of cases) {
      const live = await page.locator(`#${test.id}`).evaluate((element) => {
        const points = [
          ...
          [0, (element as HTMLElement).offsetHeight],
        ];
      ...
      const svgTransform = cssTransformToSvg(
        computed.transform,
        test.x + computed.origin[0],
        test.y + computed.origin[1],
      );
      const actual = await page.evaluate(
        ({ svgTransform, test }) => {
          ...
          const points = [
            [test.x, test.y],
            [test.x + test.w, test.y],
            [test.x + test.w, test.y + test.h],
            [test.x, test.y + test.h],
      ...
      rows.push({
        ...
        actual,
        maxAbsDelta,
        pass: maxAbsDelta <= TOLERANCE,
        source: "CSS Transforms matrix + transform-origin composition",
      });
    }
    ...
    rows.push({
      id: "projective.preserve-3d-raster-boundary",
      expected: true,
      actual: raster,
      maxAbsDelta: raster ? 0 : Infinity,
      pass: raster,
      source: "Blink projective quad; SVG affine representation boundary",
    });
    ...
    rows.push({
      id: "projective.affine-matrix3d-stays-vector",
      expected: false,
      actual: affine3dRaster,
      maxAbsDelta: affine3dRaster ? Infinity : 0,
      pass: !affine3dRaster,
      source: "fourth-corner affine residual classification",
    });
    ...
    for (const [direction, offset] of [
      ...
      const expected =
        direction === "below" || direction === "above" ? `matrix(1 0 0 -1 0 ${n})` : `matrix(-1 0 0 1 ${n} 0)`;
      rows.push({
        id: `reflection.${direction}`,
        expected,
        actual,
        maxAbsDelta: expected === actual ? 0 : Infinity,
        pass: expected === actual,
        source: "FEBoxReflect direction/offset mapping",
      });
    }
    const correct = rows[0].actual as Point[];
    const unpivoted = [
      { x: cases[0].x, y: cases[0].y },
      { x: cases[0].x + cases[0].w, y: cases[0].y },
      { x: cases[0].x + cases[0].w, y: cases[0].y + cases[0].h },
      { x: cases[0].x, y: cases[0].y + cases[0].h },
    ];
