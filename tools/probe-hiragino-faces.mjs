...
const candidates = ["/System/Library/Fonts/ヒラギノ角ゴシック W3.ttc", "/System/Library/Fonts/Hiragino Sans GB.ttc"];
// list any other Hiragino files present
for (const f of readdirSync("/System/Library/Fonts")) {
  if (/ヒラギノ|Hiragino/i.test(f) && !candidates.some((c) => c.endsWith(f)))
    candidates.push(`/System/Library/Fonts/${f}`);
}
...
for (const path of candidates) {
  ...
  for (const ff of faces) {
    ...
    for (const c of canon) {
      try {
        if (ff.glyphForCodePoint(c).id !== 0) covSample++;
      } catch {}
    }
    let covAll = 0;
    for (const c of allCanon) {
      try {
        if (ff.glyphForCodePoint(c).id !== 0) covAll++;
      } catch {}
    }
    console.log(
      `  ${ff.postscriptName?.padEnd(28)} sample ${covSample}/${canon.length}  allCanon ${covAll}/${allCanon.length}`,
    );
  }
}
