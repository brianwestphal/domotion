...
let html =
  '<!doctype html><meta charset="utf-8"><style>body{margin:24px;font-family:system-ui,sans-serif;background:white;color:black;line-height:2;}.sample{text-decoration:underline wavy red;}</style>';
for (const fs of SIZES)
  for (const tc of THICKS)
    html += `<div><span class="sample" style="font-size:${fs}px;text-decoration-thickness:${tc}px;">${TEXT}</span></div>`;
...
const samples = await page.evaluate(() =>
  Array.from(document.querySelectorAll(".sample")).map((el) => {
    ...
    return {
      ...
      rect: {
        ...
        y: Math.floor(r.y),
        w: Math.ceil(r.width),
        h: Math.ceil(r.height + parseFloat(cs.fontSize)),
      },
    };
  }),
);
...
for (const r of results)
  console.log(`fs=${String(r.fs).padStart(2)} t=${r.t} → yCenter=${r.yC.toFixed(2)}  amp=${r.amp.toFixed(2)}`);
await browser.close();
