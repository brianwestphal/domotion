...
export function readColrv1SourceFacts(path = COLRV1_FIXTURE): Colrv1SourceFacts {
  ...
  const palettes: [Color, Color][] = font.CPAL.colorRecordIndices.map(
    (start: number) =>
      stops.map(({ palette }) => {
  ...
  return {
    ...
    glyphId,
    paintFormat: 4,
    paletteIndices: [stops[0].palette, stops[1].palette],
    stopOffsets: [stops[0].offset, stops[1].offset],
    paletteEndpoints: palettes,
  };
}
...
const overrideEndpoints: [Color, Color] = [
  [255, 0, 255, 255],
  [0, 255, 255, 255],
];
...
async function paintedFace(page: Page): Promise<{ custom: boolean; count: number }> {
  ...
  try {
    ...
    return {
      custom: painted.length === 1 && painted[0].isCustomFont,
      count: painted.reduce((sum, row) => sum + row.glyphCount, 0),
    };
  } finally {
    await cdp.detach();
  }
...
export async function runFontPalettePaintGate(
  options: { artifactDir?: string } = {},
): Promise<FontPalettePaintGateReport> {
  ...
  const dynamic = await runFontPaletteDynamicGate({
    colrv1Source: source,
  ...
  const verdict =
    colrv0.verdict === "source-exact" && complete && rows.every((row) => row.pass) && dynamic.verdict === "source-exact"
      ? "source-exact"
      : "source-drift";
  return {
    schemaVersion: 1,
    ...
    JSON.stringify(
      {
        ...
        colrv0: report.colrv0.verdict,
        colrv1: `${report.colrv1Rows.filter((row) => row.pass).length}/${report.colrv1Rows.length}`,
