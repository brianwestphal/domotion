...
const cps = [0x2605, 0x2606, 0x2665, 0x2666, 0x2660, 0x2663];
const html = `<style>div{font:18px sans-serif;background:white;color:black}</style>` +
...
for (let i = 0; i < cps.length; i++) {
  ...
  console.log(`U+${cp.toString(16).toUpperCase()}  ${String.fromCodePoint(cp)}  Chrome: ${fonts.fonts.map(f=>`${f.familyName}×${f.glyphCount}`).join(", ")}`);
}
await browser.close();
