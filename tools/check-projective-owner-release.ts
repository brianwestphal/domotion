...
async function reports(path: string): Promise<string[]> {
  ...
  return (
    await Promise.all(
      entries.map((entry) =>
        entry.isDirectory()
          ? reports(resolve(path, entry.name))
