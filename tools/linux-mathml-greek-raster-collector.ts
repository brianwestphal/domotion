...
function verifyPackage(debPath: string, fontPath: string): Buffer {
  ...
  const fields = {
    name: packageField(debPath, "Package"),
    source: packageField(debPath, "Source").split(" ")[0],
    version: packageField(debPath, "Version"),
    architecture: packageField(debPath, "Architecture"),
  };
  for (const [key, expected] of Object.entries({
    name: FREE_SANS_NOBLE_PACKAGE.name,
    source: FREE_SANS_NOBLE_PACKAGE.source,
    version: FREE_SANS_NOBLE_PACKAGE.version,
    architecture: FREE_SANS_NOBLE_PACKAGE.architecture,
  }))
    if (fields[key as keyof typeof fields] !== expected)
      throw new Error(`FreeSans package ${key} ${fields[key as keyof typeof fields]} != ${expected}`);
  const normalized = resolve(fontPath).replaceAll("\\", "/");
  if (!normalized.endsWith(`/${FREE_SANS_NOBLE_PACKAGE.fontPath}`))
    throw new Error(`FreeSans path is not the authenticated package member: ${fontPath}`);
  return assertFile(
    fontPath,
    FREE_SANS_NOBLE_PACKAGE.fontByteLength,
    FREE_SANS_NOBLE_PACKAGE.fontSha256,
    "FreeSans.ttf",
  );
}
...
function isolatedFontconfig(fontBytes: Buffer, fontSource: string, artifactDir: string): IsolatedFontconfig {
  const root = resolve(artifactDir, "isolated-fontconfig"),
    fontDir = resolve(root, "fonts"),
  ...
  const configPath = resolve(root, "fonts.conf");
  ...
  if (lines.length !== 1)
    throw new Error(
      `isolated Fontconfig inventory has ${lines.length} entries, expected one: ${JSON.stringify(lines)}`,
    );
  const [listedPath, familyName, postscriptName, rawIndex] = lines[0].split("\t");
  if (
    resolve(listedPath) !== fontPath ||
    familyName !== "FreeSans" ||
  ...
  const entry = {
    ...
    familyName: "FreeSans" as const,
    postscriptName: "FreeSans" as const,
...
): Pick<LinuxMathmlGreekGlyph, "outlineSha256" | "outlineCommandCount"> {
...
function verifyFontAndSubsets(fontBytes: Buffer): { hinted: Buffer; unhinted: Buffer } {
  ...
  const metadata = {
    ...
    postscriptName: opened.postscriptName,
    unitsPerEm: opened.unitsPerEm,
    ascent: opened.ascent,
    descent: opened.descent,
    lineGap: opened.lineGap,
    glyphCount: opened.numGlyphs,
  };
  for (const [key, expected] of Object.entries({
    familyName: FREE_SANS_NOBLE_PACKAGE.familyName,
    postscriptName: FREE_SANS_NOBLE_PACKAGE.postscriptName,
    unitsPerEm: FREE_SANS_NOBLE_PACKAGE.unitsPerEm,
    ascent: FREE_SANS_NOBLE_PACKAGE.ascent,
    descent: FREE_SANS_NOBLE_PACKAGE.descent,
    lineGap: FREE_SANS_NOBLE_PACKAGE.lineGap,
    glyphCount: FREE_SANS_NOBLE_PACKAGE.glyphCount,
  }))
    if (metadata[key as keyof typeof metadata] !== expected)
      throw new Error(`FreeSans ${key} ${metadata[key as keyof typeof metadata]} != ${expected}`);
  for (const token of LINUX_MATHML_GREEK_TOKENS) {
    ...
    if (JSON.stringify(actual) !== JSON.stringify(token.glyph))
      throw new Error(`${token.id}: authenticated FreeSans glyph facts drifted`);
  }
...
function assertSubset(bytes: Buffer, expected: { byteLength: number; sha256: string }, label: string): void {
  if (bytes.length !== expected.byteLength || sha256(bytes) !== expected.sha256)
    throw new Error(`${label} FreeSans subset identity drifted`);
  ...
  for (const token of LINUX_MATHML_GREEK_TOKENS) {
    ...
    if (
      actual.gid !== token.glyph.gid ||
      actual.advanceX !== token.glyph.advanceX ||
      actual.outlineSha256 !== token.glyph.outlineSha256 ||
      actual.outlineCommandCount !== token.glyph.outlineCommandCount
    ) {
...
async function cdpFonts(
  page: Page,
  ids: string[],
): Promise<Map<string, { familyName: string; postscriptName: string; isCustomFont: boolean; glyphCount: number }>> {
  ...
  try {
    ...
    const out = new Map<
      string,
      { familyName: string; postscriptName: string; isCustomFont: boolean; glyphCount: number }
    >();
    for (const id of ids) {
...
async function sourceEvidence(
  page: Page,
  packagePath: string,
  fontconfig: IsolatedFontconfig,
  fontBytes: Buffer,
): Promise<LinuxMathmlGreekPreterminalEvidence> {
  ...
  const dom = (await page.evaluate(
    ...
    LINUX_MATHML_GREEK_TOKENS.map((token) => token.id),
  )) as Record<
    string,
    {
      source: string;
  ...
  const tokens = LINUX_MATHML_GREEK_TOKENS.map((expected) => {
    ...
    if (node.text.trim() !== expected.transformed)
      throw new Error(
        `${expected.id}: capture math-auto produced ${JSON.stringify(node.text)} instead of ${expected.transformed}`,
      );
    const textTop = node.textTop ?? node.y,
      fontAscent = node.fontAscent;
    if (fontAscent == null || !Number.isFinite(fontAscent) || fontAscent <= 0)
      throw new Error(`${expected.id}: capture omitted exact font ascent`);
    const face = faces.get(expected.id)!;
    const glyph = opened.glyphForCodePoint(expected.transformedCodePoint);
    return {
      id: expected.id,
      source: dom[expected.id].source,
      transformed: node.text.trim(),
      sourceCodePoint: expected.sourceCodePoint,
      transformedCodePoint: expected.transformedCodePoint,
      textTransform: dom[expected.id].textTransform as "math-auto",
      computedFontStyle: dom[expected.id].fontStyle as "normal",
      geometry: {
        x: node.textLeft ?? dom[expected.id].rect.x,
        y: node.textTop ?? dom[expected.id].rect.y,
        width: node.textWidth ?? dom[expected.id].rect.width,
        height: node.textHeight ?? dom[expected.id].rect.height,
        textTop,
        fontAscent,
  ...
  return {
    schemaVersion: 1,
    package: {
      suite: FREE_SANS_NOBLE_PACKAGE.suite,
      name: FREE_SANS_NOBLE_PACKAGE.name,
      source: FREE_SANS_NOBLE_PACKAGE.source,
      version: FREE_SANS_NOBLE_PACKAGE.version,
      architecture: FREE_SANS_NOBLE_PACKAGE.architecture,
      filename: FREE_SANS_NOBLE_PACKAGE.filename,
      byteLength: FREE_SANS_NOBLE_PACKAGE.byteLength,
      sha256: FREE_SANS_NOBLE_PACKAGE.sha256,
    },
    inventory: {
      fontconfigVersion: fontconfig.version,
      configSha256: fontconfig.configSha256,
      inventorySha256: fontconfig.inventorySha256,
      entries: [fontconfig.entry],
    },
    sourceFont: {
      packagePath: FREE_SANS_NOBLE_PACKAGE.fontPath,
      runtimePath: fontconfig.fontPath,
      byteLength: fontBytes.length,
      sha256: sha256(fontBytes),
      familyName: opened.familyName,
      postscriptName: opened.postscriptName,
    ...
    subset: {
      ...
      hinted: { ...LINUX_MATHML_GREEK_SUBSETS.hinted },
      unhinted: { ...LINUX_MATHML_GREEK_SUBSETS.unhinted },
    },
    tokens,
  };
}
...
function actualPaths(
  preterminal: LinuxMathmlGreekPreterminalEvidence,
  fontBytes: Buffer,
): { markup: string; defs: string; logical: LinuxMathmlGreekRasterRow["pathsLogical"] } {
  clearWebfonts();
  clearGlyphDefs();
  resetTextRunProvenance();
  ...
  const fragments = preterminal.tokens.map((token) =>
    renderTextAsPath(token.transformed, token.geometry.x, token.geometry.textTop, {
      fontSize: LINUX_MATHML_GREEK_CELL.fontSizePx,
      fontFamily: UNIQUE_PATHS_FAMILY,
      fontWeight: "400",
      fontStyle: "normal",
      ascentOverride: token.geometry.fontAscent,
      fill: "#000",
      lang: "en",
    }),
  );
  ...
  if (
    snapshot.runs.length !== LINUX_MATHML_GREEK_TOKENS.length ||
    snapshot.transitions.length !== LINUX_MATHML_GREEK_TOKENS.length ||
  ...
  const selected = getFontInstance(
    `webfont:${UNIQUE_PATHS_FAMILY.toLowerCase()}`,
    400,
    LINUX_MATHML_GREEK_CELL.fontSizePx,
    0,
    {},
  );
  if (selected?.webfontBuffer == null || sha256(selected.webfontBuffer) !== FREE_SANS_NOBLE_PACKAGE.fontSha256)
    throw new Error("production paths selected bytes are not authenticated FreeSans");
  const tokens = preterminal.tokens.map((source, index) => {
    ...
    if (
      run == null ||
      glyph == null ||
      run.sourceText !== source.transformed ||
      run.glyphs.length !== 1 ||
      run.shapeError != null ||
      run.emitter !== "paths" ||
      run.finalRepresentation !== "svg-paths" ||
      glyph.sourceOutline == null
    )
      throw new Error(`${source.id}: malformed production path provenance`);
    return {
      id: source.id,
      glyph: {
        gid: glyph.id,
      ...
      baseline: parseBaseline(fragments[index]),
      matrix: [1, 0, 0, 1, 0, 0] as [number, number, number, number, number, number],
    };
  });
  return {
    ...
    defs: getGlyphDefs(),
    logical: {
      sourceSha256: FREE_SANS_NOBLE_PACKAGE.fontSha256,
      faceIndex: 0,
      postscriptName: "FreeSans",
...
async function screenshotMarkup(page: Page, markup: string): Promise<Buffer> {
  await page.setContent(
    `<!doctype html><style>html,body{margin:0;width:${VIEWPORT.width}px;height:${VIEWPORT.height}px;overflow:hidden;background:#fff}</style>${markup}`,
    { waitUntil: "load" },
  );
...
async function hintingOffArm(
  page: Page,
  preterminal: LinuxMathmlGreekPreterminalEvidence,
  unhinted: Buffer,
): Promise<{ png: Buffer; evidence: LinuxMathmlGreekRasterRow["hintingControl"] }> {
  ...
  const texts = preterminal.tokens
    .map(
      (token) =>
        `<text id="hint-${token.id}" x="${token.geometry.x}" y="${token.geometry.baseline}" fill="#000" style="font-family:'${UNIQUE_UNHINTED_FAMILY}';font-size:${LINUX_MATHML_GREEK_CELL.fontSizePx}px;font-weight:400;font-style:normal">${html(token.transformed)}</text>`,
    )
    .join("");
  ...
  await page.setContent(
    `<!doctype html><style>html,body{margin:0;width:${VIEWPORT.width}px;height:${VIEWPORT.height}px;overflow:hidden;background:#fff}${face}</style>${svg}`,
    { waitUntil: "load" },
  );
  const loaded = await page.evaluate(
    async ({ family, text }) => {
      ...
      return document.fonts.check(`24px '${family}'`, text);
    },
    { family: UNIQUE_UNHINTED_FAMILY, text: LINUX_MATHML_GREEK_TOKENS.map((token) => token.transformed).join("") },
  );
  if (!loaded) throw new Error("browser did not load the authenticated unhinted subset");
  const faces = await cdpFonts(
    page,
    LINUX_MATHML_GREEK_TOKENS.map((token) => `hint-${token.id}`),
  );
  ...
  if (sha256(Buffer.from(base64, "base64")) !== LINUX_MATHML_GREEK_SUBSETS.unhinted.sha256)
    throw new Error("hinting-off browser source bytes are unauthenticated");
  const logicalTokens = preterminal.tokens.map((token) => ({
    id: token.id,
    glyph: token.glyph,
    baseline: token.geometry.baseline,
    matrix: token.geometry.matrix,
  }));
  return {
    ...
    evidence: {
      ...
      fontFaceRuleCount: pageFacts.fontFaceRuleCount as 1,
      sourceSha256: LINUX_MATHML_GREEK_SUBSETS.unhinted.sha256,
      sourceByteLength: unhinted.length,
      isCustomFont: [...faces.values()].every((entry) => entry.isCustomFont),
      glyphCount: [...faces.values()].reduce((sum, entry) => sum + entry.glyphCount, 0) as 4,
...
function artifact(root: string, dir: string, name: string, bytes: Buffer): LinuxMathmlGreekRasterRow["nativeArtifact"] {
  ...
  if (relativePath.startsWith("../") || relativePath.includes("/../"))
    throw new Error(`artifact directory escapes observation root: ${dir}`);
...
function runnerBootIdSha256(): string {
  ...
  if (!existsSync(path))
    throw new Error("Linux runner omitted its boot id; independent proposal/validation ownership cannot be proved");
...
export async function collectLinuxMathmlGreekRaster(
  options: CollectLinuxMathmlGreekOptions,
): Promise<LinuxMathmlGreekRasterRow[]> {
