...
export const LINUX_MATHML_GREEK_TOKENS = [
  {
    ...
    source: "α",
    transformed: "𝛼",
    sourceCodePoint: 0x03b1,
    transformedCodePoint: 0x1d6fc,
    glyph: {
      gid: 6548,
      cluster: 0,
      advanceX: 578,
      ...
      offsetX: 0,
      offsetY: 0,
      outlineSha256: "e43fd0e75f237bd43dd00aebf92921045532724135d0b57f3633b17bb934123a",
      outlineCommandCount: 31,
    },
  },
  {
    id: "beta",
    source: "β",
    transformed: "𝛽",
    sourceCodePoint: 0x03b2,
    transformedCodePoint: 0x1d6fd,
    glyph: {
      gid: 6549,
      cluster: 0,
      advanceX: 544,
      ...
      offsetX: 0,
      offsetY: 0,
      outlineSha256: "1c895fa7002ed66ad32f5c617a658479c40f8c4301180ac61e0b3035ff9563ab",
      outlineCommandCount: 33,
    },
  },
  {
    id: "pi",
    source: "π",
    transformed: "𝜋",
    sourceCodePoint: 0x03c0,
    transformedCodePoint: 0x1d70b,
    glyph: {
      gid: 6563,
      cluster: 0,
      advanceX: 594,
      ...
      offsetX: 0,
      offsetY: 0,
      outlineSha256: "001f3a368297a899f9f192eb0e0af7a39fb34136cbcdb7df96edf1005eddd69c",
      outlineCommandCount: 21,
    },
  },
  {
    id: "theta",
    source: "θ",
    transformed: "𝜃",
    sourceCodePoint: 0x03b8,
    transformedCodePoint: 0x1d703,
    glyph: {
      gid: 6555,
      cluster: 0,
      advanceX: 505,
      ...
      offsetX: 0,
      offsetY: 0,
      outlineSha256: "c6a03f961b855e495a86960949c94a24d19ca263b40301d05c59cb7022202164",
      outlineCommandCount: 28,
    },
  },
] as const;
...
export const linuxMathmlGreekCellSha256 = (): string =>
  createHash("sha256")
...
export const linuxMathmlGreekTokenEvidenceSchema = z
  .object({
...
export const linuxMathmlGreekTokenArraySchema = z
  .array(linuxMathmlGreekTokenEvidenceSchema)
  .length(LINUX_MATHML_GREEK_TOKENS.length);

export const linuxMathmlGreekPreterminalSchema = z
  .object({
    ...
    subset: z
      .object({
        ...
        hinted: z
          .object({
            byteLength: z.literal(LINUX_MATHML_GREEK_SUBSETS.hinted.byteLength),
            sha256: z.literal(LINUX_MATHML_GREEK_SUBSETS.hinted.sha256),
          })
          .strict(),
        unhinted: z
          .object({
            byteLength: z.literal(LINUX_MATHML_GREEK_SUBSETS.unhinted.byteLength),
            sha256: z.literal(LINUX_MATHML_GREEK_SUBSETS.unhinted.sha256),
          })
          .strict(),
      })
      .strict(),
...
export function validateLinuxMathmlGreekTokenEvidence(raw: unknown): string[] {
  ...
  for (let index = 0; index < LINUX_MATHML_GREEK_TOKENS.length; index++) {
    ...
    if (
      ...
      actual.sourceCodePoint !== expected.sourceCodePoint ||
      actual.transformedCodePoint !== expected.transformedCodePoint
    ) {
      problems.push(`${expected.id}: source/math-auto scalar identity mismatch`);
    }
    if (JSON.stringify(actual.glyph) !== JSON.stringify(expected.glyph))
      problems.push(`${expected.id}: authenticated glyph identity mismatch`);
    ...
    if (JSON.stringify(actual.geometry.matrix) !== JSON.stringify([1, 0, 0, 1, 0, 0]))
      problems.push(`${expected.id}: unexpected non-identity source matrix`);
  }
  return problems;
}
...
export function assertLinuxMathmlGreekPreterminalEvidence(
  raw: unknown,
): asserts raw is LinuxMathmlGreekPreterminalEvidence {
  const problems = validateLinuxMathmlGreekPreterminalEvidence(raw);
  if (problems.length > 0)
    throw new Error(`Linux MathML Greek pre-terminal evidence is not exact: ${problems.join("; ")}`);
}
