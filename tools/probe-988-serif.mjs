...
const html =
  `<style>div{font:18px serif;background:white;color:black}</style>` +
...
for (let i = 0; i < cps.length; i++) {
  ...
  console.log(
    `U+${cp.toString(16).toUpperCase()}  serif primary  Chrome: ${fonts.fonts.map((f) => `${f.familyName}×${f.glyphCount}`).join(", ")}  width=${w.toFixed(2)}px`,
  );
}
await browser.close();
