    ...
    for (const u of uses) {
      ...
      const item = {
        ...
        useAttrs: {
          ...
          h: u.getAttribute("height"),
        },
        useStyleW: ucs.width,
        useStyleH: ucs.height,
        targetCsW: targetCs.width,
        targetCsH: targetCs.height,
        targetRectW: Math.round(tr.width),
        targetRectH: Math.round(tr.height),
        targetParent:
          (t.parentElement?.tagName || "") +
          " style:" +
          ((t.parentElement as HTMLElement | null)?.style?.cssText?.slice(0, 80) ?? ""),
