...
const scenarios: Scenario[] = [
  ...
  { id: "empty-decoration", pseudo: "before", expectation: "exact", states: ["content:empty", "before", "empty-host"] },
  {
    ...
    pseudo: "before",
    expectation: "exact",
    states: ["content:string", "before", "in-flow", "host-text-first"],
  },
  {
    id: "after-string",
    pseudo: "after",
    expectation: "exact",
    states: ["content:string", "after", "in-flow", "element-child-first"],
  },
  {
    id: "absolute-before",
    pseudo: "before",
    expectation: "exact",
    states: ["before", "absolute", "fractional-origin"],
  },
  ...
  { id: "line-normal", pseudo: "before", expectation: "exact", states: ["line-height:normal", "mixed-font"] },
  {
    ...
    pseudo: "after",
    expectation: "exact",
    states: ["line-height:explicit", "fallback-glyph", "astral-utf16"],
  },
  ...["baseline", "middle", "text-top", "text-bottom", "top", "bottom", "sub", "super", "length", "percent"].map(
    (value): Scenario => ({
  ...
  ),
  {
    ...
    pseudo: "before",
    expectation: "exact",
    states: ["LTR", "mixed-bidi", "neutral-punctuation", "isolate"],
  },
  ...
  { id: "multicol", pseudo: "before", expectation: "exact", states: ["multicol", "fragmentainer-translation"] },
  {
    ...
    pseudo: "before",
    expectation: "exact",
    states: ["logical-edges:asymmetric", "box-decoration-break:slice"],
  },
  {
    id: "edges-clone",
    pseudo: "after",
    expectation: "exact",
    states: ["logical-edges:asymmetric", "box-decoration-break:clone"],
  },
  {
    id: "mixed-content",
    pseudo: "before",
    expectation: "exact",
    states: ["text-url-text", "anonymous-image", "content-item-offset-reset", "astral-utf16"],
  },
...
function fixtureHtml(): string {
  const verticalAlignRules = [
    ...
    .map(
      ([id, value]) =>
        `#vertical-align-${id}::before{content:"${id}";vertical-align:${value};font-size:13px;line-height:22px}`,
    )
    .join("\n");
  const verticalAlignHosts = [
    "baseline",
    "middle",
    "text-top",
    "text-bottom",
    ...
    .map(
      (id) =>
        `<div class="probe va" data-pseudo-oracle="vertical-align-${id}" id="vertical-align-${id}"><span>Tall</span></div>`,
    )
    .join("\n");
    ...
    if (hostId != null) {
      for (const pseudo of node.pseudoElements ?? []) {
        if (pseudo.pseudoType === "before" || pseudo.pseudoType === "after")
          result.set(`${hostId}:${pseudo.pseudoType}`, pseudo);
      }
    }
  ...
  return page.evaluate(
    ({ id, pseudo }) => {
      ...
      return {
        ...
        boxDecorationBreak:
          style.getPropertyValue("box-decoration-break") || style.getPropertyValue("-webkit-box-decoration-break"),
        border: {
...
async function measuredRows(
  page: Page,
  raw: SnapshotLayoutRow[],
  style: BrowserStyleResult,
): Promise<SnapshotLayoutRow[]> {
  const slices = raw.flatMap((row) =>
    row.text == null
      ? []
  ...
  return raw.map((row) => ({
    ...row,
    textBoxes: row.textBoxes.map((box) => ({
      ...box,
      ...(row.text == null ? {} : { shapedAdvance: advances[cursor++] }),
    })),
  }));
}
...
async function collectDprRows(browser: Browser, deviceScaleFactor: number): Promise<PseudoOracleRow[]> {
  ...
  try {
    ...
    const { root } = (await session.send("DOM.getDocument", { depth: -1, pierce: true })) as unknown as {
      root: CdpNode;
    };
    ...
    const rows = await Promise.all(
      scenarios.map(async (scenario): Promise<PseudoOracleRow> => {
        ...
        if (node == null) {
          ...
          return {
            ...
            textFragments: 0,
            imageFragments: 0,
            errors: pass ? [] : ["expected a protocol pseudo node"],
            pass,
          };
        }
...
function mutateAndReject(
  row: PseudoOracleRow | undefined,
  mutate: (record: DecodedPseudoFragmentSet) => void,
): { rejected: boolean; evidence: string } {
  if (row?.input == null || row.record?.status !== "exact")
    return { rejected: false, evidence: "target exact row unavailable" };
...
function mutationMatrix(rows: PseudoOracleRow[]): PseudoMutationResult[] {
  const exact = (id: string) =>
    rows.find((row) => row.deviceScaleFactor === 1 && row.id === id && row.record?.status === "exact");
  const mutation = (
    id: string,
    target: PseudoOracleRow | undefined,
    change: (record: DecodedPseudoFragmentSet) => void,
  ): PseudoMutationResult => ({ id, ...mutateAndReject(target, change) });
  const firstText = (record: DecodedPseudoFragmentSet) => record.fragments.find((fragment) => fragment.kind === "text");
  return [
    mutation("font-size-half-leading", exact("line-explicit"), (record) => {
      const fragment = firstText(record);
      if (fragment?.kind === "text") fragment.baseline.origin.y += 3;
    }),
    mutation("host-first-last-baseline", exact("vertical-align-super"), (record) => {
      const fragment = firstText(record);
      if (fragment?.kind === "text") fragment.baseline.origin.y += 7;
    }),
    mutation("union-all-fragments", exact("wrap-three"), (record) => {
      if (record.fragments.length > 1) record.fragments.splice(1);
    }),
    mutation("sort-bidi-by-source-offset", exact("bidi-rtl"), (record) => {
      record.fragments.sort(
        (a, b) => (a.kind === "text" ? a.sourceStartUtf16 : 0) - (b.kind === "text" ? b.sourceStartUtf16 : 0),
      );
    }),
    mutation("omit-fragmentainer-translation", exact("multicol"), (record) => {
      const box = record.boxFragments.find(
        (fragment) => fragment.fragmentainerTranslation != null && Math.abs(fragment.fragmentainerTranslation.x) > 1,
      );
      if (box != null) box.fragmentainerTranslation = { x: 0, y: 0 };
    }),
    mutation("concatenate-content-items", exact("mixed-content"), (record) => {
      for (const fragment of record.fragments) fragment.contentItemIndex = 0;
    }),
    mutation("wrong-slice-edge-ownership", exact("edges-slice"), (record) => {
      if (record.boxFragments.length > 1) record.boxFragments[0].edgeOwnership.inlineEnd = true;
    }),
    mutation("horizontal-baseline-in-vertical-writing", exact("writing-vertical-rl"), (record) => {
      const fragment = firstText(record);
      if (fragment?.kind === "text")
        fragment.baseline.origin = { x: fragment.localRect.x, y: fragment.localRect.y + fragment.baseline.ascent };
    }),
    mutation("codepoint-not-utf16-offset", exact("line-explicit"), (record) => {
      const fragment = record.fragments.find((item) => item.kind === "text" && item.text.includes("😀"));
      if (fragment?.kind === "text") fragment.sourceEndUtf16--;
    }),
    mutation("drop-anonymous-image-row", exact("mixed-content"), (record) => {
      const index = record.fragments.findIndex((fragment) => fragment.kind === "image");
      if (index >= 0) record.fragments.splice(index, 1);
    }),
  ];
}
...
async function main(): Promise<number> {
  ...
  console.log(
    `pseudo fragment geometry oracle: ${passed}/${report.rows.length} rows; ${rejected}/${report.mutations.length} mutations; ${report.chromiumVersion}; ${report.verdict}`,
  );
  for (const row of report.rows.filter((item) => !item.pass))
    console.log(`FAIL dpr=${row.deviceScaleFactor} ${row.id}: ${row.actual}; ${row.errors.join("; ")}`);
  for (const mutation of report.mutations.filter((item) => !item.rejected))
    console.log(`FAIL mutation ${mutation.id}: ${mutation.evidence}`);
