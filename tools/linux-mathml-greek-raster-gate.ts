...
const residualSchema = z
  .object({
    ...
    width: z.number().int().nonnegative(),
    height: z.number().int().nonnegative(),
    maxEdgeDistance: finite.nonnegative(),
    severity: finite.nonnegative(),
...
const fingerprintSchema = z
  .object({
    ...
    fontconfigConfigSha256: sha,
    fontInventorySha256: sha,
    rendererSourceSha256: sha,
    oracleSourceSha256: sha,
    consumerRasterizer: z.string().min(1),
    playwrightVersion: z.string().min(1),
    nodeVersion: z.string().min(1),
    icuVersion: z.string().min(1),
    sharpVersion: z.string().min(1),
    libvipsVersion: z.string().min(1),
    metricAlgorithm: z.literal(LINUX_MATHML_GREEK_CELL.metricAlgorithm),
    launchFlags: z.array(z.string()),
    locale: z.string().min(1),
  })
  .strict();

export const linuxMathmlGreekRasterRowSchema = z
  .object({
    ...
    runProvenance: z
      .object({
        ...
        githubRunAttempt: z.string().min(1),
        githubJob: z.string().min(1),
        runnerName: z.string().min(1),
        runnerBootIdSha256: sha,
    ...
    hintingControl: z
      .object({
        ...
        computedFamily: z.string().min(1),
        fontFaceRuleFamily: z.string().min(1),
        fontFaceRuleCount: z.literal(1),
        ...
        isCustomFont: z.literal(true),
        glyphCount: z.literal(LINUX_MATHML_GREEK_TOKENS.length),
        tokens: z.array(logicalTokenSchema).length(LINUX_MATHML_GREEK_TOKENS.length),
      })
      .strict(),
...
const maximaSchema = residualSchema.pick({
  ...
  width: true,
  height: true,
  maxEdgeDistance: true,
  severity: true,
  totalChannelDelta: true,
});
const roleHashesSchema = z.object({ native: sha, paths: sha, hintingOff: sha }).strict();
export const linuxMathmlGreekEnvelopeFileSchema = z
  .object({
    schemaVersion: z.literal(1),
    entries: z.array(
      z
        .object({
          ...
          proposalHashes: roleHashesSchema,
          validationHashes: roleHashesSchema,
          maxima: maximaSchema,
...
export const linuxMathmlGreekFingerprintSha256 = (fingerprint: LinuxMathmlGreekRasterRow["fingerprint"]): string =>
  sha256(canonical(fingerprint));

function logicalProblems(row: LinuxMathmlGreekRasterRow): string[] {
  const problems = [...row.warnings];
  try {
    assertLinuxMathmlGreekPreterminalEvidence(row.preterminal);
  } catch (error) {
    problems.push(String(error));
  }
  if (row.preterminal.inventory.inventorySha256 !== row.fingerprint.fontInventorySha256)
    problems.push("pre-terminal/fingerprint font inventory mismatch");
  if (row.preterminal.inventory.configSha256 !== row.fingerprint.fontconfigConfigSha256)
    problems.push("pre-terminal/fingerprint Fontconfig configuration mismatch");
  for (let index = 0; index < LINUX_MATHML_GREEK_TOKENS.length; index++) {
    ...
    if (paths.id !== expected.id || JSON.stringify(paths.glyph) !== JSON.stringify(expected.glyph))
      problems.push(`${expected.id}: paths glyph does not match the authenticated source`);
    if (hinting.id !== expected.id || JSON.stringify(hinting.glyph) !== JSON.stringify(expected.glyph))
      problems.push(`${expected.id}: hinting-off glyph does not match the authenticated source`);
    if (paths.baseline !== source.geometry.baseline || canonical(paths.matrix) !== canonical(source.geometry.matrix))
      problems.push(`${expected.id}: paths placement is not capture-owned`);
    if (
      hinting.baseline !== source.geometry.baseline ||
      canonical(hinting.matrix) !== canonical(source.geometry.matrix)
    )
      problems.push(`${expected.id}: hinting-off placement is not capture-owned`);
  }
  if (
    row.hintingControl.requestedFamily !== row.hintingControl.computedFamily ||
    row.hintingControl.requestedFamily !== row.hintingControl.fontFaceRuleFamily
  )
    problems.push("hinting-off custom-face family ownership mismatch");
  if (row.nativeArtifact.sha256 === row.pathsArtifact.sha256) problems.push("native/paths raster arms are inert");
  if (row.nativeArtifact.sha256 === row.hintingOffArtifact.sha256)
    problems.push("hinting-off negative control is inert");
  if (row.hintingOffResidual.changedPixels === 0 || row.hintingOffResidual.totalChannelDelta === 0)
    problems.push("hinting-off negative control reports no movement");
  return problems;
}
...
function safeArtifact(root: string, path: string): string {
  ...
  if (rel === "" || rel.startsWith("../") || rel.includes("/../"))
    throw new Error(`artifact path escapes bundle: ${path}`);
  return absolute;
}

/** Reopen every lossless artifact and distrust caller-supplied hashes/metrics. */
export async function reauthenticateLinuxMathmlGreekRows(
  raw: unknown,
  artifactRoot: string,
  ...
  for (const row of parsed) {
    ...
    for (const artifact of artifacts) {
      ...
      if (decoded.width !== artifact.width || decoded.height !== artifact.height)
        throw new Error(`${row.id}: ${artifact.path} dimensions are unauthenticated`);
      if (sha256(content) !== artifact.sha256)
        throw new Error(`${row.id}: ${artifact.path} SHA-256 is unauthenticated`);
      bytes.push(content);
    }
    ...
    if (canonical(residual) !== canonical(row.residual))
      throw new Error(`${row.id}: native/paths residual is unauthenticated`);
    if (canonical(hintingOffResidual) !== canonical(row.hintingOffResidual))
      throw new Error(`${row.id}: hinting-off residual is unauthenticated`);
...
function maxima(residual: LinuxMathmlGreekResidual): z.infer<typeof maximaSchema> {
  return {
    changedPixels: residual.changedPixels,
    area: residual.area,
    width: residual.width,
    height: residual.height,
    maxEdgeDistance: residual.maxEdgeDistance,
    severity: residual.severity,
    totalChannelDelta: residual.totalChannelDelta,
  };
}
...
export interface LinuxMathmlGreekReport {
  ...
  verdict:
    | "logical-mismatch"
    | "incomplete-independent-evidence"
    | "logical-exact-unratified"
    | "ratified-rasterization-only"
    | "validation-exceeds-proposal";
  problems: string[];
...
export function adjudicateLinuxMathmlGreekRows(rawRows: unknown, rawEnvelopes: unknown): LinuxMathmlGreekReport {
  ...
  if (!parsed.success || !envelopes.success)
    return {
      schemaVersion: 1,
      pass: false,
      eligibleForRatification: false,
      verdict: "logical-mismatch",
      problems: [parsed.success ? envelopes.error.message : parsed.error.message],
    };
  const rows = parsed.data;
  const problems = rows.flatMap((row) => logicalProblems(row).map((problem) => `${row.runLabel}: ${problem}`));
  if (problems.length > 0)
    return { schemaVersion: 1, pass: false, eligibleForRatification: false, verdict: "logical-mismatch", problems };
  ...
  if (proposal.length !== 1 || validation.length !== 1)
    return {
      ...
      pass: false,
      eligibleForRatification: false,
      verdict: "incomplete-independent-evidence",
      problems: [`expected one proposal and one validation row, found ${proposal.length}/${validation.length}`],
    };
  const p = proposal[0],
    v = validation[0];
  if (canonical(p.fingerprint) !== canonical(v.fingerprint)) problems.push("proposal/validation fingerprints differ");
  if (p.runProvenance.runnerBootIdSha256 === v.runProvenance.runnerBootIdSha256)
    problems.push("proposal/validation were not collected on independent Linux machines");
  if (p.cellSha256 !== v.cellSha256) problems.push("proposal/validation cell identities differ");
  const proposalMaxima = maxima(p.residual),
    validationResidual = maxima(v.residual);
  const candidateEnvelope = {
    cellSha256: p.cellSha256,
    fingerprintSha256: linuxMathmlGreekFingerprintSha256(p.fingerprint),
    proposalHashes: {
      native: p.nativeArtifact.sha256,
      paths: p.pathsArtifact.sha256,
      hintingOff: p.hintingOffArtifact.sha256,
    },
    validationHashes: {
      native: v.nativeArtifact.sha256,
      paths: v.pathsArtifact.sha256,
      hintingOff: v.hintingOffArtifact.sha256,
    },
    maxima: proposalMaxima,
  };
  if (!inside(validationResidual, proposalMaxima))
    problems.push("independent validation exceeds the proposal residual in at least one scalar dimension");
  if (problems.length > 0)
    return {
      schemaVersion: 1,
      pass: false,
      eligibleForRatification: false,
      verdict: problems.some((problem) => problem.includes("exceeds"))
        ? "validation-exceeds-proposal"
        : "incomplete-independent-evidence",
      problems,
      candidateEnvelope,
    };
  const envelope = envelopes.data.entries.find(
    (entry) =>
      entry.cellSha256 === candidateEnvelope.cellSha256 &&
      entry.fingerprintSha256 === candidateEnvelope.fingerprintSha256,
  );
  if (envelope == null)
    return {
      schemaVersion: 1,
  ...
  if (
    canonical(envelope.proposalHashes) !== canonical(candidateEnvelope.proposalHashes) ||
    ...
    return {
      ...
      eligibleForRatification: false,
      verdict: "logical-mismatch",
      problems: ["ratified envelope is not the reviewed proposal/validation evidence"],
      candidateEnvelope,
    };
  }
  return {
    ...
    pass: true,
    eligibleForRatification: true,
    verdict: "ratified-rasterization-only",
    problems: [],
    candidateEnvelope,
  };
}
...
async function main(): Promise<void> {
  ...
  if (mode === "aggregate") {
    ...
    const files = readdirSync(root, { recursive: true })
      .map(String)
      .filter((file) => file.endsWith("linux-mathml-greek-raster-rows.json"));
    const rows: LinuxMathmlGreekRasterRow[] = [];
