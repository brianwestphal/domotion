...
const rowSchema = z
  .object({
    ...
    axis: z.string().min(1),
    expectedRoute: z.enum(["marker-free-control", "custom-vector", "suppressed-captured-absence", "native-raster"]),
    deviceScaleFactor: z.union([z.literal(1), z.literal(2)]),
    cssZoom: z.union([z.literal(1), z.literal(1.25), z.literal(2)]),
    captured: z
      .object({ missingFacts: z.array(z.string()) })
      .passthrough()
      .nullable(),
  ...
  .passthrough();
export const nativeScrollbarAuditReportSchema = z
  .object({
    ...
    mutations: z.record(z.string(), z.boolean()),
    platformFingerprints: z.array(z.unknown()),
    verdict: z.literal("authoritative-capture-and-source-owned-paint-exact"),
  })
  .strict();
...
const artifactManifest = (report: NativeScrollbarAuditReport) =>
  report.rows.flatMap((row) => row.artifacts.map((artifact) => ({ row: rowKey(row), ...artifact })));
const logicalRows = (report: NativeScrollbarAuditReport) =>
  report.rows.map((row) => ({
    id: row.id,
    axis: row.axis,
...
export function adjudicateNativeScrollbarReports(
  ...
  _envelopes: readonly NativeScrollbarRasterEnvelope[] = [],
  ...
  for (const { parsed: report, raw } of reports) {
    ...
    if (
      report.sourceRevisions.chromium !== SCROLLBAR_GATE_SOURCE_REVISIONS.chromium ||
      report.sourceRevisions.skiaPinnedByChromium !== SCROLLBAR_GATE_SOURCE_REVISIONS.skiaPinnedByChromium
    )
      blockers.push(`${key}: source revision mismatch`);
    ...
    if (report.logicalRowsSha256 !== digest(logicalRows(raw)))
      blockers.push(`${key}: canonical logical-row hash mismatch`);
    if (report.artifactSetSha256 !== digest(artifactManifest(raw)))
      blockers.push(`${key}: canonical artifact-set hash mismatch`);
    if (!Object.values(report.controls).every(Boolean)) blockers.push(`${key}: ownership controls are not all active`);
    if (!Object.values(report.mutations).every(Boolean))
      blockers.push(`${key}: destructive mutations are not all active`);
    ...
    for (const row of report.rows) {
      ...
      if (row.captured == null || row.captured.missingFacts.length > 0)
        blockers.push(`${key}/${rk}: source ownership facts incomplete`);
      if (row.artifacts.length !== 2 || new Set(row.artifacts.map((artifact) => artifact.role)).size !== 2)
        blockers.push(`${key}/${rk}: lossless source/generated artifact pair incomplete`);
    }
    for (const id of SCROLLBAR_GATE_SCENARIOS)
      for (const dpr of SCROLLBAR_GATE_DPRS)
  ...
  for (const platform of SCROLLBAR_GATE_PLATFORMS)
    for (const role of ["proposal", "validation"] as const)
      if (!byRole.has(`${platform}/${role}`)) blockers.push(`missing role-bound report: ${platform}/${role}`);
  for (const platform of SCROLLBAR_GATE_PLATFORMS) {
    const proposal = byRole.get(`${platform}/proposal`);
    const validation = byRole.get(`${platform}/validation`);
    if (proposal == null || validation == null) continue;
    if (proposal.observationId === validation.observationId)
      blockers.push(`${platform}: proposal/validation observation ids are not independent`);
    if (proposal.provenance.bootId === validation.provenance.bootId)
      blockers.push(`${platform}: proposal/validation boot ids are not independent`);
    if (
      proposal.provenance.githubRunId !== validation.provenance.githubRunId ||
      proposal.provenance.githubRunAttempt !== validation.provenance.githubRunAttempt ||
      proposal.provenance.workflowRef !== validation.provenance.workflowRef
    )
      blockers.push(`${platform}: proposal/validation workflow provenance differs`);
    ...
    if (
      proposal.provenance.runnerImage !== validation.provenance.runnerImage ||
    ...
    if (proposal.logicalRowsSha256 !== validation.logicalRowsSha256)
      blockers.push(`${platform}: proposal/validation logical rows disagree`);
  }
