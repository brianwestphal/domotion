...
const mechanismSchema = z.object({
  ...
  platforms: z.array(z.enum(ACTIVATION_PLATFORMS)).min(1),
});
  ...
  for (const mechanism of ledger.mechanisms) {
    ...
    if (evidence != null)
      for (const [kind, marker] of [
        ["positive", mechanism.positive],
        ["negative", mechanism.negative],
        ["mutation", mechanism.mutation],
      ] as const) {
        if (!evidence.includes(marker)) errors.push(`${mechanism.id}: stale ${kind} marker: ${marker}`);
      }
    if (mechanism.positive === mechanism.negative)
      errors.push(`${mechanism.id}: positive and negative controls must differ`);
  }
  for (const kind of ACTIVATION_KINDS)
    if (!ledger.mechanisms.some((entry) => entry.kind === kind)) errors.push(`unclaimed mechanism kind: ${kind}`);
  return errors;
}
...
export async function writeActivationEvidence(path: string, ledger: ActivationLedger): Promise<void> {
  await writeFile(
    path,
    `${JSON.stringify({ schemaVersion: 1, generatedAt: new Date().toISOString(), platform: process.platform, mechanisms: ledger.mechanisms.map(({ id, kind, evidenceFile, positive, negative, mutation, platforms }) => ({ id, kind, evidenceFile, positive, negative, mutation, platforms })) }, null, 2)}\n`,
  );
}
