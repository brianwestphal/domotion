...
function isOptionalInstall(family, path) {
  return path.startsWith("/Library/Fonts/") || !existsSync(path) || OS_VERSION_OPTIONAL_FAMILIES.has(family);
}
...
const FAMILY_TO_PATH = {
  ...
  "Grantha Sangam MN": {
    path: `${SUPPLEMENTAL}/Tamil Sangam MN.ttc`,
    postscriptName: "GranthaSangamMN-Regular",
    extractor: "native",
  },
...
out += "\n";
out +=
  "export interface UnicodeFontEntry { family: string; path: string; postscriptName?: string; optionalInstall?: boolean }\n\n";
...
for (const r of ranges) {
  ...
  const optional = isOptionalInstall(r.family, r.resolved.path) ? ", optionalInstall: true" : "";
...
for (const [fontKey, resolved] of keyToResolved) {
  ...
  const r = spawnSync(
    ...
    [
      ...
      JSON.stringify([
        resolved.path,
        resolved.postscriptName ?? null,
        sampleRange.start.toString(16),
        sampleRange.end.toString(16),
      ]),
    ],
...
out += "\n";
out +=
  'export interface UnicodeFontEntry { family: string; path: string; postscriptName?: string; extractor?: "fontkit" | "native"; optionalInstall?: boolean }\n\n';
out += "export const UNICODE_FONT_PATHS: Record<string, UnicodeFontEntry> = {\n";
seenKeys.clear();
for (const r of ranges) {
  if (seenKeys.has(r.fontKey)) continue;
...
console.log(
  `${fontkitCrashers.size} fontkit-crashing fonts marked extractor:"native" — routed through the CoreText glyph-helper:`,
);
