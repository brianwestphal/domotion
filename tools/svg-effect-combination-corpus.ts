...
function cssFor(test: SvgEffectCombinationCase): string {
  ...
  return `
    ...
    .paint-${test.ordinal}{fill:url("#gradient-${test.ordinal}");stroke:#172554;stroke-width:8;clip-path:${clip};mask:url("#mask-${test.ordinal}");vector-effect:${test.values.vectorEffect};${transform}}
...
function tileMarkup(test: SvgEffectCombinationCase): string {
  ...
  const owner = test.values.viewport === "nested"
    ...
    : content;
  return `<svg data-effect-case="${test.id}" style="position:absolute;left:${x}px;top:${y}px" width="180" height="150" viewBox="0 0 180 150"><defs>${gradientMarkup(test)}${clipMarkup(test)}${maskMarkup(test)}${markerMarkup(test)}</defs>${owner}</svg>`;
}
