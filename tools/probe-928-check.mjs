...
const info = await page.evaluate(() => {
  const lis = Array.from(document.querySelectorAll(".features li"))
    .slice(0, 5)
    .map((li, i) => {
...
const screenshot = await page.screenshot({
  clip: { x: li0.liRect.x - 4, y: li0.liRect.y - 2, width: li0.liRect.w + 8, height: li0.liRect.h + 4 },
});
writeFileSync("/tmp/p928-li-chrome.png", screenshot);
const m = await sharp("/tmp/p928-li-chrome.png").metadata();
await sharp("/tmp/p928-li-chrome.png")
  .resize(m.width * 5, m.height * 5, { kernel: "nearest" })
  .toFile("/tmp/p928-li-chrome-z.png");
await browser.close();
