  ...
  Array.from(document.querySelectorAll(".sample")).map((el) => {
    ...
    return {
      ...
      rect: {
        ...
        y: Math.floor(r.y),
        w: Math.ceil(r.width),
        h: Math.ceil(r.height + parseFloat(getComputedStyle(el).fontSize)),
      },
    };
  }),
);
...
for (const s of samples) {
  ...
  if (centers.length < 50 || blackBottomY < 0) {
    console.log(JSON.stringify({ fontSize: s.fontSize, error: "skip" }));
    continue;
  }
