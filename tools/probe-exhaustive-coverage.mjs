...
const cps = [...html.matchAll(/U\+([0-9A-Fa-f]{4,6})/g)]
  .map((m) => parseInt(m[1], 16))
  .filter((cp) => cp >= 0x2f800 && cp <= 0x2fa1d);
...
for (const d of dirs) {
  try {
    for (const f of readdirSync(d)) if (/\.(ttf|ttc|otf|otc)$/i.test(f)) files.push(`${d}/${f}`);
  } catch {}
}
const faces = [];
for (const p of files) {
  try {
    const fk = fontkit.openSync(p);
    for (const ff of fk.fonts ?? [fk]) faces.push(ff);
  } catch {}
}
...
function litCovered(cp) {
  for (const ff of faces) {
    try {
      if (ff.glyphForCodePoint(cp).id !== 0) return true;
    } catch {}
  }
  return false;
}
function nfdCovered(cp) {
  ...
  for (const ff of faces) {
    try {
      if (ff.glyphForCodePoint(dcp).id !== 0) return true;
    } catch {}
  }
  return false;
}
