...
export function assertStandaloneDefaultIgnorableRecord(text: string, logicalRuns: TextRunProvenanceDiagnostic[]): void {
  const scalars = [...text];
  if (scalars.length !== 1 || !isHarfbuzzDefaultIgnorable(scalars[0].codePointAt(0)!)) return;
  if (logicalRuns.length !== 1)
    throw new Error(`standalone default-ignorable produced ${logicalRuns.length} terminal runs`);
  const run = logicalRuns[0];
  if (
    run.sourceText !== text ||
    run.emittedText !== text ||
  ...
  if (run.glyphs.length !== 1)
    throw new Error(`standalone default-ignorable produced ${run.glyphs.length} logical glyphs`);
  const glyph = run.glyphs[0];
  const expectedSpan: [number, number] = [0, text.length];
  if (
    glyph.id !== expectedGid ||
    ...
    throw new Error(
      `standalone default-ignorable logical record disagrees with HarfBuzz source: ${JSON.stringify(glyph)}`,
    );
  }
}
...
export function assertStandaloneDefaultIgnorableFace(
  ...
  const selected = logicalRuns[0]?.selected.instantiatedPostscriptName ?? logicalRuns[0]?.selected.postscriptName;
  const chrome = chromeFaces.map((face) => face.replace(/×\d+$/, ""));
  if (selected == null || !chrome.some((face) => normalize(face) === normalize(selected))) {
    throw new Error(
      `standalone default-ignorable selected face ${selected ?? "(none)"} != Chromium ${chrome.join(",") || "(none)"}`,
    );
  }
}
...
export async function extractRuns(
  ...
  for (const { path: file, label } of files) {
    ...
    const found = await page.evaluate(
      ({ splitEvery, implicitOuterLayerOrder }: { splitEvery: boolean; implicitOuterLayerOrder: number }) => {
        const out: string[] = [];
        ...
        const harvestableFaces = new Map<
          string,
          {
            family: string;
            mime: string;
        ...
        const decodeLayerIdent = (value: string): string => {
          ...
          for (let index = 0; index < value.length;) {
            ...
            if (hex !== "") {
              ...
              output +=
                cp === 0 || cp > 0x10ffff || (cp >= 0xd800 && cp <= 0xdfff) ? "\ufffd" : String.fromCodePoint(cp);
        ...
        const pendingFeatureRules: Array<{
          rule: CSSRule & {
            fontFamily: string;
            annotation?: Map<string, number[]>;
            ornaments?: Map<string, number[]>;
            stylistic: Map<string, number[]>;
            swash?: Map<string, number[]>;
            characterVariant?: Map<string, number[]>;
            styleset?: Map<string, number[]>;
          };
          layer: FeatureLayer;
        }> = [];
        ...
        while (pendingRules.length > 0) {
          ...
          if (kind === "CSSLayerBlockRule") {
            ...
            pendingRules.unshift(
              ...Array.from(block.cssRules ?? []).map((child) => ({
                rule: child,
                layer: nestedLayer,
              })),
            );
          ...
          if (kind === "CSSImportRule") {
            ...
            const importLayer = imported.layerName == null ? layer : addLayer(layer, imported.layerName);
            try {
              pendingRules.unshift(
                ...Array.from(imported.styleSheet?.cssRules ?? []).map((child) => ({
                  rule: child,
                  layer: importLayer,
                })),
              );
        ...
        for (const el of elements) {
          ...
          const stackFamilies = (cs.fontFamily.match(/(?:"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|[^,])+/g) ?? []).map(
            (family) =>
              family
                .trim()
                .replace(/^["']|["']$/g, "")
          ...
          for (const node of Array.from(el.childNodes)) {
            ...
            const texts =
              splitEvery || fvs !== "normal" || ffs !== "normal" || fva !== "normal" ? raw.split(/\s+/) : [raw];
    ...
    const fusedFeatureValues = fuseFontFeatureValueRules(found.featureValueRules as FontFeatureValueRuleRecord[]);
...
function probeEnvironmentKey(spec: RunSpec): string {
  return JSON.stringify([
    spec.fontFeatureEnvironment == null ? null : fontFeatureEnvironmentKey(spec.fontFeatureEnvironment),
    spec.webfont ?? null,
...
export function resolvedFeaturesForRun(spec: RunSpec): string[] {
  ...
  const resolved = resolvedFeatureValueList(spec.fontVariantAlternates, spec.fontFamily, tables);
  if (spec.resolvedFontFeatures != null && JSON.stringify(spec.resolvedFontFeatures) !== JSON.stringify(resolved)) {
    throw new Error(
      `stale font-feature-values row for ${spec.fontFamily}: expected ` +
...
function webfontFaceCss(spec: RunSpec): string {
  ...
  if (face == null) return "";
  if (!/^[a-z0-9.+-]+\/[a-z0-9.+-]+$/i.test(face.mime) || !/^[A-Za-z0-9+/=]+$/.test(face.dataBase64)) {
...
export function shapingProbePageHtml(specs: RunSpec[]): string {
  ...
  const prelude =
    specs.length === 0
      ? ""
      : webfontFaceCss(specs[0]) + serializeFontFeatureValues(environmentTables ?? specs[0].fontFeatureValues);
  return `<html lang="en"><head><meta charset="utf-8"><style>${prelude}</style></head><body style="margin:0">${specs
    .map(
      (s, i) =>
        ...
        `${s.fontStretch != null && s.fontStretch !== "100%" ? `;font-stretch:${esc(s.fontStretch)}` : ""}` +
        ...
        `${s.letterSpacing != null && s.letterSpacing !== "normal" ? `;letter-spacing:${esc(s.letterSpacing)}` : ""}` +
        `${s.textRendering != null && s.textRendering !== "auto" ? `;text-rendering:${esc(s.textRendering)}` : ""}` +
        `${
...
export async function chromeShaping(page: import("@playwright/test").Page, specs: RunSpec[]): Promise<ChromeShaping[]> {
  ...
  if (groups.size > 1) {
    ...
    for (const group of groups.values()) {
      const rows = await chromeShaping(
        page,
        group.map((entry) => entry.spec),
      );
  ...
  const geom = await page.evaluate((count: number) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      out.push({
        xs: Array.from(seen).sort((a, b) => a - b),
  ...
  for (let i = 0; i < specs.length; i++) {
    const { nodeId } = (await client.send("DOM.querySelector", { nodeId: root.nodeId, selector: `#r${i}` })) as {
      nodeId: number;
    };
...
export function ourShaping(spec: RunSpec): OurShaping {
  const authenticated =
    spec.fontFeatureEnvironment == null ? undefined : authenticateFontFeatureEnvironment(spec.fontFeatureEnvironment);
  if (authenticated != null) {
    ...
    if (!installedRunWebfonts.has(key)) {
      registerWebfont(
        face.family,
        parseInt(face.weightDescriptor, 10) || 400,
  ...
  const features = mergeFeatureLists(
    ...
    resolveFontVariantFeatures(undefined, undefined, spec.fontVariantLigatures, spec.letterSpacing, spec.textRendering),
  );
  ...
  const terminalEmitter = provenance.runs.some((run) => run.emitter === "paths") ? "paths" : "embedded-font";
  const logicalGlyphs = provenance.runs.filter((run) => run.emitter === terminalEmitter).flatMap((run) => run.glyphs);
  ...
  const logicalXs =
    logicalGlyphs.length > 0 &&
    logicalGlyphs.every(
      (glyph) => glyph.xAdvance === 0 && glyph.yAdvance === 0 && glyph.xOffset === 0 && glyph.yOffset === 0,
    )
      ? logicalGlyphs.map(() => 0)
      : xs;
  return {
...
export function compareShaping(
  c: ChromeShaping,
  o: OurShaping,
  tolerance: number,
): { verdict: Verdict; maxDelta: number | null } {
  ...
  return maxDelta <= tolerance ? { verdict: "agree-exact", maxDelta } : { verdict: "agree-count", maxDelta };
}
...
async function main(): Promise<number> {
  ...
  try {
    ...
    const allow: Set<string> = existsSync(opts.allowlistFile)
      ? new Set(
          (JSON.parse(readFileSync(opts.allowlistFile, "utf-8")) as Array<{ text: string; fontFamily: string }>).map(
            (e) => `${e.text}\0${e.fontFamily}`,
          ),
        )
      : new Set();
