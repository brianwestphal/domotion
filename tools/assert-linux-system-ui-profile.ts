  ...
  for (const fresh of [false, true]) {
    ...
    for (const [index, c] of cases.entries()) {
      await page.setContent(
        `<span id="probe" lang="${index % 2 ? "ja" : "en"}" style="font: ${c.style} ${c.weight} ${c.stretch} 24px system-ui, sans-serif">Hamburgefons</span>`,
      );
      ...
      const ours = resolveFont(
        "system-ui, sans-serif",
        c.weight,
        24,
        c.style === "italic" ? 12 : 0,
        undefined,
        c.stretch === "condensed" ? 75 : 100,
        index % 2 ? "ja" : "en",
      ) as { postscriptName?: string; familyName?: string } | null;
