...
const PHASE = {
  ...
  "read-bodyBg": "source",
  "screenshot-expected": "source",
  "discover-webfonts": "source",
  "capture-tree": "source",
  "cache-write": "source",
  "embed-remote-images": "render",
  "rasterize-conic": "render",
  "render-svg": "render",
  "goto-svg": "actual",
  "settle-svg": "actual",
...
const PHASE_LABEL = {
  setup: "setup",
  source: "capture source (cache-miss only)",
  render: "render SVG",
  actual: "rasterize actual",
  compare: "diff (serialized by lock)",
};
...
out.push(
  `<text x="${PAD}" y="34" font-size="20" font-weight="700" fill="#202124">Domotion demo-test pipeline — one fixture (serial) and a real run (parallel)</text>`,
);
out.push(
  `<text x="${PAD}" y="56" font-size="12.5" fill="#5f6368">Sample: ${data.fixtures.length} html-test-unicode fixtures, all cache-miss · ${data.workerCount} workers · ${(data.totalWallMs / 1000).toFixed(1)}s wall · source: tests/html-test-suite.tsx instrumentation (DEMO_TIMING=1)</text>`,
);
...
out.push(
  `<text x="${PAD}" y="${aTop - 10}" font-size="14" font-weight="700" fill="#202124">① One demo test, step by step (serial) — mean ms across ${data.fixtures.length} fixtures, total ${r1(meanTotal)} ms</text>`,
);
...
steps.forEach((s, i) => {
  ...
  out.push(
    `<text x="${A_X - 8}" y="${y + ROWH / 2 + 4}" font-size="12" text-anchor="end" fill="#3c4043">${esc(s.step)}</text>`,
  );
  out.push(
    `<rect x="${A_X}" y="${y + 4}" width="${r1(bw)}" height="${ROWH - 9}" rx="2.5" fill="${PHASE_COLOR[s.phase]}"/>`,
  );
  const pct = (s.mean / meanTotal) * 100;
  out.push(
    `<text x="${A_X + bw + 8}" y="${y + ROWH / 2 + 4}" font-size="11.5" fill="#202124">${s.mean.toFixed(0)} ms <tspan fill="#80868b">(${pct.toFixed(0)}%, range ${s.min.toFixed(0)}–${s.max.toFixed(0)})</tspan></text>`,
  );
});
...
out.push(
  `<text x="${PAD}" y="${rTop - 8}" font-size="14" font-weight="700" fill="#202124">①b Inside render-svg = ${r1(rSvgTotal)} ms — CoreText helper ${helperPctOfRender.toFixed(0)}%, text ${((Math.max(0, textMs - helperMs) / Math.max(1, rSvgTotal)) * 100).toFixed(0)}%, box ${((Math.max(0, rSvgTotal - textMs) / Math.max(1, rSvgTotal)) * 100).toFixed(0)}%</text>`,
);
...
for (const seg of rsplit) {
  ...
  if (sw > 150) {
    out.push(
      `<text x="${rx + 6}" y="${rTop + rBarH / 2 + 3}" font-size="11" fill="#fff">${esc(seg.label)} — ${r1(seg.ms)} ms (${pct.toFixed(0)}%)</text>`,
    );
  }
  rx += sw;
}
...
out.push(
  `<text x="${PAD}" y="${bTop - 26}" font-size="14" font-weight="700" fill="#202124">② The real run — ${data.fixtures.length} fixtures across ${W} workers (parallel). Browser launches once; each worker runs the serial pipeline above; the diff step is serialized by a shared lock.</text>`,
);
...
for (let t = 0; t <= data.totalWallMs; t += 5000) {
  ...
  out.push(
    `<text x="${x}" y="${bTop - 12}" font-size="10.5" text-anchor="middle" fill="#80868b">${(t / 1000).toFixed(0)}s</text>`,
  );
}
for (let w = 0; w < W; w++) {
  const y = bTop + w * (laneH + 8);
  out.push(
    `<text x="${B_X - 8}" y="${y + laneH / 2 + 4}" font-size="11" text-anchor="end" fill="#5f6368">worker ${w + 1}</text>`,
  );
...
for (const fx of placed) {
  ...
  for (const st of fx.steps) {
    ...
    out.push(
      `<rect x="${sx}" y="${y + 3}" width="${Math.max(0.5, sw)}" height="${laneH - 6}" fill="${PHASE_COLOR[PHASE[st.step] ?? "setup"]}" opacity="0.92"/>`,
    );
    sx += sw;
  }
  out.push(
    `<rect x="${x}" y="${y + 3}" width="${r1(bw)}" height="${laneH - 6}" fill="none" stroke="#fff" stroke-width="0.5"/>`,
  );
  const label = fx.name.length > 22 ? fx.name.slice(0, 21) + "…" : fx.name;
  if (bw > 50)
    out.push(
      `<text x="${x + 3}" y="${y + laneH / 2 + 3.5}" font-size="9.5" fill="#fff">${esc(label)} ${(fx.totalMs / 1000).toFixed(1)}s</text>`,
    );
}
out.push(
  `<text x="${B_X}" y="${bTop + W * (laneH + 8) + 26}" font-size="11.5" fill="#5f6368">Total wall time ${(data.totalWallMs / 1000).toFixed(1)}s for ${data.fixtures.length} fixtures = ${(data.totalWallMs / data.fixtures.length / 1000).toFixed(2)}s/fixture amortized (vs ${(data.fixtures.reduce((a, f) => a + f.totalMs, 0) / data.fixtures.length / 1000).toFixed(2)}s/fixture of serial work — the gap is the ${W}× worker parallelism).</text>`,
);
