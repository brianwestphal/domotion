...
function dump(n: CapturedElement, depth: number, maxDepth: number, yMin: number, yMax: number) {
  ...
  const bg =
    n.styles?.backgroundImage && n.styles.backgroundImage !== "none"
      ? ` bg=${n.styles.backgroundImage.slice(0, 50)}`
      : "";
  console.log(
    `${"  ".repeat(depth)}${n.tag} rect=(${Math.round(n.x)},${Math.round(n.y)},${Math.round(n.width)},${Math.round(n.height)}) op=${(n.styles as any)?.opacity}${src}${txt}${bg}`,
  );
