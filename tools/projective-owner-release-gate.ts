...
const artifact = z.object({
  role: z.enum(["source", "generated", "owner-crop"]),
  path: z.string().min(1),
  sha256: sha,
...
const row = z.object({
  family: z.enum(PROJECTIVE_GATE_FAMILIES),
  profile: z.enum(PROJECTIVE_GATE_PROFILES),
  dpr: z.union([z.literal(1), z.literal(2)]),
  expectedOwnerIds: z.array(z.string()),
  actualOwnerIds: z.array(z.string()),
  rasterCount: z.number().int().nonnegative(),
  directImageApplications: z.number().int().nonnegative(),
  nestedDuplicateCount: z.number().int().nonnegative(),
  sampledApproximationCount: z.number().int().nonnegative(),
  vectorSentinelExact: z.boolean(),
  sentinelBakedIntoRaster: z.boolean(),
  restorationExact: z.boolean(),
  warnings: z.array(z.string()),
  maxFinalPixelDelta: z.number().int().nonnegative(),
...
export const projectiveOwnerReleaseReportSchema = z.object({
  ...
  environment: z.object({
    platform: z.enum(PROJECTIVE_GATE_PLATFORMS),
...
export function projectiveGateRowKey(
  value: Pick<ProjectiveOwnerReleaseReport["rows"][number], "family" | "profile" | "dpr">,
): string {
  return `${value.family}/${value.profile}/dpr${value.dpr}`;
}

export function adjudicateProjectiveOwnerRelease(
  ...
  for (const platform of PROJECTIVE_GATE_PLATFORMS) {
    ...
    if (matches.length !== 1)
      blockers.push(`${matches.length === 0 ? "missing" : "duplicate"} native platform report: ${platform}`);
  }
  for (const report of reports) {
    ...
    for (const family of PROJECTIVE_GATE_FAMILIES)
      for (const profile of PROJECTIVE_GATE_PROFILES)
        for (const dpr of PROJECTIVE_GATE_DPRS) {
    ...
    for (const value of report.rows) {
      ...
      if (JSON.stringify([...value.actualOwnerIds].sort()) !== JSON.stringify([...value.expectedOwnerIds].sort()))
        blockers.push(`${key}: owner identity drift`);
      if (
        value.rasterCount !== value.expectedOwnerIds.length ||
        value.directImageApplications !== value.expectedOwnerIds.length
      )
        blockers.push(`${key}: not one direct atomic image per owner`);
      if (value.nestedDuplicateCount || value.sampledApproximationCount)
        blockers.push(`${key}: duplicate or sampled projective paint`);
      if (!value.vectorSentinelExact || value.sentinelBakedIntoRaster)
        blockers.push(`${key}: vector sentinel ownership drift`);
      if (value.maxFinalPixelDelta > 4) blockers.push(`${key}: final image exceeds four device pixels`);
      for (const item of value.artifacts) {
        if (item.pngWidth !== item.deviceRect.width || item.pngHeight !== item.deviceRect.height)
          blockers.push(`${key}: artifact dimensions disagree with device crop`);
        const frame = item.sourceFrameDeviceRect;
        const crop = item.deviceRect;
        if (
          crop.x < frame.x ||
          crop.y < frame.y ||
          crop.x + crop.width > frame.x + frame.width ||
          crop.y + crop.height > frame.y + frame.height
        )
          blockers.push(`${key}: crop escapes source frame`);
        if (!item.path.includes(expectedFingerprint.replaceAll("/", "-")))
          blockers.push(`${key}: artifact fingerprint mismatch`);
      }
    }
    const mutationIds = new Set(report.mutations.filter((m) => m.killed).map((m) => m.id));
    for (const id of PROJECTIVE_GATE_MUTATIONS)
      if (!mutationIds.has(id)) blockers.push(`${report.environment.platform}: mutation not killed: ${id}`);
  }
