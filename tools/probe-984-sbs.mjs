...
async function sbsTest(name) {
  ...
  const W = Math.min(800, m.width),
    H = Math.min(500, m.height);
  await sharp(`${base}-expected.png`)
    .extract({ left: 0, top: 0, width: W, height: H })
    .toFile(`/tmp/p984-${name}-e.png`);
  ...
  await sharp({ create: { width: W * 2 + 30, height: H + 20, channels: 3, background: { r: 255, g: 255, b: 255 } } })
    .composite([
      { input: `/tmp/p984-${name}-e.png`, left: 5, top: 10 },
      { input: `/tmp/p984-${name}-a.png`, left: W + 15, top: 10 },
    ])
