...
export const LINUX_TERMINAL_MASK_CASES: readonly LinuxTerminalMaskCase[] = [
  {
    id: "ui-meta-13",
    face: "wqy-ui",
    fontPath: FONT_PATHS.wqy,
    postscriptName: "WenQuanYiZenHei",
    nativeFamily: "WenQuanYi Zen Hei",
    sourceCodepoint: 0x43,
    fontSizePx: 13,
    geometricPrecisionPolicy: "enabled",
  },
  {
    id: "mono-label-17",
    face: "wqy-mono",
    ...
    nativeFamily: "WenQuanYi Zen Hei Mono",
    sourceCodepoint: 0x55,
    fontSizePx: 17,
    geometricPrecisionPolicy: "enabled",
  },
  {
  ...
  },
  {
    ...
    postscriptName: "WenQuanYiZenHei",
    nativeFamily: "WenQuanYi Zen Hei",
    sourceCodepoint: 0x6c94,
    fontSizePx: 32,
    geometricPrecisionPolicy: "enabled",
  },
  {
    id: "freesans-malayalam-32",
    face: "free-sans",
    fontPath: FONT_PATHS.freeSans,
    postscriptName: "FreeSans",
    nativeFamily:
      "'Malayalam Sangam MN','Arial Unicode MS','Apple Symbols','Apple Color Emoji','Noto Sans','Noto Serif',sans-serif",
    sourceCodepoint: 0x0d10,
    fontSizePx: 32,
    geometricPrecisionPolicy: "disabled",
  },
  {
    id: "unifont-malayalam-32",
    face: "unifont",
    fontPath: FONT_PATHS.unifont,
    postscriptName: "Unifont",
    nativeFamily:
      "'Malayalam Sangam MN','Arial Unicode MS','Apple Symbols','Apple Color Emoji','Noto Sans','Noto Serif',sans-serif",
    sourceCodepoint: 0x0d04,
    fontSizePx: 32,
    geometricPrecisionPolicy: "disabled",
  },
] as const;
...
export const LINUX_TERMINAL_MASK_VARIANTS: ReadonlyArray<{
  ...
  bytes:
    "source-cmap" | "pua-only" | "pua-with-source" | "pua-no-hinting" | "target-hinted-full" | "target-hinted-y" | null;
  ...
  {
    id: "pua-production-policy",
    embedded: true,
    addressing: "pua",
    bytes: "pua-only",
    textRendering: "productionPolicy",
  },
  {
    id: "pua-target-hinted-full",
    embedded: true,
    addressing: "pua",
    bytes: "target-hinted-full",
    textRendering: "productionPolicy",
  },
  {
    id: "pua-target-hinted-y",
    embedded: true,
    addressing: "pua",
    bytes: "target-hinted-y",
    textRendering: "productionPolicy",
  },
  {
    id: "pua-geometric-precision",
    embedded: true,
    addressing: "pua",
    bytes: "pua-only",
...
function buildTargetHintedCaseFaces(
  faces: Record<LinuxTerminalMaskFaceId, FaceBuild>,
): Map<string, TargetHintedCaseFace> {
  ...
  const envelope = {
    ...
    queries: cases.flatMap((testCase) => [
      { type: "glyphs", fontRef: testCase.id, glyphs: [{ cp: testCase.sourceCodepoint }] },
      {
        type: "hintedGlyphs",
        fontRef: testCase.id,
        fontSizePx: testCase.fontSizePx,
        glyphs: [{ cp: testCase.sourceCodepoint }],
      },
    ]),
  };
  const child = spawnSync(GLYPH_HELPER, [], {
  ...
  for (let index = 0; index < cases.length; index++) {
    ...
    if (designGlyph == null || hintedGlyph == null || designGlyph.d.length === 0 || hintedGlyph.d.length === 0) {
      throw new Error(
        `${testCase.id}: target-strike outline unavailable (${design?.error ?? hinted?.error ?? "empty path"})`,
      );
    }
...
function variantDocument(
  ...
  const targetKind =
    variant.bytes === "target-hinted-full" || variant.bytes === "target-hinted-y" ? variant.bytes : null;
  if (targetKind != null) {
  ...
  const texts = matrix
    .map((cell, index) => {
      ...
      const codepoint =
        variant.addressing === "source" ? cell.sourceCodepoint : face.puaForSourceCodepoint.get(cell.sourceCodepoint)!;
      const family =
        targetKind != null
          ...
          : variant.embedded
            ...
            : cell.nativeFamily;
      const textRendering =
        variant.textRendering === "productionPolicy"
          ? cell.geometricPrecisionPolicy === "enabled"
            ? "geometricPrecision"
            : "auto"
...
export function validateLinuxTerminalMaskResults(results: readonly LinuxTerminalMaskScoreRow[]): string[] {
  ...
  if (production.global.diffPct >= control.global.diffPct * 0.85) {
    errors.push(
      `production-policy aggregate improvement is below 15% (${control.global.diffPct} -> ${production.global.diffPct})`,
    );
  }
  ...
  const policyByCase = new Map(
    LINUX_TERMINAL_MASK_CASES.map((testCase) => [testCase.id, testCase.geometricPrecisionPolicy]),
  );
  ...
  for (const controlCase of control.cases ?? []) {
    ...
    if (controlCase.id === "mono-label-17") {
      ...
      if (targetYCase.centroidDelta != null && Math.abs(targetYCase.centroidDelta.y) >= 0.05) {
        errors.push(
          `mono-label-17: target strike vertical centroid is not near-native (${targetYCase.centroidDelta.y})`,
        );
      }
    }
    if (policyByCase.get(controlCase.id) === "enabled") {
      ...
      const controlPhases = new Map(
        (controlCase.phases ?? []).map((phase) => [`${phase.phaseX},${phase.phaseY}`, phase]),
      );
      for (const productionPhase of productionCase.phases ?? []) {
        ...
        if (controlPhase != null && productionPhase.totalChannelDelta > controlPhase.totalChannelDelta) {
          errors.push(
            `${controlCase.id}: production phase ${productionPhase.phaseX},${productionPhase.phaseY} regressed terminal-mask delta`,
          );
        }
      }
    } else {
      ...
      for (const geometricPhase of geometricCase?.phases ?? []) {
        ...
        if (controlPhase != null && geometricPhase.totalChannelDelta > controlPhase.totalChannelDelta)
          negativeWitnesses++;
      }
    }
  }
...
export async function runLinuxTerminalMaskOracle(options: LinuxTerminalMaskOracleOptions) {
  ...
  const report = {
    ...
    targetHintedFaces: Object.fromEntries(
      [...targetHintedFaces].map(([id, face]) => [
        id,
        {
          pua: face.pua,
