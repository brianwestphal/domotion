...
const stixCands = [
  ...
  "/System/Library/Fonts/STIX 2.otf",
...
export function runMathItalicProbe() {
  ...
  for (const directory of ["/System/Library/Fonts", "/Library/Fonts", join(homedir(), "Library", "Fonts")]) {
  ...
  for (const [name, path] of fonts) {
    ...
    try {
      ...
      f = opened.fonts ? opened.fonts[0] : opened;
    } catch (e) {
      console.log(`${name.padEnd(15)} FAILED (${e.message})`);
      continue;
    }
