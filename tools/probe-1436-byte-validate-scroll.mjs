...
const seg = (scrollY, segStartMs, segEndMs, easing) => ({
  ...
  tree: [el({ tag: "div", x: 0, y: scrollY, height: 600 })],
  diffFromPrev: null,
  ...(easing ? { easing } : {}),
});
const segX = (scrollX, s, e, easing) => ({
  ...
  segmentStartMs: s,
  segmentEndMs: e,
  tree: [el({ tag: "div", x: scrollX, y: 0, width: 600 })],
  diffFromPrev: null,
  ...(easing ? { easing } : {}),
});
...
const configs = {
  ...
  "y-easing-named": [
    seg(0, 0, 1000, { kind: "named", name: "ease-in" }),
    seg(400, 1000, 2000, { kind: "named", name: "ease-out" }),
    seg(900, 2000, 3000, { kind: "named", name: "ease-in-out" }),
  ],
  "y-easing-steps": [
    seg(0, 0, 1000, { kind: "steps", count: 4, position: "end" }),
    seg(500, 1000, 2500, { kind: "steps", count: 3 }),
  ],
  "y-easing-bezier": [
    seg(0, 0, 1000, { kind: "cubic-bezier", values: [0.4, 0, 0.2, 1] }),
    seg(600, 1000, 2000, { kind: "cubic-bezier", values: [0.25, 0.1, 0.25, 1] }),
  ],
...
for (const [name, segs] of Object.entries(configs)) {
  try {
    manifest[name] = createHash("sha256").update(composeScrollSvg(segs, OPT)).digest("hex").slice(0, 16);
  } catch (e) {
...
for (const cs of [1, 2, 3]) {
  ...
  try {
    manifest[`y-chunk${cs}`] = createHash("sha256")
      .update(composeScrollSvg(segs, { ...OPT, chunkSize: cs }))
      .digest("hex")
      .slice(0, 16);
  } catch (e) {
    manifest[`y-chunk${cs}`] = `ERROR:${e.message}`;
  }
}
const xsegs = [segX(0, 0, 1000, { kind: "named", name: "ease-in" }), segX(500, 1000, 2000), segX(1100, 2000, 3000)];
try {
  manifest["x-easing"] = createHash("sha256")
    .update(composeScrollSvg(xsegs, { ...OPT, axis: "x" }))
    .digest("hex")
    .slice(0, 16);
} catch (e) {
  manifest["x-easing"] = `ERROR:${e.message}`;
}
