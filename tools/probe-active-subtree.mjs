...
const info = await page.evaluate(() => {
  ...
  function dump(el, depth, maxDepth) {
    ...
    const bg =
      cs.backgroundColor !== "rgba(0, 0, 0, 0)" ? cs.backgroundColor : cs.backgroundImage !== "none" ? "img" : "";
    const node = {
      t: el.tagName,
      c: (el.getAttribute("class") || "").slice(0, 32),
      y: Math.round(r.top + window.scrollY),
      h: Math.round(r.height),
      f: flags.join(","),
      bg,
      after: getComputedStyle(el, "::after").content !== "none" ? "::after" : "",
    };
