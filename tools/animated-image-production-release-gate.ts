...
export interface AnimatedImageProductionReleaseReport {
  ...
  artifacts: Array<{
    ...
    runEnvironmentSha256: string;
    productionTestsSha256: string;
    testsPassed: number;
  }>;
  failures: string[];
...
const readJson = async <T>(path: string): Promise<{ value: T; bytes: Uint8Array }> => {
  ...
  return { value: JSON.parse(bytes.toString()) as T, bytes };
};

export async function adjudicateAnimatedImageProductionRelease(
  root: string,
): Promise<AnimatedImageProductionReleaseReport> {
  const failures: string[] = [];
  const artifacts: AnimatedImageProductionReleaseReport["artifacts"] = [];
  ...
  for (const platform of ["macOS", "Linux", "Windows"] as const) {
    ...
    if (directory == null) {
      failures.push(`${platform}: native artifact missing`);
      continue;
    }
    try {
      const base = join(root, directory);
      const decoder = await readJson<AnimatedImageFrameSelectionReport>(join(base, "decoder.json"));
      const tests = await readJson<{ numPassedTests?: number; numFailedTests?: number; success?: boolean }>(
        join(base, "production-tests.json"),
      );
      const environment = await readJson<Record<string, unknown>>(join(base, "run-env.json"));
      if (
        decoder.value.verdict !== "decoder-frame-exact" ||
        decoder.value.errors.length !== 0 ||
        !decoder.value.browser.headless
      ) {
      ...
      if (
        tests.value.success !== true ||
        (tests.value.numFailedTests ?? 1) !== 0 ||
        (tests.value.numPassedTests ?? 0) < 18
      ) {
      ...
      artifacts.push({
        platform,
        decoderSha256: sha256(decoder.bytes),
        runEnvironmentSha256: sha256(environment.bytes),
        productionTestsSha256: sha256(tests.bytes),
        testsPassed: tests.value.numPassedTests ?? 0,
      });
    } catch (error) {
  ...
  return {
    ...
    artifacts,
    failures,
    verdict: failures.length === 0 && artifacts.length === 3 ? "macos-linux-windows-production-exact" : "blocked",
  };
}
