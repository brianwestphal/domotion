...
const findCanvas = (tree: CapturedElement[]): CapturedElement | undefined =>
  tree.flatMap((node) => [node, ...(node.children ?? [])]).find((node) => node.tag === "canvas");
...
export async function runLiveReplacedMediaOwnershipAudit() {
  ...
  try {
    ...
    const paint = (color: string) =>
      page.locator("#c").evaluate((node, value) => {
    ...
    const controls = {
      ...
      expectedWarningsOnly: warnings.every(
        (warning) =>
    ...
    return {
      ...
      unsupported: [
        ...
        "decoder-owned animation continuation",
        "media-time effects after capture",
      ],
      warnings,
      controls,
