...
if (!old || !next)
  throw new Error(
    "usage: compare-roll-evidence --old old.json --new new.json [--review review.json] [--out report.json]",
  );
...
const result = compareRollArtifacts(
  hydrate(old),
  hydrate(next),
  arg("--review") ? load<RollReview>(arg("--review")!) : undefined,
);
const text = JSON.stringify(result, null, 2) + "\n";
if (arg("--out")) writeFileSync(resolve(arg("--out")!), text);
