...
export function selectCorpusRuns(
  runs: CorpusRun[],
  mode: CorpusMode,
  shardIndex: number,
  shardCount: number,
  representativeLimit = 4096,
): CorpusRun[] {
  if (
    ...
    throw new Error("invalid shard i/n");
...
export function reduceMismatches(rows: Array<Record<string, unknown>>) {
  ...
  for (const row of rows) {
    ...
    if (hit == null) groups.set(signature, { signature, count: 1, example: row });
    else hit.count++;
  }
...
async function main(): Promise<void> {
  ...
  const selected = selectCorpusRuns(
    corpus.runs,
    mode,
    shard[0],
    shard[1],
  ...
  if (existsSync(manifestPath)) {
    const old = JSON.parse(readFileSync(manifestPath, "utf8")) as {
      shardDigest?: string;
      completed?: boolean;
      exitCode?: number;
    };
    if (old.shardDigest === shardDigest && old.completed === true) {
      process.stdout.write(`resume hit ${manifestPath}\n`);
      process.exitCode = old.exitCode ?? 2;
      return;
    }
  }
  ...
  const child = spawnSync(
    command,
    [resolve("tools/shaping-conformance.ts"), "--runs", shardCorpus, "--out", shardDir],
    ...
    `${JSON.stringify(
      {
        ...
        totalRuns: corpus.runs.length,
        startedAt,
        completedAt: new Date().toISOString(),
        completed: child.status != null,
        exitCode: child.status,
        fingerprint: parityEnvironment({
          chromium: String((report && (report as any).meta?.chromium) || "unavailable"),
          launchFlags: [],
          deviceScaleFactor: 1,
          zoom: 1,
          writingMode: "mixed-corpus",
          direction: "mixed-corpus",
          corpusIdentity: corpusDigest,
          sampleIdentity: shardDigest,
        }),
        report: "report.json",
        mismatchReductions: "mismatch-reductions.json",
      },
      null,
      2,
    )}\n`,
