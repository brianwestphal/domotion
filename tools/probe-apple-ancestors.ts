...
async function main() {
  ...
  for (const cls of interesting) {
    ...
    for (const n of matches.slice(0, 3)) {
      console.log(
        `  cls="${(n.styles?.className || "").slice(0, 55)}" tag=${n.tagName} rect=(${Math.round(n.x)},${Math.round(n.y)},${Math.round(n.width)},${Math.round(n.height)}) opacity=${n.styles?.opacity} display=${n.styles?.display}`,
      );
    }
  }
