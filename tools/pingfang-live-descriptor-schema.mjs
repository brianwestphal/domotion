...
export function validatePingFangDescriptorArtifact(artifact) {
  ...
  if (JSON.stringify(artifact.codepoints) !== JSON.stringify(PINGFANG_DESCRIPTOR_CODEPOINTS))
    errors.push("codepoint corpus drift");
  if (artifact.coldProcesses?.length < 3) errors.push("need at least three cold processes");
  if (!artifact.warmProcess?.samples?.some((sample) => sample.iteration >= 2))
    errors.push("need three warm iterations");
  for (const [label, process] of [
    ...(artifact.coldProcesses ?? []).map((value, index) => [`cold[${index}]`, value]),
    ["warm", artifact.warmProcess],
  ]) {
    for (const sample of process?.samples ?? []) {
      const arms = sample.arms?.map((arm) => arm.arm) ?? [];
      for (const arm of PINGFANG_DESCRIPTOR_ARMS)
        if (!arms.includes(arm)) errors.push(`${label} U+${sample.queryCodepoint?.toString(16)} missing ${arm}`);
      for (const record of sample.arms ?? [])
        for (const key of ["descriptor", "variationAxes", "variation", "unitsPerEm", "matrix", "glyphs"])
