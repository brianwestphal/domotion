...
await page.setContent(
  `<!doctype html><body style="margin:0"><img src="file:///tmp/claude/fx.svg" width="${W}" height="${H}"></body>`,
  { waitUntil: "load" },
);
await page.evaluate(() => document.fonts.ready);
...
function cellSig(g, rc) {
  ...
  for (let y = 2; y < rc.h - 2; y++)
    for (let x = 2; x < rc.w - 2; x++) {
      const px = rc.x + x,
        py = rc.y + y;
...
const eg = await gray(expected),
  ag = await gray(actual);
...
function tofuSet(sigs) {
  ...
  for (let i = 0; i < sigs.length; i++) {
    const k = `${Math.round(sigs[i].ink / 20)}:${sigs[i].hash}`;
...
for (let i = 0; i < rects.length; i++) {
  ...
  if (!e && !a) bothReal++;
    ...
    over++; // Chrome tofu, ours real => OVER-render
...
console.log(
  `  bothReal=${bothReal} bothTofu=${bothTofu} OVER(ours-real/chrome-tofu)=${over} UNDER(ours-tofu/chrome-real)=${under}`,
);
