...
for (const [path, psn, cp, label] of fontsToTest) {
  const r = spawnSync("node", ["/tmp/_probe.mjs", JSON.stringify([path, psn, cp.toString(16)])], {
    encoding: "utf8",
    timeout: 10000,
  });
  const ok = r.status === 0 && r.stdout.includes("OK");
  console.log(
    `${label.padEnd(12)} U+${cp.toString(16).toUpperCase().padStart(4, "0")}  ${ok ? "OK" : "CRASH"}  (${path.split("/").pop()})`,
  );
}
