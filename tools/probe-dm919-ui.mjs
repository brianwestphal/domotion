...
for (const psName of ["AppleColorEmoji", ".AppleColorEmojiUI"]) {
  ...
  console.log(
    `${psName}: advance=${sm.advanceWidth} (${(sm.advanceWidth * scale).toFixed(1)}px), unitsPerEm=${font.unitsPerEm}, bbox=${JSON.stringify(sm.bbox)}, ascent=${font.ascent}, descent=${font.descent}`,
  );
}
