...
let html =
  '<!doctype html><meta charset="utf-8"><style>body{margin:24px;font-family:system-ui,sans-serif;background:white;color:black;line-height:1.8;}.sample{text-decoration:underline wavy red;text-decoration-thickness:2px;}</style>';
...
const samples = await page.evaluate(() => {
  return Array.from(document.querySelectorAll(".sample")).map((el) => {
    ...
    return {
      ...
      rect: { x: r.x, y: r.y, w: r.width, h: r.height + parseFloat(cs.fontSize) * 0.7 },
...
for (const s of samples) {
  ...
  if (redYs.length < 50) {
    console.log(JSON.stringify({ fontSize: s.fontSize, error: "too-few-red-pixels" }));
    continue;
  }
