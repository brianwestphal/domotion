...
import {
  ...
  parseGradientStops,
} from "../src/render/gradient-defs.js";
import {
  clipPathShapeForElement,
  clipReferenceBox,
  parseSameDocumentClipPathUrl,
  svgEffectReferenceBox,
  translateClipPath,
  type HtmlClipGeometryBox,
  type SvgEffectGeometryBox,
} from "../src/render/clip-path.js";
import {
  buildMaskDef,
  positionFragmentClipPathDef,
  positionFragmentMaskDef,
  positionObjectBoundingBoxClipPathDef,
} from "../src/render/mask.js";
...
function lineFromMarkup(markup: string): LineRecord {
  return {
    p0: { x: numberAttr(markup, "x1"), y: numberAttr(markup, "y1") },
    p1: { x: numberAttr(markup, "x2"), y: numberAttr(markup, "y2") },
  };
}
...
export function blinkCornerLine(
...
function gradientRows(): OracleRow[] {
  ...
  return boxes.flatMap(({ width, height }, boxIndex) =>
    dirs.map((direction) => {
      ...
      return {
        id: `linear.magic-corner.${boxIndex}.${direction}`,
        stage: "gradient" as const,
...
function legacyGradientRows(): OracleRow[] {
  const source =
    "css_parsing_utils.cc:2767-2935; css_gradient_value.cc:839-919,1371-1456,1825-1840,1924-2016,411-442; SkGradientBaseShader.cpp@62efacd3:213-265";
  const rect = { x: 13, y: 17, w: 300, h: 120 };
  const rows: OracleRow[] = [];

  const percent = parseExactGradient("-webkit-gradient(linear, 10% 25%, 80% 75%, from(red), to(blue))")!;
  const percentMarkup = buildExactLinearGradientDef(
    percent as Extract<typeof percent, { kind: "linear" }>,
    "legacy-percent",
    rect,
  );
  ...
  rows.push({
    id: "legacy-gradient.linear.physical-percent-axes",
    stage: "gradient",
    source,
    expected: percentExpected,
    actual: percentActual,
    maxAbsDelta: percentDelta,
    pass: percentDelta <= TOLERANCE,
  });
  ...
  const zoomActual = lineFromMarkup(
    buildExactLinearGradientDef(zoomed as Extract<typeof zoomed, { kind: "linear" }>, "legacy-zoom", rect),
  );
  const zoomExpected = { p0: { x: 37, y: 33 }, p1: { x: 253, y: 107 } };
  const zoomDelta = maxDelta(zoomExpected, zoomActual);
  rows.push({
    id: "legacy-gradient.linear.unitless-effective-zoom",
    stage: "gradient",
    source,
    expected: zoomExpected,
    actual: zoomActual,
    maxAbsDelta: zoomDelta,
    pass: zoomDelta <= TOLERANCE,
  });

  const radialCss = physicalComputedGradientImage(
    "-webkit-gradient(radial, 10 5, 4, 75% 80%, 30, from(red), to(blue))",
    2,
  );
  const radial = parseExactGradient(radialCss)!;
  const radialMarkup = buildExactRadialGradientDef(
    radial as Extract<typeof radial, { kind: "radial" }>,
    "legacy-radial",
    rect,
  );
  const radialActual = {
    x1: numberAttr(radialMarkup, "fx"),
    y1: numberAttr(radialMarkup, "fy"),
    r1: numberAttr(radialMarkup, "fr"),
    x2: numberAttr(radialMarkup, "cx"),
    y2: numberAttr(radialMarkup, "cy"),
    r2: numberAttr(radialMarkup, "r"),
  };
  const radialExpected = { x1: 33, y1: 27, r1: 8, x2: 238, y2: 113, r2: 60 };
  const radialDelta = maxDelta(radialExpected, radialActual);
  rows.push({
    id: "legacy-gradient.radial.two-circle-geometry",
    stage: "gradient",
    source,
    expected: radialExpected,
    actual: radialActual,
    maxAbsDelta: radialDelta,
    pass: radialDelta <= TOLERANCE && !radialMarkup.includes("spreadMethod"),
  });

  const shrinking = parseExactGradient(
    "-webkit-gradient(radial, 75% 80%, 30, 10 5, 4, from(red), color-stop(.25, lime), to(blue))",
  )!;
  const shrinkingMarkup = buildExactRadialGradientDef(
    shrinking as Extract<typeof shrinking, { kind: "radial" }>,
    "legacy-radial-shrinking",
    rect,
  );
  const shrinkingActual = {
    x1: numberAttr(shrinkingMarkup, "fx"),
    y1: numberAttr(shrinkingMarkup, "fy"),
    r1: numberAttr(shrinkingMarkup, "fr"),
    x2: numberAttr(shrinkingMarkup, "cx"),
    y2: numberAttr(shrinkingMarkup, "cy"),
    r2: numberAttr(shrinkingMarkup, "r"),
  };
  const shrinkingExpected = { x1: 23, y1: 22, r1: 4, x2: 238, y2: 113, r2: 30 };
  const shrinkingStops = [...shrinkingMarkup.matchAll(/<stop offset="([^"]+)" stop-color="([^"]+)"/g)].map((match) => [
    Number(match[1]),
    match[2],
  ]);
  const shrinkingDelta = maxDelta(shrinkingExpected, shrinkingActual);
  rows.push({
    id: "legacy-gradient.radial.shrinking-two-circle-reversal",
    stage: "gradient",
    source,
    expected: {
      geometry: shrinkingExpected,
      stops: [
        [0, "blue"],
        [0.75, "lime"],
        [1, "red"],
    ...
    actual: { geometry: shrinkingActual, stops: shrinkingStops },
    maxAbsDelta: shrinkingDelta,
    pass:
      shrinkingDelta <= TOLERANCE &&
      JSON.stringify(shrinkingStops) ===
        JSON.stringify([
          [0, "blue"],
          [0.75, "lime"],
          [1, "red"],
  ...
  const stopsGradient = parseExactGradient(
    "-webkit-gradient(linear, left top, right bottom, color-stop(1.2, yellow), to(blue), color-stop(-.2, lime), from(red))",
  )!;
  const stopsMarkup = buildExactLinearGradientDef(
    stopsGradient as Extract<typeof stopsGradient, { kind: "linear" }>,
    "legacy-stops",
    rect,
  );
  const stopActual = [...stopsMarkup.matchAll(/<stop offset="([^"]+)" stop-color="([^"]+)"/g)].map((match) => [
    Number(match[1]),
    match[2],
  ]);
  const stopExpected = [
  ...
  rows.push({
    id: "legacy-gradient.stops.stable-sort-before-terminal-clamp",
    stage: "gradient",
    source,
    expected: stopExpected,
    actual: stopActual,
    maxAbsDelta: stopsPass ? 0 : Infinity,
    pass: stopsPass,
  });
  ...
  rows.push({
    id: "legacy-gradient.mutation.modern-line-substitution",
    stage: "gradient",
    source,
    expected: { killed: true },
    actual: { killed: modernMovement > 1, movement: modernMovement },
    maxAbsDelta: 0,
    pass: modernMovement > 1,
  });
  ...
  const swappedLine = lineFromMarkup(
    buildExactLinearGradientDef(percent as Extract<typeof percent, { kind: "linear" }>, "swapped-mutation", {
      ...rect,
      w: rect.h,
      h: rect.w,
    }),
  ...
  rows.push({
    id: "legacy-gradient.mutation.width-height-swap",
    stage: "gradient",
    source,
    expected: { killed: true },
    actual: { killed: swappedMovement > 1, movement: swappedMovement },
    maxAbsDelta: 0,
    pass: swappedMovement > 1,
  });
  ...
  const unzoomedLine = lineFromMarkup(
    buildExactLinearGradientDef(unzoomed as Extract<typeof unzoomed, { kind: "linear" }>, "zoom-mutation", rect),
  );
  const zoomMovement = maxDelta(zoomActual, unzoomedLine);
  rows.push({
    id: "legacy-gradient.mutation.omit-effective-zoom",
    stage: "gradient",
    source,
    expected: { killed: true },
    actual: { killed: zoomMovement > 1, movement: zoomMovement },
    maxAbsDelta: 0,
    pass: zoomMovement > 1,
  });

  const modernRepeat = buildLinearGradientDef(
    "repeat-control",
    "90deg, red 0 10px, blue 10px 20px",
    true,
    300,
    120,
    13,
  ...
  rows.push({
    id: "legacy-gradient.activation.nonrepeating-only",
    stage: "gradient",
    source,
    expected: { legacyNonRepeat: true, modernRepeat: true },
    actual: { legacyNonRepeat, modernRepeat: repeatUnchanged },
    maxAbsDelta: legacyNonRepeat && repeatUnchanged ? 0 : Infinity,
    pass: legacyNonRepeat && repeatUnchanged,
  });
  return rows;
}
...
function interpolationRows(): OracleRow[] {
  const cases = [
    {
      id: "srgb",
      layer: "linear-gradient(90deg in srgb, red, blue)",
      raster: false,
      attr: 'color-interpolation="sRGB"',
    },
    {
      id: "srgb-linear",
      layer: "linear-gradient(90deg in srgb-linear, red, blue)",
      raster: false,
      attr: 'color-interpolation="linearRGB"',
    },
    ...["lab", "oklab", "lch", "oklch", "hsl", "hwb"].map((space) => ({
      id: space,
      layer: `linear-gradient(90deg in ${space}, red, blue)`,
      raster: true,
      attr: "",
    })),
    { id: "oklch-longer-hue", layer: "linear-gradient(90deg in oklch longer hue, red, blue)", raster: true, attr: "" },
    {
      ...
      layer: "linear-gradient(90deg in srgb, rgba(255,0,0,0), blue)",
      raster: true,
      attr: "",
    },
  ];
  return cases.map((test) => {
    ...
    const markup = raster
      ? ""
      : buildLinearGradientDef("gi", test.layer.slice(test.layer.indexOf("(") + 1, -1), false, 100, 20);
    ...
    return {
      id: `gradient.interpolation.${test.id}`,
      stage: "gradient" as const,
      source: "css_gradient_value.cc:1163-1188,1454-1460; Gradient::SetColorInterpolationSpace",
      expected,
      actual,
      maxAbsDelta: pass ? 0 : Infinity,
      pass,
    };
  });
}
...
function radialAndStopRows(): OracleRow[] {
  const radialCases = [
    {
      id: "circle.closest-side",
      args: "circle closest-side at 25% 75%, red, blue",
      expected: { cx: 60, cy: 110, r: 30, scale: 1 },
    },
    {
      id: "circle.farthest-corner",
      args: "circle farthest-corner at 25% 75%, red, blue",
      expected: { cx: 60, cy: 110, r: Math.hypot(150, 90), scale: 1 },
    },
    {
      id: "ellipse.closest-side",
      args: "ellipse closest-side at 25% 75%, red, blue",
      expected: { cx: 60, cy: 110, r: 50, scale: 0.6 },
    },
    {
      id: "ellipse.farthest-corner",
      args: "ellipse farthest-corner at 25% 75%, red, blue",
      expected: { cx: 60, cy: 110, r: 150 * Math.SQRT2, scale: 0.6 },
    },
  ];
  const rows: OracleRow[] = radialCases.map(({ id, args, expected }) => {
    ...
    const actual = {
      cx: numberAttr(markup, "cx"),
      cy: numberAttr(markup, "cy"),
      r: numberAttr(markup, "r"),
      scale: transform == null ? 1 : Number(transform[1]),
    };
    const delta = maxDelta(expected, actual);
    return {
      id: `radial.${id}`,
      stage: "gradient",
      source: "css_gradient_value.cc:1846-2008",
      expected,
      actual,
      maxAbsDelta: delta,
      pass: delta <= TOLERANCE,
  ...
  const repeatActual = {
    fr: numberAttr(repeating, "fr"),
    r: numberAttr(repeating, "r"),
    spread: /spreadMethod="([^"]+)"/.exec(repeating)?.[1] ?? "",
    offsets: [...repeating.matchAll(/<stop offset="([^"]+)"/g)].map((m) => Number(m[1])),
  };
  rows.push({
    id: "radial.repeating-positive-domain",
    stage: "gradient",
    source: "css_gradient_value.cc:506-653,809-824",
    expected: repeatExpected,
    actual: repeatActual,
    maxAbsDelta: maxDelta(
      { fr: 10, r: 30, offsets: [0, 1] },
      { fr: repeatActual.fr, r: repeatActual.r, offsets: repeatActual.offsets },
    ),
    pass:
      repeatActual.spread === "repeat" &&
      maxDelta(repeatExpected.offsets, repeatActual.offsets) <= TOLERANCE &&
      repeatActual.fr === 10 &&
      repeatActual.r === 30,
  });
  ...
  const negativeRepeatActual = {
    innerRadius: negativeRepeat.innerRadius,
    outerRadius: negativeRepeat.outerRadius,
    offsets: negativeRepeat.stops.map((stop) => stop.pos),
    repeat: negativeRepeat.repeat,
  };
  const negativeRepeatDelta = maxDelta(negativeRepeatExpected, negativeRepeatActual);
  rows.push({
    id: "radial.repeating-negative-domain-shift",
    stage: "gradient",
    source: "css_gradient_value.cc:593-633,809-824",
    expected: negativeRepeatExpected,
    actual: negativeRepeatActual,
    maxAbsDelta: negativeRepeatDelta,
    pass: negativeRepeatDelta <= TOLERANCE,
  });
  ...
  const nonRepeatActual = {
    innerRadius: nonRepeat.innerRadius,
    outerRadius: nonRepeat.outerRadius,
    offsets: nonRepeat.stops.map((stop) => stop.pos),
    boundaryColor: [nonRepeat.stops[0].color.r, nonRepeat.stops[0].color.g, nonRepeat.stops[0].color.b],
  };
  const nonRepeatDelta = maxDelta(nonRepeatExpected, nonRepeatActual);
  rows.push({
    id: "radial.non-repeating-negative-domain-clamp",
    stage: "gradient",
    source: "css_gradient_value.cc:549-588,809-824",
    expected: nonRepeatExpected,
    actual: nonRepeatActual,
    maxAbsDelta: nonRepeatDelta,
    pass: nonRepeatDelta <= TOLERANCE,
  });

  const linearLight = normalizeRadialGradientDomain(
    [opaqueStop(-0.2, 255, 0), opaqueStop(0.8, 0, 255)],
    100,
    false,
    "srgb-linear",
  );
  ...
  rows.push({
    id: "radial.non-repeating-negative-domain-linear-light",
    stage: "gradient",
    source: "css_gradient_value.cc:549-588; Color::InterpolateColors",
    expected: linearExpected,
    actual: linearActual,
    maxAbsDelta: linearDelta,
    pass: linearDelta <= TOLERANCE,
  });
  ...
  const outOfRangeActual = {
    innerRadius: outOfRange.innerRadius,
    outerRadius: outOfRange.outerRadius,
    offsets: outOfRange.stops.map((stop) => stop.pos),
  };
  const outOfRangeDelta = maxDelta(outOfRangeExpected, outOfRangeActual);
  rows.push({
    id: "radial.non-repeating-out-of-range-domain",
    stage: "gradient",
    source: "css_gradient_value.cc:490-547,630-633,809-824",
    expected: outOfRangeExpected,
    actual: outOfRangeActual,
    maxAbsDelta: outOfRangeDelta,
    pass: outOfRangeDelta <= TOLERANCE,
  });
  ...
  rows.push({
    id: "radial.negative-domain-mutation-control",
    stage: "gradient",
    source: "css_gradient_value.cc:609-627",
    expected: { moved: true },
    actual: {
      moved:
        negativeRepeat.innerRadius !== legacyClamp.innerRadius ||
        negativeRepeat.outerRadius !== legacyClamp.outerRadius,
    },
    maxAbsDelta: 0,
    pass:
      negativeRepeat.innerRadius !== legacyClamp.innerRadius || negativeRepeat.outerRadius !== legacyClamp.outerRadius,
  });
  ...
  for (const test of stopCases) {
    ...
    rows.push({
      id: `stops.${test.id}`,
      stage: "gradient",
      source: "css_gradient_value.cc:656-790",
  ...
  rows.push({
    id: "stops.chromium-nine-stop-hint",
    stage: "gradient",
    source: "css_gradient_value.cc:266-399",
    expected: expectedHintPositions,
    actual: hintActual,
    maxAbsDelta: hintDelta,
    pass: hintDelta <= TOLERANCE,
  });
  return rows;
}
...
function conicRows(): OracleRow[] {
  const parsed = parseConicGradient(
    "conic-gradient(from 90deg at 25% 75%, black 0 25%, white 25% 50%, black 50% 75%, white 75% 100%)",
  )!;
  ...
  const rows: OracleRow[] = [
    {
      id: "conic.center-angle-stop-domain",
      stage: "gradient",
      source: "css_gradient_value.cc:2241-2270,656-824",
      expected: geometryExpected,
      actual: geometryActual,
      maxAbsDelta: geometryDelta,
      pass: geometryDelta <= TOLERANCE,
    },
  ];

  const repeating = parseConicGradient("repeating-conic-gradient(red -25%, blue 25%)")!;
  const repeatActual = {
    repeating: repeating.repeating === true,
    offsets: resolveConicStops(repeating.stops).map((stop) => stop.offset),
  };
  const repeatExpected = { repeating: true, offsets: [-0.25, 0.25] };
  const repeatDelta = maxDelta(repeatExpected, repeatActual);
  rows.push({
    id: "conic.repeating-negative-domain",
    stage: "gradient",
    source: "css_gradient_value.cc:490-547,809-821,2241-2270",
    expected: repeatExpected,
    actual: repeatActual,
    maxAbsDelta: repeatDelta,
    pass: repeatDelta <= TOLERANCE,
  });
  ...
  rows.push({
    id: "conic.tile-effective-zoom",
    stage: "gradient",
    source: "CSSImageGeneratorValue sizing in effective zoom space",
    expected: tileExpected,
    actual: tileActual,
    maxAbsDelta: tileDelta,
    pass: tileDelta <= TOLERANCE,
  });

  const routed = needsChromiumGradientRaster("conic-gradient(in oklch longer hue, red, blue)");
  rows.push({
    id: "conic.chromium-raster-boundary",
    stage: "gradient",
    source: "Gradient::CreateConic plus SVG conic paint-server boundary",
    expected: { chromiumRaster: true },
    actual: { chromiumRaster: routed },
    maxAbsDelta: 0,
    pass: routed,
  });
  return rows;
...
function clipRows(): OracleRow[] {
  ...
  const cases = [
    ...
    {
      id: "circle",
      value: "circle(25% at 20% 75%)",
      expected: { cx: 50, cy: 110, r: svgNumber(Math.sqrt((200 ** 2 + 120 ** 2) / 2) * 0.25) },
    },
  ...
  const rows = cases.map(({ id, value, expected }) => {
    ...
    if (id === "inset")
      actual = {
        x: numberAttr(markup, "x"),
        y: numberAttr(markup, "y"),
        width: numberAttr(markup, "width"),
        height: numberAttr(markup, "height"),
      };
    else if (id === "circle")
      actual = { cx: numberAttr(markup, "cx"), cy: numberAttr(markup, "cy"), r: numberAttr(markup, "r") };
    else if (id === "ellipse")
      actual = {
        cx: numberAttr(markup, "cx"),
        cy: numberAttr(markup, "cy"),
        rx: numberAttr(markup, "rx"),
        ry: numberAttr(markup, "ry"),
      };
    else
      actual = {
        points: [...markup.matchAll(/(?:points="| )(-?\d+(?:\.\d+)?),(-?\d+(?:\.\d+)?)/g)].flatMap((m) => [
          Number(m[1]),
          Number(m[2]),
        ]),
      };
    ...
    return {
      id: `clip.${id}`,
      stage: "clip" as const,
      source: "basic_shape_functions.cc and clip_path_clipper.cc reference-box path",
      expected,
      actual,
      maxAbsDelta: delta,
      pass: delta <= TOLERANCE,
  ...
  for (const test of referenceCases) {
    ...
    const delta = maxDelta(test.expected, actual);
    rows.push({
      id: `clip.reference-box.${test.box}`,
      stage: "clip",
      source: "geometry_box_utils.cc:13-49",
      expected: test.expected,
      actual,
      maxAbsDelta: delta,
      pass: delta <= TOLERANCE,
  ...
  const svgReferenceCases: Array<{
    box: SvgEffectGeometryBox;
    expected: { x: number; y: number; width: number; height: number };
  }> = [
  ...
  for (const test of svgReferenceCases) {
    ...
    const delta = maxDelta(test.expected, actual);
    rows.push({
      id: `clip.svg-reference-box.${test.box}`,
      stage: "clip",
  ...
  rows.push({
    id: "clip.url-reference.html-border-box",
    stage: "clip",
    source: "clip_path_clipper.cc:364-400 ReferenceClipPathOperation non-SVG branch",
    expected: { x: 10, y: 20, width: 200, height: 120 },
    actual: htmlUrlBox,
    maxAbsDelta: htmlUrlDelta,
    pass: htmlUrlDelta <= TOLERANCE,
  });
  ...
  rows.push({
    id: "clip.url-reference.html-object-bbox-map",
    stage: "clip",
    source: "layout_svg_resource_clipper.cc:237-247 explicit reference-box translate/scale",
    expected: htmlObjectBoundingBoxExpected,
    actual: htmlObjectBoundingBoxActual,
    maxAbsDelta: 0,
    pass: JSON.stringify(htmlObjectBoundingBoxActual) === JSON.stringify(htmlObjectBoundingBoxExpected),
  });
  const svgUrlBox = svgEffectReferenceBox(svgBoxes, "fill-box");
  const svgUrlDelta = maxDelta(svgBoxes.fillBox, svgUrlBox);
  rows.push({
    id: "clip.url-reference.svg-forced-fill-box",
    stage: "clip",
    source: "clip_path_clipper.cc:368-374 ReferenceClipPathOperation SVG-child override",
    expected: svgBoxes.fillBox,
    actual: svgUrlBox,
    maxAbsDelta: svgUrlDelta,
    pass: svgUrlDelta <= TOLERANCE,
  });
  const svgUrlMutationDelta = maxDelta(svgUrlBox, svgEffectReferenceBox(svgBoxes, "stroke-box"));
  rows.push({
    id: "clip.url-reference.svg-stroke-mutation",
    stage: "clip",
    source: "SVG URL references force fill-box rather than author-selectable stroke-box",
    expected: { differsFromStrokeBox: true },
    actual: { differsFromStrokeBox: svgUrlMutationDelta > 1 },
    maxAbsDelta: 0,
    pass: svgUrlMutationDelta > 1,
  });
  ...
  rows.push({
    id: "clip.svg-reference-box.activation-control",
    stage: "clip",
    source: "SVGResources::ReferenceBoxForEffects SVG-child branch",
    expected: { differsFromHtmlLayoutBox: true },
    actual: { differsFromHtmlLayoutBox: svgActivationDelta > 1 },
    maxAbsDelta: 0,
    pass: svgActivationDelta > 1,
  });
  ...
  for (const test of [
    { box: "padding-box" as const, expected: { x: 15, y: 25, width: 190, height: 110, rx: 25 } },
    { box: "content-box" as const, expected: { x: 25, y: 35, width: 170, height: 90, rx: 15 } },
  ]) {
    ...
    const delta = maxDelta(test.expected, actual);
    rows.push({
      id: `clip.rounded-reference-box.${test.box}`,
      stage: "clip",
      source: "clip_path_clipper.cc:242-261",
      expected: test.expected,
      actual,
      maxAbsDelta: delta,
      pass: delta <= TOLERANCE,
  ...
  const marginRounded = clipPathShapeForElement(
    {
      ...roundedElement,
      styles: {
        ...roundedElement.styles,
        borderRadius: "10px",
        marginTop: "20px",
        marginRight: "20px",
        marginBottom: "20px",
  ...
  const marginActual = {
    x: numberAttr(marginRounded, "x"),
    y: numberAttr(marginRounded, "y"),
    width: numberAttr(marginRounded, "width"),
    height: numberAttr(marginRounded, "height"),
    rx: numberAttr(marginRounded, "rx"),
  };
  const marginDelta = maxDelta(marginExpected, marginActual);
  rows.push({
    id: "clip.rounded-reference-box.margin-correction",
    stage: "clip",
    source: "float_rounded_rect.cc OutsetWithCornerCorrection",
    expected: marginExpected,
    actual: marginActual,
    maxAbsDelta: marginDelta,
    pass: marginDelta <= TOLERANCE,
  });
  return rows;
}
...
function overflowClipMarginRows(): OracleRow[] {
  ...
  const corrected = overflowClipMarginGeometry(
    ...
    zero,
    zero,
    corner([4, 8], [12, 16], [64, 0], [0, 32]),
    value("padding-box", 32),
  );
  ...
  const contourActual = {
    ...
    radii: [
      corrected.corners.tl.h,
      corrected.corners.tl.v,
      corrected.corners.tr.h,
      corrected.corners.tr.v,
      corrected.corners.br.h,
      corrected.corners.br.v,
      corrected.corners.bl.h,
      corrected.corners.bl.v,
    ],
  };
  const contourDelta = maxDelta(contourExpected, contourActual);
  rows.push({
    id: "clip.overflow-margin.coverage-contour",
    stage: "clip",
    source,
    expected: contourExpected,
    actual: contourActual,
    maxAbsDelta: contourDelta,
    pass: contourDelta <= 0.001,
  });
  ...
  for (const test of [
    ...
    {
      id: "content-zero-negative-outsets",
      value: value("content-box", 0),
      expected: { x: 19, y: 32, width: 77, height: 32 },
    },
  ]) {
    ...
    const delta = maxDelta(test.expected, actual);
    rows.push({
      id: `clip.overflow-margin.reference.${test.id}`,
      stage: "clip",
      source,
      expected: test.expected,
      actual,
  ...
  const activations = [
    ...
    { id: "auto", input: { overflowX: "auto", overflowY: "auto" }, expected: false },
    {
      ...
      input: { overflowX: "visible", overflowY: "visible", contain: "paint" },
      expected: true,
    },
  ...
  for (const test of activations) {
    ...
    rows.push({
      id: `clip.overflow-margin.activation.${test.id}`,
      stage: "clip",
      source,
      expected: test.expected,
      actual,
      maxAbsDelta: actual === test.expected ? 0 : Infinity,
      pass: actual === test.expected,
    });
  }
  const negativeRejected = parseOverflowClipMargin("-4px") == null;
  rows.push({
    id: "clip.overflow-margin.negative-declaration-control",
    stage: "clip",
    source: "longhands_custom.cc ParseSingleValue ValueRange::kNonNegative",
    expected: true,
    actual: negativeRejected,
    maxAbsDelta: negativeRejected ? 0 : Infinity,
    pass: negativeRejected,
  });
  ...
  const base = overflowClipMarginGeometry(
    ...
    zero,
    zero,
    corner([6, 4], [6, 4], [6, 4], [6, 4]),
    value("padding-box", 10),
  );
  const mutated = overflowClipMarginGeometry(
    ...
    zero,
    corner([6, 4], [6, 4], [6, 4], [6, 4]),
    value("padding-box", 11),
  );
  const movementActual = [
    mutated.x - base.x,
    mutated.y - base.y,
    mutated.width - base.width,
    mutated.height - base.height,
  ];
  const movementExpected = [-1, -1, 2, 2];
  const movementDelta = maxDelta(movementExpected, movementActual);
  rows.push({
    id: "clip.overflow-margin.margin-mutation",
    stage: "clip",
    source,
    expected: movementExpected,
    actual: movementActual,
    maxAbsDelta: movementDelta,
    pass: movementDelta <= TOLERANCE,
  });
  return rows;
}
...
function maskRows(): OracleRow[] {
  ...
  const rows: OracleRow[] = [
    {
      id: "mask.linear.magic-corner.alpha",
      stage: "mask",
      source: "CSSMaskPainter FillLayer geometry plus css_gradient_value.cc",
      expected: wanted,
      actual,
      maxAbsDelta: delta,
      pass: delta <= TOLERANCE && actual.maskType === "alpha",
    },
  ];
  const positioned = buildMaskDef(
    "mp",
  ...
  rows.push({
    id: "mask.size-position.no-repeat",
    stage: "mask",
    source: "CSSMaskPainter FillLayer image geometry",
    expected: true,
    actual: positionedRect,
    maxAbsDelta: positionedRect ? 0 : Infinity,
    pass: positionedRect,
  });
  const repeated = buildMaskDef(
    "mr",
    "linear-gradient(red, black)",
    10,
    20,
  ...
  const repeatOrigins = [
    ...repeated.matchAll(/<rect x="(-?\d+(?:\.\d+)?)" y="(-?\d+(?:\.\d+)?)" width="80" height="40"/g),
  ].map((match) => [Number(match[1]), Number(match[2])]);
  const expectedRepeatOrigins = [
    [-40, -15],
    [40, -15],
    [120, -15],
    [200, -15],
  ...
  rows.push({
    id: "mask.size-position.repeat",
    stage: "mask",
    source: "CSSMaskPainter FillLayer tiling geometry",
    expected: expectedRepeatOrigins,
    actual: repeatOrigins,
    maxAbsDelta: repeatDelta,
    pass: repeatDelta <= TOLERANCE,
  });
  const axisCases = [
  ...
  for (const test of axisCases) {
    ...
    const origins = [...def.matchAll(/<rect x="(-?\d+(?:\.\d+)?)" y="(-?\d+(?:\.\d+)?)" width="80" height="40"/g)].map(
      (match) => [Number(match[1]), Number(match[2])],
    );
    const axisDelta = maxDelta(test.expected, origins);
    rows.push({
      id: `mask.repeat.${test.id}`,
      stage: "mask",
      source: "background_image_geometry.cc:589-642",
      expected: test.expected,
      actual: origins,
      maxAbsDelta: axisDelta,
      pass: axisDelta <= TOLERANCE,
    });
  }
  const roundDef = buildMaskDef(
    "mround",
    "linear-gradient(red, black)",
    10,
    20,
  ...
  const roundTiles = [
    ...roundDef.matchAll(/<rect x="(-?\d+(?:\.\d+)?)" y="65" width="(-?\d+(?:\.\d+)?)" height="40"/g),
  ].map((match) => [Number(match[1]), Number(match[2])]);
  const expectedRoundTiles = [
    [-26.7, 66.7],
    [40, 66.7],
    [106.7, 66.7],
    [173.3, 66.7],
  ...
  rows.push({
    id: "mask.repeat.round",
    stage: "mask",
    source: "background_image_geometry.cc:26-36,547-588",
    expected: expectedRoundTiles,
    actual: roundTiles,
    maxAbsDelta: roundDelta,
    pass: roundDelta <= TOLERANCE,
  });
  const spaceDef = buildMaskDef(
    "mspace",
    "linear-gradient(red, black)",
    10,
    20,
  ...
  const spaceOrigins = [...spaceDef.matchAll(/<rect x="(-?\d+(?:\.\d+)?)" y="65" width="80" height="40"/g)].map(
    (match) => Number(match[1]),
  );
  const expectedSpaceOrigins = [10, 130];
  const spaceDelta = maxDelta(expectedSpaceOrigins, spaceOrigins);
  rows.push({
    id: "mask.repeat.space",
    stage: "mask",
    source: "background_image_geometry.cc:17-30,596-632",
    expected: expectedSpaceOrigins,
    actual: spaceOrigins,
    maxAbsDelta: spaceDelta,
    pass: spaceDelta <= TOLERANCE,
  });
  const dataMask =
    'url("data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%2280%22 height=%2240%22%3E%3Crect width=%2280%22 height=%2240%22 fill=%22black%22/%3E%3C/svg%3E")';
  const urlMaskBoxes = {
  ...
  for (const test of [
    ...
    const readGeometry = (markup: string) => {
      ...
      return {
        pattern: {
          x: numberAttr(pattern, "x"),
          y: numberAttr(pattern, "y"),
          width: numberAttr(pattern, "width"),
          height: numberAttr(pattern, "height"),
        },
        image: {
  ...
  const cyclicActual = [0, 1, 2, 3].map((index) =>
    new RegExp(
      `<rect x="0" y="0" width="${index % 2 === 0 ? 10 : 20}" height="${index % 2 === 0 ? 10 : 20}" fill="url\\(#mcycleg${index}\\)"`,
    ).test(cyclicDef),
  );
  rows.push({
    id: "mask.layer-list.cyclic",
    stage: "mask",
    source: "FillLayer linked-list repetition",
    expected: [true, true, true, true],
    actual: cyclicActual,
    maxAbsDelta: cyclicActual.every(Boolean) ? 0 : Infinity,
    pass: cyclicActual.every(Boolean),
  });
  ...
  for (const test of [
    ...
    rows.push({
      id: `mask.composite.${test.op}`,
      stage: "mask",
      source: "CSS mask-composite Porter-Duff mapping",
      expected: true,
      actual: matched,
      maxAbsDelta: matched ? 0 : Infinity,
      pass: matched,
    });
  }
  const threeLayers = `${twoLayers},linear-gradient(blue, transparent)`;
  const mixed = buildMaskDef(
    "mmix",
    threeLayers,
    0,
    0,
  ...
  const mixedActual = {
    ...
    intersectThenSubtract:
      mixed.includes(
        '<g mask="url(#mmixraw1)"><rect x="0" y="0" width="100" height="80" fill="#fff" mask="url(#mmixraw2)"',
      ) &&
      mixed.includes('id="mmixnota0"') &&
      mixed.includes(
        '<g mask="url(#mmixnota0)"><rect x="0" y="0" width="100" height="80" fill="#fff" mask="url(#mmixraw0)"',
      ),
  };
  ...
  rows.push({
    id: "mask.composite.mixed-sequential",
    stage: "mask",
    source: "svg_mask_painter.cc:91-113, IterateFillLayersReveresed",
    expected: mixedExpected,
    actual: mixedActual,
    maxAbsDelta: mixedPass ? 0 : Infinity,
    pass: mixedPass,
  });
  const fragmentMask = positionFragmentMaskDef(
    '<mask id="fm" x="0" y="0" width="1" height="1"><rect width="10" height="8"/></mask>',
    13,
    17,
    90,
    70,
  );
  const fragmentMaskExpected =
    '<mask id="fm" maskUnits="userSpaceOnUse" x="13" y="17" width="90" height="70"><g transform="translate(13, 17)"><rect width="10" height="8"/></g></mask>';
  rows.push({
    id: "mask.fragment.user-space",
    stage: "mask",
    source: "SVGResourceClipper/Masker HTML reference coordinate mapping",
    expected: fragmentMaskExpected,
    actual: fragmentMask,
    maxAbsDelta: fragmentMask === fragmentMaskExpected ? 0 : Infinity,
    pass: fragmentMask === fragmentMaskExpected,
  });
  const fragmentClip = positionFragmentClipPathDef(
    '<clipPath id="fc" clipPathUnits="userSpaceOnUse"><rect width="10" height="8"/></clipPath>',
    13,
    17,
  );
  const fragmentClipExpected =
    '<clipPath id="fc" clipPathUnits="userSpaceOnUse" transform="translate(13, 17)"><rect width="10" height="8"/></clipPath>';
  rows.push({
    id: "clip.fragment.user-space",
    stage: "clip",
    source: "ClipPathClipper local reference coordinate mapping",
    expected: fragmentClipExpected,
    actual: fragmentClip,
    maxAbsDelta: fragmentClip === fragmentClipExpected ? 0 : Infinity,
    pass: fragmentClip === fragmentClipExpected,
  });
  return rows;
}
...
function resizerRows(): OracleRow[] {
  ...
  for (const test of [
    ...
    {
      id: "replaced",
      input: { resize: "both", overflowX: "auto", overflowY: "auto", isLayoutReplaced: true },
      expected: false,
    },
  ]) {
    const actual = blinkCanResize(test.input);
    rows.push({
      id: `resizer.activation.${test.id}`,
      stage: "resizer",
      source,
      expected: test.expected,
      actual,
  ...
  const noBars = blinkResizerThickness({
    themeThickness: 16,
    verticalScrollbarThickness: null,
    horizontalScrollbarThickness: null,
  });
  const bothBars = blinkResizerThickness({
    themeThickness: 16,
    verticalScrollbarThickness: 13,
    horizontalScrollbarThickness: 9,
  });
  for (const test of [
    ...
    rows.push({
      id: `resizer.thickness.${test.id}`,
      stage: "resizer",
      source,
      expected: test.expected,
      actual: test.actual,
      maxAbsDelta: delta,
      pass: delta <= TOLERANCE && test.actual.hasScrollbar === test.expected.hasScrollbar,
    });
  }
  ...
  rows.push({
    id: "resizer.paint.platform-right",
    stage: "resizer",
    source,
    expected: rightExpected,
    actual: rightPaint,
    maxAbsDelta: paintDelta,
    pass: paintDelta <= TOLERANCE,
  });
  ...
  rows.push({
    id: "resizer.side.horizontal-rtl-only",
    stage: "resizer",
    source,
    expected: sideExpected,
    actual: sideActual,
    maxAbsDelta: sideActual.join() === sideExpected.join() ? 0 : Infinity,
    pass: sideActual.join() === sideExpected.join(),
  });
  return rows;
}
...
function main(): number {
  ...
  const report = {
    schemaVersion: 1,
    stage: "gradient-mask-clip-resizer-geometry",
