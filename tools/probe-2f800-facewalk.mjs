...
function covers(face, cp) {
  ...
  try {
    return face.glyphForCodePoint(cp).id !== 0;
  } catch {
    return false;
  }
}
...
function firstCovering(list, cp) {
  ...
  for (const f of list) {
    ...
    if (singleton != null && covers(f.face, singleton))
      return { face: f.key, via: "decomp", dcp: singleton.toString(16) };
  }
  return null;
}
...
for (const cp of cps) {
  ...
  if (prim) primaryOnly++;
  const walk = firstCovering(
    STACK.map((s) => faces.find((f) => f.key === s.key)),
    cp,
  ); // full installed CSS-stack walk
  if (walk) {
    walkCovers++;
    viaCounts[walk.via]++;
  ...
  else if (walkGlyph && !chromeGlyph) {
    walkOverPaints++;
    mismatches.push({ cp: cp.toString(16), kind: "OVER", walk });
  } else if (!walkGlyph && chromeGlyph) {
    walkUnderPaints++;
    mismatches.push({ cp: cp.toString(16), kind: "UNDER" });
  }
}
...
console.log(
  `Full CSS-stack walk covers:           ${walkCovers} cells   (+${walkCovers - primaryOnly} vs primary-only)`,
);
...
if (mismatches.length) {
  ...
  for (const m of mismatches.slice(0, 20))
    console.log(`  U+${m.cp} ${m.kind}${m.walk ? " " + JSON.stringify(m.walk) : ""}`);
}
