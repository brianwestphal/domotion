...
async function main() {
  ...
  function walk(node: any, depth: number = 0): void {
    ...
    const looks =
      node.styles?.fontStyle?.includes?.("italic") ||
      node.styles?.textDecorationLine?.includes?.("underline") ||
      (typeof text === "string" && text.includes("Most Discussed"));
    if (looks) {
      console.log(
        `[${depth}] <${node.tag ?? "?"}> rect=(${node.x?.toFixed?.(0)},${node.y?.toFixed?.(0)} ${node.width?.toFixed?.(0)}×${node.height?.toFixed?.(0)}) fontStyle=${node.styles?.fontStyle} td=${node.styles?.textDecorationLine} text=${JSON.stringify(typeof text === "string" ? text.slice(0, 60) : "")}`,
      );
