...
const S0 = scene({
  ...
  sub: "One SVG chaining parameterized and custom transitions back to back.",
  via: "exits via <b>crossfade →</b>",
});
const S1 = scene({
  bg: "radial-gradient(130% 130% at 100% 0%, #2a1450 0%, #0a0f1e 62%)",
  accent: "#c4a3ff",
  kicker: "Scene 2",
  title: "Origin-aware zoom",
  sub: "It fades in, then brings the next scene forward from a deliberately off-center origin.",
  via: "enters <b>fade</b> · exits <b>zoom { origin } →</b>",
});
const S2 = scene({
  bg: "radial-gradient(130% 130% at 100% 100%, #0c3a3a 0%, #0a0f1e 62%)",
  accent: "#5eead4",
  kicker: "Scene 3",
  title: "Clock reveal",
  sub: "The zoom hands off to a counterclockwise reveal centered below and left of the canvas midpoint.",
  via: "enters <b>dolly</b> · exits <b>reveal { clock } →</b>",
});
const S3 = scene({
  ...
  kicker: "Scene 4",
  title: "Angled push",
  sub: "The clock opens onto this scene; an arbitrary-angle push then carries it into the final recipe.",
  via: "enters <b>clock reveal</b> · exits <b>push { angle } →</b>",
});
const S4 = scene({
  bg: "radial-gradient(130% 130% at 0% 0%, #0c3a2a 0%, #0a0f1e 62%)",
  accent: "#4ade80",
  kicker: "Scene 5",
  title: "Custom, safely",
  sub: "Opacity, translation, scale, and a shine band combine through bounded primitives — no raw CSS or script.",
  via: "enters via <b>custom recipe</b> · crossfades to loop ↺",
});
  ...
  try {
    ...
    frames = [
      { svgContent: c0, duration: 1600, transition: { type: "crossfade", duration: 600 } }, // → crossfade
      {
        svgContent: c1,
        duration: 1600,
        transition: { type: "zoom", duration: 650, zoom: { fromScale: 0.82, origin: { x: 0.68, y: 0.36 } } },
      },
      {
        svgContent: c2,
        duration: 1700,
        transition: {
          type: "reveal",
          duration: 650,
          reveal: { shape: "clock", origin: { x: 0.35, y: 0.62 }, startAngle: 35, direction: "counterclockwise" },
        },
      },
      {
        svgContent: c3,
        duration: 1700,
        transition: { type: "push", duration: 650, push: { angle: 205, distance: 0.85 } },
      },
      {
        svgContent: c4,
        duration: 1800,
        transition: {
          ...
          custom: {
            incoming: {
              opacity: 0.1,
              translate: { x: 0.12, y: -0.08 },
              scale: { from: 0.88, origin: { x: 0.5, y: 0.5 } },
            },
