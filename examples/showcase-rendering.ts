/**
 * Showcase: Rendering Capabilities
 *
 * Demonstrates text rendering across different styles:
 * - Headings (bold, large)
 * - Body text (regular, secondary color)
 * - Monospace / code blocks
 * - Colored text (links, badges, success/error)
 * - Small text / labels
 * - Inline SVG icons
 * - Buttons and form elements
 * - Cards with borders and rounded corners
 *
 * No server needed — uses static HTML pages.
 */
...
const OUTPUT = resolve(OUT_DIR, "showcase-rendering.svg");
...
.container { padding: 32px; width: ${WIDTH}px; height: ${HEIGHT}px; }
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < FRAMES.length; i++) {
    ...
    const tmpPath = resolve(OUT_DIR, `showcase-tmp-${i}.html`);
    writeFileSync(tmpPath, frame.html);
    await pg.goto(`file://${tmpPath}`);
    await pg.waitForTimeout(200);

    const tree = await captureElementTree(pg, "body", { x: 0, y: 0, width: WIDTH, height: HEIGHT });
    // DM-512: demos always emit self-contained SVGs so they load correctly
  ...
  let svg = generateAnimatedSvg({ width: WIDTH, height: HEIGHT, frames: animFrames });
  svg = optimizeSvg(svg);
