...
| `inner-element-scroll-foreground/` | An element-owned scroll beneath a separate **static foreground sibling**: the higher-`z-index` release card overlaps the feed and stays pinned while every activity row passes behind it. |
...
| `overlay-window/` | The **per-overlay window** and **per-state overlays** (docs/104). Frame 1 carries TWO typing annotations in ONE frame — the first bounded by `endAt`, the second starting after it — which is what `endAt` buys standalone: an overlay that ends where the author says, without splitting the frame in two. Frame 2 is a hand-authored `states:` run whose four states each carry an `interact` overlay **anchored to an element that moves**, so each ring is resolved against its own state's layout rather than the run's last. |
...
> The `npm install` demo previously lived here as `progress-install/`. It is now
> a realistic terminal-cast demo — `examples/progress-install.ts` (run via
> `npm run demos:examples`), rendered through the `domotion term` pipeline
> (doc 67) rather than an animate config. Output:
> `examples/output/progress-install.svg`.
