...
async function waitForServer(url: string, process: ChildProcess): Promise<void> {
  for (let attempt = 0; attempt < 80; attempt++) {
    if (process.exitCode != null)
      throw new Error(`Glassbox server exited before it became ready (${process.exitCode})`);
    try {
...
async function runSemanticHook(page: Page, hookId: string): Promise<void> {
  ...
  if (hookId === "apply-reviewed-fix") {
    await page.evaluate(() => {
      document
        .querySelectorAll(".annotation-row, .annotation-form-container, .annotation-count")
        .forEach((element) => element.remove());
      const reopen = document.getElementById("reopen-review");
      if (reopen != null) {
        reopen.textContent = "Complete Review";
        reopen.id = "complete-review";
      }
      const code = document.querySelector('.diff-line.split-right[data-side="new"][data-line="23"] .code');
      if (code != null)
        code.textContent =
          "  await redis.set(`session:${encodeURIComponent(id)}`, JSON.stringify(session), 'EX', SESSION_TTL);";
    });
    return;
  }
...
export function glassboxBenchmarkHealDecision(
  request: Parameters<RunStudioHealingLoopOptions["heal"]>[0],
): StudioAiHealingDecision {
  ...
  return {
    ...
    evidence: {
      ...
      data: {
        ...
        role: candidate.roleAttribute ?? "button",
        name: candidate.ariaLabelAttribute ?? "Risk review",
        rect: candidate.rect,
...
export function glassboxBenchmarkReviewDecision(
  request: Parameters<RunStudioHealingLoopOptions["review"]>[0],
): StudioAiReviewDecision {
  const hasBrandRevision = request.project.review.revisions.some(
    (revision) => revision.metadata?.automation != null && revision.summary.includes("browser frame"),
  );
  if (!hasBrandRevision) {
    const project = structuredClone(request.project);
    const scene = project.scenes.find((item) => item.id === "scene-live-review")!;
    ...
    return {
      ...
      project,
      summary:
        "Clarified the live browser frame so the local-first product promise remains visible during the longest scene.",
      evidence: {
        summary:
          "The live scene occupies most of the runtime; its persistent chrome should reinforce the product promise without obscuring UI.",
      },
    };
  }
  ...
  return {
    ...
    evidence: {
      summary:
        "The candidate preserves all five scenes, every core interaction, readable pacing, restrained transitions, and Glassbox brand framing.",
      data: {
        sceneCount: request.project.scenes.length,
        eventCount: eventIds.length,
        meaningfulEffects: segment?.evidence.filter((item) => item.meaningful).length ?? 0,
      },
    },
  };
}
...
function videoReview(project: StudioProject): StudioVideoReviewReport {
  ...
  return {
    kind: "report",
    summary:
      "The rendered benchmark tells a complete local review-to-repair story with purposeful motion and a restrained Glassbox visual voice.",
    findings: [
      finding(
        "video-pacing",
        "pacing",
        "The opening promise and terminal launch establish context before the 16.6-second live workflow.",
        "The scene lengths follow the viewer's cognitive load instead of cutting every beat uniformly.",
        "Preserve the longer typing and export holds on recapture.",
        0,
        5200,
      ),
      finding(
        "video-cursor",
        "cursor-realism",
        "Every cursor waypoint is derived from the live target box and computed cursor style.",
        "The path reads as a user making intentional choices rather than a decorative pointer.",
        "Regenerate cursor choreography whenever target geometry changes.",
        5200,
        21800,
      ),
      finding(
        "video-readability",
        "readability",
        "The product UI is captured at 1280 by 800 and terminal beats use high-contrast 22–24px text.",
        "Code, feedback, and status changes remain legible in the review render.",
        "Keep future captions out of the diff content area.",
        1900,
        25000,
      ),
      finding(
        "video-transitions",
        "transitions",
        "Cuts are used inside the live workflow while crossfades and one shine transition separate narrative chapters.",
        "State changes stay crisp and chapter changes feel cinematic without ghosting similar UIs.",
        "Avoid crossfading adjacent near-identical application states.",
        0,
        27000,
      ),
      finding(
        "video-narrative",
        "narrative-clarity",
        "The sequence shows launch, AI risk triage, a precise bug note, structured export, agent repair, and resolution.",
        "The demo demonstrates Glassbox's causal loop rather than presenting disconnected features.",
        "Retain all six causal beats when shortening the demo.",
        0,
        27000,
      ),
      finding(
        "video-brand",
        "brand-presentation",
        "Terracotta, blue, and GitHub-dark-adjacent surfaces frame the actual Glassbox UI, with the official logo embedded in the close.",
        "The visual system feels like Glassbox while letting the product remain the hero.",
        "Keep the persistent browser label focused on local inspection.",
        0,
        27000,
      ),
      finding(
        "video-polish",
        "overall-polish",
        "The final SVG is self-contained, font-backed, revision-provenanced, and rendered through the production video exporter.",
        "The candidate is suitable for human review and repeatable release use.",
        "Use the controlled-change run as the regression gate after UI updates.",
        0,
        27000,
      ),
    ],
    comparison: {
      summary:
        "Compared with the legacy hand-orchestrated hero, the Studio candidate makes story, semantic intent, healing evidence, AI review, and generated artifacts durable and inspectable.",
      improvements: [
        "A static JSON storyboard owns the narrative.",
        "The real live workflow is one evidence-backed semantic segment.",
        "Brand treatments and transitions are data, not compositor-only code.",
        "A UI name change is repaired from live DOM evidence and recaptured.",
      ],
      regressions: [],
    },
  };
...
function assertSelfContained(svg: string): void {
  ...
  if (
    /(?:href|src)\s*=\s*["'](?:https?:)?\/\//i.test(withoutNamespaces) ||
    /url\(["']?(?:https?:)?\/\//i.test(withoutNamespaces)
  ) {
...
async function runCandidate(
  ...
  const result = await runStudioHealingLoop(browser, source, {
    ...
    runSceneHook: async ({ page, hookId, phase }) => {
      if (hookId !== "prepare-glassbox" || phase !== "beforeCapture")
        throw new Error(`Unexpected scene hook ${hookId}:${phase}`);
  ...
  try {
    ...
    process.stderr.write(`SVG optimization skipped: ${String(error)}\n`);
  }
  ...
  if (renderVideo) {
    ...
    if (reviewed.status !== "reviewed")
      throw new Error(`Glassbox video review stopped for clarification: ${reviewed.checkpoint.question}`);
  ...
  return {
    ...
    projectPath,
    ...(media == null ? {} : { media }),
    ...(report == null ? {} : { videoReview: report }),
  };
}
    ...
    const report = {
      ...
      baseline: {
        ...
        video:
          baseline.media == null
            ? null
            : { path: relative(DOMOTION_ROOT, baseline.media.path), sha256: baseline.media.sha256 },
      ...
      controlledChange:
        changed == null
          ...
          : {
              ...
              healingRevisions: changed.project.review.revisions
                .filter((revision) => revision.metadata?.automation != null)
                .map((revision) => ({ id: revision.id, summary: revision.summary, metadata: revision.metadata })),
            },
