...
cmake -S "$Root\packages\text-engine\tools\icu-helper" -B "$BuildRoot\helper" -A $env:CMAKE_GENERATOR_PLATFORM -DDOMOTION_ICU_ROOT="$IcuRoot"
...
Copy-Item "$BuildRoot\helper\Release\domotion-icu.exe" "$Root\packages\text-engine\tools\icu-helper\domotion-icu.exe"
Copy-Item "$IcuRoot\$BinDir\icuuc78.dll" "$Root\packages\text-engine\tools\icu-helper\icuuc78.dll"
Copy-Item "$IcuRoot\$BinDir\icudt78.dll" "$Root\packages\text-engine\tools\icu-helper\icudt78.dll"
