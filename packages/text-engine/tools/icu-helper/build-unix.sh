...
cmake -S "$ROOT/packages/text-engine/tools/icu-helper" -B "$BUILD_ROOT/helper" \
  -DCMAKE_BUILD_TYPE=Release -DDOMOTION_ICU_ROOT="$INSTALL_ROOT"
cmake --build "$BUILD_ROOT/helper" --config Release --parallel
cp "$BUILD_ROOT/helper/domotion-icu" "$ROOT/packages/text-engine/tools/icu-helper/domotion-icu"
