 ...
 * underlying registries remain module-local implementation details, but their
 * ownership and lifetime are explicit here.
 */
...
export interface TextEngineDocumentRequest {
  /** Reuse an isolated renderer lifetime across related documents. Omit to
...
export function withTextEngineDocument<T>(
  ...
  try {
    ...
    // producers: their clear/render/collect sequence still expects generated
    // glyphs and embedded fonts to remain readable after this callback. Keep
    // the completed generation installed in that case instead of restoring
    // the pre-render snapshot and silently dropping its @font-face output.
    if (!implicitSession) {
