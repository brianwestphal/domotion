...
describe("embedded system-font terminal rendering (DM-2623)", () => {
  ...
  it("limits target strikes to individually measured Linux surfaces", () => {
    ...
    expect(embeddedLinuxTargetStrikeEnabled("linux", "LiberationSans-Bold", 16, 700, 0, 100)).toBe(false);
...
// Exhaustive resolver loops are long-lived workloads: use the production
// helper channel for them, then close it at the describe boundary. The default
// unit configuration remains one-shot so ordinary short-lived Vitest forks do
// not leak native children when the worker is reaped.
function withPersistentGlyphHelper(): void {
  let previousNoServe: string | undefined;
  beforeAll(() => {
    previousNoServe = process.env.DOMOTION_HELPER_NO_SERVE;
  ...
  afterAll(() => {
    clearFontResolutionCaches();
    clearGlyphHelperCache();
    if (previousNoServe == null) delete process.env.DOMOTION_HELPER_NO_SERVE;
    else process.env.DOMOTION_HELPER_NO_SERVE = previousNoServe;
  });
}
...
describe("commandsFor (per-glyph fallback routing)", () => {
  ...
  const HELPER = "packages/text-engine/tools/macos-glyph-extractor/domotion-glyph-paths";
...
describe("resolveFontForCodepoint: primary-only NFD decomposition (DM-1080)", () => {
  // This is a decomposition-boundary test, not a live platform-fallback
  // integration test. Keeping the nomination walk armed makes every negative
  // compatibility-ideograph probe spawn the native helper in the unit lane,
  // obscuring the invariant behind host-dependent transport time.
  withNominationWalkDisarmed();
  withPersistentGlyphHelper();
...
describe("renderTextAsPath: embedded-font emits custom-built TTFs (DM-655)", () => {
  const SFNS_PATH = "/System/Library/Fonts/SFNS.ttf";
  const fontBuf = fs.existsSync(SFNS_PATH) ? fs.readFileSync(SFNS_PATH) : null;
  const fixtureFontBuf = fs.readFileSync("packages/text-engine/assets/fonts/fixture/DomotionFixtureSerif-Regular.ttf");
...
const macCutHelper =
  process.platform === "darwin" && existsSync("packages/text-engine/tools/macos-glyph-extractor/domotion-glyph-paths");
