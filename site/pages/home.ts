...
const tocCards = SECTIONS.map((sec) => {
  const titles = sec.pages.slice(0, 4).map((p) => `<li>${escape(p.title)}</li>`).join("");
  const more = sec.pages.length > 4 ? `<li>+${sec.pages.length - 4} more</li>` : "";
  const firstSlug = sec.pages[0]?.slug ?? "";
  return `
    <a class="toc-card" href="${firstSlug}/">
      <h3>${escape(sec.title)}</h3>
      <p>${escape(sec.blurb)}</p>
      <ul>${titles}${more}</ul>
    </a>`;
}).join("");
...
<section class="hero">
  ...
  <div class="cta">
    <a class="btn primary" href="start/quickstart/">Quick start →</a>
    <a class="btn" href="start/what-is-domotion/">Why Domotion?</a>
    <a class="btn" href="api/overview/">API reference</a>
  </div>
...
<section>
  ...
  <div class="toc-grid" style="grid-template-columns:repeat(auto-fit,minmax(260px,1fr));">
    <div class="toc-card">
...
<section style="margin-top:40px;text-align:center;">
  ...
  <pre style="display:inline-block;text-align:left;margin:0 auto;">npm install -g domotion
