...
const TOP_LEVEL_ORDER = ["width", "height", "frames", "output", "optimize", "mobile", "colorScheme", "vars", "cursor"];
const FRAME_ORDER = [
...
function propsTable(node, requiredSet, order) {
  ...
  return ["| Field | Type | Required | Default | Description |", "|---|---|---|---|---|", ...rows].join("\n");
}
...
function emitSection(out, node, title, level, seen) {
  ...
  for (const [k, p] of Object.entries(obj.properties)) {
    ...
    const hasShape = child && (child.properties || (child.oneOf || child.anyOf || []).some((v) => v.properties));
...
function generateAnimateConfigReference() {
  ...
  out.push("description: Every field of the domotion animate JSON config, generated from the published JSON Schema.");
  out.push("---");
  out.push("");
  out.push(
